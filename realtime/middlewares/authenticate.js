...
function authenticate_with_mrinimitable(socket, next) {
	...
	socket.mrinimitable_request = (path, args = {}, opts = {}) => {
	...
	socket
		.mrinimitable_request("/api/method/mrinimitable.realtime.get_user_info")
		.then((res) => res.json())
		.then(({ message }) => {
...
function get_site_name(socket) {
	...
	} else if (socket.request.headers["x-mrinimitable-site-name"]) {
		socket.site_name = get_hostname(socket.request.headers["x-mrinimitable-site-name"]);
	} else if (
		conf.default_site &&
...
module.exports = authenticate_with_mrinimitable;
