...
class EMail:
	...
	def attach_file(self, n):
		...
		_file = mrinimitable.get_doc("File", {"file_name": n})
	...
	def validate(self):
		...
		self.recipients = [strip(r) for r in self.recipients if r not in mrinimitable.STANDARD_USERS]
		self.cc = [strip(r) for r in self.cc if r not in mrinimitable.STANDARD_USERS]
		self.bcc = [strip(r) for r in self.bcc if r not in mrinimitable.STANDARD_USERS]
	...
	def add_headers(self, headers):
		...
		if not isinstance(headers, dict):
			mrinimitable.throw(_("Headers must be a dictionary"))
	...
	def make(self):
		...
		for hook in mrinimitable.get_hooks("make_email_body_message"):
			mrinimitable.get_attr(hook)(self)
...
def get_formatted_html(
	...
	email_account=None,
	header=None,
	unsubscribe_link: mrinimitable._dict | None = None,
	sender=None,
	with_container=False,
):
	email_account = email_account or EmailAccount.find_outgoing(match_by_email=sender)

	rendered_email = mrinimitable.get_template("templates/emails/standard.html").render(
		{
...
def get_email_html(template, args, subject, header=None, with_container=False):
	...
	email = mrinimitable.utils.jinja.get_email_from_template(template, args)
...
def inline_style_in_html(html):
	...
	css_files = mrinimitable.get_hooks("email_css")
...
def get_message_id():
	...
	return email.utils.make_msgid(domain=mrinimitable.local.site)
...
def get_footer(email_account, footer=None):
	...
	sender_address = mrinimitable.db.get_default("email_footer_address")

	if sender_address:
		args.update({"sender_address": sender_address})

	if not cint(mrinimitable.db.get_default("disable_standard_email_footer")):
		args.update({"default_mail_footer": mrinimitable.get_hooks("default_mail_footer")})

	footer += mrinimitable.utils.jinja.get_email_from_template("email_footer", args)[0]

	return footer


def replace_filename_with_cid(message):
	"""Replaces <img embed="assets/mrinimitable/images/filename.jpg" ...> with
	...
	while True:
		...
		# found match
		img_path = groups[0]
		img_path_escaped = mrinimitable.utils.html_utils.unescape_html(img_path)
...
def get_filecontent_from_path(path):
	...
	if path.startswith("assets/"):
		...
		# public file
		full_path = mrinimitable.get_site_path("public", path)
	elif path.startswith("private/files/"):
		# private file
		full_path = mrinimitable.get_site_path(path)
	else:
		full_path = path
...
def get_header(header=None):
	...
	from mrinimitable.utils.jinja import get_email_from_template
	...
	if not title:
		title = mrinimitable.get_hooks("app_title")[-1]
