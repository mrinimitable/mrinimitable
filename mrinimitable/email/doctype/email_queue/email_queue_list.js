mrinimitable.listview_settings["Email Queue"] = {
...
function show_toggle_sending_button(list_view) {
	...
	const sending_disabled = cint(mrinimitable.sys_defaults.suspend_email_queue);
	const label = sending_disabled ? __("Resume Sending") : __("Suspend Sending");

	list_view.page.add_inner_button(label, async () => {
		await mrinimitable.xcall(
			"mrinimitable.email.doctype.email_queue.email_queue.toggle_sending",
		...
		mrinimitable.sys_defaults.suspend_email_queue = sending_disabled ? 0 : 1;
