...
import mrinimitable
from mrinimitable import _, safe_encode, task
...
from mrinimitable.email.email_body import add_attachment, get_email, get_formatted_html
from mrinimitable.email.mrinimitablemail import MrinimitableMail
from mrinimitable.email.queue import get_unsubcribed_url, get_unsubscribe_message
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from mrinimitable.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if mrinimitable.session.user != "Administrator":
			mrinimitable.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = mrinimitable.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False) -> EmailQueue:
		...
		doc = mrinimitable.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = mrinimitable.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return mrinimitable.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			mrinimitable.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(mrinimitable.db.get_default("suspend_email_queue")) == 1
		):
			return False
	...
	def send(
		...
		mrinimitable_mail_client: MrinimitableMail = None,
		...
		with SendMailContext(self, smtp_server_instance, mrinimitable_mail_client) as ctx:
			...
			for recipient in self.recipients:
				...
				elif not mrinimitable.in_test or mrinimitable.flags.testing_email:
					if ctx.email_account_doc.service == "Mrinimitable Mail":
						is_newsletter = self.reference_doctype == "Newsletter"
						ctx.mrinimitable_mail_client.send_raw(
							sender=self.sender,
			...
			if mrinimitable.in_test and not mrinimitable.flags.testing_email:
				mrinimitable.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = mrinimitable.qb.DocType("Email Queue")
		email_recipient = mrinimitable.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			mrinimitable.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
		...
		(
			mrinimitable.qb.from_(email_recipient)
			.delete()
			.where(email_recipient.creation < (Now() - Interval(days=days)))
		).run()


from mrinimitable.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	def __init__(
		...
		self.mrinimitable_mail_client: MrinimitableMail = mrinimitable_mail_client
	...
	def fetch_outgoing_server(self):
		...
		if self.email_account_doc.service == "Mrinimitable Mail":
			if not self.mrinimitable_mail_client:
				self.mrinimitable_mail_client = self.email_account_doc.get_mrinimitable_mail_client()
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": mrinimitable.get_traceback()}
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		if self.queue_doc.get("email_read_tracker_url"):
			...
			tracker_url = f"{get_url()}/api/method/mrinimitable.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = mrinimitable.get_doc("File", file_filters)
				...
				print_format_file = mrinimitable.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not mrinimitable.get_system_settings("store_attached_pdf_document"):
			return

		file_data = mrinimitable._dict(file_name=file_name, is_private=1)
		...
		if mrinimitable.db.exists("File", file_data):
			return

		file = mrinimitable.new_doc("File", **file_data)
...
def retry_sending(queues: str | list[str]):
	if not mrinimitable.has_permission("Email Queue", throw=True):
		return
	...
	email_queue = mrinimitable.qb.DocType("Email Queue")
	mrinimitable.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, mrinimitable.session.user
...
def toggle_sending(enable):
	...
	mrinimitable.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	mrinimitable.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	mrinimitable.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(mrinimitable.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/mrinimitable.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/mrinimitable.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				mrinimitable.qb.from_(EmailUnsubscribe)
			...
			for att in self._attachments:
				if att.get("fid") or att.get("file_url"):
					...
					if not att.get("lang", None):
						att["lang"] = mrinimitable.local.lang
	...
	def prepare_email_content(self):
		...
		if self.in_reply_to:
			if message_id := mrinimitable.db.get_value("Communication", self.in_reply_to, "message_id"):
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in mrinimitable.utils.create_batch(final_recipients, 1000):
				mrinimitable.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=mrinimitable.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=mrinimitable.in_test or send_now,
					queue="long",
				)
	...
	def send_emails(self, queue_data, final_recipients):
		...
		mrinimitable_mail_client = None
		smtp_server_instance = None
		for r in final_recipients:
			recipients = list(set([r, *self.final_cc(), *self.bcc]))
			q = EmailQueue.new({**queue_data, **{"recipients": recipients}}, ignore_permissions=True)
			if not mrinimitable_mail_client and not smtp_server_instance:
				email_account = q.get_email_account(raise_error=True)

				if email_account.service == "Mrinimitable Mail":
					mrinimitable_mail_client = email_account.get_mrinimitable_mail_client()
				else:
					smtp_server_instance = email_account.get_smtp_server()

			with suppress(Exception):
				q.send(smtp_server_instance=smtp_server_instance, mrinimitable_mail_client=mrinimitable_mail_client)
	...
	def as_dict(self, include_recipients=True):
		...
		except mrinimitable.InvalidEmailAddressError:
