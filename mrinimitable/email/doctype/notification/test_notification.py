...
def get_test_notification(config):
	try:
		notification = mrinimitable.get_doc(doctype="Notification", **config).insert()
		yield notification
	finally:
		notification.delete()
...
class TestNotification(IntegrationTestCase):
	def setUp(self):
		...
		if not mrinimitable.db.exists("Notification", {"name": "ToDo Status Update"}, "name"):
			notification = mrinimitable.new_doc("Notification")
		...
		if not mrinimitable.db.exists("Notification", {"name": "Contact Status Update"}, "name"):
			notification = mrinimitable.new_doc("Notification")
	...
	def test_new_and_save(self):
		...
		communication = mrinimitable.new_doc("Communication")
		...
		self.assertEqual(mrinimitable.db.get_value("Communication", communication.name, "subject"), "__testing__")

	def test_condition(self):
		"""Check notification is triggered based on a condition."""
		event = mrinimitable.new_doc("Event")
		event.subject = "test"
	...
	def test_invalid_condition(self):
		...
		recipent = mrinimitable.new_doc("Notification Recipient")
		recipent.receiver_by_document_field = "owner"

		notification.recipents = recipent
		notification.condition = "test"

		self.assertRaises(mrinimitable.ValidationError, notification.save)
		notification.delete()
	...
	def test_minutes_positive_offset(self):
		...
		with get_test_notification(notification) as n:
			mrinimitable.db.delete("Notification Log", {"subject": n.subject})
			# Run the offset alerts
			trigger_notifications(None, "offset")

			# Check if the notification was triggered
			self.assertEqual(1, mrinimitable.db.count("Notification Log", {"subject": n.subject}))
	...
	def test_minutes_offset_validation(self):
		...
		self.assertRaises(mrinimitable.ValidationError, notification.insert)
	...
	def test_alert_disabled_on_wrong_field(self):
		mrinimitable.set_user("Administrator")
		notification = mrinimitable.get_doc(
			{
	...
	def test_date_changed(self):
		...
		event.starts_on = mrinimitable.utils.add_days(mrinimitable.utils.nowdate(), 2) + " 12:00:00"
		event.save()
	...
	def test_cc_jinja(self):
		mrinimitable.db.delete("User", {"email": "test_jinja@example.com"})
		mrinimitable.db.delete("Email Queue")
		mrinimitable.db.delete("Email Queue Recipient")

		test_user = mrinimitable.new_doc("User")
		...
		self.assertTrue(mrinimitable.db.get_value("Email Queue Recipient", {"recipient": "test_jinja@example.com"}))
	...
	def test_notification_to_assignee(self):
		...
		email_queue = mrinimitable.get_doc(
	...
	def test_notification_value_change_casted_types(self):
		...
		with get_test_notification(notification) as n:
			...
			user.birth_date = mrinimitable.utils.add_days(user.birth_date, 1).date()
			user.save()

			user.reload()
			user.birth_date = mrinimitable.utils.getdate(user.birth_date)
			user.save()
			self.assertEqual(1, mrinimitable.db.count("Notification Log", {"subject": n.subject}))

	@classmethod
	def tearDownClass(cls):
		mrinimitable.delete_doc_if_exists("Notification", "ToDo Status Update")
		mrinimitable.delete_doc_if_exists("Notification", "Contact Status Update")
		...
		notification = mrinimitable.get_doc(
			{
				...
				"message": "{% set val = mrinimitable.get_doc('ToDo', doc.name) %} ToDo allocated to {{ doc.allocated_to }}",
				"channel": "Email",
	...
	def test_filters_condition(self):
		...
		mrinimitable.delete_doc_if_exists("Notification", "Test Filters Condition")
...
❯ shashi run-tests --module mrinimitable.email.doctype.notification.test_notification --case TestNotificationOffsetRange
...
Updating Dashboard for mrinimitable
...
# 		self.event = mrinimitable.new_doc("Event")
...
# 		mrinimitable.db.delete("Event", {"name": self.event.name})
# 		mrinimitable.db.delete("Notification", {"name": self.notification["name"]})
# 		mrinimitable.db.delete("Notification Log", {"subject": self.notification["subject"]})
...
# 						self.assertEqual(
# 							1,
# 							mrinimitable.db.count("Notification Log", {"subject": n.subject}),
