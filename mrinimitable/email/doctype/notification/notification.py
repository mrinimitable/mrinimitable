...
from mrinimitable.core.doctype.role.role import get_info_based_on_role, get_user_info
from mrinimitable.core.doctype.sms_settings.sms_settings import send_sms
from mrinimitable.desk.doctype.notification_log.notification_log import enqueue_create_notification
from mrinimitable.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
from mrinimitable.model.document import Document
from mrinimitable.modules.utils import export_module_json, get_doc_module
from mrinimitable.utils import add_to_date, cast, now_datetime, nowdate, validate_email_address
...
class Notification(Document):
	...
	if TYPE_CHECKING:
		from mrinimitable.email.doctype.notification_recipient.notification_recipient import NotificationRecipient
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition and not self.filters:
			return _("Yes")
		try:
			doc = mrinimitable.get_cached_doc(self.document_type, preview_document)
			if self.condition_type == "Python":
				context = get_context(doc)
				return _("Yes") if mrinimitable.safe_eval(self.condition, eval_locals=context) else _("No")
	...
	def preview_message(self, preview_document):
		try:
			...
			msg = mrinimitable.render_template(self.message, context)
			if self.channel == "SMS":
				return mrinimitable.utils.strip_html_tags(msg)
			return msg
		except Exception as e:
	...
	def preview_subject(self, preview_document):
		try:
			...
			if not self.subject:
				return _("No subject")
			if "{" in self.subject:
				return mrinimitable.render_template(self.subject, context)
			return self.subject
		except Exception as e:
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			mrinimitable.throw(_("Please specify which date field must be checked"))

		if self.event in ("Minutes Before", "Minutes After"):
			if not self.datetime_changed:
				mrinimitable.throw(_("Please specify which datetime field must be checked"))
			if not self.minutes_offset:
				mrinimitable.throw(_("Please specify the minutes offset"))
		...
		if self.event == "Value Change" and not self.value_changed:
			mrinimitable.throw(_("Please specify which value field must be checked"))
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not mrinimitable.conf.developer_mode:
			mrinimitable.throw(
	...
	def validate_condition(self):
		...
		temp_doc = mrinimitable.new_doc(self.document_type)
		try:
			mrinimitable.safe_eval(self.condition, None, get_context(temp_doc.as_dict()))
		except Exception:
			mrinimitable.throw(_("The Condition '{0}' is invalid").format(self.condition))
	...
	def validate_filters(self):
		...
		dummy_doc = mrinimitable.new_doc(self.document_type)
	...
	def validate_forbidden_document_types(self):
		if self.document_type in FORBIDDEN_DOCUMENT_TYPES or (
			mrinimitable.get_meta(self.document_type).istable and self.event not in DATE_BASED_EVENTS
		):
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = mrinimitable.get_lazy_doc(self.document_type, d.name)

			if (
				self.condition_type == "Python"
				and self.condition
				and not mrinimitable.safe_eval(self.condition, None, get_context(doc))
			):
				continue
	...
	def queue_send(self, doc, enqueue_after_commit=True):
		...
		Example:
		              ...
		              ```python
		              notification = mrinimitable.get_doc("Notification", "My Notification", ignore_permissions=True)
		...
		return enqueue(
			"mrinimitable.email.doctype.notification.notification.evaluate_alert",
			doc=doc,
			alert=self,
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in mrinimitable.model.numeric_fieldtypes:
						value = mrinimitable.utils.cint(value)
	...
	def create_system_notification(self, doc, context):
		...
		if "{" in subject:
			subject = mrinimitable.render_template(self.subject, context)
		...
		notification_doc = {
			...
			"email_content": mrinimitable.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from mrinimitable.core.doctype.communication.email import _make as make_communication
		...
		sender = None
		message = mrinimitable.render_template(self.message, context)
		...
		if doc.doctype != "Communication":
			...
			comm = mrinimitable.get_lazy_doc("Communication", communication)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=mrinimitable.utils.strip_html_tags(mrinimitable.render_template(self.message, context)),
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not mrinimitable.safe_eval(recipient.condition, None, context):
					continue
	...
	def get_attachment(self, doc):
		...
		print_settings = mrinimitable.get_doc("Print Settings", "Print Settings")
			...
			return [
				{
					...
					"lang": mrinimitable.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self, md_as_html=False):
		...
		path = os.path.join(os.path.dirname(module.__file__), mrinimitable.scrub(self.name))
		extension = FORMATS.get(self.message_type, ".md")
		file_path = path + extension
		...
		if extension == ".md":
			return mrinimitable.utils.md_to_html(template)
...
def clear_notification_cache():
	mrinimitable.client_cache.delete_keys("notifications::")


@mrinimitable.whitelist()
def get_documents_for_today(notification):
	notification = mrinimitable.get_doc("Notification", notification)
...
def trigger_notifications(doc, method=None):
	if mrinimitable.flags.in_import or mrinimitable.flags.in_patch:
	...
	if method == "daily":
		...
		for d in doc_list:
			alert = mrinimitable.get_doc("Notification", d.name)
...
def evaluate_alert(doc: Document, alert, event=None):
	...
	try:
		if isinstance(alert, str):
			alert = mrinimitable.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition_type == "Python" and alert.condition:
			if not mrinimitable.safe_eval(alert.condition, None, context):
				return
		...
		if event == "Value Change" and not doc.is_new():
			if not mrinimitable.db.has_column(doc.doctype, alert.value_changed):
		...
		message = _("Error while evaluating Notification {0}. Please fix your template.").format(
			mrinimitable.utils.get_link_to_form("Notification", alert.name)
		)
		mrinimitable.throw(message, title=_("Error in Notification"))
	except Exception as e:
		title = str(e)
		message = mrinimitable.get_traceback(with_context=True)
		mrinimitable.log_error(title=title, message=message)
		msg = f"<details><summary>{title}</summary>{message}</details>"
		mrinimitable.throw(msg, title=_("Error in Notification"))


def get_context(doc):
	Mrinimitable = namedtuple("Mrinimitable", ["mrinimitable"])
	mrinimitable = Mrinimitable(mrinimitable=get_safe_globals().get("mrinimitable"))
	return {
		"doc": doc,
		"nowdate": nowdate,
		"mrinimitable": mrinimitable.mrinimitable,
	}


def get_assignees(doc):
	assignees = []
	assignees = mrinimitable.get_all(
		"ToDo",
...
def get_emails_from_template(template, context):
	...
	emails = mrinimitable.render_template(template, context) if "{" in template else template
