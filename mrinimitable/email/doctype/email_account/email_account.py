...
import mrinimitable
from mrinimitable import _, are_emails_muted, safe_encode
from mrinimitable.desk.doctype.notification_log.notification_log import enqueue_create_notification
from mrinimitable.desk.form import assign_to
from mrinimitable.email.doctype.email_domain.email_domain import EMAIL_DOMAIN_FIELDS
from mrinimitable.email.mrinimitablemail import MrinimitableMail
from mrinimitable.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from mrinimitable.email.smtp import SMTPServer
from mrinimitable.email.utils import get_port
from mrinimitable.model.document import Document
from mrinimitable.utils import cint, comma_or, cstr, parse_addr, validate_email_address
from mrinimitable.utils.background_jobs import enqueue, get_jobs
from mrinimitable.utils.jinja import render_template
...
def cache_email_account(cache_name):
	def decorator_cache_email_account(func):
		...
		def wrapper_cache_email_account(*args, **kwargs):
			if not hasattr(mrinimitable.local, cache_name):
				setattr(mrinimitable.local, cache_name, {})

			cached_accounts = getattr(mrinimitable.local, cache_name)
...
class EmailAccount(Document):
	...
	if TYPE_CHECKING:
		from mrinimitable.email.doctype.imap_folder.imap_folder import IMAPFolder
		...
		mrinimitable_mail_site: DF.Data | None
		...
		service: DF.Literal[
			"", "Mrinimitable Mail", "GMail", "Sendgrid", "SparkPost", "Yahoo Mail", "Outlook.com", "Yandex.Mail"
		]
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				mrinimitable.throw(_("Login Id is required"))
		else:
			self.login_id = None
		...
		if self.service == "Mrinimitable Mail":
			self.use_imap = 0
			self.always_use_account_email_id_as_sender = 1

			if self.auth_method == "Basic" or self.get_oauth_token():
				self.validate_mrinimitable_mail_settings()

		# validate the imap settings
		if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
			mrinimitable.throw(_("You need to set one IMAP folder for {0}").format(mrinimitable.bold(self.email_id)))

		if mrinimitable.local.flags.in_patch or mrinimitable.in_test:
			return
		...
		if (
			not mrinimitable.local.flags.in_install
			and not self.awaiting_password
			and not self.service == "Mrinimitable Mail"
		):
			if validate_oauth or self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					if not use_oauth:
						mrinimitable.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				mrinimitable.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
			...
			for folder in self.imap_folder:
				if folder.append_to:
					...
					if folder.append_to not in valid_doctypes:
						mrinimitable.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))

	@mrinimitable.whitelist()
	def validate_mrinimitable_mail_settings(self):
		if self.service == "Mrinimitable Mail":
			mrinimitable_mail_client = self.get_mrinimitable_mail_client()
			mrinimitable_mail_client.validate()

	def validate_smtp_conn(self):
		if not self.smtp_server:
			mrinimitable.throw(_("SMTP Server is required"))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					mrinimitable.bold(_("Default Incoming")),
					mrinimitable.bold(_("Enable Incoming")),
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					mrinimitable.bold(_("Default Outgoing")),
					mrinimitable.bold(_("Enable Outgoing")),
				)
			)
		if messages:
			if len(messages) == 1:
	...
	def clear_cache(self):
		super().clear_cache()
		mrinimitable.client_cache.delete_value("automatic_linking_email")
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in mrinimitable.get_all("Email Account", filters={field: 1}):
	...
	def get_domain_values(self, domain: str):
		return mrinimitable.db.get_value("Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True)
	...
	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		...
		if not args.get("host"):
			mrinimitable.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(mrinimitable._dict(args))
	...
	def check_email_server_connection(self, email_server, in_receive):
		...
		try:
			...
			if in_receive and any(map(lambda t: t in message, all_error_codes)):
				...
				mrinimitable.throw(cstr(e))

		except OSError:
			if in_receive:
				# timeout while connecting, see receive.py connect method
				description = mrinimitable.message_log.pop() if mrinimitable.message_log else "Socket Error"
	...
	@property
	def _password(self):
		raise_exception = not (self.auth_method == "OAuth" or self.no_smtp_authentication or mrinimitable.in_test)
	...
	def from_record(cls, record):
		email_account = mrinimitable.new_doc(cls.DOCTYPE)
		email_account.update(record)
		return email_account

	@classmethod
	def find(cls, name):
		return mrinimitable.get_doc(cls.DOCTYPE, name)

	@classmethod
	def find_one_by_filters(cls, **kwargs) -> "EmailAccount":
		name = mrinimitable.db.get_value(cls.DOCTYPE, kwargs)
	...
	def find_outgoing(cls, match_by_email=None, match_by_doctype=None, _raise_error=False):
		...
		if _raise_error:
			mrinimitable.throw(
				...
				mrinimitable.OutgoingEmailError,
			)
	...
	def get_account_details_from_site_config(cls):
		if not mrinimitable.conf.get("mail_server"):
			return {}
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "Mrinimitable"},
		...
		for doc_field_name, d in field_to_conf_name_map.items():
			...
			value = [mrinimitable.conf.get(k) for k in conf_names if mrinimitable.conf.get(k)]
	...
	def get_mrinimitable_mail_client(self):
		return self._mrinimitable_mail_client

	@functools.cached_property
	def _mrinimitable_mail_client(self):
		if self.auth_method == "OAuth":
			if access_token := self.get_access_token():
				return MrinimitableMail(self.mrinimitable_mail_site, self.email_id, access_token=access_token)

			mrinimitable.throw(
				_("Please Authorize OAuth for Email Account {0}").format(
					mrinimitable.bold(self.email_account_name)
				),
				title=_("Mrinimitable Mail OAuth Error"),
			)
		else:
			return MrinimitableMail(
				self.mrinimitable_mail_site, self.email_id, self.api_key, self.get_password("api_secret")
			)
	...
	def handle_incoming_connect_error(self, description):
		if self.get_failed_attempts_count() > 5:
			...
			mrinimitable.enqueue(self._disable_broken_incoming_account, description=description)
		else:
	...
	def get_failed_attempts_count(self):
		return cint(mrinimitable.cache.get_value(f"{self.name}:email-account-failed-attempts"))
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				try:
					...
					if self.use_imap:
						self.handle_bad_emails(mail.uid, mail.raw_message, mrinimitable.get_traceback())
					exceptions.append(mrinimitable.get_traceback())
				except Exception:
					mrinimitable.db.rollback()
				else:
					mrinimitable.db.commit()
		...
		if exceptions:
			raise Exception(mrinimitable.as_json(exceptions))
	...
	def get_inbound_mails(self) -> list[InboundMail]:
		...
		try:
			if self.service == "Mrinimitable Mail":
				mrinimitable_mail_client = self.get_mrinimitable_mail_client()
				messages = mrinimitable_mail_client.pull_raw(last_received_at=self.last_received_at)
	...
	def handle_bad_emails(self, uid, raw, reason):
		...
		if cint(self.use_imap):
			...
			unhandled_email = mrinimitable.get_doc(
				{
					"raw": raw_str,
					"uid": uid,
					"reason": reason,
	...
	def send_auto_reply(self, communication, email):
		...
		from mrinimitable.core.doctype.communication.email import (
		...
		if self.enable_auto_reply:
			...
			mrinimitable.sendmail(
				...
				or mrinimitable.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		Communication = mrinimitable.qb.DocType("Communication")
		mrinimitable.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def after_rename(self, old, new, merge=False):
		mrinimitable.db.set_value("Email Account", new, "email_account_name", new)
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				mrinimitable.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if mrinimitable.db.exists(
				"Email Account",
				{"enable_automatic_linking": 1, "name": ("!=", self.name)},
			):
				mrinimitable.throw(_("Automatic Linking can be activated only for one Email Account."))
	...
	def get_oauth_token(self):
		if self.auth_method == "OAuth":
			connected_app = mrinimitable.get_doc("Connected App", self.connected_app)
...
def get_append_to(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	...
	email_append_to_list = [
		dt.name for dt in mrinimitable.get_all("DocType", filters=filters, fields=["name", "email_append_to"])
	]
	# Set Email Append To DocTypes set via Customize Form
	email_append_to_list.extend(
		dt.doc_type
		for dt in mrinimitable.get_list(
			"Property Setter",
...
def notify_unreplied():
	...
	for email_account in mrinimitable.get_all(
		"Email Account",
		"name",
		...
		if append_to:
			...
			for comm in mrinimitable.get_all(
				"Communication",
				"name",
				filters=[
				...
				comm = mrinimitable.get_doc("Communication", comm.name)

				if mrinimitable.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	from mrinimitable.integrations.doctype.connected_app.connected_app import has_token

	doctype = mrinimitable.qb.DocType("Email Account")
	email_accounts = (
	...
	for email_account in email_accounts:
		...
		if now:
			...
			queued_jobs = get_jobs(site=mrinimitable.local.site, key="job_name")[mrinimitable.local.site]
...
def pull_emails(email_account: str) -> None:
	...
	mrinimitable.has_permission("Email Account", "read", throw=True)
	...
	if job_name not in queued_jobs:
		...
		mrinimitable.msgprint(_("Emails are already being pulled from this account."))


def pull_from_email_account(email_account):
	"""Runs within a worker process"""
	email_account = mrinimitable.get_doc("Email Account", email_account)
...
def get_max_email_uid(email_account):
	...
	if result := mrinimitable.get_all(
		"Communication",
		filters={
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing, used_oauth):
	...
	from mrinimitable.core.doctype.user.user import ask_pass_update
	...
	user_names = mrinimitable.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		UserEmail = mrinimitable.qb.DocType("User Email")
		mrinimitable.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
		...
		users = " and ".join([mrinimitable.bold(user.get("name")) for user in user_names])
		mrinimitable.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def remove_user_email_inbox(email_account):
	...
	for user in users:
		doc = mrinimitable.get_doc("User", user.get("name"))
...
def set_email_password(email_account, password):
	account = mrinimitable.get_doc("Email Account", email_account)
...
def get_automatic_email_link():
	...
	return mrinimitable.client_cache.get_value("automatic_linking_email", generator=check_db)
