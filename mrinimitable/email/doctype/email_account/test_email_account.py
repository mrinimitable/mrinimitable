...
from mrinimitable.email.doctype.email_account.email_account import notify_unreplied
from mrinimitable.email.email_body import get_message_id
from mrinimitable.email.receive import Email, InboundMail, SentEmailInInboxError
...
class TestEmailAccount(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		email_account = mrinimitable.get_doc("Email Account", "_Test Email Account 1")
	...
	def setUp(self):
		...
		mrinimitable.flags.sent_mail = None
		mrinimitable.db.delete("Email Queue")
		mrinimitable.db.delete("Unhandled Email")
	...
	def test_incoming(self):
		...
		comm = mrinimitable.get_doc("Communication", {"sender": "test_sender@example.com"})
		self.assertTrue("test_receiver@example.com" in comm.recipients)
		# check if todo is created
		self.assertTrue(mrinimitable.db.get_value(comm.reference_doctype, comm.reference_name, "name"))

	def test_unread_notification(self):
		todo = mrinimitable.get_last_doc("ToDo")

		comm = mrinimitable.new_doc(
			"Communication",
	...
	def test_incoming_with_attach(self):
		...
		existing_file = mrinimitable.get_doc({"doctype": "File", "file_name": "okayblue-conf-14.png"})
		mrinimitable.delete_doc("File", existing_file.name)
	...
	def test_sendmail(self):
		...
		sent_mail = email.message_from_string(mrinimitable.safe_decode(mrinimitable.flags.sent_mail))
	...
	def test_threading(self):
		...
		sent_name = make(
			...
			doctype="ToDo",
			name=mrinimitable.get_last_doc("ToDo").name,
			send_email=True,
		)["name"]

		sent_mail = email.message_from_string(mrinimitable.get_last_doc("Email Queue").message)
		...
		sent = mrinimitable.get_doc("Communication", sent_name)
	...
	def test_threading_by_subject(self):
		...
		comm_list = mrinimitable.get_all(
			"Communication",
	...
	def test_threading_by_message_id(self):
		...
		event = mrinimitable.get_doc(doctype="Event", subject="test-message").insert()
		...
		last_mail = mrinimitable.get_doc("Email Queue", dict(reference_name=event.name))
	...
	def test_handle_bad_emails(self):
		...
		self.assertTrue(mrinimitable.db.get_value("Unhandled Email", {"message_id": message_id}))
	...
	def test_handle_bad_encoding(self):
		...
		self.assertTrue(mrinimitable.db.get_value("Unhandled Email", {"uid": uid}))
	...
	def test_append_to(self):
		...
		self.assertTrue(mrinimitable.db.exists(communication.reference_doctype, communication.reference_name))
	...
	def test_append_to_with_imap_folders(self):
		...
		for mail in mails:
			...
			if mail.append_to == "ToDo":
				...
				self.assertTrue(communication.reference_name)
				self.assertTrue(
					mrinimitable.db.exists(communication.reference_doctype, communication.reference_name)
				)
			else:
				test_folder_mails += 1
	...
	@patch("mrinimitable.email.receive.EmailServer.select_imap_folder", return_value=True)
	@patch("mrinimitable.email.receive.EmailServer.logout", side_effect=lambda: None)
	def mocked_get_inbound_mails(
		email_account, messages=None, mocked_logout=None, mocked_select_imap_folder=None
	):
		from mrinimitable.email.receive import EmailServer
...
class TestInboundMail(IntegrationTestCase):
	...
	def test_find_parent_communication_from_header(self):
		...
		communication = self.new_communication()
		mail_content = self.get_test_mail(fname="reply-4.raw").replace(
			"{{ message_id }}", f"<{communication.name}@{mrinimitable.local.site}>"
		)
...
def cleanup(sender=None):
	...
	names = mrinimitable.get_list("Communication", filters=filters, fields=["name"])
	for name in names:
		mrinimitable.delete_doc_if_exists("Communication", name.name)
		mrinimitable.delete_doc_if_exists("Communication Link", {"parent": name.name})
