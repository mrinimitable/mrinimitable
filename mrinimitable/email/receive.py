...
import mrinimitable
from mrinimitable import _, safe_decode, safe_encode
from mrinimitable.core.doctype.file.exceptions import MaxFileSizeReachedError
from mrinimitable.core.doctype.file.utils import get_random_filename
...
from mrinimitable.utils.user import is_system_user
...
class EmailSizeExceededError(mrinimitable.ValidationError):
	pass


class LoginLimitExceeded(mrinimitable.ValidationError):
	pass
...
class EmailServer:
	...
	def __init__(self, args=None):
		self.retry_limit = 3
		self.retry_count = 0
		self.settings = args or mrinimitable._dict()
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = imaplib.IMAP4_SSL(
					...
					timeout=mrinimitable.conf.pop_timeout,
					ssl_context=ssl.create_default_context(),
				)
			else:
				self.imap = imaplib.IMAP4(
					self.settings.host, self.settings.incoming_port, timeout=mrinimitable.conf.pop_timeout
				)
			...
			mrinimitable.msgprint(_("Invalid Mail Server. Please rectify and try again."))
			raise
	...
	def connect_pop(self):
		...
		try:
			...
			mrinimitable.log_error("POP: Unable to connect")
			...
			if self.is_temporary_system_problem(e):
				...
				mrinimitable.msgprint(_("Invalid User Name or Support Password. Please rectify and try again."))
				raise
	...
	def check_imap_uidvalidity(self, folder):
		...
		mrinimitable.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)

		if uid_validity is None:
			mrinimitable.flags.initial_sync = True
	...
	def retrieve_message(self, uid, msg_num, folder):
		try:
			...
			mrinimitable.log_error("Unable to fetch email", self.make_error_msg(uid, msg_num))
	...
	def get_email_seen_status(self, uid, flag_string):
		...
		for flag in imaplib.ParseFlags(flag_string) or []:
			match = WORDS_PATTERN.search(mrinimitable.as_unicode(flag))
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not mrinimitable.db.get_value(
	...
	def decode_email(email: bytes | str | None) -> str | None:
		if not email:
			return
		email = mrinimitable.as_unicode(email)
		try:
	...
	def save_attachments_in_doc(self, doc):
		...
		for attachment in self.attachments:
			try:
				...
				pass
			except mrinimitable.FileAlreadyAttachedException:
				pass
...
class InboundMail(Email):
	...
	def _build_communication_doc(self):
		...
		communication = mrinimitable.get_doc(data)
	...
	def is_reply_to_system_sent_mail(self):
		...
		return self.is_reply() and mrinimitable.local.site in self.in_reply_to
	...
	def match_record_by_subject_and_sender(self, doctype):
		...
		3. Find by matching subject alone (Special case)
		        ...
		        the system user is replying to via the common email account in Mrinimitable. This fix bypasses
		...
		if not record:
			...
			name = mrinimitable.db.get_value(self.email_account.append_to, filters=filters)
			record = self.get_doc(doctype, name, ignore_error=True) if name else None
		return record

	def _create_reference_document(self, doctype):
		"""Create reference document if it does not exist in the system."""
		parent = mrinimitable.new_doc(doctype)
		email_fields = self.get_email_fields(doctype)

		if email_fields.subject_field:
			parent.set(email_fields.subject_field, mrinimitable.as_unicode(self.subject)[:140])

		if email_fields.sender_field:
			parent.set(email_fields.sender_field, mrinimitable.as_unicode(self.from_email))

		if email_fields.sender_name_field:
			parent.set(email_fields.sender_name_field, mrinimitable.as_unicode(self.from_real_name))
		...
		try:
			...
			return mrinimitable.db.get_value(doctype, {email_fields.sender_field: self.from_email})

	@staticmethod
	def get_doc(doctype, docname, ignore_error=False):
		try:
			return mrinimitable.get_doc(doctype, docname)
	...
	def get_users_linked_to_account(email_account):
		...
		users = mrinimitable.get_all("User Email", filters={"email_account": email_account.name}, fields=["parent"])
	...
	def clean_subject(subject):
		...
		return mrinimitable.as_unicode(strip(re.sub(regex, "", subject, count=0, flags=re.IGNORECASE)))

	@staticmethod
	def get_email_fields(doctype):
		"""Return Email related fields of a doctype."""
		fields = mrinimitable._dict()
	...
	def get_document(self, doctype, name):
		"""Is same as mrinimitable.get_doc but suppresses the DoesNotExist error."""
		try:
			return mrinimitable.get_doc(doctype, name)
		except mrinimitable.DoesNotExistError:
			return None
