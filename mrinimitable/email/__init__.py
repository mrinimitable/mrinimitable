...
def sendmail_to_system_managers(subject, content):
	mrinimitable.sendmail(recipients=get_system_managers(), subject=subject, content=content)
...
def get_contact_list(txt, page_length=20, extra_filters: str | None = None) -> list[dict]:
	...
	if extra_filters:
		extra_filters = mrinimitable.parse_json(extra_filters)
	...
	contacts = mrinimitable.get_list(
		"Contact",
	...
	# https://github.com/mrinimitable/mrinimitable/blob/6c6a89bcdd9454060a1333e23b855d0505c9ebc2/mrinimitable/public/js/mrinimitable/form/controls/autocomplete.js#L29-L35
	return [
		mrinimitable._dict(
			value=d.email_id,
			label=d.email_id,
...
def get_system_managers():
	return mrinimitable.db.sql_list(
...
def get_communication_doctype(doctype, txt, searchfield, start, page_len, filters):
	user_perms = mrinimitable.utils.user.UserPermissions(mrinimitable.session.user)
	...
	com_doctypes = []
	if len(txt) < 2:
		for name in mrinimitable.get_hooks("communication_doctypes"):
			try:
		...
		com_doctypes = [
			d[0] for d in mrinimitable.db.get_values("DocType", {"issingle": 0, "istable": 0, "hide_toolbar": 0})
		]
...
def sendmail(
	...
	if as_markdown:
		from mrinimitable.utils import md_to_html
