...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "mrinimitable", "dist", type)
	...
	assets_json = mrinimitable.read_file("assets/assets.json")
	if assets_json:
		assets_json = mrinimitable.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/mrinimitable"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["mrinimitable/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="mrinimitable")


def get_assets_link(mrinimitable_head) -> str:
	import requests

	tag = getoutput(
		r"cd ../apps/mrinimitable && git show-ref --tags -d | grep {} | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{{}}//'".format(mrinimitable_head)
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/mrinimitable/mrinimitable/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.mrinimitableframework.com/{mrinimitable_head}.tar.gz"

	if not requests.head(url):
		reference = f"Release {tag}" if tag else f"Commit {mrinimitable_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, mrinimitable_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="mrinimitable-assets-", suffix=mrinimitable_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded Mrinimitable assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./mrinimitable-shashi/sites", "")
...
def download_mrinimitable_assets(verbose=True) -> bool:
	"""Download and set up Mrinimitable assets if they exist based on the current commit HEAD.
	Return True if correctly setup else return False.
	"""
	mrinimitable_head = getoutput("cd ../apps/mrinimitable && git rev-parse HEAD")

	if not mrinimitable_head:
		return False

	try:
		url = get_assets_link(mrinimitable_head)
		assets_archive = fetch_assets(url, mrinimitable_head)
...
def symlink(target, link_name, overwrite=False):
	...
	while True:
		temp_link_name = f"tmp{mrinimitable.generate_hash()}"
...
def setup():
	...
	for app in mrinimitable.get_all_apps(True):
		try:
			pymodules.append(mrinimitable.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(mrinimitable.local.sites_path, "assets")
...
def bundle(
	...
	if skip_mrinimitable:
		command += " --skip_mrinimitable"
	...
	mrinimitable_app_path = mrinimitable.get_app_source_path("mrinimitable")
	mrinimitable.commands.popen(command, cwd=mrinimitable_app_path, env=get_node_env(), raise_err=True)
...
def watch(apps=None):
	...
	live_reload = mrinimitable.utils.cint(os.environ.get("LIVE_RELOAD", mrinimitable.conf.live_reload))
	...
	check_node_executable()
	mrinimitable_app_path = mrinimitable.get_app_source_path("mrinimitable")
	mrinimitable.commands.popen(command, cwd=mrinimitable_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in mrinimitable.get_all_apps():
		app_doc_path = None

		pymodule = mrinimitable.get_module(app_name)
...
def html_to_js_template(path, content):
	"""Return HTML template content as Javascript code, by adding it to `mrinimitable.templates`."""
	return """mrinimitable.templates["{key}"] = '{content}';\n""".format(
