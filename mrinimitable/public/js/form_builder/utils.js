...
export async function load_doctype_model(doctype) {
	await mrinimitable.call("mrinimitable.desk.form.load.getdoctype", { doctype });
}

export async function get_table_columns(df, child_doctype) {
	let table_columns = [];

	if (!mrinimitable.get_meta(df.options)) {
		await load_doctype_model(df.options);
	}
	if (!child_doctype) {
		child_doctype = mrinimitable.get_meta(df.options);
	}
	...
	for (let tf of table_fields) {
		if (!mrinimitable.model.layout_fields.includes(tf.fieldtype) && tf.in_list_view && tf.label) {
			let colsize;
	...
	function adjust_column_size(total_colsize, increase) {
		...
		while (start_condition && passes < 11) {
			for (var i in table_columns) {
				...
				if (colsize > 1 && colsize < 11 && mrinimitable.model.is_non_std_field(_df.fieldname)) {
					if (
						passes < 3 &&
		...
		if (d.fieldtype) {
			if (!d.fieldname) {
				if (d.label) {
					...
					if (mrinimitable.model.restricted_fields.includes(d.fieldname)) {
					...
					d.fieldname =
						...
						"_" +
						mrinimitable.utils.get_random(4);
				}
			} else {
				if (mrinimitable.model.restricted_fields.includes(d.fieldname)) {
					mrinimitable.throw(__("Fieldname {0} is restricted", [d.fieldname]));
				}
			}
...
export function clone_field(field) {
	...
	cloned_field.df.name = mrinimitable.utils.get_random(8);
	return cloned_field;
}
