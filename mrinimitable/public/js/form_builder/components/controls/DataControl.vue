...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = mrinimitable.boot.time_zone
		? mrinimitable.boot.time_zone.user
		: mrinimitable.sys_defaults.time_zone;
...
<template>
	<div class="control mrinimitable-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="mrinimitable.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="mrinimitable.utils.icon('down', 'sm')"
		/>
	</div>
</template>
