...
function remove_field() {
	if (store.is_customize_form && props.field.df.is_custom_field == 0) {
		mrinimitable.msgprint(__("Cannot delete standard field. You can hide it if you want"));
...
function duplicate_field() {
	...
	duplicate_field.df.owner = mrinimitable.session.user;
...
function make_dialog(frm) {
	frm.dialog = new mrinimitable.ui.Dialog({
	...
	if (frm.doctype === "Customize Form") {
		...
		mrinimitable.db.exists("Property Setter", property_setter_id).then((exits) => {
			if (exits) {
				frm.dialog.set_secondary_action_label(__("Reset To Default"));
				frm.dialog.set_secondary_action(() => {
					mrinimitable.call({
						method: "mrinimitable.custom.doctype.customize_form.customize_form.get_link_filters_from_doc_without_customisations",
						args: {
...
function edit_filters() {
	...
	mrinimitable.model.with_doctype(field_doctype, () => {
		frm.dialog.show();
		add_existing_filter(frm, props.field.df);
	});
}
			...
			<template #label>
				<div class="field-label">
					...
					<div
						...
						v-html="mrinimitable.utils.icon('help', 'sm')"
					/>
				</div>
			</template>
			<template #actions>
				<div class="field-actions" :hidden="store.read_only">
					<button
						...
						@click="edit_filters"
					>
						<div v-html="mrinimitable.utils.icon('filter', 'sm')" />
					</button>
					<AddFieldButton ref="add_field_ref" :column="column" :field="field">
						<div v-html="mrinimitable.utils.icon('add', 'sm')" />
					</AddFieldButton>
					<button
					...
					>
						<div v-html="mrinimitable.utils.icon('move', 'sm')" />
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="mrinimitable.utils.icon('duplicate', 'sm')" />
					</button>
					<button
					...
					>
						<div v-html="mrinimitable.utils.icon('external-link', 'sm')" />
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="mrinimitable.utils.icon('remove', 'sm')" />
					</button>
				</div>
			</template>
		</component>
