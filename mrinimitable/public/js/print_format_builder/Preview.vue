...
function get_default_docname() {
	return mrinimitable.db.get_list(doctype.value, { limit: 1 }).then((doc) => {
...
let url = computed(() => {
	...
	let _url =
		type.value == "PDF" ? `/api/method/mrinimitable.utils.weasyprint.download_pdf` : "/printpreview";
	return `${_url}?${params.toString()}`;
});

// mounted
onMounted(() => {
	doc_select.value = mrinimitable.ui.form.make_control({
	...
	preview_type.value = mrinimitable.ui.form.make_control({
