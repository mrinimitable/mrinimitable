...
function toggle_edit_letterhead() {
	...
	if (!control.value) {
		control.value = mrinimitable.ui.form.make_control({
			parent: editor.value,
			df: {
...
// mounted
onMounted(() => {
	if (!letterhead.value && mrinimitable.boot.sysdefaults.letter_head) {
		set_letterhead(mrinimitable.boot.sysdefaults.letter_head);
	}
