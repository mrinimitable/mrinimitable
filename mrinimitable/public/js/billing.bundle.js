let mrinimitableCloudBaseEndpoint = "https://mrinimitablecloud.com";
let isFCUser = false;

$(document).ready(function () {
	if (
		mrinimitable.boot.is_fc_site &&
		mrinimitable.boot.setup_complete === 1 &&
		!mrinimitable.is_mobile() &&
		mrinimitable.user.has_role("System Manager")
	) {
		mrinimitable.call({
			method: "mrinimitable.integrations.mrinimitable_providers.mrinimitablecloud_billing.current_site_info",
			callback: (r) => {
				if (!r?.message) return;

				const response = r.message;
				const trial_end_date = new Date(response.trial_end_date);
				mrinimitableCloudBaseEndpoint = response.base_url;
				...
				$(".login-to-fc, .upgrade-plan-button").on("click", function () {
					openMrinimitableCloudDashboard();
				});
			},
		});
	}
...
function openMrinimitableCloudDashboard() {
	window.open(`${mrinimitableCloudBaseEndpoint}/dashboard/sites/${mrinimitable.boot.sitename}`, "_blank");
}
