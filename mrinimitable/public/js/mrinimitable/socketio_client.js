...
mrinimitable.provide("mrinimitable.realtime");
...
class RealTimeClient {
	...
	init(port = 9000, lazy_connect = false) {
		if (mrinimitable.boot.disable_async) {
			return;
		}
	...
	get_host(port = 9000) {
		...
		if (window.dev_server) {
			...
			port = mrinimitable.boot.socketio_port || port.toString() || "9000";
			if (parts.length > 2) {
				host = parts[0] + ":" + parts[1];
			}
			host = host + ":" + port;
		}
		return host + `/${mrinimitable.boot.sitename}`;
	}
	...
	doc_subscribe(doctype, docname) {
		if (mrinimitable.flags.doc_subscribe) {
		...
		mrinimitable.flags.doc_subscribe = true;

		// throttle to 1 per sec
		setTimeout(function () {
			mrinimitable.flags.doc_subscribe = false;
		}, 1000);
	...
	process_response(data, method) {
		...
		// "callback" is std mrinimitable term
		if (method === "success") {
...
mrinimitable.realtime = new RealTimeClient();

// backward compatibility
mrinimitable.socketio = mrinimitable.realtime;
