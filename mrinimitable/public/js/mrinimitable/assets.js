...
// depends on mrinimitable.versions to manage versioning

mrinimitable.require = function (items, callback) {
	if (typeof items === "string") {
		items = [items];
	}
	items = items.map((item) => mrinimitable.assets.bundled_asset(item));

	return new Promise((resolve) => {
		mrinimitable.assets.execute(items, () => {
			resolve();
			callback && callback();
		});
	});
...
class AssetManager {
	constructor() {
		...
		this._handlers = {
			js: function (txt) {
				mrinimitable.dom.eval(txt);
			},
			css: function (txt) {
				mrinimitable.dom.set_style(txt);
			},
		};
	}
	check() {
	...
	init_local_storage() {
		...
		if (mrinimitable.boot) localStorage.metadata_version = mrinimitable.boot.metadata_version;
	}
	...
	execute(items, callback) {
		...
		const version_string =
			mrinimitable.boot.developer_mode || window.dev_server ? Date.now() : window._version_number;
...
mrinimitable.assets = new AssetManager();
