	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="mrinimitable.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && mrinimitable.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: mrinimitable.utils.icon("folder-open", "md"),
		closed: mrinimitable.utils.icon("folder-normal", "md"),
		leaf: mrinimitable.utils.icon("primitive-dot", "xs"),
		search: mrinimitable.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return mrinimitable.utils.get_form_link("File", filename);
};
