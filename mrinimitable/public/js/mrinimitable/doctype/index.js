...
*/
mrinimitable.model.DocTypeController = class DocTypeController extends mrinimitable.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		mrinimitable.meta.docfield_map[
	...
	show_db_utilization() {
		...
		mrinimitable
			.xcall("mrinimitable.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = mrinimitable.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			mrinimitable.model.with_doctype(link_doctype, () => {
				let fields = mrinimitable.meta
