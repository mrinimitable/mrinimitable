...
mrinimitable.standard_pages["Workspaces"] = function () {
	var wrapper = mrinimitable.container.add_page("Workspaces");
	...
	mrinimitable.workspace = new mrinimitable.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		mrinimitable.workspace.show();
	});
};

mrinimitable.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = mrinimitable.workspace_block.blocks;
		...
		this.sidebar = mrinimitable.app.sidebar;
		this.app_switcher_menu = mrinimitable.app.app_switcher_menu;
		this.sidebar.setup_pages();
		this.cached_pages = $.extend(true, {}, mrinimitable.boot.sidebar_pages);
		this.has_access = mrinimitable.boot.sidebar_pages.has_access;
		this.has_create_access = mrinimitable.boot.sidebar_pages.has_create_access;
		this.show();
	...
	`).appendTo(this.body);
		...
		this.body.find(".btn-edit-workspace").on("click", async () => {
			...
			this.editor.isReady.then(() => {
				...
				mrinimitable.app.sidebar.toggle_sorting();
			});
		});
	}

	get_pages() {
		return mrinimitable.xcall("mrinimitable.desk.desktop.get_workspace_sidebar_items", null, "GET");
	}
	...
	show() {
		...
		if (!mrinimitable.router.current_route[0]) {
			mrinimitable.route_flags.replace_route = true;
			mrinimitable.set_route(mrinimitable.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return mrinimitable
			.call({
				method: "mrinimitable.desk.desktop.get_desktop_page",
				args: {
			...
			.then((data) => {
				...
				return mrinimitable.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		if (mrinimitable.boot.user.default_workspace) {
			default_page = {
				name: mrinimitable.boot.user.default_workspace.name,
				public: mrinimitable.boot.user.default_workspace.public,
			};
		} else if (
		...
		if (this.sidebar.all_pages.length) {
			...
			if (!this._page.public) {
				...
				if (!app && this._page.module) {
					app = mrinimitable.boot.module_app[mrinimitable.router.slug(this._page.module)];
				}
				if (!app) app = "mrinimitable";
			}
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await mrinimitable.after_ajax(() => this.get_data(current_page));
			}
			...
			this.wrapper
				.find(".workspace-icon")
				.html(mrinimitable.utils.icon(this._page.icon || "folder-normal", "md"));
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			await this.editor.readOnly.toggle();
			this.is_read_only = true;
			mrinimitable.boot.sidebar_pages = this.cached_pages;
			this.reload();
			mrinimitable.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				mrinimitable.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	initialize_new_page() {
		...
		const d = new mrinimitable.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + mrinimitable.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					for_user: values.is_public ? "" : mrinimitable.session.user,
					icon: values.icon,
					indicator_color: values.indicator_color,
					parent_page: values.parent || "",
					is_editable: true,
					selected: true,
					app: mrinimitable.current_app,
					type: values.type,
					...
					this.editor
						...
						.then(async () => {
							...
							this.create_page(new_page).then(() => {
								...
								let route = pre_url + mrinimitable.router.slug(new_page.title);
	...
	create_page(new_page) {
		return new Promise((resolve) => {
			mrinimitable.call({
				method: "mrinimitable.desk.doctype.workspace.workspace.new_page",
				args: {
					new_page: new_page,
				},
				callback: (r) => {
					if (r.message) {
						...
						mrinimitable.boot.sidebar_pages = r.message;
						this.sidebar.setup_pages();

						if (!mrinimitable.boot.app_data_map["private"] && new_page.public === 0) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: mrinimitable.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				mrinimitable.call({
					method: "mrinimitable.desk.doctype.workspace.workspace.save_page",
					args: {
						name: page.name,
	...
	reload() {
		...
		return this.get_pages().then((r) => {
			mrinimitable.boot.sidebar_pages = r;
			this.sidebar.setup_pages();
			this.show();
	...
	create_page_skeleton() {
		...
		this.body.prepend(mrinimitable.render_template("workspace_loading_skeleton"));
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			mrinimitable.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			mrinimitable.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};
