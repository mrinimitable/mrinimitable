...
export default class Block {
	...
	make(block, block_name, widget_type = block) {
		let block_data = this.config.page_data[block + "s"].items.find((obj) => {
			return (
				mrinimitable.utils.unescape_html(obj.label) == mrinimitable.utils.unescape_html(__(block_name))
			);
		});
		if (!block_data) return false;
		this.wrapper.innerHTML = "";
		block_data.in_customize_mode = !this.readOnly;
		this.block_widget = new mrinimitable.widget.SingleWidgetGroup({
	...
	new(block, widget_type = block) {
		...
		this.dialog = new dialog_class({
			...
			primary_action: (widget) => {
				...
				this.block_widget = mrinimitable.widget.make_widget({
					...widget,
	...
	add_new_block_button() {
		let $new_button = $(`
			<div class="new-block-button">${mrinimitable.utils.icon("add-round", "lg")}</div>
		`);
		...
		this.dropdown_list = [
			{
				label: "Delete",
				title: "Delete Block",
				icon: mrinimitable.utils.icon("delete-active", "sm"),
				action: () => this.api.blocks.delete(),
			},
			{
				label: "Expand",
				title: "Expand Block",
				icon: mrinimitable.utils.icon("expand-alt", "sm"),
				action: () => this.increase_width(),
			},
			{
				label: "Shrink",
				title: "Shrink Block",
				icon: mrinimitable.utils.icon("shrink", "sm"),
				action: () => this.decrease_width(),
			},
			{
				label: "Move Up",
				title: "Move Up",
				icon: mrinimitable.utils.icon("up-arrow", "sm"),
				action: () => this.move_block("up"),
			},
			{
				label: "Move Down",
				title: "Move Down",
				icon: mrinimitable.utils.icon("down-arrow", "sm"),
				action: () => this.move_block("down"),
			},
			{
				label: "Duplicate",
				title: "Duplicate",
				icon: mrinimitable.utils.icon("copy", "sm"),
		...
		let $button = $(`
			<div class="dropdown-btn">
				<button class="btn btn-secondary btn-xs setting-btn" title="${__("Setting")}">
					${mrinimitable.utils.icon("dot-horizontal", "xs")}
				</button>
