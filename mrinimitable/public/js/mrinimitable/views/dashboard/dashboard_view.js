...
mrinimitable.views.DashboardView = class DashboardView extends mrinimitable.views.ListView {
	get view_name() {
		return "Dashboard";
	}
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.dashboard_settings =
				mrinimitable.get_user_settings(this.doctype)["dashboard_settings"] || null;
		});
	}
	...
	reset_dashboard_customization() {
		mrinimitable.confirm(__("Are you sure you want to reset all customizations?"), () => {
			this.dashboard_settings = null;
			mrinimitable.model.user_settings
		...
		const dialog = new mrinimitable.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							query: "mrinimitable.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user",
							filters: {
								document_type: this.doctype,
							},
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == "New Chart") {
					...
					mrinimitable
						.xcall(
							"mrinimitable.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart",
							{ args: chart }
						)
						.then((doc) => {
	...
	get_field_options() {
		...
		mrinimitable.get_meta(this.doctype).fields.map((df) => {
