...
mrinimitable.last_edited_communication = {};
const separator_element = "<div>---</div>";

mrinimitable.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	get_fields() {
		...
		const fields = [
			...
			{
				fieldtype: "Button",
				label: mrinimitable.utils.icon("down", "xs"),
			...
			},
			{
				...
				fieldname: "content",
				onchange: mrinimitable.utils.debounce(this.save_as_draft.bind(this), 300),
			},
			{
				fieldtype: "Button",
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: mrinimitable.boot.user.send_me_a_copy,
			},
			{
			...
			},
			{
				...
				default: mrinimitable.boot.lang,
		...
		const email_accounts = mrinimitable.boot.email_accounts.filter((account) => {
			return (
		...
		if (email_accounts.length) {
			...
			} else if (this.user_email_accounts.includes(mrinimitable.session.user_email)) {
				this["sender"] = mrinimitable.session.user_email;
			}
		}
	...
	guess_language() {
		...
		if (print_format != "Standard") {
			print_format_lang = mrinimitable.get_doc(
				"Print Format",
				print_format
			)?.default_print_language;
		}

		let lang = document_lang || print_format_lang || mrinimitable.boot.lang;
	...
	toggle_more_options(show_options) {
		...
		const label = mrinimitable.utils.icon(show_options ? "up-line" : "down", "xs");
	...
	setup_multiselect_queries() {
		["recipients", "cc", "bcc"].forEach((field) => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				mrinimitable.call({
					method: "mrinimitable.email.get_contact_list",
					args: args,
					callback: (r) => {
	...
	setup_email_template() {
		...
		function add_template() {
			...
			mrinimitable.call({
				method: "mrinimitable.email.doctype.email_template.email_template.get_email_template",
				args: {
		...
		mrinimitable.utils.add_select_group_button(clear_and_add_template, email_template_actions);
	}
	...
	get_last_edited_communication(clear) {
		if (!mrinimitable.last_edited_communication[this.doctype]) {
			mrinimitable.last_edited_communication[this.doctype] = {};
		}

		if (clear || !mrinimitable.last_edited_communication[this.doctype][this.key]) {
			mrinimitable.last_edited_communication[this.doctype][this.key] = {};
		}

		return mrinimitable.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = this.subject ? mrinimitable.utils.html2text(this.subject) : "";
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = mrinimitable.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		$(`
			...
			<p class='add-more-attachments'>
				<button class='btn btn-xs btn-default'>
					${mrinimitable.utils.icon("small-add", "xs")}&nbsp;
					${__("Add Attachment")}
				</button>
			</p>
		...
		attach
			...
			.on("click", () => new mrinimitable.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = mrinimitable.urllib.get_full_url(f.file_url);
	...
	get_attachment_row(attachment, checked) {
		return $(`<p class="checkbox flex">
			<label title="${attachment.file_name}" style="max-width: 100%">
				...
				>
					${mrinimitable.utils.icon("link-url", "sm")}
				</a>
			</label>
		</p>`);
	}
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on("click", () => {
			...
			mrinimitable.db.set_value("User", mrinimitable.session.user, "send_me_a_copy", val);
			mrinimitable.boot.user.send_me_a_copy = val;
		});
	}
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			mrinimitable.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !mrinimitable.model.can_email(this.doc.doctype, this.frm)) {
			mrinimitable.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}

		return mrinimitable.call({
			method: "mrinimitable.core.doctype.communication.email.make",
			args: {
				...
				sender_full_name: form_values.sender ? mrinimitable.user.full_name() : undefined,
			...
			callback(r) {
				if (!r.exc) {
					mrinimitable.utils.play_sound("email");

					if (r.message["emails_not_sent_to"]) {
						mrinimitable.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								mrinimitable.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					}
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find(".form-print-wrapper").is(":visible")) {
			...
			return (
				mrinimitable.model.get_doc(":Print Settings", "Print Settings") || { with_letterhead: 1 }
			).with_letterhead
				? 1
				: 0;
		}
	...
	async get_signature(sender_email) {
		let signature = mrinimitable.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await mrinimitable.db.get_list("Email Account", {
				filters: filters,
		...
		if (!mrinimitable.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = mrinimitable.datetime.global_date_format(
	...
	html2text(html) {
		...
		const text = mrinimitable.utils.html2text(html);
		return text.replace(/\n{3,}/g, "\n\n");
	}
	...
	sanitize_emails(control) {
		...
		let sanitized = emails
			.split(",")
			.map((email) => mrinimitable.utils.xss_sanitise(email.trim()))
			.join(",");
		if (sanitized != emails) {
