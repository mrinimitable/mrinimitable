/**
 * mrinimitable.views.ImageView
 */
mrinimitable.provide("mrinimitable.views");

mrinimitable.views.ImageView = class ImageView extends mrinimitable.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = mrinimitable.utils.escape_html(title);
		...
		const expand_button_html = item._image_url
			? `<div class="zoom-view" data-name="${encoded_name}">
				${mrinimitable.utils.icon("expand", "xs")}
			</div>`
			: "";
	...
	get_attached_images() {
		return mrinimitable
			.call({
				method: "mrinimitable.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new mrinimitable.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
	...
	get required_libs() {
		return [
			"assets/mrinimitable/node_modules/photoswipe/src/photoswipe.css",
			"photoswipe.bundle.js",
		];
	}
};

mrinimitable.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
		me.prepare();
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = mrinimitable.render_template("photoswipe_dom");
	...
	_show(docname) {
		...
		// init
		this.pswp = new mrinimitable.PhotoSwipe(options);
		this.pswp.init();
	}
};
