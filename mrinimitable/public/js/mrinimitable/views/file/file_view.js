...
mrinimitable.views.FileView = class FileView extends mrinimitable.views.ListView {
	static load_last_view() {
		const route = mrinimitable.get_route();
		if (route.length === 2) {
			const view_user_settings = mrinimitable.get_user_settings("File", "File");
			mrinimitable.set_route(
				"List",
				"File",
				view_user_settings.last_folder || mrinimitable.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			mrinimitable.views.FileView.grid_view = !mrinimitable.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (mrinimitable.views.FileView.grid_view) {
			Object.assign(args, {
		...
		return [
			{
				label: __("Home"),
				action: () => {
					mrinimitable.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					mrinimitable.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								mrinimitable.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							mrinimitable.call({
								method: "mrinimitable.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new mrinimitable.ui.FileUploader({
						...
						on_success: (file) => {
							mrinimitable.show_alert(__("Unzipping files..."));
							mrinimitable
								.call("mrinimitable.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										mrinimitable.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				mrinimitable.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => mrinimitable.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/mrinimitable.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => mrinimitable.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (mrinimitable.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
		...
		d.subject_html = `
			${mrinimitable.utils.icon(icon_class)}
			<span>${title}</span>
	...
	before_render() {
		super.before_render();
		mrinimitable.model.user_settings.save("File", "grid_view", mrinimitable.views.FileView.grid_view);
	...
	get_header_html() {
		...
		let header_selector_html = !mrinimitable.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !mrinimitable.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === mrinimitable.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = mrinimitable.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? mrinimitable.form.formatters.FileSize(file.file_size) : "";
		...
		return `
			<div class="list-row-col ellipsis list-subject level">
				...
				<span class="level-item  ellipsis" title="${mrinimitable.utils.escape_html(file.file_name)}">
					<a class="ellipsis" href="${route_url}" title="${mrinimitable.utils.escape_html(file.file_name)}">
						${file.subject_html}
					</a>
				</span>
			</div>
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					mrinimitable.file_manager.paste(folder_name);
					mrinimitable.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!mrinimitable.file_manager.can_paste ||
			mrinimitable.file_manager.old_folder === this.current_folder;
...
mrinimitable.views.FileView.grid_view = mrinimitable.get_user_settings("File").grid_view || false;
