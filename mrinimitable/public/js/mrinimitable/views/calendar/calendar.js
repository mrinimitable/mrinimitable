...
mrinimitable.provide("mrinimitable.views.calendar");
mrinimitable.provide("mrinimitable.views.calendars");

mrinimitable.views.CalendarView = class CalendarView extends mrinimitable.views.ListView {
	static load_last_view() {
		const route = mrinimitable.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = mrinimitable.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = mrinimitable.views.Calendar[this.doctype] || {};
			this.calendar_name = mrinimitable.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new mrinimitable.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, mrinimitable.views.calendar[this.doctype]);
				resolve(options);
			} else {
				mrinimitable.model.with_doc("Calendar View", calendar_name, () => {
					const doc = mrinimitable.get_doc("Calendar View", calendar_name);
					if (!doc) {
						...
						mrinimitable.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
...
mrinimitable.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(mrinimitable.boot.calendars, function (i, doctype) {
			if (mrinimitable.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					mrinimitable.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = mrinimitable.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
		...
		this.fullCalendar = new mrinimitable.FullCalendar(this.$cal[0], this.cal_options);
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(mrinimitable.utils.icon("left"));
		this.$wrapper
			.find(`.fc-next-button span`)
			.attr("class", "")
			.html(mrinimitable.utils.icon("right"));
		if (this.$wrapper.find(".fc-today-button svg").length == 0)
			this.$wrapper.find(".fc-today-button").prepend(mrinimitable.utils.icon("today"));
	...
	get_system_datetime(date) {
		return mrinimitable.datetime.convert_to_system_tz(date, true);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			plugins: mrinimitable.FullCalendar.Plugins,
			initialView: defaults.initialView || "dayGridMonth",
			locale: mrinimitable.boot.lang,
			firstDay: 1,
			headerToolbar: {
			...
			events: function (info, successCallback, failureCallback) {
				return mrinimitable.call({
					method: me.get_events_method || "mrinimitable.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (info) {
				...
				if (mrinimitable.model.can_read(doctype)) {
					mrinimitable.set_route("Form", doctype, info.event.id);
				}
			},
			...
			select: function (info) {
				...
				var event = mrinimitable.model.get_new_doc(me.doctype);
				...
				mrinimitable.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = mrinimitable.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = mrinimitable.datetime.convert_to_user_tz(d.start);
				d.end = mrinimitable.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!mrinimitable.datetime.validate(d.start) && d.end) {
				d.start = mrinimitable.datetime.add_days(d.end, -1);
			}

			if (d.start && !mrinimitable.datetime.validate(d.end)) {
				d.end = mrinimitable.datetime.add_days(d.start, 1);
			}

			me.prepare_colors(d);

			d.title = mrinimitable.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = mrinimitable.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = mrinimitable.ui.color.get(color_name, "extra-light");
			d.textColor = mrinimitable.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!mrinimitable.ui.color.validate_hex(color) || !color) {
				color = mrinimitable.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = mrinimitable.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		mrinimitable.model.remove_from_locals(me.doctype, event.id);
		return mrinimitable.call({
			method: me.update_event_method || "mrinimitable.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					mrinimitable.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {
