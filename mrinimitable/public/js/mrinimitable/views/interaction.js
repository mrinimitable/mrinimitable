...
mrinimitable.provide("mrinimitable.interaction_settings");

mrinimitable.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		mrinimitable.model.with_doctype("Event", () => {
			let categories = mrinimitable.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new mrinimitable.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = mrinimitable.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = mrinimitable.session.user;
		}
	...
	add_attachments(doc, attachments) {
		mrinimitable.call({
			method: "mrinimitable.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
