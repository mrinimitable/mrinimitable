...
mrinimitable.provide("mrinimitable.widget.utils");
mrinimitable.provide("mrinimitable.views");
mrinimitable.provide("mrinimitable.query_reports");

mrinimitable.standard_pages["query-report"] = function () {
	var wrapper = mrinimitable.container.add_page("query-report");
	...
	mrinimitable.query_report = new mrinimitable.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind("show", function () {
		mrinimitable.query_report.show();
	});
};

mrinimitable.views.QueryReport = class QueryReport extends mrinimitable.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = mrinimitable.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = mrinimitable.utils.throttle(this.refresh, 300);
	...
	update_url_with_filters() {
		if (mrinimitable.get_route_str() == this.page_name) {
	...
	setup_events() {
		mrinimitable.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					mrinimitable.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (mrinimitable.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, mrinimitable.route_options);

		if (this.report_name !== mrinimitable.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (mrinimitable.has_route_options()) {
	...
	add_card_button_to_toolbar() {
		if (!mrinimitable.model.can_create("Number Card")) return;
	...
	add_chart_buttons_to_toolbar(show) {
		if (!mrinimitable.model.can_create("Dashboard Chart")) return;
		if (show) {
	...
	add_card_to_dashboard() {
		let field_options = mrinimitable.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = mrinimitable.dashboard_utils.get_dashboard_link_field();
		const set_standard = mrinimitable.boot.developer_mode;
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			mrinimitable.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"mrinimitable.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || mrinimitable.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"mrinimitable.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		mrinimitable.xcall(method, { args: args }).then(() => {
			let message;
			if (dashboard_name) {
			...
			mrinimitable.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	refresh_report(route_options) {
		...
		return mrinimitable.run_serially([
			() => this.setup_filters(),
	...
	get_report_doc() {
		return mrinimitable.model
			.with_doc("Report", this.report_name)
			.then((doc) => {
				this.report_doc = doc;
			})
			.then(() => mrinimitable.model.with_doctype(this.report_doc?.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (mrinimitable.query_reports[this.report_name]) {
				this.report_settings = mrinimitable.query_reports[this.report_name];
				resolve();
			} else {
				mrinimitable
					.xcall("mrinimitable.desk.query_report.get_script", {
						report_name: this.report_name,
					})
					.then((settings) => {
						mrinimitable.dom.eval(settings.script || "");
						mrinimitable.after_ajax(() => {
							...
							mrinimitable.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings(custom_report_name) {
		let report_script_name =
			this.report_doc.report_type === "Custom Report"
				...
				: this.report_name;
		return mrinimitable.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			mrinimitable.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = mrinimitable.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					mrinimitable.throw(
	...
	add_collapse_button() {
		...
		if (this.filters[filter_no]) {
			this.$collapse_button = $(`<div>${mrinimitable.utils.icon("chevron-down", "md")}</div>`);
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = mrinimitable.route_options;
		...
		if (route_options) {
			...
			return mrinimitable.run_serially(promises);
		}
	}
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			this.last_ajax = mrinimitable.call({
				method: "mrinimitable.desk.query_report.run",
				type: "GET",
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = mrinimitable.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			mrinimitable.utils.build_summary_item(summary).appendTo(this.$summary);
		});
	...
	get_query_params() {
		const query_string = mrinimitable.utils.get_query_string(mrinimitable.get_route_str());
		return mrinimitable.utils.get_query_params(query_string);
	}
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
				...
				function () {
					window.open(
						mrinimitable.urllib.get_full_url(
							"/api/method/mrinimitable.core.doctype.prepared_report.prepared_report.download_attachment?" +
								"dn=" +
			...
			let pretty_diff = mrinimitable.datetime.comment_when(doc.report_end_time);
			const days_old = mrinimitable.datetime.get_day_diff(
				mrinimitable.datetime.now_datetime(),
				doc.report_end_time
			);
			if (days_old > 1) {
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					mrinimitable.set_route(mrinimitable.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		mrinimitable
			.xcall(
				"mrinimitable.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = mrinimitable.warn(
					...
					mrinimitable.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		mrinimitable
			.xcall("mrinimitable.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				mrinimitable.call({
					method: "mrinimitable.core.doctype.prepared_report.prepared_report.make_prepared_report",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
				...
				let alert_message =
					...
					`</a>`;
				mrinimitable.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (data.length > (cint(mrinimitable.boot.sysdefaults.max_report_rows) || 100000)) {
			let msg = __(
			...
			this.toggle_message(true, `${mrinimitable.utils.icon("solid-warning")} ${msg}`);
			return;
		}
		...
		if (
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: mrinimitable.boot.lang,
				translations: mrinimitable.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: "fixed",
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: mrinimitable.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: mrinimitable.utils.report_column_total,
				},
			};
	...
	get_chart_options(data) {
		...
		options.axisOptions = {
			...
			numberFormatter: mrinimitable.utils.format_chart_axis_number,
		};
		options.height = 280;
		return options;
	}
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new mrinimitable.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = mrinimitable.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new mrinimitable.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		...
		return columns.map((column) => {
			column = mrinimitable.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = mrinimitable.datetime.user_to_obj(keyword);
					const cellValue = mrinimitable.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = mrinimitable.get_meta(this.report_doc.ref_doctype);
		mrinimitable.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		mrinimitable.call("mrinimitable.core.doctype.access_log.access_log.make_access_log", {
	...
	async pdf_report(print_settings) {
		const base_url = mrinimitable.urllib.get_base_url();
		const print_css = mrinimitable.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = mrinimitable.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = mrinimitable.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: mrinimitable.boot.lang,
			layout_direction: mrinimitable.utils.is_rtl() ? "rtl" : "ltr",
			can_use_smaller_font: this.report_doc.is_standard === "Yes" && custom_format ? 0 : 1,
		});
		...
		mrinimitable.render_pdf(html, print_settings);
	}
	...
	async get_report_print_format(report_name) {
		...
		const r = await mrinimitable.db.get_value("Print Format", filters, ["html", "css"]);
		if (r && r.message && r.message.html) {
			const css = r.message.css || "";
			const html = r.message.html || "";
			return `<style>${css}</style>${html}`;
		} else {
			mrinimitable.msgprint(__("Print Format not found"));
			return null;
		}
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				const docfield = mrinimitable.query_report.get_filter(fieldname).df;
				const value = applied_filters[fieldname];

				if (mrinimitable.utils.is_empty(value) || docfield.hidden_due_to_dependency) {
					return null;
				}

				return `<div class="filter-row">
					<b>${__(docfield.label, null, docfield.parent)}:</b> ${mrinimitable.format(value, docfield)}
				</div>`;
			})
			.join("");
	}
	...
	export_report() {
		...
		this.export_dialog = mrinimitable.report_utils.get_export_dialog(
			__(this.report_name),
			extra_fields,
			({
				file_format,
				...
				for (const [key, value] of Object.entries(filters)) {
					const df = mrinimitable.query_report.get_filter(key).df;
				...
				const args = {
					cmd: "mrinimitable.desk.query_report.export_query",
					report_name: this.report_name,
				...
				open_url_post(mrinimitable.request.url, args);
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = mrinimitable.utils.get_formatted_duration(cell.content);
				}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __("Edit"),
				action: () => mrinimitable.set_route("Form", "Report", this.report_name),
				condition: () => mrinimitable.user.is_report_manager(),
				standard: true,
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = mrinimitable.ui.get_print_settings(
						false,
				...
				condition: () => mrinimitable.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => mrinimitable.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("Setup Auto Email"),
				action: () =>
					mrinimitable.set_route("List", "Auto Email Report", { report: this.report_name }),
				standard: true,
			},
			{
				label: __("Add Column"),
				action: () => {
					let d = new mrinimitable.ui.Dialog({
						...
						fields: [
							{
								...
								options: this.linked_doctypes?.map((df) => ({
									label: df.doctype + " (" + mrinimitable.unscrub(df.fieldname) + ")",
								...
								change: () => {
									...
									mrinimitable.model.with_doctype(doctype, () => {
										let options = mrinimitable.meta
											.get_docfields(doctype)
											.filter(mrinimitable.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = mrinimitable.meta.get_docfield(values.doctype, values.field);
							...
							custom_columns.push({
								fieldname: this.columns
									...
									? df.fieldname + "-" + mrinimitable.scrub(values.doctype)
									: df.fieldname,
							...
							mrinimitable.call({
								method: "mrinimitable.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
									names: Array.from(
			...
			},
			{
				...
				condition: () => mrinimitable.user.has_role("System Manager"),
				standard: true,
			},
		];

		if (mrinimitable.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new mrinimitable.ui.Dialog({
						...
						primary_action: (values) => {
							mrinimitable.call({
								method: "mrinimitable.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									mrinimitable.set_route("query-report", r.message);
								},
							});
						},
					});
	...
	add_custom_column(custom_column, custom_data, new_column_data, insert_after_index) {
		...
		this.data.forEach((row) => {
			if (column[0].fieldname.includes("-")) {
				row[column_field + "-" + mrinimitable.scrub(new_column_data.doctype)] =
