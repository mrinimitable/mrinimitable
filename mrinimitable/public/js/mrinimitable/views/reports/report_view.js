...
mrinimitable.views.ReportView = class ReportView extends mrinimitable.views.ListView {
	get view_name() {
		return "Report";
	}
	...
	setup_events() {
		...
		mrinimitable.realtime.on("list_update", (data) => this.on_update(data));
	...
	setup_sort_selector() {
		this.sort_selector = new mrinimitable.ui.SortSelector({
		...
		this.group_by_control = new mrinimitable.ui.GroupBy(this);
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse mrinimitable.route_options if this is a Custom Report
	...
	async get_link_title_field_value(doctype, value) {
		return (
			mrinimitable.utils.get_link_title(doctype, value) ||
			(await mrinimitable.utils.fetch_link_title(doctype, value))
		);
	}
	...
	set_dirty_state_for_custom_report() {
		...
		if (!mrinimitable.utils.deep_equal(current_settings, report_settings)) {
	...
	prepare_data(r) {
		let data = r.message || {};
		data = mrinimitable.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== mrinimitable.session.user;
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			// numeric values in y
			if (col.docfield && mrinimitable.model.numeric_fieldtypes.includes(col.docfield.fieldtype)) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
					value: col.id,
			...
			fields: [
				...
				},
				{
					...
					default: defaults.chart_type
						? mrinimitable.utils.to_title_case(defaults.chart_type)
						: "Bar",
				},
			],
			primary_action: (data) => {
	...
	make_chart() {
		...
		this.chart = new mrinimitable.Chart(this.$charts_wrapper.find(".charts-inner-wrapper")[0], {
			...
			tooltipOptions: {
				formatTooltipY: (value) => {
					return mrinimitable.format(value, get_df(this.chart_args.y_axes[0]), {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === "Text Editor") {
			...
			// make control
			control = mrinimitable.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true,
			});
	...
	evaluate_read_only_depends_on(expression, data) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = mrinimitable.utils.eval(expression.substr(5), { doc: data });
	...
	is_editable(df, data) {
		if (
			df &&
			mrinimitable.model.can_write(this.doctype) &&
			...
			mrinimitable.model.is_non_std_field(df.fieldname)
		) {
	...
	set_default_fields() {
		...
		const fields = this.meta.fields.filter((df) => {
			return (
				...
				mrinimitable.perm.has_perm(this.doctype, df.permlevel, "read") &&
				mrinimitable.model.is_value_type(df.fieldtype) &&
				!df.report_hide
			);
		});
	...
	get_fields() {
		let fields = this.fields.map((f) => {
			let column_name = mrinimitable.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields = this.get_unique_cdt_in_view().map(
			(cdt) => mrinimitable.model.get_full_column_name("name", cdt) + " as " + `'${cdt}:name'`
		);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = mrinimitable.meta.get_docfield(doctype, fieldname);
		if (
			df &&
			df.fieldtype === "Currency" &&
			df.options &&
			!df.options.includes(":") &&
			mrinimitable.meta.has_field(doctype, df.options)
		) {
			...
			const field_label = mrinimitable.meta.get_label(doctype, field[0]);
			mrinimitable.show_alert(
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === "name") {
			this.refresh();
			mrinimitable.throw(__("Cannot remove ID field"));
		}
	...
	get_columns_for_picker() {
		...
		const standard_fields_filter = (df) => !mrinimitable.model.no_value_type.includes(df.fieldtype);

		let doctype_fields = mrinimitable.meta
			.get_docfields(this.doctype)
			.filter(standard_fields_filter);

		// filter out docstatus field from picker
		let std_fields = mrinimitable.model.std_fields.filter(
		...
		if (this.data) {
			has_status_values = mrinimitable.get_indicator(this.data[0], this.doctype);
		}

		if (!mrinimitable.meta.has_field(this.doctype, "status") && has_status_values) {
			doctype_fields = [
				{
					label: __("Status"),
					fieldname: "docstatus",
		...
		const table_fields = mrinimitable.meta.get_table_fields(this.doctype);

		table_fields.forEach((df) => {
			const cdt = df.options;
			const child_table_fields = mrinimitable.meta
				.get_docfields(cdt)
				.filter(standard_fields_filter);
	...
	get_dialog_fields() {
		...
		const table_fields = mrinimitable.meta.get_table_fields(this.doctype).filter((df) => !df.hidden);
		...
		for (let f of this.fields) {
			...
			if (f[0] !== "docstatus") {
				...
				if (
					...
					!mrinimitable.meta.has_field(this.doctype, "status")
				) {
	...
	build_column(c) {
		...
		let docfield = mrinimitable.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		if (!docfield) {
			docfield = mrinimitable.model.get_std_field(fieldname, true);
			...
			if (docfield) {
				...
				if (fieldname == "docstatus" && !mrinimitable.meta.has_field(this.doctype, "status")) {
		...
		const editable =
			mrinimitable.model.is_non_std_field(fieldname) &&
			!docfield.read_only &&
			!docfield.is_virtual;

		const align = (() => {
			const is_numeric = mrinimitable.model.is_numeric_field(docfield);
			if (is_numeric) {
				return "right";
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				if (Array.isArray(row)) {
					doc = row.reduce((acc, curr) => {
						...
						if (
							...
							mrinimitable.boot.link_title_doctypes.includes(curr.column.docfield.options)
						) {
				...
				return mrinimitable.format(value, column.docfield, { always_show_decimals: true }, doc);
			},
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: (value) => {
						let formatted_value = mrinimitable.format(
							value,
							col.docfield,
							{
								always_show_decimals: true,
	...
	format_total_cell(formatted_value, df) {
		...
		if (mrinimitable.model.is_numeric_field(df.docfield)) {
	...
	build_row(d) {
		return this.columns.map((col) => {
			...
			if (col.field === "docstatus" && !mrinimitable.meta.has_field(this.doctype, "status")) {
				// get status from docstatus
				let status = mrinimitable.get_indicator(d, this.doctype);
				if (status) {
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return mrinimitable.call({
				method: "mrinimitable.desk.reportview.save_report",
				args: {
					name: name,
					doctype: this.doctype,
				...
				callback: (r) => {
					if (r.exc) {
						mrinimitable.msgprint(__("Report was not saved (there were errors)"));
						return;
					}
					if (r.message != this.report_name) {
						// Rerender the reports dropdown,
						// so that this report is included in the dropdown as well.
						mrinimitable.boot.user.all_reports[r.message] = {
							ref_doctype: this.doctype,
							report_type: "Report Builder",
							title: r.message,
						};

						mrinimitable.set_route("List", this.doctype, "Report", r.message);
						return;
					}
	...
	delete_report() {
		return mrinimitable.call({
			method: "mrinimitable.desk.reportview.delete_report",
	...
	get_report_doc() {
		return new Promise((resolve) => {
			mrinimitable.model.with_doc("Report", this.report_name, () => {
				resolve(mrinimitable.get_doc("Report", this.report_name));
			});
		});
	}
		...
		return (
			...
			filters
				.map((f) => {
					...
					const docfield = mrinimitable.meta.get_docfield(doctype, fieldname);
					const label = `<b>${__(mrinimitable.meta.get_label(doctype, fieldname))}</b>`;
					switch (condition) {
						case "=":
							return __("{0} is equal to {1}", [
								label,
								mrinimitable.format(value, docfield),
							]);
						case "!=":
						...
						case "Between":
							return __("{0} is between {1} and {2}", [
								__(label),
								mrinimitable.format(value[0], docfield),
								mrinimitable.format(value[1], docfield),
							]);
						case "Timespan":
						...
						case "in":
							return __("{0} is one of {1}", [
								__(label),
								mrinimitable.utils.comma_or(
									value.map((v) => mrinimitable.format(v, docfield))
								),
							]);
						case "not in":
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && mrinimitable.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					mrinimitable.ui.get_print_settings(false, (print_settings) => {
			...
			},
			{
				...
				action: () => {
					...
					mrinimitable.utils.setup_search(d.$body, ".unit-checkbox", ".label-area");
					d.show();
				},
			},
		];

		if (mrinimitable.model.can_export(this.doctype)) {
			items.push({
				label: __("Export"),
				action: () => {
					...
					const d = mrinimitable.report_utils.get_export_dialog(
						__(this.doctype),
						extra_fields,
						(data) => {
							args.cmd = "mrinimitable.desk.reportview.export_query";
		...
		items.push({
			...
			action: () => {
				if (this.report_name) {
					...
					mrinimitable.msgprint(__("Please save the report first"));
				}
			},
		});
		...
		const can_edit_or_delete = (action) => {
			...
			return (
				...
				(mrinimitable.model[method]("Report") || this.report_doc.owner === mrinimitable.session.user)
			);
		};
		...
		if (can_edit_or_delete("delete")) {
			items.push({
				label: __("Delete"),
				action: () =>
					mrinimitable.confirm("Are you sure you want to delete this report?", () =>
						this.delete_report()
					),
		...
		// user permissions
		if (this.report_name && mrinimitable.user.has_role("System Manager")) {
			items.push({
				...
				action: () => {
					...
					mrinimitable.set_route("List", "User Permission", args);
				},
			});
		}
	...
	parse_filters_from_route_options() {
		if (mrinimitable.route_options?._group_by) {
			try {
				let config = JSON.parse(mrinimitable.route_options._group_by);
				this.group_by_control.apply_settings({
					group_by: config[0],
					aggregate_on: config[1],
					aggregate_function: config[2],
				});
				delete mrinimitable.route_options["_group_by"];
			} catch (e) {
