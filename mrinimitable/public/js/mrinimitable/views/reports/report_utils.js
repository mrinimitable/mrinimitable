mrinimitable.provide("mrinimitable.report_utils");

mrinimitable.report_utils = {
	make_chart_options: function (
		...
		function get_translated_column_label(fieldname) {
			...
			return column?.label ?? __(mrinimitable.model.unscrub(fieldname));
		}
	},
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = mrinimitable.report_utils.prepare_field_from_column(field);
					return { label: field.label, value: field.fieldname };
				}
			});
		}
	...
	get_report_filters: function (report_name) {
		if (mrinimitable.query_reports[report_name]) {
			let filters = mrinimitable.query_reports[report_name].filters;
		...
		return mrinimitable
			...
			.then((r) => {
				mrinimitable.dom.eval(r.script || "");
				return mrinimitable.after_ajax(() => {
					if (
						mrinimitable.query_reports[report_name] &&
						!mrinimitable.query_reports[report_name].filters &&
						r.filters
					) {
						return (mrinimitable.query_reports[report_name].filters = r.filters);
					}
					return (
						mrinimitable.query_reports[report_name] &&
						mrinimitable.query_reports[report_name].filters
	...
	get_export_dialog(report_name, extra_fields, callback) {
		...
		function update_csv_preview(dialog) {
			const is_query_report = mrinimitable.get_route()[0] === "query-report";
			const report = is_query_report ? mrinimitable.query_report : cur_list;
			...
			dialog.set_value(
				"csv_preview",
				mrinimitable.report_utils.get_csv_preview(
					PREVIEW_DATA,
	...
	get_csv_preview(data, quoting, delimiter, decimal_sep) {
		...
		if (delimiter.length > 1) {
			mrinimitable.throw(__("Delimiter must be a single character"));
		}

		if (decimal_sep.length > 1) {
			mrinimitable.throw(__("Decimal Separator must be a single character"));
		}

		if (0 > quoting || quoting > 3) {
			mrinimitable.throw(__("Quoting must be between 0 and 3"));
		}

		if (decimal_sep !== "." && quoting === QUOTING.NonNumeric) {
			mrinimitable.throw(__("Decimal Separator must be '.' when Quoting is set to Non-numeric"));
		}
