/**
 * mrinimitable.views.MapView
 */
mrinimitable.provide("mrinimitable.utils");
mrinimitable.provide("mrinimitable.views");

mrinimitable.views.MapView = class MapView extends mrinimitable.views.ListView {
	get view_name() {
		return "Map";
	}
	...
	setup_view() {
		...
		this.map = L.map(this.map_id).setView(
			mrinimitable.utils.map_defaults.center,
			mrinimitable.utils.map_defaults.zoom
		);
	...
	get_popup_content(feature) {
		return mrinimitable.utils.get_form_link(this.doctype, feature.properties.name, true);
	}
