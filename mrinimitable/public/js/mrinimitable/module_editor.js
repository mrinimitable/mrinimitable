mrinimitable.ModuleEditor = class ModuleEditor {
	...
	set_modules_in_table() {
		...
		block_modules.map((module_doc) => {
			if (!unchecked_options.includes(module_doc.module)) {
				mrinimitable.model.clear_doc(module_doc.doctype, module_doc.name);
			}
		});

		unchecked_options.map((module) => {
			if (!block_modules.find((d) => d.module === module)) {
				let module_doc = mrinimitable.model.add_child(
					this.frm.doc,
					"Block Module",
					"block_modules"
				);
