...
mrinimitable.provide("mrinimitable.dom");
...
mrinimitable.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + mrinimitable.dom.id_count;
		mrinimitable.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + mrinimitable.dom.id_count;
		$ele.attr("id", id);
		mrinimitable.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = mrinimitable.dom.by_id(parent);
		...
		if (cs) mrinimitable.dom.css(c, cs);
	...
	freeze: function (msg, css_class) {
		...
		mrinimitable.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!mrinimitable.dom.freeze_count) return; // anything open?
		mrinimitable.dom.freeze_count--;
		if (!mrinimitable.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				mrinimitable.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
mrinimitable.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
mrinimitable.run_serially = function (tasks) {
...
mrinimitable.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
mrinimitable.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

mrinimitable.scrub = mrinimitable.slug = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

mrinimitable.unscrub = function (txt) {
	return mrinimitable.model.unscrub(txt);
};

mrinimitable.get_data_pill = (
	label,
	target_id = null,
	remove_action = null,
	image = null,
	...
	let color = "",
		style = "";
	if (colored) {
		color = mrinimitable.get_palette(label);
	}
	...
	if (remove_action) {
		let remove_btn = $(`
			<span class="remove-btn cursor-pointer">
				${mrinimitable.utils.icon("close", "sm", "es-icon")}
			</span>
		`);
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${mrinimitable.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${mrinimitable.utils.icon("close", "sm")}
						</button>
					</div>
				</div>
...
mrinimitable.is_online = function () {
	if (mrinimitable.boot.developer_mode == 1) {
...
mrinimitable.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + mrinimitable.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = mrinimitable.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = mrinimitable.assets.bundled_asset("desk.bundle.css");
