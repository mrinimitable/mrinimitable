function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = mrinimitable.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff =
		(new Date(mrinimitable.datetime.now_datetime().replace(/-/g, "/")).getTime() - date.getTime()) /
		1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = mrinimitable.datetime.str_to_user ? mrinimitable.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="mrinimitable-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
mrinimitable.datetime.comment_when = comment_when;
mrinimitable.datetime.prettyDate = prettyDate;

mrinimitable.datetime.refresh_when = function () {
	if (jQuery) {
		$(".mrinimitable-timestamp").each(function () {
