mrinimitable.dashboard_utils = {
	render_chart_filters: function (filters, button_class, container, append) {
		filters.forEach((filter) => {
			...
			if (filter.icon) {
				icon_html = mrinimitable.utils.icon(filter.icon);
			}
	...
	get_filters_for_chart_type: function (chart) {
		if (chart.chart_type === "Custom" && chart.source) {
			...
			return mrinimitable.xcall(method, { name: chart.source }).then((config) => {
				mrinimitable.dom.eval(config);
				return mrinimitable.dashboards.chart_sources[chart.source].filters;
			});
		} else if (chart.chart_type === "Report" && chart.report_name) {
			return mrinimitable.report_utils.get_report_filters(chart.report_name).then((filters) => {
				return filters;
			});
		} else {
	...
	create_dashboard_settings() {
		return mrinimitable
			.xcall(
				"mrinimitable.desk.doctype.dashboard_settings.dashboard_settings.create_dashboard_settings",
				{
	...
	get_years_since_creation(creation) {
		...
		let current_year = this.get_year(mrinimitable.datetime.now_date());
		let years_list = [];
		...
		let fields = [
			{
				...
				options: `<div>
						...
						<p>${__("For example:")}
							<code>mrinimitable.defaults.get_user_default("Company")</code>
						</p>
					</div>`,
			},
		];
		...
		if (Array.isArray(dynamic_filters)) {
			dynamic_filters.forEach((f) => {
				try {
					...
					mrinimitable.throw(__("Invalid expression set in filter {0} ({1})", [f[1], f[0]]));
				}
			});
			filters = [...filters, ...dynamic_filters];
		} else {
			for (let key of Object.keys(dynamic_filters)) {
				try {
					...
					mrinimitable.throw(__("Invalid expression set in filter {0}", [key]));
				}
			}
	...
	get_add_to_dashboard_dialog(docname, doctype, method) {
		...
		const dialog = new mrinimitable.ui.Dialog({
			...
			primary_action: (values) => {
				values.name = docname;
				values.set_standard = mrinimitable.boot.developer_mode;
				mrinimitable.xcall(method, { args: values }).then(() => {
