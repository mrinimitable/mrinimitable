mrinimitable.provide("mrinimitable.contacts");

$.extend(mrinimitable.contacts, {
	...
	render_address_and_contact: function (frm) {
		...
		if (frm.fields_dict["address_html"] && "addr_list" in frm.doc.__onload) {
			$(frm.fields_dict["address_html"].wrapper)
				.html(mrinimitable.render_template("address_list", frm.doc.__onload))
				.find(".btn-address")
		...
		if (frm.fields_dict["contact_html"] && "contact_list" in frm.doc.__onload) {
			$(frm.fields_dict["contact_html"].wrapper)
				.html(mrinimitable.render_template("contact_list", frm.doc.__onload))
				.find(".btn-contact")
				.on("click", () => new_record("Contact", frm));
		}
	},
	get_last_doc: function (frm) {
		const reverse_routes = mrinimitable.route_history.slice().reverse();
	...
	get_address_display: function (frm, address_field, display_field) {
		...
		mrinimitable
			.xcall("mrinimitable.contacts.doctype.address.address.get_address_display", {
...
function new_record(doctype, frm) {
	...
	if (mrinimitable.boot.enable_address_autocompletion === 1 && doctype === "Address") {
		new mrinimitable.ui.AddressAutocompleteDialog({
