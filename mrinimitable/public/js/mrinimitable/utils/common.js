...
mrinimitable.avatar = function (
	...
	if (user) {
		// desk
		user_info = mrinimitable.user_info(user);
	} else {
		// website
		let full_name = title || mrinimitable.get_cookie("full_name");
		user_info = {
			image: image_url === null ? mrinimitable.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: mrinimitable.get_abbr(full_name),
			color: mrinimitable.get_palette(full_name),
		};
	}
	...
	return mrinimitable.get_avatar(
		css_class,
		title,
...
mrinimitable.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let abbr = mrinimitable.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = mrinimitable.get_palette(title);
...
mrinimitable.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			mrinimitable.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
	if (extra_users.length === 1) {
		html += mrinimitable.avatar(
			extra_users[0],
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => mrinimitable.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${mrinimitable.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
mrinimitable.ui.scroll = function (element, animate, additional_offset) {
...
mrinimitable.get_palette = function (txt) {
	if (!txt) return mrinimitable.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return mrinimitable.palette[idx % 8];
};

mrinimitable.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
mrinimitable.gravatars = {};
mrinimitable.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!mrinimitable.gravatars[email_id]) {
		// TODO: check if gravatar exists
		mrinimitable.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return mrinimitable.gravatars[email_id];
};
...
mrinimitable.get_cookie = function getCookie(name) {
	return mrinimitable.get_cookies()[name];
};

mrinimitable.get_cookies = function getCookies() {
...
mrinimitable.is_mobile = function () {
	return $(document).width() < 768;
};

mrinimitable.is_large_screen = function () {
	return $(document).height() > 1180;
};

mrinimitable.utils.xss_sanitise = function (string, options) {
...
mrinimitable.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = mrinimitable.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(mrinimitable.utils.xss_sanitise(url, { strategies: ["js"] }));
};

mrinimitable.utils.strip_url = (url) => {
...
mrinimitable.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
		...
		},
		{
			...
			label: __("Start Date"),
			reqd: 1,
			default: mrinimitable.datetime.nowdate(),
		},
		{
			fieldname: "end_date",
			fieldtype: "Date",
	...
	mrinimitable.prompt(
		...
		function (values) {
			mrinimitable.call({
				method: "mrinimitable.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
mrinimitable.utils.get_page_view_count = function (route) {
	return mrinimitable.call("mrinimitable.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};
