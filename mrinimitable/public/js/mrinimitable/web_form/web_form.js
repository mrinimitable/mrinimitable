...
mrinimitable.provide("mrinimitable.web_form");
import EventEmitterMixin from "../../mrinimitable/event_emitter";

export default class WebForm extends mrinimitable.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		mrinimitable.web_form = this;
		mrinimitable.web_form.events = {};
		Object.assign(mrinimitable.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		mrinimitable.init_client_script && mrinimitable.init_client_script();
		this.setup_listeners();
		mrinimitable.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = mrinimitable.utils.get_query_params();
		delete values.new;
	...
	delete_form() {
		...
		mrinimitable.confirm(__("Are you sure you want to delete this record?"), () => {
			mrinimitable.call({
				method: "mrinimitable.website.doctype.web_form.web_form.delete",
				args: {
					web_form_name: this.name,
					docname: this.doc.name,
				},
				callback: () => {
					mrinimitable.msgprint(__("Deleted!"));
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${mrinimitable.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".mrinimitable-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		mrinimitable.form_dirty = false;

		mrinimitable.call({
			type: "POST",
			method: "mrinimitable.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					mrinimitable.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						mrinimitable.call({
							type: "POST",
							method: "mrinimitable.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			mrinimitable.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
