...
mrinimitable.ready(function () {
	let web_form_doc = mrinimitable.web_form_doc;
	let reference_doc = mrinimitable.reference_doc;
	...
	function show_login_prompt() {
		if (mrinimitable.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new mrinimitable.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(mrinimitable.form.formatters.FileSize($(this).text()));
		});
	}
