mrinimitable.provide("mrinimitable.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
mrinimitable.ui.keys.shortcut_groups = shortcut_groups;

mrinimitable.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new mrinimitable.ui.keys.AltShortcutGroup());
	}
...
mrinimitable.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (mrinimitable.ui.keys.key_map[e.which] || "").toLowerCase();
...
mrinimitable.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = mrinimitable.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
