export class DropdownConsole {
	...
	async load_contextual_boilerplate() {
		...
		if (cur_frm && !cur_frm.is_new()) {
			default_code = `doc = mrinimitable.get_doc("${cur_frm.doc.doctype}", "${cur_frm.doc.name}")\n`;
		} else if (cur_list && mrinimitable.get_route()[0] == "List") {
			let args = cur_list.get_args();
			default_code = `docs = mrinimitable.get_all("${args.doctype}",
				fields="*",
