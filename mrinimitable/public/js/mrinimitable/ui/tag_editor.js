mrinimitable.ui.TagEditor = class TagEditor {
	constructor(opts) {
		/* docs:
		Arguments
	...
	setup_tags() {
		...
		this.tags = new mrinimitable.ui.Tags({
			...
			onTagRemove: (tag) => {
				if (!me.refreshing) {
					return mrinimitable.call({
						method: "mrinimitable.desk.doctype.tag.tag.remove_tag",
						args: me.get_args(tag),
						callback: function (r) {
	...
	setup_awesomplete() {
		...
		$input.on("input", function (e) {
			...
			mrinimitable.call({
				method: "mrinimitable.desk.doctype.tag.tag.get_tags",
				args: {
