import { Chart } from "mrinimitable-charts/dist/mrinimitable-charts.esm";

mrinimitable.provide("mrinimitable.ui");
mrinimitable.Chart = Chart;

mrinimitable.ui.RealtimeChart = class RealtimeChart extends mrinimitable.Chart {
	constructor(element, socketEvent, maxLabelPoints = 8, data) {
		...
		this.start_updating = function () {
			mrinimitable.realtime.on(this.socketEvent, (data) => {
				this.update_chart(data.label, data.points);
			});
		};

		this.stop_updating = function () {
			mrinimitable.realtime.off(this.socketEvent);
		};
