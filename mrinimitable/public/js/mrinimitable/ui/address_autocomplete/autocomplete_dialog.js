...
mrinimitable.ui.AddressAutocompleteDialog = class AddressAutocompleteDialog {
	constructor(opts) {
	...
	_get_dialog() {
		...
		const dialog = new mrinimitable.ui.Dialog({
			...
			fields: [
				{
					...
					reqd: 1,
					get_query:
						"mrinimitable.integrations.doctype.geolocation_settings.geolocation_settings.autocomplete",
					onchange: () => {
						// Disable "Create Address" button if mandatory fields are missing
						mrinimitable.model.with_doctype("Address", () => {
							const address = this.parse_selected_value();
							const mandatory_fields = mrinimitable
								.get_meta("Address")
								.fields.filter(
									(field) =>
										field.reqd &&
			...
			primary_action: () => {
				...
				mrinimitable.db.insert(address).then((doc) => {
			...
			secondary_action: () => {
				...
				const address = this.parse_selected_value();
				mrinimitable.new_doc("Address", address);
			},
		});
