...
mrinimitable.ui.GroupBy = class {
	...
	init_group_by_popover() {
		...
		const group_by_template = $(
			mrinimitable.render_template("group_by", {
			...
			if (!this.aggregate_on_html.length) {
				...
				for (let doctype in this.all_fields) {
					...
					doctype_fields.forEach((field) => {
						...
						if (mrinimitable.model.is_numeric_field(field.fieldtype)) {
							let field_label = field.label || mrinimitable.model.unscrub(field.fieldname);
							let option_text =
								doctype == this.doctype
			...
			$(`<div class="group-by-selector">
				<button class="btn btn-default btn-sm group-by-button ellipsis">
					<span class="group-by-icon button-icon">
						${mrinimitable.utils.icon("es-line-folder-alt")}
					</span>
	...
	get_group_by_docfield() {
		...
		if (this.aggregate_function === "count") {
			...
			docfield = Object.assign(
				{},
				mrinimitable.meta.docfield_map[this.aggregate_on_doctype][this.aggregate_on_field]
			);
	...
	get_group_by_fields() {
		...
		const standard_fields = mrinimitable.model.std_fields.filter(
		...
		const standard_fields_filter = (df) =>
			!mrinimitable.model.no_value_type.includes(df.fieldtype) && !df.report_hide;
