...
mrinimitable.ui.Tags = class {
	...
	bind() {
		...
		const select_tag = function () {
			const tagValue = mrinimitable.utils.xss_sanitise(me.$input.val());
			me.addTag(tagValue);
			me.$input.val("");
		};
	...
	removeTag(label) {
		label = mrinimitable.utils.xss_sanitise(label);
	...
	get_tag(label) {
		let colored = true;
		let $tag = mrinimitable.get_data_pill(
			label,
			label,
			(target, pill_wrapper) => {
