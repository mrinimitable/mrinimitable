...
mrinimitable.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = mrinimitable.ui.keys.get_key(e);
			let increment_by;
	...
	toggle_theme(theme) {
		...
		mrinimitable.show_alert(__("Theme Changed"), 3);

		mrinimitable.xcall("mrinimitable.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
mrinimitable.ui.add_system_theme_switch_listener = () => {
	mrinimitable.ui.dark_theme_media_query.addEventListener("change", () => {
		mrinimitable.ui.set_theme();
	});
};

mrinimitable.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

mrinimitable.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = mrinimitable.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
