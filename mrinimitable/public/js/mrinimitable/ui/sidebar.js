mrinimitable.ui.Sidebar = class Sidebar {
	constructor() {
		this.items = {};
		this.parent_items = [];
	...
	make_dom() {
		...
		this.wrapper = $(mrinimitable.render_template("sidebar")).prependTo("body");
		...
		this.apps_switcher = new mrinimitable.ui.AppsSwitcher(this);
	...
	set_all_pages() {
		this.sidebar_pages = mrinimitable.boot.sidebar_pages;
	...
	set_default_app() {
		...
		mrinimitable.boot.app_data.sort((a, b) => (a.workspaces.length < b.workspaces.length ? 1 : -1));
		mrinimitable.current_app = mrinimitable.boot.app_data[0].app_name;
		mrinimitable.mrinimitable_toolbar.set_app_logo(mrinimitable.boot.app_data[0].app_logo_url);
	}

	set_active_workspace_item() {
		if (!mrinimitable.get_route()) return;
		let current_route = mrinimitable.get_route();
		let current_route_str = mrinimitable.get_route_str();
		let current_item;
		if (current_route[0] == "Workspaces") {
			current_item = current_route[1];
		} else if (mrinimitable.breadcrumbs) {
			if (Object.keys(mrinimitable.breadcrumbs.all).length == 0) return;
			if (mrinimitable.breadcrumbs.all[current_route_str]) {
				current_item =
					mrinimitable.breadcrumbs.all[current_route_str].workspace ||
					mrinimitable.breadcrumbs.all[current_route_str].module;
			}
		}
	...
	setup_pages() {
		...
		if (this.all_pages) {
			...
			mrinimitable.workspace_list = [];
			mrinimitable.workspace_map = {};
			for (let page of this.all_pages) {
				mrinimitable.workspaces[mrinimitable.router.slug(page.name)] = {
					name: page.name,
					public: page.public,
				};
				if (!page.app && page.module) {
					page.app = mrinimitable.boot.module_app[mrinimitable.slug(page.module)];
				}
				mrinimitable.workspace_map[page.name] = page;
				mrinimitable.workspace_list.push(page);
			}
			this.make_sidebar();
		}
		this.set_hover();
	...
	make_sidebar() {
		...
		let app_workspaces = mrinimitable.boot.app_data_map[mrinimitable.current_app || "mrinimitable"].workspaces;

		let parent_pages = this.all_pages.filter((p) => !p.parent_page).uniqBy((p) => p.name);
		if (mrinimitable.current_app === "private") {
		...
		this.wrapper.find(".selected").length &&
			!mrinimitable.dom.is_element_in_viewport(this.wrapper.find(".selected")) &&
	...
	sidebar_item_container(item) {
		...
		if (item.type === "Link") {
			if (item.link_type === "Report") {
				path = mrinimitable.utils.generate_route({
					type: item.link_type,
					name: item.link_to,
				...
				path = mrinimitable.utils.generate_route({ type: item.link_type, name: item.link_to });
			}
		} else if (item.type === "URL") {
			path = item.external_link;
		} else {
			if (item.public) {
				path = "/app/" + mrinimitable.router.slug(item.name);
			} else {
				path = "/app/private/" + mrinimitable.router.slug(item.name.split("-")[0]);
			}
		}
						...
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>
							${
								item.public || item.icon
									? mrinimitable.utils.icon(item.icon || "folder-normal", "md")
	...
	add_toggle_children(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(`<button class="btn-reset drop-icon hidden">`)
			.html(mrinimitable.utils.icon(drop_icon, "sm"))
		...
		for (let container of this.$sidebar.find(".nested-container")) {
			this.sorting_items[this.sorting_items.length] = Sortable.create(container, {
				...
				onEnd: () => {
					...
					mrinimitable.xcall(
						"mrinimitable.desk.doctype.workspace_settings.workspace_settings.set_sequence",
						{
	...
	close_sidebar() {
		...
		if (mrinimitable.is_mobile()) mrinimitable.app.sidebar.prevent_scroll();
	}
	open_sidebar() {
		this.sidebar_expanded = true;
		this.expand_sidebar();
	...
	reload() {
		return mrinimitable.workspace.get_pages().then((r) => {
