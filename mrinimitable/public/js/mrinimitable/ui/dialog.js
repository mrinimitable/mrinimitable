...
mrinimitable.ui.open_dialogs = [];

mrinimitable.ui.Dialog = class Dialog extends mrinimitable.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		this.$wrapper = mrinimitable.get_modal("", "");
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				mrinimitable.ui.form.get_open_grid_form?.()?.hide_form();

				if (mrinimitable.ui.open_dialogs[mrinimitable.ui.open_dialogs.length - 1] === me) {
					mrinimitable.ui.open_dialogs.pop();
					if (mrinimitable.ui.open_dialogs.length) {
						window.cur_dialog =
							mrinimitable.ui.open_dialogs[mrinimitable.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				mrinimitable.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("mrinimitable.ui.Dialog:shown");
	...
	handle_focus() {
		const me = this;
		if (mrinimitable.get_route) {
			if (mrinimitable.get_route()[0] == "Form") {
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(mrinimitable.utils.icon(icon));
