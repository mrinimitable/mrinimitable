// mrinimitable.ui.Capture
// Author - Achilles Rasquinha <achilles@mrinimitable.io>
 ...
 *
 * @example
 * mrinimitable._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
mrinimitable._.get_data_uri = (element) => {
...
/**
 * @description Mrinimitable's Capture object.
 *
 * @example
 * const capture = mrinimitable.ui.Capture()
 * capture.show()
 *
 ...
 */
mrinimitable.ui.Capture = class {
	constructor(options = {}) {
		this.options = mrinimitable.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...mrinimitable.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(mrinimitable.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (mrinimitable.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(mrinimitable.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					mrinimitable.show_alert(mrinimitable.ui.Capture.ERR_MESSAGE, 3);
				}
			...
			images += `
				<div class="mt-1 p-1 rounded col-md-3 col-sm-4 col-xs-4" data-idx="${idx}">
					<span class="capture-remove-btn" data-idx="${idx}">
						${mrinimitable.utils.icon("close", "lg")}
					</span>
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = mrinimitable._.get_data_uri(me.video);
...
mrinimitable.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
mrinimitable.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
mrinimitable.ui.Capture.TEMPLATE = `
<div class="mrinimitable-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${mrinimitable.ui.Capture.ERR_MESSAGE}</video>
	</div>
