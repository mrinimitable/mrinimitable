...
mrinimitable.tags.utils = {
	get_tags: function (txt) {
		txt = txt.slice(1);
		let out = [];

		if (!mrinimitable.tags.tags) {
			mrinimitable.tags.utils.fetch_tags();
			return [];
		}

		mrinimitable.tags.tags.forEach((tag) => {
			const search_result = mrinimitable.search.utils.fuzzy_search(txt, tag, true);
			if (search_result.score) {
				out.push({
					...
					onclick() {
						...
						mrinimitable.searchdialog.search.init_search("#".concat(tag), "tags");
					},
				});
			}
		});
	...
	fetch_tags() {
		mrinimitable.call({
			method: "mrinimitable.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function (r) {
				if (r && r.message) {
					mrinimitable.tags.tags = $.extend([], r.message);
				}
			},
		});
	},
	...
	get_tag_results: function (tag) {
		...
		return new Promise(function (resolve) {
			mrinimitable.call({
				method: "mrinimitable.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag,
				},
				callback: function (r) {
