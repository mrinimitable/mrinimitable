...
mrinimitable.provide("mrinimitable.tags");

mrinimitable.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
		...
		var awesomplete = new Awesomplete(input, {
			...
			item: function (item, term) {
				...
				let target = "#";
				if (d.route) {
					target = mrinimitable.router.make_url(
						mrinimitable.router.convert_from_standard_route(
							mrinimitable.router.get_route_from_arguments(
			...
			mrinimitable.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(mrinimitable.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(mrinimitable.search.utils.get_frequent_links());
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				mrinimitable.route_options = item.route_options;
			}
			...
			if (item.onclick) {
				...
				mrinimitable.set_route(item.route);
			}
			$input.val("");
			$input.trigger("blur");
		});
		...
		mrinimitable.search.utils.setup_recent();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function () {
				...
				mrinimitable.msgprint(txt, __("Search Help"));
			},
		});
	}
	...
	build_options(txt) {
		var options = mrinimitable.search.utils
			.get_creatables(txt)
			.concat(
				mrinimitable.search.utils.get_search_in_list(txt),
				mrinimitable.search.utils.get_doctypes(txt),
				mrinimitable.search.utils.get_reports(txt),
				mrinimitable.search.utils.get_pages(txt),
				mrinimitable.search.utils.get_workspaces(txt),
				mrinimitable.search.utils.get_dashboards(txt),
				mrinimitable.search.utils.get_recent_pages(txt || ""),
				mrinimitable.search.utils.get_executables(txt),
				mrinimitable.search.utils.get_marketplace_apps(txt)
			);
		if (txt.charAt(0) === "#") {
			options = mrinimitable.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${mrinimitable.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	mrinimitable.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [mrinimitable.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			const doctype = mrinimitable.container.page?.list_view?.doctype;
			if (!doctype) return;
			...
			this.options.push({
				label: __("Find {0} in {1}", [
					mrinimitable.utils.xss_sanitise(txt).bold(),
					__(route[1]).bold(),
				]),
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function () {
						mrinimitable.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: mrinimitable.utils.get_random(16),
				onclick: function () {
					mrinimitable.msgprint(mrinimitable.utils.get_random(16), __("Result"));
				},
			});
		}
	}
