...
mrinimitable.search.SearchDialog = class {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		this.search_dialog = new mrinimitable.ui.Dialog({
			minimizable: true,
			size: "large",
		});
		this.set_header();
		...
		this.search_dialog.header
			...
			.html(
				...
				<span class="search-icon">
					${mrinimitable.utils.icon("search")}
				</span>`
			);
	}
		...
		this.searches = {
			global_search: {
				...
				get_results: (keywords, callback) => {
					let start = 0,
						limit = 100;
					let results = mrinimitable.search.utils.get_nav_results(keywords);
					mrinimitable.search.utils.get_global_results(keywords, start, limit).then(
						(global_results) => {
			...
			tags: {
				...
				get_results: (keywords, callback) => {
					var results = mrinimitable.search.utils.get_nav_results(keywords);
					mrinimitable.tags.utils.get_tag_results(keywords).then(
						(global_results) => {
							results = results.concat(global_results);
							callback(results, keywords);
						},
		...
		var $placeholder = $(`<div class="row search-results hide">
			<div class="empty-state">
				<div class="text-center">
					<img src="/assets/mrinimitable/images/ui-states/search-empty-state.svg"
	...
	render_data(result_sets) {
		let $search_results = $(mrinimitable.render_template("search")).addClass("hide");
	...
	get_link(result) {
		let link = "";
		if (result.route) {
			link = `href="${mrinimitable.router.make_url(result.route)}"`;
	...
	render_result(type, result) {
		...
		if (result.image !== undefined) {
			let avatar_html = mrinimitable.get_avatar("avatar-medium", result.label, result.image);
	...
	handle_result_click(result, $result) {
		if (result.route_options) {
			mrinimitable.route_options = result.route_options;
		}
		$result.on("click", () => {
			...
			if (result.onclick) {
				...
				mrinimitable.set_route(result.route);
