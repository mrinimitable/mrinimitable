...
mrinimitable.ui.FieldGroup = class FieldGroup extends mrinimitable.ui.form.Layout {
	constructor(opts) {
		super(opts);
		this.dirty = false;
		this.fetch_dict = {};
	...
	make() {
		...
		if (this.fields) {
			...
			$.each(this.fields_list, function (i, field) {
				...
				if (
					def_value == null ||
					(!def_value && !mrinimitable.model.is_numeric_field(field.df.fieldtype))
				)
					return;

				if (def_value == "Today" && field.df["fieldtype"] == "Date") {
					def_value = mrinimitable.datetime.get_today();
				}
