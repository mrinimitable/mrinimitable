...
mrinimitable.ui.color = {
	get: function (color_name, shade) {
		...
		return mrinimitable.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(mrinimitable.ui.color_map);
		if (color_names.includes(color_name)) {
			return mrinimitable.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(mrinimitable.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(mrinimitable.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in mrinimitable.ui.color_map) {
			const colors = mrinimitable.ui.color_map[key];
...
mrinimitable.ui.color_map = mrinimitable.ui.color.get_color_map();
