 ...
 *
 * @returns {mrinimitable.ui.Page}
 */

/**
 * @typedef {Object} mrinimitable.ui.Page
 */

mrinimitable.ui.make_app_page = function (opts) {
	opts.parent.page = new mrinimitable.ui.Page(opts);
	return opts.parent.page;
};

mrinimitable.ui.pages = {};

mrinimitable.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		this.make();
		mrinimitable.ui.pages[mrinimitable.get_route_str()] = this;
	}
	...
	load_lib(callback) {
		mrinimitable.require(this.required_libs, callback);
	}

	add_main_section() {
		$(mrinimitable.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (mrinimitable.utils.is_xs() || mrinimitable.utils.is_sm()) {
	...
	add_action_icon(icon, click, css_class = "", tooltip_label) {
		...
		if (!tooltip_label) {
			...
			tooltip_label = mrinimitable.unscrub(icon);
		}
	...
	get_icon_label(icon, label) {
		...
		return `${icon ? mrinimitable.utils.icon(icon_name, size) : ""} <span class="hidden-xs"> ${__(
			label
		)} </span>`;
	}
	...
	set_action(btn, opts) {
		...
		mrinimitable.ui.keys.get_shortcut_group(this).add(btn, text_span.length ? text_span : btn);
	}
	...
	add_dropdown_item({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${mrinimitable.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			mrinimitable.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
	...
	set_title(title, icon = null, strip = true, tab_title = "", tooltip_label = "") {
		...
		this.title = title;
		mrinimitable.utils.set_title(tab_title || title);
		if (icon) {
			title = `${mrinimitable.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || "btn-default"} ${opts.btn_size || "btn-sm"} ellipsis">
				${opts.icon ? mrinimitable.utils.icon(opts.icon) : ""}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		mrinimitable.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = mrinimitable.ui.form.make_control({
			df: df,
