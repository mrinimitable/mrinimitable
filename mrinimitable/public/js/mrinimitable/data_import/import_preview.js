...
mrinimitable.data_import.ImportPreview = class ImportPreview {
	...
	prepare_columns() {
		this.columns = this.preview_data.columns.map((col, i) => {
			...
			if (col.skip_import) {
				...
				return {
					id: mrinimitable.utils.get_random(6),
		...
		this.data = this.data.map((row) => {
			return row.map((cell) => {
				...
				if (typeof cell === "string") {
					cell = mrinimitable.utils.xss_sanitise(cell);
				}
				return cell;
			});
		});
	...
	setup_styles() {
		...
		this.datatable.style.setStyle(`svg.import-success`, {
			width: "16px",
			fill: mrinimitable.ui.color.get_color_shade("green", "dark"),
		});
		...
		this.datatable.style.setStyle(row_classes, {
			pointerEvents: "none",
			backgroundColor: mrinimitable.ui.color.get_color_shade("gray", "extra-light"),
			color: mrinimitable.ui.color.get_color_shade("gray", "dark"),
		});
	}
	...
	show_column_warning(_, $target) {
		...
		mrinimitable.utils.scroll_to($warning, true, 30);
	}
...
function get_fields_as_options(doctype, column_map) {
	let keys = [doctype];
	mrinimitable.meta.get_table_fields(doctype).forEach((df) => {
	...
	return [].concat(
		...keys.map((key) => {
			return column_map[key].map((df) => {
				...
				if (doctype !== key) {
					let table_field = mrinimitable.meta.get_docfield(doctype, key);
