...
mrinimitable.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = mrinimitable.utils.to_title_case(route[2] || "");
		if (route.length > 2 && mrinimitable.views.view_modes.includes(view_name)) {
	...
	set_route(view, calendar_name) {
		...
		if (search_params) {
			mrinimitable.route_options = Object.fromEntries(search_params);
		}
	...
	setup_views() {
		const views = {
			...
			Report: {
				...
				current_view_handler: () => {
					...
					if (mrinimitable.get_route().length > 3) {
						default_action = {
			...
			Calendar: {
				condition: mrinimitable.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && mrinimitable.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(mrinimitable.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => mrinimitable.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					mrinimitable.treeview_settings[this.doctype] ||
					mrinimitable.get_meta(this.doctype).is_tree,
		...
		mrinimitable.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => mrinimitable.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return mrinimitable.utils.to_title_case(mrinimitable.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(mrinimitable.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			mrinimitable.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						mrinimitable.set_route("list", doctype, "kanban", board.name);
					} else {
						mrinimitable.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			mrinimitable.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			mrinimitable.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					mrinimitable.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return mrinimitable.db
			...
			.then((result) => {
				...
				if (mrinimitable.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = mrinimitable.boot.email_accounts;
	...
	slug() {
		return mrinimitable.router.slug(mrinimitable.router.doctype_layout || this.doctype);
	}
};
