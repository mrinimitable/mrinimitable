export default class BulkOperations {
	constructor({ doctype }) {
		if (!doctype) mrinimitable.throw(__("Doctype required"));
		this.doctype = doctype;
	}

	print(docs) {
		const print_settings = mrinimitable.model.get_doc(":Print Settings", "Print Settings");
		const allow_print_for_draft = cint(print_settings.allow_print_for_draft);
		const is_submittable = mrinimitable.model.is_submittable(this.doctype);
		...
		if (invalid_docs.length > 0) {
			mrinimitable.msgprint(__("You selected Draft or Cancelled documents"));
			return;
		}

		if (valid_docs.length === 0) {
			mrinimitable.msgprint(__("Select atleast 1 record for printing"));
			return;
		}
		...
		const dialog = new mrinimitable.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					fieldname: "print_sel",
					options: mrinimitable.meta.get_print_formats(this.doctype),
					default: mrinimitable.get_meta(this.doctype).default_print_format,
				},
				{
					fieldtype: "Select",
					label: __("Page Size"),
					fieldname: "page_size",
					options: mrinimitable.meta.get_print_sizes(),
		...
		dialog.set_primary_action(__("Print"), (args) => {
			if (!args) return;
			const default_print_format = mrinimitable.get_meta(this.doctype).default_print_format;
			...
			if (args.page_size === "Custom") {
				if (args.page_height === 0 || args.page_width === 0) {
					mrinimitable.throw(__("Page height and width cannot be zero"));
				}
			...
			if (args.background_print) {
				mrinimitable
					.call("mrinimitable.utils.print_format.download_multi_pdf_async", {
					...
					.then((response) => {
						...
						mrinimitable.realtime.task_subscribe(task_id);
						mrinimitable.realtime.on(`task_complete:${task_id}`, (data) => {
							mrinimitable.msgprint({
							...
							mrinimitable.realtime.task_unsubscribe(task_id);
							mrinimitable.realtime.off(`task_complete:${task_id}`);
						});
					});
			} else {
				const w = window.open(
					"/api/method/mrinimitable.utils.print_format.download_multi_pdf?" +
						"doctype=" +
	...
	get_letterhead_options() {
		...
		mrinimitable.call({
			method: "mrinimitable.client.get_list",
			args: {
				doctype: "Letter Head",
	...
	delete(docnames, done = null) {
		mrinimitable
			.call({
				method: "mrinimitable.desk.reportview.delete_items",
				freeze: true,
				freeze_message:
					docnames.length <= 10
	...
	assign(docnames, done) {
		if (docnames.length > 0) {
			const assign_to = new mrinimitable.ui.form.AssignToDialog({
				obj: this,
				method: "mrinimitable.desk.form.assign_to.add_multiple",
				doctype: this.doctype,
				docname: docnames,
				bulk_assign: true,
				re_assign: true,
			...
			mrinimitable.msgprint(__("Select records for assignment"));
		}
	}
	...
	clear_assignment(docnames, done) {
		if (docnames.length > 0) {
			mrinimitable
				.call({
					method: "mrinimitable.desk.form.assign_to.remove_multiple",
					args: {
						doctype: this.doctype,
						names: docnames,
			...
			mrinimitable.msgprint(__("Select records for removing assignment"));
		}
	}
	...
	apply_assignment_rule(docnames, done) {
		if (docnames.length > 0) {
			mrinimitable
				.call("mrinimitable.automation.doctype.assignment_rule.assignment_rule.bulk_apply", {
					doctype: this.doctype,
					docnames: docnames,
				})
				.then(() => done());
	...
	submit_or_cancel(docnames, action = "submit", done = null) {
		...
		mrinimitable.realtime.task_subscribe(task_id);
		return mrinimitable
			.xcall("mrinimitable.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs", {
				doctype: this.doctype,
				action: action,
				docnames: docnames,
				task_id: task_id,
			...
			.then((failed_docnames) => {
				if (failed_docnames?.length) {
					const comma_separated_records = mrinimitable.utils.comma_and(failed_docnames);
					switch (action) {
						case "submit":
							mrinimitable.throw(__("Cannot submit {0}.", [comma_separated_records]));
							break;
						case "cancel":
							mrinimitable.throw(__("Cannot cancel {0}.", [comma_separated_records]));
							break;
						default:
							mrinimitable.throw(__("Cannot {0} {1}.", [action, comma_separated_records]));
					}
				}
				if (failed_docnames?.length < docnames.length) {
					mrinimitable.utils.play_sound(action);
					if (done) done();
				}
			})
			.finally(() => {
	...
	edit(docnames, field_mappings, done) {
		...
		const dialog = new mrinimitable.ui.Dialog({
			...
			primary_action: ({ value }) => {
				...
				mrinimitable
					.call({
						method: "mrinimitable.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs",
						args: {
							doctype: this.doctype,
							freeze: true,
							docnames: docnames,
					...
					.then((r) => {
						...
						dialog.hide();
						mrinimitable.show_alert(__("Updated successfully"));
					});
			},
		...
		const dialog = new mrinimitable.ui.Dialog({
			...
			fields: [
				{
					...
					get_data: function (txt) {
						return mrinimitable.db.get_link_options("Tag", txt);
					},
				},
			],
			...
			primary_action: () => {
				...
				if (args && args.tags) {
					...
					mrinimitable.call({
						method: "mrinimitable.desk.doctype.tag.tag.add_tags",
						args: {
							tags: args.tags,
							dt: this.doctype,
							docs: docnames,
	...
	export(doctype, docnames) {
		mrinimitable.require("data_import_tools.bundle.js", () => {
			const data_exporter = new mrinimitable.data_import.DataExporter(
				doctype,
				"Insert New Records"
			);
