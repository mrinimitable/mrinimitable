...
mrinimitable.views.ListView = class ListView extends mrinimitable.views.BaseList {
	static load_last_view() {
		const route = mrinimitable.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = mrinimitable.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			mrinimitable.set_route(
				"list",
				mrinimitable.router.doctype_layout || doctype,
				mrinimitable.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		const meta = mrinimitable.get_meta(this.doctype);
		this.is_large_table = meta?.is_large_table;

		this.debounced_refresh = mrinimitable.utils.debounce(
	...
	has_permissions() {
		return mrinimitable.perm.has_perm(this.doctype, 0, "read");
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			mrinimitable.set_route("");
			mrinimitable.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(mrinimitable.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = mrinimitable.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${mrinimitable.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		mrinimitable.msgprint(
			mrinimitable.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? mrinimitable.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						mrinimitable.boot.link_title_doctypes.includes(df.options)
					) {
						mrinimitable.model.with_doctype(df.options, () => {
							const meta = mrinimitable.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = mrinimitable.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = mrinimitable.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				mrinimitable.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !mrinimitable.boot.read_only) {
			const doctype_name = __(mrinimitable.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (allowed_filter_types.includes(f[2]) && mrinimitable.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		mrinimitable.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = mrinimitable.meta.get_docfield.bind(null, this.doctype);
		...
		if (mrinimitable.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (mrinimitable.has_indicator(this.doctype) && df.fieldname === "status") {
						return false;
					}
	...
	before_refresh() {
		if (mrinimitable.route_options && this.filter_area) {
	...
	before_render() {
		...
		mrinimitable.model.user_settings.save(this.doctype, "last_view", this.view_name);
		...
		let $columns = this.columns
			.map((col) => {
				let classes = [
					...
					mrinimitable.model.is_numeric_field(col.df) ? "text-right" : "",
					col.df?.fieldname,
				].join(" ");
	...
	get_left_html(doc) {
		...
		for (let i = 0; i < this.columns.length; i++) {
			...
			if (mrinimitable.is_mobile() && col.type == "Field" && [3, 4].includes(i)) {
	...
	get_column_html(col, doc, show_in_mobile) {
		...
		let translated_doctypes = mrinimitable.boot?.translated_doctypes || [];
		...
		const format = () => {
			if (df.fieldtype === "Percent") {
				...
				return mrinimitable.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? mrinimitable.utils.escape_html(value_display)
						: value_display;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${mrinimitable.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${mrinimitable.utils.guess_colour(
					_value
				)} ellipsis"
			...
			} else if (mrinimitable.model.html_fieldtypes.includes(df.fieldtype)) {
				html = `<span class="ellipsis">
					${_value}
				</span>`;
			} else {
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${mrinimitable.utils.escape_html(value)}">${format()}</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${mrinimitable.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		let css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			mrinimitable.model.is_numeric_field(df) ? "text-right" : "",
			fieldname,
		].join(" ");
		...
		if (mrinimitable.is_mobile() && col.type == "Subject") {
			css_class += " bold";
		}
	...
	apply_column_widths() {
		...
		Object.entries(this.column_max_widths).forEach(([fieldname, width]) => {
			$(`.list-view .mrinimitable-list .result .level-left .list-row-col.${fieldname}`).css({
				width: width,
				flex: `1 0 ${width}px`,
			});
		});
	...
	update_listview_classes(has_assignto, assign_to_count) {
		...
		// disable scrolling
		if (this.list_view_settings?.disable_scrolling && !mrinimitable.is_mobile()) {
	...
	get_tags_html(user_tags, limit, colored = false) {
		let get_tag_html = (tag) => {
			...
			if (tag) {
				if (colored) {
					color = mrinimitable.get_palette(tag);
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments d-flex align-items-center">
					${
						mrinimitable.avatar_group(assigned_users, this.max_number_of_avatars - 1, {
							filterable: true,
						})[0].outerHTML
					}
				</div>`;
		...
		if (this.list_view_settings && !this.list_view_settings.disable_comment_count) {
			comment_count = `<span class="comment-count d-flex align-items-center">
				${mrinimitable.utils.icon("es-line-chat-alt")}
			...
			if (button_actions) {
				dropdown_buttons = `
					<button type="button" class="btn btn-xs btn-default ellipsis" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
						...
						${mrinimitable.utils.icon("select", "xs")}
					</button>
	...
	get_form_link(doc) {
		...
		return `/app/${encodeURIComponent(
			mrinimitable.router.slug(mrinimitable.router.doctype_layout || this.doctype)
		)}/${encodeURIComponent(cstr(doc.name))}`;
	}

	get_seen_class(doc) {
		const seen_by = doc._seen ? JSON.parse(doc._seen) : [];
		return seen_by.includes(mrinimitable.session.user) ? "" : "bold";
	}

	get_like_html(doc) {
		const liked_by = doc._liked_by ? JSON.parse(doc._liked_by) : [];
		const is_liked = liked_by.includes(mrinimitable.session.user);
		const title = liked_by.map((u) => mrinimitable.user_info(u).fullname).join(", ");
	...
	get_subject_text(doc, title) {
		...
		if (mrinimitable.model.html_fieldtypes.includes(subject_field.fieldtype)) {
			// NOTE: this is very slow, so only do it for HTML fields
			return mrinimitable.utils.html2text(value);
		} else {
			return value;
		}
	}
	...
	get_indicator_html(doc, show_workflow_state) {
		const indicator = mrinimitable.get_indicator(doc, this.doctype, show_workflow_state);
	...
	get_indicator_dot(doc) {
		const indicator = mrinimitable.get_indicator(doc, this.doctype);
		if (!indicator) return "";
	...
	get_image_url(doc) {
		...
		if (window.cordova && !mrinimitable.utils.is_url(url)) {
			url = mrinimitable.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = mrinimitable.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = mrinimitable.session.user;
				}
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if (link) {
				mrinimitable.set_route(link.pathname);
				return false;
			}
		});
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", (e) => {
			...
			mrinimitable.ui.toggle_like($this, doctype, name);
	...
	setup_realtime_updates() {
		...
		mrinimitable.realtime.doctype_subscribe(this.doctype);
		mrinimitable.realtime.off("list_update");
		mrinimitable.realtime.on("list_update", (data) => {
	...
	disable_realtime_updates() {
		mrinimitable.realtime.doctype_unsubscribe(this.doctype);
	...
	process_document_refreshes() {
		...
		mrinimitable.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = mrinimitable.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return mrinimitable.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (mrinimitable.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (mrinimitable.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					mrinimitable.set_route("list", "data-import", {
		...
		if (
			mrinimitable.model.can_create("Custom Field") &&
			mrinimitable.model.can_create("Property Setter")
		) {
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						mrinimitable.set_route("form", "doctype", doctype);
		...
		if (mrinimitable.user.has_role("System Manager") && mrinimitable.boot.developer_mode) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => mrinimitable.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		mrinimitable.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: mrinimitable.get_meta(this.doctype),
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (mrinimitable.model.has_workflow(this.doctype)) {
			const actions = mrinimitable.workflow.get_all_transition_actions(this.doctype);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						...
						mrinimitable
							.xcall("mrinimitable.model.workflow.bulk_workflow_approval", {
	...
	toggle_workflow_actions() {
		if (!mrinimitable.model.has_workflow(this.doctype)) return;
		...
		mrinimitable
			.xcall("mrinimitable.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				mrinimitable.model.is_value_type(field_doc) &&
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					mrinimitable.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk printing
		if (mrinimitable.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			mrinimitable.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!mrinimitable.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (mrinimitable.model.can_cancel(doctype) && !mrinimitable.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (mrinimitable.model.can_delete(doctype) && is_bulk_edit_allowed(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		if (!params.toString() && mrinimitable.route_options) {
			params = new Map(Object.entries(mrinimitable.route_options));
		}
		...
		params.forEach((value, field) => {
			...
			if (!doctype) {
				doctype = mrinimitable.meta.get_doctype_for_field(this.doctype, field);
			}
...
mrinimitable.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return mrinimitable.views.list_view[route];
};
