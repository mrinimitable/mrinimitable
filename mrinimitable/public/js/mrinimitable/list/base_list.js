...
mrinimitable.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || mrinimitable.router.doctype_layout || __(this.doctype);
		this.meta = mrinimitable.get_meta(this.doctype);
		this.settings = mrinimitable.listview_settings[this.doctype] || {};
		this.user_settings = mrinimitable.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = mrinimitable.is_large_screen() ? 100 : 20;
		this.selected_page_count = this.page_length;
		this.data = [];
		this.method = "mrinimitable.desk.reportview.get";

		this.can_create = mrinimitable.model.can_create(this.doctype);
	...
	get_list_view_settings() {
		return mrinimitable
			.call("mrinimitable.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(mrinimitable.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			mrinimitable.model.std_fields_list.includes(fieldname) ||
			mrinimitable.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = mrinimitable.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!mrinimitable.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return mrinimitable.model.with_doctype(this.doctype);
	}
	...
	setup_view_menu() {
		if (mrinimitable.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			const icon_map = {
				Image: "image-view",
				List: "list",
				Report: "small-file",
			...
			this.views_list = new mrinimitable.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		mrinimitable.breadcrumbs.add(this.meta.module, this.doctype);
	}
	...
	setup_side_bar() {
		...
		this.list_sidebar = new mrinimitable.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$mrinimitable_list = $('<div class="mrinimitable-list">').appendTo(this.page.main);
	}
	 ...
	 * to the `mrinimitable_list` container. This container is used to create a scrollable area for the result content.
	 */
	setup_result_container_area() {
		if (this.view == "List") {
			this.$mrinimitable_list.append($(`<div class="result-container">`));
		}
	}

	setup_result_area() {
		this.$result = $(`<div class="result">`);
		let mrinimitable_list = this.$mrinimitable_list;
		if (this.view == "List") {
			mrinimitable_list = this.$mrinimitable_list.find(".result-container");
		}
		mrinimitable_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$mrinimitable_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$mrinimitable_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$mrinimitable_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map((f) => mrinimitable.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return mrinimitable.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	refresh() {
		...
		return mrinimitable.call(args).then((r) => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		...
		Object.assign(mrinimitable.boot.user_info, data.user_info);
...
class FilterArea {
	constructor(list_view) {
		...
		this.debounced_refresh_list_view = mrinimitable.utils.debounce(
			this.refresh_list_view.bind(this),
			300
		);
		this.setup();
		if (mrinimitable.is_mobile()) this.setup_mobile(list_view);
	}
	setup_mobile(list_view) {
		...
		$(`<button class="filter-toggle btn btn-default btn-sm filter-button">
					<span class="filter-icon button-icon">
						${mrinimitable.utils.icon("funnel-plus")}
					</span>
				</button>
			</div>`)
	...
	clear(refresh = true) {
		...
		return mrinimitable.run_serially(promises).then(() => {
			this.trigger_refresh = true;
				...
				.filter(
					(df) =>
						(df.fieldname === title_field ||
							(df.in_standard_filter && mrinimitable.model.is_value_type(df.fieldtype))) &&
						mrinimitable.perm.has_perm(this.list_view.doctype, df.permlevel)
				)
				.map((df) => {
					...
					if (
						df.fieldtype == "Link" &&
						df.options &&
						mrinimitable.boot.treeviews.includes(df.options)
					) {
						condition = "descendants of (inclusive)";
					}
	...
	make_filter_list() {
		$(`<div class="filter-selector">
			<div class="btn-group">
				<button class="btn btn-default btn-sm filter-button">
					<span class="filter-icon button-icon">
						${mrinimitable.utils.icon("es-line-filter")}
					</span>
				...
				<button class="btn btn-default btn-sm filter-x-button" title="${__("Clear all filters")}">
					<span class="filter-icon button-icon">
						${mrinimitable.utils.icon("es-small-close")}
					</span>
				</button>
			</div>
		...
		this.filter_list = new mrinimitable.ui.FilterGroup({
...
mrinimitable.views.is_valid = (view_mode) => mrinimitable.views.view_modes.includes(view_mode);
