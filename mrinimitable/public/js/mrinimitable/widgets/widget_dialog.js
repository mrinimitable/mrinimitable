		...
		this.dialog = new mrinimitable.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${mrinimitable.utils.get_random(20)}`;
				}
	...
	get_title() {
		if (this.editing) {
			switch (this.type) {
				...
				default:
					return __("Edit {0}", [__(mrinimitable.model.unscrub(this.type))]);
			}
		}
		...
		switch (this.type) {
			...
			default:
				return __("New {0}", [__(mrinimitable.model.unscrub(this.type))]);
		}
	}
...
class QuickListDialog extends WidgetDialog {
	...
	generate_filter_from_json() {
		if (this.values && this.values.quick_list_filter) {
			this.filters = mrinimitable.utils.get_filter_from_json(
...
class ShortcutDialog extends WidgetDialog {
			...
			},
			{
				...
				onchange: () => {
					...
					if (doctype && this.dialog.get_value("type") == "DocType") {
						mrinimitable.model.with_doctype(doctype, async () => {
							let meta = mrinimitable.get_meta(doctype);

							if (doctype && mrinimitable.boot.single_types.includes(doctype)) {
								this.hide_filters();
							} else if (doctype) {
							...
							if (mrinimitable.boot.calendars.includes(doctype)) views.push("Calendar");

							const response = await mrinimitable.db.get_value(
								"Kanban Board",
			...
			},
			{
				...
				depends_on: (state) => {
					if (this.dialog) {
						...
						let is_single = mrinimitable.boot.single_types.includes(doctype);
	...
	process_data(data) {
		...
		data.label = data.label ? data.label : mrinimitable.model.unscrub(data.link_to);

		if (data.url) {
			let _url = data.url;
			if (data.url.startsWith("/")) {
				_url = mrinimitable.urllib.get_base_url() + data.url;
			}
			!validate_url(_url) &&
				mrinimitable.throw({
...
class NumberCardDialog extends WidgetDialog {
			...
			return [
				{
					...
					get_query: () => {
						return {
							query: "mrinimitable.desk.doctype.number_card.number_card.get_cards_for_user",
							filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type && mrinimitable.get_meta(this.document_type)) {
			mrinimitable.get_meta(this.document_type).fields.map((df) => {
		...
		return [
			{
				...
				get_query: () => {
					return {
						query: "mrinimitable.desk.doctype.custom_html_block.custom_html_block.get_custom_blocks_for_user",
					};
				},
			},
		];
