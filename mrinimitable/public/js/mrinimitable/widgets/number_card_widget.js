...
export default class NumberCardWidget extends Widget {
	...
	make_card() {
		mrinimitable.model.with_doc("Number Card", this.number_card_name || this.name).then((card) => {
			if (!card) {
	...
	create_number_card() {
		this.set_doc_args();
		return mrinimitable
			.xcall("mrinimitable.desk.doctype.number_card.number_card.create_number_card", {
				args: this.card_doc,
			})
			.then((doc) => {
				this.name = doc.name;
	...
	set_route() {
		...
		const route = mrinimitable.utils.generate_route({
			name: name,
		...
		if (is_document_type) {
			mrinimitable.route_options = filters.reduce((acc, filter) => {
	...
	set_route_for_custom_card() {
		...
		if (this.data.route_options) {
			mrinimitable.route_options = this.data.route_options;
		}

		mrinimitable.set_route(this.data.route);
	}
	...
	get_settings(type) {
		...
		const settings_map = {
			...
			"Document Type": {
				method: "mrinimitable.desk.doctype.number_card.number_card.get_result",
				args: {
					doc: this.card_doc,
					filters: this.filters,
				},
	...
	get_filters() {
		return mrinimitable.dashboard_utils.get_all_filters(this.card_doc);
	}
	...
	async get_data() {
		this.data = await mrinimitable.xcall(this.settings.method, this.settings.args);
	...
	get_number_for_doctype_card(res) {
		...
		if (this.card_doc.function !== "Count") {
			return mrinimitable.model.with_doctype(this.card_doc.document_type, () => {
				const based_on_df = mrinimitable.meta.get_docfield(
	...
	get_number_for_report_card(res) {
		...
		this.number = mrinimitable.report_utils.get_result_of_fn(this.card_doc.report_function, vals);
	...
	set_formatted_number(df, doc) {
		...
		if (this.card_doc.show_full_number) {
			...
			const shortened_number = mrinimitable.utils.shorten_number(this.number, default_country, 5);
		...
		const formatted_number = mrinimitable.format(number_parts[0], df, null, doc);
		this.formatted_number =
		...
		return this.get_percentage_stats().then(() => {
			if (this.percentage_stat == 0 || this.percentage_stat == undefined) {
				...
				caret_html = `<span class="indicator-pill-round green">
						${mrinimitable.utils.icon("es-line-arrow-up-right", "xs")}
					</span>`;
				color_class = "green-stat";
			} else {
				caret_html = `<span class="indicator-pill-round red">
						${mrinimitable.utils.icon("arrow-down-right", "xs")}
					</span>`;
	...
	get_percentage_stats() {
		return mrinimitable
			.xcall("mrinimitable.desk.doctype.number_card.number_card.get_percentage_difference", {
				doc: this.card_doc,
				filters: this.filters,
				result: this.number,
			})
			.then((res) => {
				if (res !== undefined) {
					this.percentage_stat = mrinimitable.utils.shorten_number(res);
				}
			});
	}
		...
		let actions = [
			...
			},
			{
				...
				handler: () => {
					...
					mrinimitable.set_route("Form", "Number Card", number_card);
				},
			},
		];
