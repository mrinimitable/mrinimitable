...
mrinimitable.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						mrinimitable.assets._executed = mrinimitable.assets._executed.filter(
		...
		// update assets json
		mrinimitable.call("mrinimitable.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				mrinimitable.boot.assets_json = r.message;

				if (mrinimitable.hot_update) {
					mrinimitable.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
