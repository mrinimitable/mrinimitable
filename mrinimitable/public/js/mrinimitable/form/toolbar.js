...
mrinimitable.ui.form.Toolbar = class Toolbar {
	constructor(opts) {
		$.extend(this, opts);
		this.refresh();
	...
	set_title() {
		...
		if (this.frm.is_new()) {
			...
			if (
				...
				this.page.$sub_title_area.on("click", (event) => {
					...
					mrinimitable.utils.copy_to_clipboard(this.frm.docname);
				});
			}
		} else {
			title = this.frm.docname;
		...
		if (this.frm.meta.title_field) {
			mrinimitable.utils.set_title(title + " - " + this.frm.docname);
		}
	...
	rename_document_title(input_name, input_title, merge = false) {
		...
		let rename_document = () => {
			if (input_name != docname) mrinimitable.realtime.doctype_subscribe(doctype, input_name);
			return mrinimitable
				.xcall("mrinimitable.model.rename_doc.update_document_title", {
					doctype,
					docname,
					name: input_name,
					title: input_title,
	...
	set_indicator() {
		var indicator = mrinimitable.get_indicator(this.frm.doc);
		if (
	...
	add_discard() {
		if (
			mrinimitable.model.is_submittable(this.frm.doc.doctype) &&
	...
	add_print() {
		...
		if (
			...
			if (mrinimitable.model.can_print(null, this.frm) && !this.frm.meta.issingle) {
				this.page.add_menu_item(
					__("Print"),
					() => {
						this.frm.print_doc();
	...
	add_email() {
		// email
		if (mrinimitable.model.can_email(null, this.frm) && this.frm.doc.docstatus < 2) {
			this.page.add_menu_item(
				__("Email"),
				() => {
					this.frm.email_doc();
	...
	add_duplicate() {
		if (mrinimitable.boot.user.can_create.includes(this.frm.doctype) && !this.frm.meta.allow_copy) {
			this.page.add_menu_item(
				__("Duplicate"),
				() => {
					this.frm.copy_doc();
	...
	add_delete() {
		...
		if (
			...
			!this.frm.doc.__islocal &&
			!mrinimitable.model.is_single(this.frm.doctype) &&
			mrinimitable.model.can_delete(this.frm.doctype)
		) {
			this.page.add_menu_item(
				__("Delete"),
				() => {
	...
	add_follow() {
		if (this.frm.meta.track_changes && mrinimitable.boot.user.document_follow_notify) {
	...
	add_copy_to_clipboard() {
		...
		this.page.add_menu_item(
			...
			() => {
				mrinimitable.utils.copy_to_clipboard(JSON.stringify(this.frm.doc));
			},
			true
		);
	}
	...
	add_auto_repeat() {
		...
		if (this.can_repeat()) {
			this.page.add_menu_item(
				__("Repeat"),
				() => {
					mrinimitable.utils.new_auto_repeat_prompt(this.frm);
				},
				true
			);
		}
	...
	add_new() {
		...
		if (p[CREATE] && !this.frm.meta.issingle && !this.frm.meta.in_create) {
			this.page.add_menu_item(
				...
				() => {
					mrinimitable.new_doc(this.frm.doctype, true);
				},
				true,
				{
					shortcut: "Ctrl+B",
	...
	add_audit_trail() {
		if (
			...
			mrinimitable.model.get_value("DocType", this.frm.doc.doctype, "track_changes")
		) {
			this.page.add_menu_item(
				__("View Audit Trail"),
				() => {
					mrinimitable.set_route("audit-trail");
				},
				true
			);
		}
	...
	make_customize_buttons() {
		...
		if (mrinimitable.model.can_create("DocType")) {
			if (mrinimitable.boot.developer_mode && !is_doctype_form) {
				// edit doctype
				this.page.add_menu_item(
					__("Edit DocType"),
					() => {
						mrinimitable.set_route("Form", "DocType", this.frm.doctype);
					},
					true
				);
			}
	...
	has_workflow() {
		if (this._has_workflow === undefined)
			this._has_workflow = mrinimitable.model.has_workflow(this.frm.doctype);
	...
	show_linked_with() {
		if (!this.frm.linked_with) {
			this.frm.linked_with = new mrinimitable.ui.form.LinkedWith({
				frm: this.frm,
			});
		}
	...
	set_primary_action(dirty) {
		...
		if (status) {
			...
			if (status !== this.current_status && status === "Amend") {
				...
				mrinimitable
					.xcall(
						"mrinimitable.client.is_document_amended",
						{
							doctype: doc.doctype,
							docname: doc.name,
						},
	...
	set_page_actions(status) {
		...
		if (status === "Edit") {
			...
			if (this.has_workflow()) {
				mrinimitable
					.xcall("mrinimitable.model.workflow.can_cancel_document", {
	...
	follow() {
		...
		mrinimitable
			.call("mrinimitable.desk.form.document_follow.update_follow", {
			...
			.then((r) => {
				...
				mrinimitable.model.set_docinfo(
					this.frm.doctype,
					this.frm.doc.name,
