...
mrinimitable.provide("mrinimitable.model.docinfo");
...
mrinimitable.ui.form.Controller = class FormController {
	constructor(opts) {
		$.extend(this, opts);
	}
};

mrinimitable.ui.form.Form = class MrinimitableForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.sections = [];
		this.grids = [];
		this.cscript = new mrinimitable.ui.form.Controller({ frm: this });
		this.events = {};
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = mrinimitable.get_meta(doctype_layout_name);
		this.undo_manager = new UndoManager({ frm: this });
		this.setup_meta(doctype);
		this.debounced_reload_doc = mrinimitable.utils.debounce(this.reload_doc.bind(this), 1000);
	...
	setup_meta() {
		this.meta = mrinimitable.get_meta(this.doctype);

		if (this.meta.istable) {
			this.meta.in_dialog = 1;
		}

		this.perm = mrinimitable.perm.get_perm(this.doctype); // for create
	...
	setup() {
		this.fields = [];
		this.fields_dict = {};
		this.state_fieldname = mrinimitable.workflow.get_state_fieldname(this.doctype);
		...
		this.toolbar = new mrinimitable.ui.form.Toolbar({
			frm: this,
			page: this.page,
		});

		this.viewers = new mrinimitable.ui.form.FormViewers({
			frm: this,
		...
		this.script_manager = new mrinimitable.ui.form.ScriptManager({
			frm: this,
		});
		this.script_manager.setup();
		this.watch_model_updates();

		if (!this.meta.hide_toolbar && mrinimitable.boot.desk_settings.timeline) {
			// this.footer_tab = new mrinimitable.ui.form.Tab(this.layout, {
			// 	label: __("Activity"),
			// 	fieldname: 'timeline'
			// });

			this.footer = new mrinimitable.ui.form.Footer({
				frm: this,
	...
	add_form_keyboard_shortcuts() {
		...
		mrinimitable.ui.keys.add_shortcut({
			...
			condition: () => mrinimitable.model.can_print(this.doctype, this) && !this.meta.issingle,
		});
	...
	setup_std_layout() {
		...
		// layout
		this.layout = new mrinimitable.ui.form.Layout({
			parent: this.body,
		...
		this.dashboard = new mrinimitable.ui.form.Dashboard(dashboard_parent, this);

		this.tour = new mrinimitable.ui.form.FormTour({
			frm: this,
		});

		// workflow state
		this.states = new mrinimitable.ui.form.States({
			frm: this,
		});
	}
	...
	watch_model_updates() {
		...
		// on table fields
		var table_fields = mrinimitable.get_children("DocType", me.doctype, "fields", {
			fieldtype: ["in", mrinimitable.model.table_fields],
		});
	...
	setup_notify_on_rename() {
		...
		mrinimitable.realtime.on("doc_rename", (data) => {
			...
			if (data.doctype == this.doctype && data.old == this.docname) {
				...
				mrinimitable.set_route("Form", this.doctype, data.new);
			}
		});
	}
	...
	setup_file_drop() {
		...
		this.$wrapper.on("dragenter dragover", false).on("drop", function (e) {
			...
			if (me.doc.__islocal) {
				mrinimitable.msgprint(__("Please save before attaching."));
				throw "attach error";
			}
		...
		this.fields.map((field) => {
			if (field.df.fieldtype === "Markdown Editor") {
				this.set_df_property(field.df.fieldname, "autocompletions", () => {
					...
					return attachments
						.filter((file) => mrinimitable.utils.is_image_file(file.file_url))
						.map((file) => {
							return {
	...
	refresh(docname) {
		...
		if (this.docname) {
			...
			// set the doc
			this.doc = mrinimitable.get_doc(this.doctype, this.docname);

			// check permissions
			this.fetch_permissions();
			if (!this.has_read_permission()) {
				mrinimitable.show_not_permitted(__(this.doctype) + " " + __(cstr(this.docname)));
				return;
			}
			...
			// read only (workflow)
			this.read_only = mrinimitable.workflow.is_read_only(this.doctype, this.docname);
			if (this.read_only) {
				this.set_read_only();
			}
	...
	setup_doctype_actions() {
		if (this.meta.actions) {
			for (let action of this.meta.actions) {
				mrinimitable.ui.form.on(this.doctype, "refresh", () => {
					if (!this.is_new()) {
						if (!action.hidden) {
	...
	execute_action(action) {
		if (typeof action === "string") {
			...
			if (typeof action === "string") {
				mrinimitable.throw(`Action ${action} not found`);
			}
		}
		if (action.action_type === "Server Action") {
			return mrinimitable.xcall(action.action, { doc: this.doc }).then((doc) => {
				if (doc.doctype) {
					// document is returned by the method,
					// apply the changes locally and refresh
					mrinimitable.model.sync(doc);
					this.refresh();
				}
			...
			return mrinimitable.set_route(action.action);
		}
	}
	...
	initialize_new_doc() {
		...
		this.script_manager.trigger("before_load", this.doctype, this.docname).then(() => {
			...
			mrinimitable.after_ajax(function () {
				me.trigger_link_fields();
			});

			mrinimitable.breadcrumbs.add(me.meta.module, me.doctype);
		});
	...
	render_form(switched) {
		if (!this.meta.istable) {
			...
			if (mrinimitable.boot.desk_settings.form_sidebar) {
				this.sidebar = new mrinimitable.ui.form.Sidebar({
					frm: this,
					page: this.page,
				});
				this.sidebar.make();
	...
	run_after_load_hook() {
		if (mrinimitable.route_hooks.after_load) {
			let route_callback = mrinimitable.route_hooks.after_load;
			delete mrinimitable.route_hooks.after_load;
	...
	refresh_header(switched) {
		...
		if (!this.meta.in_dialog || this.in_form) {
			mrinimitable.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
		}
	...
	validate_and_save(save_action, callback, btn, on_error, resolve, reject) {
		...
		var after_save = function (r) {
			...
			if (!r.exc) {
				...
				if (mrinimitable.route_hooks.after_save) {
					let route_callback = mrinimitable.route_hooks.after_save;
					delete mrinimitable.route_hooks.after_save;
		...
		if (save_action != "Update") {
			// validate
			mrinimitable.validated = true;
			mrinimitable
				.run_serially([
					() => this.script_manager.trigger("validate"),
					() => this.script_manager.trigger("before_save"),
					() => {
						if (!mrinimitable.validated) {
							fail();
							return;
						}

						mrinimitable.ui.form.save(me, save_action, after_save, btn);
					},
				])
				.catch(fail);
		} else {
	...
	discard(btn, callback, on_error) {
		...
		return new Promise((resolve) => {
			mrinimitable.confirm(__("Discard {0}", [this.docname]), function () {
						...
						me.save(
							...
							function (r) {
								if (r.exc) {
									...
									mrinimitable.utils.play_sound("submit");
									callback && callback();
									me.script_manager
										.trigger("on_submit")
										.then(() => resolve(me))
										.then(() => {
											if (mrinimitable.route_hooks.after_submit) {
												let route_callback =
													mrinimitable.route_hooks.after_submit;
												delete mrinimitable.route_hooks.after_submit;
												route_callback(me);
											}
										});
								}
	...
	savecancel(btn, callback, on_error) {
		...
		mrinimitable
			.call({
				method: "mrinimitable.desk.form.linked_with.get_submitted_linked_docs",
				args: {
	...
	_cancel_all(r, btn, callback, on_error) {
		...
		for (let doctype of doctypes) {
			if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
				let docnames = links
					...
					.map((link) => mrinimitable.utils.get_form_link(link.doctype, link.name, true))
					.join(", ");
		...
		let can_cancel = links.every((link) => mrinimitable.model.can_cancel(link.doctype));
		if (can_cancel) {
		...
		let d = new mrinimitable.ui.Dialog(
			{
				...
				fields: [
					{
						fieldtype: "HTML",
						options: `<p class="mrinimitable-confirm-message">${confirm_message}</p>`,
					},
				],
			},
			() => me.handle_save_fail(btn, on_error)
		...
		if (can_cancel) {
			d.set_primary_action(__("Cancel All"), () => {
				d.hide();
				mrinimitable.call({
					method: "mrinimitable.desk.form.linked_with.cancel_all_linked_docs",
					args: {
						docs: links,
						ignore_doctypes_on_cancel_all: me.ignore_doctypes_on_cancel_all || [],
					},
			...
			me.script_manager.trigger("before_cancel").then(() => {
				...
				var after_cancel = function (r) {
					if (r.exc) {
						...
						mrinimitable.utils.play_sound("cancel");
						me.refresh();
						callback && callback();
						me.script_manager.trigger("after_cancel");
					}
				};
				mrinimitable.ui.form.save(me, "cancel", after_cancel, btn);
			});
		};
	...
	_discard(btn, on_error, skip_confirm) {
		...
		const discard_doc = () => {
			...
			me.script_manager.trigger("before_discard").then(() => {
				...
				//mrinimitable.ui.form.discard(me, after_discard, btn);
				mrinimitable.call({
					freeze: true,
					method: "mrinimitable.desk.form.save.discard",
					args: {
						doctype: me.doc.doctype,
						name: me.doc.name,
					},
	...
	savetrash() {
		...
		mrinimitable.model.delete_doc(this.doctype, this.docname, function () {
			window.history.back();
		});
	}
	...
	amend_doc() {
		if (!this.fields_dict["amended_from"]) {
			mrinimitable.msgprint(__('"amended_from" field must be present to do an amendment.'));
			return;
		}

		mrinimitable
			.xcall("mrinimitable.client.is_document_amended", {
			...
			.then((is_amended) => {
				...
				var fn = function (newdoc) {
					...
					if (me.fields_dict && me.fields_dict["amendment_date"])
						newdoc.amendment_date = mrinimitable.datetime.obj_to_str(new Date());
				};
				this.copy_doc(fn, 1);
				mrinimitable.utils.play_sound("click");
			});
	...
	validate_form_action(action, resolve) {
		...
		var perms = mrinimitable.perm.get_perm(this.doc.doctype)[0];
		...
		if (
			(mrinimitable.workflow.is_read_only(this.doctype, this.docname) &&
	...
	show_submit_message() {
		if (
			...
			!this.is_dirty() &&
			!this.is_new() &&
			!mrinimitable.model.has_workflow(this.doctype) && // show only if no workflow
	...
	fetch_permissions() {
		...
		this.perm = mrinimitable.perm.get_perm(dt, this.doc);
	}
	...
	check_doctype_conflict(docname) {
		if (this.doctype == "DocType" && docname == "DocType") {
			mrinimitable.msgprint(__("Allowing DocType, DocType. Be careful!"));
		} else if (this.doctype == "DocType") {
			if (mrinimitable.views.formview[docname] || mrinimitable.pages["List/" + docname]) {
				window.location.reload();
			}
		} else {
			if (
				mrinimitable.views.formview.DocType &&
				mrinimitable.views.formview.DocType.frm.opendocs[this.doctype]
			) {
				window.location.reload();
			}
		}
	...
	rename_notify(dt, old, name) {
		...
		// cleanup
		if (this && this.opendocs[old] && mrinimitable.meta.docfield_copy[dt]) {
			// delete docfield copy
			mrinimitable.meta.docfield_copy[dt][name] = mrinimitable.meta.docfield_copy[dt][old];
			delete mrinimitable.meta.docfield_copy[dt][old];
		}
		...
		mrinimitable.re_route[mrinimitable.router.get_sub_path()] = `${encodeURIComponent(
			mrinimitable.router.slug(this.doctype)
		)}/${encodeURIComponent(name)}`;
		!mrinimitable._from_link && mrinimitable.set_route("Form", this.doctype, name);
	}
	...
	print_doc() {
		...
		mrinimitable.set_route("print", this.doctype, this.doc.name);
	}

	navigate_records(prev) {
		let filters, sort_field, sort_order;
		let list_view = mrinimitable.get_list_view(this.doctype);
		if (list_view) {
			filters = list_view.get_filters_for_args();
			sort_field = list_view.sort_by;
			sort_order = list_view.sort_order;
		} else {
			let list_settings = mrinimitable.get_user_settings(this.doctype)["List"];
			if (list_settings) {
		...
		mrinimitable.call("mrinimitable.desk.form.utils.get_next", args).then((r) => {
			if (r.message) {
				mrinimitable.set_route("Form", this.doctype, r.message);
	...
	rename_doc() {
		mrinimitable.model.rename_doc(this.doctype, this.docname, () => this.refresh_header());
	}
	...
	email_doc(message) {
		new mrinimitable.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	copy_doc(onload, from_amend) {
		...
		var newdoc = mrinimitable.model.copy_doc(this.doc, from_amend);
	...
	reload_doc() {
		...
		if (!this.doc.__islocal) {
			mrinimitable.model.remove_from_locals(this.doctype, this.docname);
			return mrinimitable.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	has_perm(ptype) {
		return mrinimitable.perm.has_perm(this.doctype, 0, ptype, this.doc);
	}
	...
	get_docinfo() {
		return mrinimitable.model.docinfo[this.doctype][this.docname];
	}
	...
	set_footnote(txt) {
		this.footnote_area = mrinimitable.utils.set_footnote(this.footnote_area, this.body, txt);
	}
	...
	scroll_to_element() {
		if (mrinimitable.route_options && mrinimitable.route_options.scroll_to) {
			var scroll_to = mrinimitable.route_options.scroll_to;
			delete mrinimitable.route_options.scroll_to;
			...
			if (selector.length) {
				mrinimitable.utils.scroll_to(selector);
			}
		} else if (window.location.hash) {
			if ($(window.location.hash).length) {
				mrinimitable.utils.scroll_to(window.location.hash, true, 200, null, null, true);
			} else {
	...
	show_success_action() {
		...
		const success_action = new mrinimitable.ui.form.SuccessAction(this);
		success_action.show();
	}
	...
	set_currency_labels(fields_list, currency, parentfield) {
		...
		$.each(fields_list, function (i, fname) {
			var docfield = mrinimitable.meta.docfield_map[doctype][fname];
			if (docfield) {
	...
	field_map(fnames, fn) {
		...
		for (var i = 0, l = fnames.length; i < l; i++) {
			...
			var field = mrinimitable.meta.get_docfield(this.doctype, fieldname, this.docname);
			if (field) {
				fn(field);
	...
	get_docfield(fieldname1, fieldname2) {
		if (fieldname2) {
			...
			return mrinimitable.meta.get_docfield(doctype, fieldname2, this.docname);
		} else {
			// for parent
			return mrinimitable.meta.get_docfield(this.doctype, fieldname1, this.docname);
		}
	}
	...
	set_df_property(fieldname, property, value, docname, table_field, table_row_name = null) {
		...
		if (!docname || !table_field) {
			...
			const filtered_fields = mrinimitable.utils.filter_dict(grid.docfields, {
				fieldname: table_field,
			});
			if (filtered_fields.length) {
				df = mrinimitable.meta.get_docfield(
	...
	get_files() {
		return this.attachments
			? mrinimitable.utils.sort(this.attachments.get_attachments(), "file_name", "string")
			: [];
	}
	...
	clear_table(fieldname) {
		mrinimitable.model.clear_table(this.doc, fieldname);
	}

	add_child(fieldname, values) {
		var doc = mrinimitable.model.add_child(
			this.doc,
			mrinimitable.meta.get_docfield(this.doctype, fieldname).options,
			fieldname
		);
		if (values) {
	...
	set_value(field, value, if_missing, skip_dirty_trigger = false) {
		...
		var _set = function (f, v) {
			...
			if (fieldobj) {
				if (!if_missing || !mrinimitable.model.has_value(me.doctype, me.doc.name, f)) {
					if (
						mrinimitable.model.table_fields.includes(fieldobj.df.fieldtype) &&
						$.isArray(v)
					) {
						// set entire child table from specified array as value
						mrinimitable.model.clear_table(me.doc, fieldobj.df.fieldname);

						const standard_fields = [
							...mrinimitable.model.std_fields_list,
							...mrinimitable.model.child_table_field_list,
						];
						v.forEach((d, idx) => {
							let child = mrinimitable.model.add_child(
								me.doc,
								fieldobj.df.options,
						...
						return mrinimitable.model.set_value(
							me.doctype,
							me.doc.name,
							f,
							v,
				...
				mrinimitable.msgprint(__("Field {0} not found.", [f]));
	...
	call(opts, args, callback) {
		...
		if (!opts.doc) {
			if (opts.method.indexOf(".") === -1)
				opts.method = mrinimitable.model.get_server_module_name(me.doctype) + "." + opts.method;
					...
					if (opts.child) {
						...
						if (opts.child) {
							var std_field_list = ["doctype"]
								.concat(mrinimitable.model.std_fields_list)
								.concat(mrinimitable.model.child_table_field_list);
	...
	set_read_only() {
		const docperms = mrinimitable.perm.get_perm(this.doc.doctype);
	...
	get_formatted(fieldname) {
		return mrinimitable.format(
			this.doc[fieldname],
			mrinimitable.meta.get_docfield(this.doctype, fieldname, this.docname),
			{ no_icon: true },
			this.doc
		);
	}
	...
	get_title() {
		return mrinimitable.model.get_doc_title(this.doc);
	}
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		mrinimitable.meta.get_table_fields(this.doctype).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		var doctype;
		if (mrinimitable.meta.docfield_map[this.doctype][fieldname]) {
			doctype = this.doctype;
		} else {
			mrinimitable.meta.get_table_fields(this.doctype).every(function (df) {
				if (mrinimitable.meta.docfield_map[df.options][fieldname]) {
					doctype = df.options;
					return false;
				} else {
					return true;
		...
		mrinimitable.meta.docfield_map[doctype][fieldname].formatter = function (
			value,
			df,
			options,
			doc
			...
			if (value) {
				...
				if (get_text) {
					label = get_text(doc);
				} else if (mrinimitable.form.link_formatters[df.options]) {
					label = mrinimitable.form.link_formatters[df.options](value, doc, df);
				} else {
					label = value;
				}
				...
				return `
						<a class="indicator ${get_color(doc || {})}"
							href="/app/${mrinimitable.router.slug(df.options)}/${escaped_name}"
							data-doctype="${df.options}"
							data-name="${mrinimitable.utils.escape_html(value)}">
							${label}
						</a>
					`;
			} else {
	...
	can_create(doctype) {
		...
		if (!mrinimitable.model.can_create(doctype)) {
			return false;
		}
	...
	make_new(doctype) {
		...
		if (this.make_methods && this.make_methods[doctype]) {
			...
			mrinimitable.model.with_doctype(doctype, function () {
				let new_doc = mrinimitable.model.get_new_doc(doctype, null, null, true);

				// set link fields (if found)
				me.set_link_field(doctype, new_doc);

				mrinimitable.ui.form.make_quick_entry(doctype, null, null, new_doc);
				// mrinimitable.set_route('Form', doctype, new_doc.name);
			});
		}
	}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		mrinimitable.get_meta(doctype).fields.forEach(function (df) {
	...
	update_in_all_rows(table_fieldname, fieldname, value) {
		...
		mrinimitable.model
			...
			.filter((child) => !mrinimitable.model.has_value(child.doctype, child.name, fieldname))
			.forEach((child) =>
				mrinimitable.model.set_value(child.doctype, child.name, fieldname, value)
			);
	}
	...
	scroll_to_field(fieldname, focus = true) {
		...
		// scroll to input
		mrinimitable.utils.scroll_to($el, true, 15);
		...
		let control_element = $el.closest(".mrinimitable-control");
	...
	setup_docinfo_change_listener() {
		...
		if (this.doc && !this.is_new()) {
			mrinimitable.realtime.doc_subscribe(doctype, docname);
		}
		mrinimitable.realtime.off("docinfo_update");
		mrinimitable.realtime.on("docinfo_update", ({ doc, key, action = "update" }) => {
			if (
			...
			let doc_list = mrinimitable.model.docinfo[doctype][docname][key] || [];
			let docindex = doc_list.findIndex((old_doc) => {
				return old_doc.name === doc.name;
			});

			if (action === "add") {
				mrinimitable.model.docinfo[doctype][docname][key].push(doc);
			}
			if (docindex > -1) {
				if (action === "update") {
					mrinimitable.model.docinfo[doctype][docname][key].splice(docindex, 1, doc);
				}
				if (action === "delete") {
					mrinimitable.model.docinfo[doctype][docname][key].splice(docindex, 1);
				}
			}
	...
	set_fields_as_options(
		...
		return new Promise((resolve) => {
			mrinimitable.model.with_doctype(reference_doctype, () => {
				mrinimitable.get_meta(reference_doctype).fields.map((df) => {
					filter_function(df) &&
	...
	get_involved_users() {
		...
		return involved_users
			.uniqBy((u) => u)
			.filter((user) => !["Administrator", mrinimitable.session.user].includes(user))
			.filter(Boolean);
	}
	...
	show_submission_queue_banner() {
		...
		mrinimitable
			.call({
				method: "mrinimitable.core.doctype.submission_queue.submission_queue.get_latest_submissions",
	...
	show_workflow_read_only_banner() {
		...
		if (this.dashboard) {
			...
			mrinimitable.after_ajax(_show_read_only_banner);
		}
	}
};

mrinimitable.validated = 0;
