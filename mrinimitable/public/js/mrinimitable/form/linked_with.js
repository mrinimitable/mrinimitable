...
mrinimitable.ui.form.LinkedWith = class LinkedWith {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	make_dialog() {
		...
		this.dialog.on_page_show = () => {
			mrinimitable
				.xcall("mrinimitable.desk.form.linked_with.get", {
		...
		return `<div class="list-row-container">
			<div class="level list-row small">
				<div class="level-left bold">
					<a href="/app/${mrinimitable.router.slug(doctype)}/${doc.name}">${doc.name}</a>
				</div>
			</div>
		</div>`;
	}
