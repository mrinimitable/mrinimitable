mrinimitable.ui.form.ControlButton = class ControlButton extends mrinimitable.ui.form.ControlData {
	can_write() {
	...
	make_input() {
		...
		this.$input = $(
			`<button
				class="btn ${mrinimitable.utils.escape_html(btn_size)} ${mrinimitable.utils.escape_html(btn_type)}"
				title="${mrinimitable.utils.escape_html(this.df.label)}"
			>`
		)
	...
	set_label(label) {
		...
		label =
			(this.df.icon ? mrinimitable.utils.icon(this.df.icon) : "") +
