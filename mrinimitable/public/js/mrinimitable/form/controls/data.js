mrinimitable.provide("mrinimitable.phone_call");

mrinimitable.ui.form.ControlData = class ControlData extends mrinimitable.ui.form.ControlInput {
	...
	make_input() {
		...
		this.$input.on("paste", (e) => {
			...
			if (maxlength && pasted_data.length > maxlength) {
				...
				if (this.frm && mrinimitable.model.can_write(this.frm.doctype)) {
					let doctype_edit_link = null;
					if (this.frm.meta.custom) {
						doctype_edit_link = mrinimitable.utils.get_form_link(
							"DocType",
							this.frm.doctype,
							true,
							__("this form")
		...
		if (this.df.with_copy_button) {
			this.$wrapper
				...
				.append(
					...
					${mrinimitable.utils.icon("clipboard", "sm")}
				</button>`
				)
				.find(".action-btn")
				.click(() => {
					mrinimitable.utils.copy_to_clipboard(this.value);
				});
		}
	}
	...
	setup_barcode_field() {
		this.$wrapper.find(".control-input").append(
			`<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Scan")}">
					${mrinimitable.utils.icon("scan", "sm")}
				</a>
			</span>`
		);
		...
		const me = this;
		$(document).on("mrinimitable.ui.Dialog:shown", function () {
			me.$scan_btn.toggle(true);
		});
		this.$scan_btn.on("click", "a", () => {
			new mrinimitable.ui.Scanner({
				dialog: true,
				multiple: false,
				on_scan(data) {
	...
	bind_change_event() {
		...
		if (this.constructor.trigger_change_on_input_event && !this.in_grid()) {
			...
			this.$input.on("input", mrinimitable.utils.debounce(change_handler, 500));
		}
	}
	setup_autoname_check() {
		if (!this.df.parent) return;
		this.meta = mrinimitable.get_meta(this.df.parent);
		if (
			this.meta &&
			((this.meta.autoname &&
	...
	validate(v) {
		...
		if (this.df.options == "Phone") {
			...
			var email_list = mrinimitable.utils.split_emails(v);
			if (!email_list) {
				return "";
			} else {
...
mrinimitable.ui.form.ControlReadOnly = mrinimitable.ui.form.ControlData;
