mrinimitable.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	mrinimitable.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${mrinimitable.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
