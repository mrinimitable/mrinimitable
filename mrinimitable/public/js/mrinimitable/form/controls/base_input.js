mrinimitable.ui.form.ControlInput = class ControlInput extends mrinimitable.ui.form.Control {
	...
	make_wrapper() {
		if (this.only_input) {
			this.$wrapper = $('<div class="form-group mrinimitable-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $(
				`<div class="mrinimitable-control">
	...
	refresh_input() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if (me.frm) {
				me.value = mrinimitable.model.get_value(me.doctype, me.docname, me.df.fieldname);
			} else if (me.doc) {
				...
				if (is_fetch_from_read_only) {
					$(me.disp_area).attr(
						...
						__(
							...
							me.df.fetch_from.split(".").map((value) => __(mrinimitable.unscrub(value)))
						)
					);
				}
			}
	...
	set_disp_area(value) {
		...
		if (["Data", "Long Text", "Small Text", "Text", "Password"].includes(this.df.fieldtype)) {
			value = mrinimitable.utils.escape_html(value);
		}
		let doc = this.doc || (this.frm && this.frm.doc);
		let display_value = mrinimitable.format(value, this.df, { no_icon: true, inline: true }, doc);
	...
	set_doc_url() {
		...
		$help.empty();
		$(`<a
			href="${mrinimitable.utils.escape_html(this.df.documentation_url)}"
			target="_blank"
			title="${mrinimitable.utils.escape_html(__("Documentation"))}"
		>
