mrinimitable.ui.form.ControlTime = class ControlTime extends mrinimitable.ui.form.ControlDate {
	...
	set_time_options() {
		...
		this.time_format = mrinimitable.defaultTimeFormat;
		this.datepicker_options = {
			language: "en",
			timepicker: true,
			onlyTimepicker: true,
			timeFormat: time_format.toLowerCase().replace("mm", "ii"),
			startDate: mrinimitable.datetime.now_time(true),
			onSelect: () => {
	...
	set_input(value) {
		...
		let time_format = mrinimitable.sys_defaults.time_format || "HH:mm:ss";
		if (!should_refresh) {
		...
		if (should_refresh) {
			this.datepicker.selectDate(
				mrinimitable.datetime.moment_to_date_obj(moment(value, time_format))
			);
		}
	}
	set_datepicker() {
		...
		this.datepicker.$datepicker.find('[data-action="today"]').click(() => {
			this.datepicker.selectDate(mrinimitable.datetime.now_time(true));
	...
	set_description() {
		...
		const { time_zone } = mrinimitable.sys_defaults;
	...
	format_for_input(value) {
		if (value) {
			return mrinimitable.datetime.str_to_user(value, true);
		}
		return "";
	}
	validate(value) {
		if (value && !mrinimitable.datetime.validate(value)) {
			...
			let time_format =
				sysdefaults && sysdefaults.time_format ? sysdefaults.time_format : "HH:mm:ss";
			mrinimitable.msgprint(__("Time {0} must be in format: {1}", [value, time_format]));
			return "";
		}
		return value;
	}
