mrinimitable.ui.form.ControlCode = class ControlCode extends mrinimitable.ui.form.ControlText {
	make_input() {
	...
	setup_copy_button() {
		...
		this.copy_button.on("click", () => {
			mrinimitable.utils.copy_to_clipboard(this.get_model_value() || this.get_value());
		});
	...
	make_ace_editor() {
		...
		let change_content = mrinimitable.utils.debounce(() => {
	...
	set_language() {
		...
		this.editor.setKeyboardHandler(
			`ace/keyboard/${mrinimitable.boot.user.code_editor_type || "vscode"}`
		);
	}
	...
	load_lib() {
		...
		if (mrinimitable.boot.developer_mode) {
			this.root_lib_path = "/assets/mrinimitable/node_modules/ace-builds/src-noconflict/";
		} else {
			this.root_lib_path = "/assets/mrinimitable/node_modules/ace-builds/src-min-noconflict/";
		}

		this.library_loaded = new Promise((resolve) => {
			mrinimitable.require(this.root_lib_path + "ace.js", () => {
