mrinimitable.ui.form.ControlTableMultiSelect = class ControlTableMultiSelect extends (
	mrinimitable.ui.form.ControlLink
) {
	...
	make_input() {
			...
			this.rows = this.rows.filter((row) => {
				if (row[link_field.fieldname] !== value) {
					...
					mrinimitable.run_serially([
						...
						() => {
							mrinimitable.model.clear_doc(this.df.options, row.name);
		...
		this.$input_area.on("click", ".btn-link-to-form", (e) => {
			...
			const value = decodeURIComponent($value.data().value);
			const link_field = this.get_link_field();
			mrinimitable.set_route("Form", link_field.options, value);
		});
		this.$input.on("keydown", (e) => {
			// if backspace key pressed on empty input, delete last value
			if (e.keyCode == mrinimitable.ui.keyCode.BACKSPACE && e.target.value === "") {
	...
	parse(value, label) {
		...
		if (value) {
			...
			if (this.awesomplete.get_item(value)) {
				if (this.frm) {
					const new_row = mrinimitable.model.add_child(
						this.frm.doc,
						this.df.options,
						this.df.fieldname
					);
				...
				mrinimitable.utils.add_link_title(link_field.options, value, label);
			}
		}
	...
	get_pill_html(value) {
		...
		const pill_name = mrinimitable.utils.get_link_title(link_field.options, value) || value;

		return `
			<button class="data-pill btn tb-selected-value" data-value="${encoded_value}">
				<span class="btn-link-to-form">${__(mrinimitable.utils.escape_html(pill_name))}</span>
				<span class="btn-remove">${mrinimitable.utils.icon("close")}</span>
			</button>
		`;
	}
	get_options() {
	...
	get_link_field() {
		if (!this._link_field) {
			const meta = mrinimitable.get_meta(this.df.options);
