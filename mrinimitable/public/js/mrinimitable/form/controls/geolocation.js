...
mrinimitable.ui.form.ControlGeolocation = class ControlGeolocation extends mrinimitable.ui.form.ControlData {
	...
	set_disp_area(value) {
		...
		if (!this.map_id) {
			this.map_id = mrinimitable.dom.get_unique_id();
			this.map_area = $(
		...
		if (this.frm) {
			...
			$(document).on("mrinimitable.ui.Dialog:shown", () => {
				this.make_map();
				if (value) {
					this.bind_leaflet_data(value);
				}
	...
	customize_draw_controls() {
		...
		L.Icon.Default.imagePath = mrinimitable.utils.map_defaults.image_path;
	}

	bind_leaflet_map() {
		this.map = L.map(this.map_id);
		this.map.setView(mrinimitable.utils.map_defaults.center, mrinimitable.utils.map_defaults.zoom);

		L.tileLayer(mrinimitable.utils.map_defaults.tiles, mrinimitable.utils.map_defaults.options).addTo(
			this.map
		);
	...
	get_leaflet_controls() {
		return new L.Control.Draw({
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: mrinimitable.ui.color.get("blue"),
						weight: 10,
					},
				},
				polygon: {
					...
					drawError: {
						color: mrinimitable.ui.color.get("orange"), // Color the shape will turn when intersects
