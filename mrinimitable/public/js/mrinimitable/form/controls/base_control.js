mrinimitable.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='mrinimitable-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = mrinimitable.perm.get_field_display_status(
			this.df,
			mrinimitable.model.get_doc(this.doctype, this.docname),
		...
		let value = mrinimitable.model.get_value(this.doctype, this.docname, this.df.fieldname);
		...
		if (
			...
			!this.only_input &&
			is_null(value) &&
			cint(mrinimitable.boot.sysdefaults.hide_empty_read_only_fields) &&
	...
	show_translatable_button(value) {
		...
		if (
			...
			!mrinimitable.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!mrinimitable.views || !mrinimitable.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new mrinimitable.views.TranslationManager({
						df: this.df,
