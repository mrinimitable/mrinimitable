mrinimitable.ui.form.ControlImage = class ControlImage extends mrinimitable.ui.form.Control {
	make() {
		super.make();
	...
	refresh_input() {
		...
		if (doc && this.df.options && doc[this.df.options]) {
			...
			this.$buffer = $(
				`<div class='missing-image'>${mrinimitable.utils.icon("restriction", "md")}</div>`
			).appendTo(this.$body);
		}
		return false;
	}
