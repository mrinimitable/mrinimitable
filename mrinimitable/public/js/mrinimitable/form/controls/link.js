...
mrinimitable.ui.form.recent_link_validations = {};

mrinimitable.ui.form.ControlLink = class ControlLink extends mrinimitable.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-open" tabIndex='-1' style="display: inline-block;" title="${__("Open Link")}">
					${mrinimitable.utils.icon("arrow-right", "xs")}
				</a>
			</span>
	...
	show_link_and_clear_buttons() {
		if (this.$input.val() && this.get_options()) {
			...
			this.$link_open.attr("href", mrinimitable.utils.get_form_link(doctype, name));
		}
	}
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return mrinimitable.get_route && mrinimitable.get_route()[0] === "List"
				? mrinimitable.get_route()[1]
				: null;
		}
	}
	setup_buttons() {
	...
	is_translatable() {
		return (mrinimitable.boot?.translated_doctypes || []).includes(this.get_options());
	}
	is_title_link() {
		return (mrinimitable.boot?.link_title_doctypes || []).includes(this.get_options());
	}
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			mrinimitable.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			mrinimitable.route_options = df.get_route_options_for_new_doc(this);
		} else {
			mrinimitable.route_options = {};
		}

		// partially entered name field
		mrinimitable.route_options.name_field = this.get_label_value();

		// reference to calling link
		mrinimitable._from_link = mrinimitable.utils.deep_clone(this);
		mrinimitable._from_link_scrollY = $(document).scrollTop();

		mrinimitable.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				let _label = mrinimitable.utils.escape_html(me.get_translated(d.label));
				...
				if (
					...
					html +=
						'<br><span class="small">' +
						__(mrinimitable.utils.escape_html(d.description)) +
						"</span>";
				}
				return $(`<div role="option">`)
					...
					.html(`<p title="${mrinimitable.utils.escape_html(_label)}">${html}</p>`)
					.get(0);
			},
			sort: function () {
				return 0;
		...
		this.$input.on(
			"input",
			mrinimitable.utils.debounce(function (e) {
				...
				var args = {
					...
					page_length: cint(mrinimitable.boot.sysdefaults?.link_field_results_limit) || 10,
				};
					...
					callback: function (r) {
						...
						if (!me.df.only_select) {
							...
							let custom__link_options =
								mrinimitable.ui.form.ControlLink.link_options &&
								mrinimitable.ui.form.ControlLink.link_options(me);
						...
						r.message.forEach((item) => {
							mrinimitable.utils.add_link_title(doctype, item.value, item.label);
						});
					},
				});
			}, 500)
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				mrinimitable.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (mrinimitable.model.can_select(doctype) && !mrinimitable.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = mrinimitable.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : mrinimitable.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = mrinimitable.meta.get_docfield(this.frm.doctype, key);
	...
	parse_filters(link_filters) {
		...
		link_filters.forEach((filter) => {
			...
			if (value?.startsWith?.("eval:")) {
				...
				value = mrinimitable.utils.eval(value, context);
			}
	...
	validate_link_and_fetch(value) {
		...
		if (value) {
			return mrinimitable
				.xcall(
					"mrinimitable.client.validate_link",
					{
						doctype: options,
						docname: value,
