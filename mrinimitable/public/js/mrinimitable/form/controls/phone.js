...
mrinimitable.ui.form.ControlPhone = class ControlPhone extends mrinimitable.ui.form.ControlData {
	async make_input() {
	...
	async setup_country_codes() {
		...
		if (data) {
			...
			const data = await mrinimitable.xcall("mrinimitable.geo.country_info.get_country_timezone_info");
	...
	input_events() {
		this.$input.keydown((e) => {
			...
			if ([mrinimitable.ui.keyCode.BACKSPACE].includes(key_code)) {
	...
	setup_country_code_picker() {
		...
		if (!this.selected_icon.length) {
			this.selected_icon = $(
				`<div class="selected-phone">${mrinimitable.utils.icon("down", "sm")}</div>`
			);
	...
	get_country_flag(country) {
		...
		return mrinimitable.utils.flag(code);
	}
