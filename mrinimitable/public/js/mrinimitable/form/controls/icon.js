...
mrinimitable.ui.form.ControlIcon = class ControlIcon extends mrinimitable.ui.form.ControlData {
	make_input() {
	...
	get_all_icons() {
		mrinimitable.symbols = [];
		$("#all-symbols > svg > symbol[id]").each(function () {
			this.id.includes("icon-") && mrinimitable.symbols.push(this.id.replace("icon-", ""));
		});
	}
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(
				`<div class="selected-icon">${mrinimitable.utils.icon("folder-normal", "md")}</div>`
			);
