...
mrinimitable.ui.form.get_open_grid_form = function () {
	return $(".grid-row-open").data("grid_row");
};

mrinimitable.ui.form.close_grid_form = function () {
	var open_form = mrinimitable.ui.form.get_open_grid_form();
	open_form && open_form.hide_form();

	// hide editable row too
	if (mrinimitable.ui.form.editable_row) {
...
export default class Grid {
	constructor(opts) {
		...
		this.debounced_refresh = mrinimitable.utils.debounce(this.debounced_refresh, 100);
	}
	...
	make() {
		...
		mrinimitable.utils.bind_actions_with_object(this.wrapper, this);
	...
	set_doc_url() {
		let unsupported_fieldtypes = mrinimitable.model.no_value_type.filter(
			(x) => mrinimitable.model.table_fields.indexOf(x) === -1
		);
		...
		$(`<a href="${this.df.documentation_url}" target="_blank">
			${mrinimitable.utils.icon("help", "sm")}
		</a>`).appendTo($help);
	}
	...
	delete_rows() {
		...
		selected_children.forEach((doc) => {
			...
			tasks.push(() => mrinimitable.timeout(0.1));
		});
		...
		mrinimitable.run_serially(tasks);
	...
	delete_all_rows() {
		mrinimitable.confirm(__("Are you sure you want to delete all rows?"), () => {
	...
	scroll_to_top() {
		mrinimitable.utils.scroll_to(this.wrapper);
	}
	...
	debounced_refresh_remove_rows_button = mrinimitable.utils.debounce(
		this.refresh_remove_rows_button,
		100
	);
	...
	refresh() {
		...
		if (this.frm) {
			this.display_status = mrinimitable.perm.get_field_display_status(
				this.df,
				this.frm.doc,
				this.perm
			);
	...
	setup_fields() {
		...
		if (this.frm && this.frm.docname) {
			...
			this.df = mrinimitable.meta.get_docfield(
				this.frm.doctype,
				this.df.fieldname,
				this.frm.docname
			);
			...
			if (this.df.options) {
				this.df =
					mrinimitable.meta.get_docfield(this.df.options, this.df.fieldname) ||
					this.df ||
					null;
			}
		}
		...
		if (this.doctype && this.frm) {
			this.docfields = mrinimitable.meta.get_docfields(this.doctype, this.frm.docname);
		} else {
	...
	get_data_based_on_fieldtype(df, data, value) {
		...
		if (fieldtype === "Check") {
			value = mrinimitable.utils.string_to_boolean(value);
			...
			let formatted_duration = mrinimitable.utils.get_formatted_duration(fieldvalue);
			...
			let user_formatted_date = mrinimitable.datetime.str_to_user(fieldvalue);
				...
				row.columns_list.forEach((column) => {
					...
					if (column.df.fieldname == fieldname) {
						if (show) {
							...
							if (row != mrinimitable.ui.form.editable_row) {
	...
	get_docfield(fieldname) {
		return mrinimitable.meta.get_docfield(
			this.doctype,
			fieldname,
	...
	add_new_row(idx, callback, show, copy_doc, go_to_last_page = false, go_to_first_page = false) {
		if (this.is_editable()) {
			...
			if (this.frm) {
				var d = mrinimitable.model.add_child(
					this.frm.doc,
					this.df.options,
					this.df.fieldname,
					idx
	...
	setup_visible_columns() {
		...
		for (var ci in fields) {
			...
			if (
				...
				if (
					...
					const docfield = mrinimitable.meta.docfield_map[df.parent][df.fieldname];
		...
		while (total_colsize < 11 && passes < 12) {
			for (var i in this.visible_columns) {
				...
				if (colsize > 1 && colsize < 11 && mrinimitable.model.is_non_std_field(df.fieldname)) {
					if (
						passes < 3 &&
	...
	setup_user_defined_columns() {
		...
		let user_settings = mrinimitable.get_user_settings(this.frm.doctype, "GridView");
		if (user_settings && user_settings[this.doctype] && user_settings[this.doctype].length) {
			this.user_defined_columns = user_settings[this.doctype]
				.map((row) => {
					let column = mrinimitable.meta.get_docfield(this.doctype, row.fieldname);
	...
	set_multiple_add(link, qty) {
		...
		var link_field = mrinimitable.meta.get_docfield(this.df.options, link);
	...
	setup_allow_bulk_edit() {
		...
		if (this.frm && this.frm.get_docfield(this.df.fieldname)?.allow_bulk_edit) {
			...
			const value_formatter_map = {
				Date: (val) => (val ? mrinimitable.datetime.user_to_str(val) : val),
				Int: (val) => cint(val),
				Check: (val) => cint(val),
				Float: (val) => flt(val),
			...
			// upload
			mrinimitable.flags.no_socketio = true;
			$(this.wrapper)
				.find(".grid-upload")
				.removeClass("hidden")
				.on("click", () => {
					new mrinimitable.ui.FileUploader({
						...
						on_success(file) {
							var data = mrinimitable.utils.csv_to_array(
								mrinimitable.utils.get_decoded_string(file.dataurl)
							);
							if (cint(data.length) - 7 > 5000) {
								mrinimitable.throw(__("Cannot import table with more than 5000 rows."));
							}
								...
								if (i > 6) {
									...
									if (!blank_row) {
										...
										$.each(row, (ci, value) => {
											...
											var df = mrinimitable.meta.get_docfield(
												me.df.options,
												fieldname
											);
											if (df) {
	...
	setup_download() {
		let title = this.df.label || mrinimitable.model.unscrub(this.df.fieldname);
		$(this.wrapper)
			.find(".grid-download")
			.removeClass("hidden")
			.on("click", () => {
				...
				data.push(["------"]);
				$.each(mrinimitable.get_meta(this.df.options).fields, (i, df) => {
					// don't include the read-only field in the template
					if (mrinimitable.model.is_value_type(df.fieldtype)) {
						data[1].push(df.label);
						data[2].push(df.fieldname);
						let description = (df.description || "") + " ";
						if (df.fieldtype === "Date") {
							description += mrinimitable.boot.sysdefaults.date_format;
						}
				...
				$.each(this.frm.doc[this.df.fieldname] || [], (i, d) => {
					...
					$.each(data[2], (i, fieldname) => {
						...
						if (docfields[i].fieldtype === "Date" && value) {
							value = mrinimitable.datetime.str_to_user(value);
						}
				...
				mrinimitable.tools.downloadify(data, null, title);
				return false;
			});
	}
