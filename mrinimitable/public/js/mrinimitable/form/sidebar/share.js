...
mrinimitable.ui.form.Share = class Share {
	constructor(opts) {
		$.extend(this, opts);
	...
	render_sidebar() {
		...
		this.shares.show();
		let avatar_group = mrinimitable.avatar_group(shared_users, 5, { align: "left", overlap: true });
	...
	show() {
		...
		mrinimitable.call({
			method: "mrinimitable.share.get_users",
			args: {
	...
	render_shared(shared) {
		...
		$(
			mrinimitable.render_template("set_sharing", {
				frm: this.frm,
				shared: this.shared,
				everyone: everyone,
			})
		...
		if (mrinimitable.model.can_share(null, this.frm)) {
	...
	make_user_input() {
		// make add-user input
		this.dialog.share_with = mrinimitable.ui.form.make_control({
	...
	set_edit_share_events() {
		...
		$(d.body)
			...
			.on("click", function () {
				...
				mrinimitable.call({
					method: "mrinimitable.share.set_permission",
					args: {
						doctype: me.frm.doctype,
						name: me.frm.doc.name,
						user: user,
