...
mrinimitable.ui.form.Attachments = class Attachments {
	constructor(opts) {
		$.extend(this, opts);
	...
	add_attachment(attachment) {
		...
		let file_label = `
			<a href="${file_url}" target="_blank" title="${mrinimitable.utils.escape_html(file_name)}"
		...
		if (this.can_delete_attachment()) {
			remove_action = function (target_id) {
				mrinimitable.confirm(__("Are you sure you want to delete the attachment?"), function () {
		...
		const icon = `<a href="/app/file/${fileid}" class="attachment-icon">
				${mrinimitable.utils.icon(attachment.is_private ? "es-line-lock" : "es-line-unlock", "sm ml-0")}
			</a>`;

		$(`<div class="attachment-row"></div>`)
			.append(mrinimitable.get_data_pill(file_label, fileid, remove_action, icon))
	...
	remove_attachment(fileid, callback) {
		...
		return mrinimitable.call({
			method: "mrinimitable.desk.form.utils.remove_attach",
			type: "DELETE",
			args: {
				fid: fileid,
				dt: me.frm.doctype,
			...
			callback: function (r, rt) {
				if (r.exc) {
					if (!r._server_messages) mrinimitable.msgprint(__("There were errors"));
					return;
				}
