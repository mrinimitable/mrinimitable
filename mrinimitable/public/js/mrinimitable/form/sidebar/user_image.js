mrinimitable.ui.form.set_user_image = function (frm) {
	...
	if (image) {
		image = window.cordova && image.indexOf("http") === -1 ? mrinimitable.base_url + image : image;
		...
		image_section
			...
			.removeClass("hide")
			.find(".standard-image")
			.html(mrinimitable.get_abbr(title));

		title_image.css("background-image", "").html(mrinimitable.get_abbr(title));
...
mrinimitable.ui.form.setup_user_image_event = function (frm) {
	// re-draw image on change of user image
	if (frm.meta.image_field) {
		mrinimitable.ui.form.on(frm.doctype, frm.meta.image_field, function (frm) {
			mrinimitable.ui.form.set_user_image(frm);
		});
	}
