...
mrinimitable.quick_edit = function (doctype, name) {
	if (!name) name = doctype; // single
	mrinimitable.db.get_doc(doctype, name).then((doc) => {
		mrinimitable.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

mrinimitable.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (mrinimitable.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	mrinimitable.quick_entry = new mrinimitable.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return mrinimitable.quick_entry.setup();
};

mrinimitable.ui.form.QuickEntryForm = class QuickEntryForm extends mrinimitable.ui.Dialog {
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = mrinimitable.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.onhide = () => (mrinimitable.quick_entry = null);
		this.show();
			...
			if (data) {
				...
				me.script_manager.trigger("validate").then(() => {
					me.insert().then(() => {
						...
						me.dialog.hide();
						setTimeout(function () {
							mrinimitable.show_alert({ message: messagetxt, indicator: "green" }, 3);
						}, 500);
					});
				});
			}
	...
	insert() {
		...
		return new Promise((resolve) => {
			me.update_doc();
			mrinimitable.call({
				method: "mrinimitable.client.save",
				args: {
					doc: me.dialog.doc,
				},
				callback: function (r) {
					if (
						...
						mrinimitable.model.can_submit(me.doctype) &&
						!mrinimitable.model.has_workflow(me.doctype)
					) {
	...
	submit(doc) {
		var me = this;
		mrinimitable.call({
			method: "mrinimitable.client.submit",
			args: {
				doc: doc,
			},
			callback: function (r) {
	...
	process_after_insert(r) {
		...
		} else if (mrinimitable._from_link) {
			mrinimitable.ui.form.update_calling_link(this.doc);
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			mrinimitable.run_serially([() => mrinimitable.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			mrinimitable.route_options = mrinimitable.route_options || {};
			mrinimitable.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		this.doc.__run_link_triggers = false;
		mrinimitable.set_route("Form", this.doctype, this.doc.name);
	}
