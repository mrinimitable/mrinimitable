...
mrinimitable.provide("mrinimitable.ui.form.handlers");
...
mrinimitable.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!mrinimitable.ui.form.handlers[doctype]) {
		mrinimitable.ui.form.handlers[doctype] = {};
	}
	if (!mrinimitable.ui.form.handlers[doctype][fieldname]) {
		mrinimitable.ui.form.handlers[doctype][fieldname] = [];
	}
	return mrinimitable.ui.form.handlers[doctype][fieldname];
};

mrinimitable.ui.form.on = mrinimitable.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = mrinimitable.ui.form.get_event_handler_list(doctype, fieldname);
...
mrinimitable.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = mrinimitable.ui.form.get_event_handler_list(doctype, fieldname);
...
mrinimitable.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

mrinimitable.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = mrinimitable.get_doc(doctype, name);
	...
	has_handler(event_name) {
		...
		return (
			mrinimitable.ui.form.handlers[this.frm.doctype] &&
			mrinimitable.ui.form.handlers[this.frm.doctype][event_name]
		);
	}
	...
	get_handlers(event_name, doctype) {
		...
		if (mrinimitable.ui.form.handlers[doctype] && mrinimitable.ui.form.handlers[doctype][event_name]) {
			$.each(mrinimitable.ui.form.handlers[doctype][event_name], function (i, fn) {
				handlers.new_style.push(fn);
			});
		}
		if (mrinimitable.ui.form.handlers["*"] && mrinimitable.ui.form.handlers["*"][event_name]) {
			$.each(mrinimitable.ui.form.handlers["*"][event_name], function (i, fn) {
				handlers.new_style.push(fn);
			});
		}
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (mrinimitable.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					mrinimitable.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && mrinimitable.dom.set_style(doctype.__css);

		this.trigger("setup");
	}

	log_error(caller, e) {
		mrinimitable.show_alert({ message: __("Error in Client Script."), indicator: "error" });
