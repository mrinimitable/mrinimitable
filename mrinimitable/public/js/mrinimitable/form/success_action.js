...
mrinimitable.provide("mrinimitable.success_action");

mrinimitable.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = mrinimitable.boot.success_action.find(
	...
	show_alert() {
		mrinimitable.db.get_list(this.form.doctype, { limit: 2 }).then((result) => {
	...
	get default_actions() {
		return {
			new: {
				label: __("New"),
				action: (frm) => mrinimitable.new_doc(frm.doctype),
			},
			print: {
				label: __("Print"),
			...
			list: {
				label: __("View All"),
				action: (frm) => {
					mrinimitable.set_route("List", frm.doctype);
				},
			},
		};
	}
