...
class BaseTimeline {
	...
	add_action_button(label, action, icon = null, btn_class = null) {
		let icon_element = icon ? mrinimitable.utils.icon(icon, "xs") : null;
	...
	get_timeline_item(item) {
		...
		if (item.icon) {
			timeline_item.append(`
				<div class="timeline-badge" title='${item.title || mrinimitable.utils.to_title_case(item.icon)}'>
					${mrinimitable.utils.icon(item.icon, item.icon_size || "md", item.icon_class || "")}
				</div>
			`);
		...
		timeline_item.append(
			`<div class="timeline-content ${item.is_card ? "mrinimitable-card" : ""}">`
		);
			...
			timeline_content.append(
				`<span> · ${
					...
					cint(mrinimitable.boot.sysdefaults.show_absolute_datetime_in_timeline)
						? mrinimitable.datetime.str_to_user(item.creation)
...
mrinimitable.ui.BaseTimeline = BaseTimeline;
