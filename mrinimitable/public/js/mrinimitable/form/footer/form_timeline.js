...
class FormTimeline extends BaseTimeline {
	...
	setup_timeline_actions() {
		if (mrinimitable.model.can_email(null, this.frm)) {
		...
		if (this.frm.meta.allow_events_in_timeline) {
			let create_event = () => {
				const args = {
					...
					txt: mrinimitable.markdown(this.frm.comment_box.get_value()),
				};
				return new mrinimitable.views.InteractionComposer(args);
			};
	...
	setup_document_email_link() {
		...
		if (doc_info.document_email) {
			...
			this.document_email_link_wrapper.find(".document-email-link").on("click", (e) => {
				...
				mrinimitable.utils.copy_to_clipboard(text);
			});
		}
	}
	...
	render_timeline_items() {
		...
		mrinimitable.utils.bind_actions_with_object(this.timeline_items_wrapper, this);
	}

	add_web_page_view_count() {
		if (this.frm.doc.route && cint(mrinimitable.boot.website_tracking_enabled)) {
			mrinimitable.utils.get_page_view_count(this.frm.doc.route).then((res) => {
	...
	async get_more_communication_timeline_contents() {
		...
		let response = await mrinimitable.call({
			method: "mrinimitable.desk.form.load.get_communications",
			args: {
	...
	get_communication_timeline_content(doc, allow_reply = true) {
		doc._url = mrinimitable.utils.get_form_link("Communication", doc.name);
		...
		doc.content = mrinimitable.dom.remove_script_and_style(doc.content);
		let communication_content = $(mrinimitable.render_template("timeline_message_box", { doc }));
		if (allow_reply) {
	...
	get_comment_timeline_content(doc) {
		doc.content = mrinimitable.dom.remove_script_and_style(doc.content);
		const comment_content = $(mrinimitable.render_template("timeline_message_box", { doc }));
	...
	get_milestone_timeline_contents() {
		...
		(this.doc_info.milestones || []).forEach((milestone_log) => {
			const field = mrinimitable.meta.get_label(this.frm.doctype, milestone_log.track_field);
		...
		(this.doc_info.additional_timeline_content || []).forEach((custom_item) => {
			if (custom_item.timeline_badge) {
				custom_timeline_contents.push({
					...
					content: mrinimitable.utils.eval(custom_item.method, {
				...
				custom_timeline_contents.push({
					...
					content:
						custom_item.content ||
						mrinimitable.render_template(custom_item.template, custom_item.template_data),
				});
			}
		});
	...
	setup_reply(communication_box, communication_doc) {
		...
		let reply = $(`<a class="action-btn reply">${mrinimitable.utils.icon("reply", "md")}</a>`).click(
			() => {
				this.compose_mail(communication_doc);
			}
		);
		let reply_all = $(
			`<a class="action-btn reply-all">${mrinimitable.utils.icon("reply-all", "md")}</a>`
		).click(() => {
	...
	add_dropdown_item(timeline_box, menu_items) {
		...
		menu_items.forEach((m) => {
			...
			m.empty();
			m.append(
				__("{0}", [mrinimitable.utils.to_title_case(formatted_action_name.replace("-", " "))])
			);
			m.removeClass();
	...
	compose_mail(communication_doc = null, reply_all = false) {
		const args = {
			...
			frm: this.frm,
			recipients:
				communication_doc && communication_doc.sender != mrinimitable.session.user_email
		...
		const email_accounts = mrinimitable.boot.email_accounts
			.filter((account) => {
				return (
		...
		if (communication_doc && args.is_a_reply) {
			args.cc = "";
			if (
				email_accounts.includes(mrinimitable.session.user_email) &&
				communication_doc.sender != mrinimitable.session.user_email
			) {
				...
				args.cc =
					recipients.filter((r) => r != mrinimitable.session.user_email).join(", ") + ", ";
			}
			if (reply_all) {
		...
		if (this.frm.doctype === "Communication") {
			...
			const comment_value = mrinimitable.markdown(this.frm.comment_box.get_value());
			args.message = strip_html(comment_value) ? comment_value : "";
		}

		new mrinimitable.views.CommunicationComposer(args);
	}
	...
	setup_comment_actions(comment_wrapper, doc) {
		...
		if (mrinimitable.session.user == doc.owner || mrinimitable.user.has_role("System Manager")) {
			if (mrinimitable.model.can_delete("Comment")) {
				const delete_option = $(`
		...
		let edit_button = $();
		let current_user = mrinimitable.session.user;
	...
	make_editable(container) {
		return mrinimitable.ui.form.make_control({
			parent: container,
			df: {
				fieldtype: "Comment",
				fieldname: "comment",
	...
	update_comment(name, content) {
		return mrinimitable
			.xcall("mrinimitable.desk.form.utils.update_comment", { name, content })
			.then(() => {
	...
	delete_comment(comment_name) {
		mrinimitable.confirm(__("Delete comment?"), () => {
			return mrinimitable
				.xcall("mrinimitable.client.delete", {
					doctype: "Comment",
					name: comment_name,
				})
				.then(() => {
	...
	update_comment_publicity(comment_name, publish) {
		...
		mrinimitable.confirm(message, () => {
			return mrinimitable
				.xcall("mrinimitable.desk.form.utils.update_comment_publicity", {
					name: comment_name,
					publish,
				})
				.then(() => {
	...
	copy_link(ev) {
		let doc_link = mrinimitable.urllib.get_full_url(
			mrinimitable.utils.get_form_link(this.frm.doctype, this.frm.docname)
		);
		let element_id = $(ev.currentTarget).closest(".timeline-content").attr("id");
		mrinimitable.utils.copy_to_clipboard(`${doc_link}#${element_id}`);
	}
}
