...
mrinimitable.ui.form.LinkSelector = class LinkSelector {
	constructor(opts) {
		...
		if (this.doctype != "[Select]") {
			mrinimitable.model.with_doctype(this.doctype, function (r) {
				me.make();
			});
		} else {
			this.make();
				...
				if (results.length) {
					...
					$(
						'<p><br><span class="text-muted">' +
							__("No Results") +
							"</span>" +
							(mrinimitable.model.can_create(me.doctype)
						...
						.find(".new-doc")
						.click(function () {
							mrinimitable.new_doc(me.doctype);
						});
				}
	...
	set_in_grid(value) {
		return new Promise((resolve) => {
			if (this.qty_fieldname) {
					...
					(data) => {
						...
						if (!updated) {
							...
							mrinimitable.run_serially([
								...
								() => mrinimitable.timeout(0.1),
								() => {
									let args = {};
									args[this.fieldname] = value;
									args[this.qty_fieldname] = data.qty;
									return mrinimitable.model.set_value(d.doctype, d.name, args);
								},
								() => mrinimitable.show_alert(__("Added {0} ({1})", [value, data.qty])),
								() => resolve(),
							]);
						}
					},
				...
				mrinimitable.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					mrinimitable.show_alert(__("{0} {1} added", [this.dynamic_link_reference, value]));
					resolve();
				});
			} else {
				let d = this.target.add_new_row();
				mrinimitable.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					mrinimitable.show_alert(__("{0} added", [value]));
					resolve();
				});
			}
		});
...
mrinimitable.link_search = function (doctype, args, callback, btn) {
	if (!args) {
		args = {
			txt: "",
		};
