mrinimitable.ui.form.FormTour = class FormTour {
	constructor({ frm }) {
		this.frm = frm;
		this.driver_steps = [];
	}
	...
	init_driver() {
		...
		mrinimitable.router.on("change", () => this.driver.reset());
	...
	async init({ tour_name, on_finish }) {
		if (tour_name) {
			...
			const doctype_tour_exists = await mrinimitable.db.exists("Form Tour", this.frm.doctype);
			if (doctype_tour_exists) {
				this.tour = await mrinimitable.db.get_doc("Form Tour", this.frm.doctype);
			} else {
				this.tour = { steps: mrinimitable.tour[this.frm.doctype] };
			}
		}
		...
		this.tour.steps.forEach((step) => {
			const on_next = () => {
				...
				if (field?.tab && !field.tab.is_active()) {
					...
					mrinimitable.utils.sleep(200).then(() => {
						this.start(step.idx);
						this.driver.overlay.refresh();
					});
				}
	...
	get_step(step_info, on_next, on_prev) {
		...
		let element = `.mrinimitable-control[data-fieldname='${fieldname}']`;
		...
		if (is_table_field) {
			...
			element = `.grid-row-open .mrinimitable-control[data-fieldname='${fieldname}']`;
		}
	...
	add_new_row_step(step) {
		const $add_row = `.mrinimitable-control[data-fieldname='${step.fieldname}'] .grid-add-row`;
		const add_row_step = {
			element: $add_row,
	...
	expand_row_and_proceed(step, start_from) {
		...
		mrinimitable.utils.sleep(300).then(() => this.driver.start(start_from));
	}
	...
	open_first_row_of(fieldname) {
		...
		$($close_row).one("click", () => {
			...
			mrinimitable.utils.scroll_to(next_element, true, 150, null, () => {
				this.driver.moveNext();
				mrinimitable.flags.disable_auto_scroll = false;
			});
			mrinimitable.flags.disable_auto_scroll = true;
		});
	}
