mrinimitable.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new mrinimitable.Driver({
			className: "mrinimitable-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				mrinimitable.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!mrinimitable.boot.user.onboarding_status[this.tour.name] &&
								(mrinimitable.boot.user.onboarding_status[this.tour.name] = {});
							mrinimitable.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								mrinimitable.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							mrinimitable.call({
								method: "mrinimitable.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(mrinimitable.boot.user.onboarding_status),
				...
				if ($input) mrinimitable.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await mrinimitable.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: mrinimitable.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
mrinimitable.ui.init_onboarding_tour = () => {
	...
	typeof mrinimitable.boot.onboarding_tours == "undefined" && mrinimitable.boot.onboarding_tours == [];
	typeof mrinimitable.boot.user.onboarding_status == "undefined" &&
		mrinimitable.boot.user.onboarding_status == {};
	let route = mrinimitable.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		mrinimitable.boot.onboarding_tours &&
			mrinimitable.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (mrinimitable.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (mrinimitable.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(mrinimitable.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == mrinimitable.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == mrinimitable.ui.next_form_tour);
	if (current_tour) {
		...
		mrinimitable.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (mrinimitable.ui.currentTourInstance?.driver) {
		mrinimitable.ui.currentTourInstance.driver_steps = [];
		mrinimitable.ui.currentTourInstance.driver.reset(true);
		mrinimitable.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (mrinimitable.ui.currentTourInstance = new mrinimitable.ui.OnboardingTour());
