...
export const useStore = defineStore("workflow-builder-store", () => {
	...
	async function fetch() {
		await mrinimitable.model.clear_doc("Workflow", workflow_name.value);
		await mrinimitable.model.with_doc("Workflow", workflow_name.value);

		workflow_doc.value = mrinimitable.get_doc("Workflow", workflow_name.value);
		await mrinimitable.model.with_doctype(workflow_doc.value.document_type);

		if (!workflowfields.value.length) {
			await mrinimitable.model.with_doctype("Workflow");
			workflowfields.value = mrinimitable.get_meta("Workflow").fields;
		}

		if (!statefields.value.length) {
			await mrinimitable.model.with_doctype("Workflow Document State");
			statefields.value = mrinimitable.get_meta("Workflow Document State").fields;
		}

		if (!transitionfields.value.length) {
			await mrinimitable.model.with_doctype("Workflow Transition");
			transitionfields.value = mrinimitable.get_meta("Workflow Transition").fields;
		}

		if (!workflow_doc_fields.value.length) {
			let doc_type = workflow_doc.value.document_type;
			await mrinimitable.model.with_doctype(doc_type);
			workflow_doc_fields.value = mrinimitable.meta
	...
	async function save_changes() {
		mrinimitable.dom.freeze(__("Saving..."));
		...
		try {
			...
			await mrinimitable.call("mrinimitable.client.save", { doc });
			mrinimitable.toast(__("Workflow updated successfully"));
			fetch();
		} catch (e) {
			console.error(e);
		} finally {
	...
	function setup_breadcrumbs() {
		...
		`;
		mrinimitable.breadcrumbs.clear();
		mrinimitable.breadcrumbs.$breadcrumbs.append(breadcrumbs);
	}
