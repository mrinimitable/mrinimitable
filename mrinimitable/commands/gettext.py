...
from mrinimitable.commands import pass_context
...
@click.option("--app", help="Only generate for this app. eg: mrinimitable")
@pass_context
def generate_pot_file(context: CliCtxObj, app: str | None = None):
	from mrinimitable.gettext.translate import generate_pot
...
@click.option("--app", help="Only compile for this app. eg: mrinimitable")
@click.option(
	"--force",
	is_flag=True,
	default=False,
...
def compile_translations(context: CliCtxObj, app: str | None = None, locale: str | None = None, force=False):
	from mrinimitable.gettext.translate import compile_translations as _compile_translations
...
@click.option("--app", help="Only migrate for this app. eg: mrinimitable")
...
def csv_to_po(context: CliCtxObj, app: str | None = None, locale: str | None = None):
	from mrinimitable.gettext.translate import migrate
...
@click.option("--app", help="Only update for this app. eg: mrinimitable")
...
def update_po_files(context: CliCtxObj, app: str | None = None, locale: str | None = None):
	from mrinimitable.gettext.translate import update_po
...
@click.option("--app", help="Only create for this app. eg: mrinimitable")
@pass_context
def create_po_file(context: CliCtxObj, locale: str, app: str | None = None):
	"""Create PO file for lang code"""
	from mrinimitable.gettext.translate import new_po
...
def connect_to_site(site):
	from mrinimitable import connect
