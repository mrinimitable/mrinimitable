...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			...
			mrinimitable.exceptions.SiteNotSpecifiedError,
			mrinimitable.exceptions.IncorrectSitePath,
			mrinimitable.exceptions.CommandFailedError,
		) as e:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise mrinimitable.SiteNotSpecifiedError
		return None
...
def get_commands():
	...
	clickable_link = "https://mrinimitableframework.com/docs"
	all_commands = (
		scheduler_commands
		+ site_commands
		+ testing_commands
