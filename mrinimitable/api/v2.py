...
from mrinimitable import _, cint, cstr, get_newargs, is_whitelisted
from mrinimitable.core.doctype.server_script.server_script_utils import get_server_script_map
from mrinimitable.handler import is_valid_http_method, run_server_script, upload_file
...
def handle_rpc_call(method: str, doctype: str | None = None):
	from mrinimitable.modules.utils import load_doctype_module
	...
	method = mrinimitable.override_whitelisted_method(method)
	...
	try:
		...
		mrinimitable.throw(_("Failed to get method {0} with {1}").format(method, e))
...
def document_list(doctype: str) -> list[dict[str, Any]]:
	...
	Query Parameters (accessible via mrinimitable.form_dict):
	...
	Response:
		mrinimitable.response["data"]: List of document records as dicts
		mrinimitable.response["has_next_page"]: Indicates if more pages are available
	...
	Controller Customization:
		...
		Example:
			class Project(Document):
				@staticmethod
				def get_list(query):
					Project = mrinimitable.qb.DocType("Project")
					if user_has_role("Project Owner"):
						query = query.where(Project.owner == mrinimitable.session.user)
					else:
	...
	args = mrinimitable.form_dict
	fields: list | None = mrinimitable.parse_json(args.get("fields", None))
	filters: dict | None = mrinimitable.parse_json(args.get("filters", None))
	...
	if hasattr(controller, "get_list"):
		try:
			...
			mrinimitable.throw(_("Error in {0}.get_list: {1}").format(doctype, str(e)))

	data = query.run(as_dict=as_dict, debug=debug)
	mrinimitable.response["has_next_page"] = len(data) > limit
	return data[:limit]


def count(doctype: str) -> int:
	from mrinimitable.desk.reportview import get_count
...
def create_doc(doctype: str):
	data = mrinimitable.form_dict
	data.pop("doctype", None)

	doc = mrinimitable.new_doc(doctype, **data)
...
def copy_doc(doctype: str, name: str, ignore_no_copy: bool = True):
	...
	copy = mrinimitable.copy_doc(doc, ignore_no_copy=ignore_no_copy)
...
def delete_doc(doctype: str, name: str):
	mrinimitable.client.delete_doc(doctype, name)
	mrinimitable.response.http_status_code = 202
	return "ok"


def get_meta(doctype: str):
	mrinimitable.only_for("All")
	return mrinimitable.get_meta(doctype)
...
def execute_doc_method(doctype: str, name: str, method: str | None = None):
	...
	doc.check_permission(PERMISSION_MAP[mrinimitable.request.method])
	result = doc.run_method(method, **mrinimitable.form_dict)
	mrinimitable.response.docs.append(doc.as_dict())
	return result
...
def run_doc_method(method: str, document: dict[str, Any] | str, kwargs=None):
	...
	if isinstance(document, str):
		document = mrinimitable.parse_json(document)

	if kwargs is None:
		kwargs = {}

	doc = mrinimitable.get_doc(document)
	...
	mrinimitable.response.docs.append(doc)  # send modified document and result both.
	return response
...
url_rules = [
	...
	Rule("/method/ping", endpoint=mrinimitable.ping),
	Rule("/method/upload_file", endpoint=upload_file),
	Rule("/method/<method>", endpoint=handle_rpc_call),
	Rule(
		"/method/run_doc_method",
		methods=["GET", "POST"],
		endpoint=lambda: mrinimitable.call(run_doc_method, **mrinimitable.form_dict),
	),
