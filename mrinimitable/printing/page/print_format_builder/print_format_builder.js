mrinimitable.pages["print-format-builder"].on_page_load = function (wrapper) {
	mrinimitable.print_format_builder = new mrinimitable.PrintFormatBuilder(wrapper);
	mrinimitable.breadcrumbs.add("Setup", "Print Format");
};

mrinimitable.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = mrinimitable.get_route();
	if (route.length > 1) {
		mrinimitable.model.with_doc("Print Format", route[1], function () {
			mrinimitable.print_format_builder.print_format = mrinimitable.get_doc("Print Format", route[1]);
			mrinimitable.print_format_builder.refresh();
		});
	} else if (mrinimitable.route_options) {
		if (mrinimitable.route_options.make_new) {
			let { doctype, name, based_on, beta } = mrinimitable.route_options;
			mrinimitable.route_options = null;
			mrinimitable.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			mrinimitable.print_format_builder.print_format = mrinimitable.route_options.doc;
			mrinimitable.route_options = null;
			mrinimitable.print_format_builder.refresh();
		}
	}
...
mrinimitable.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = mrinimitable.ui.make_app_page({
			parent: this.parent,
	...
	show_start() {
		this.page.main.html(mrinimitable.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = mrinimitable.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			mrinimitable.model.with_doc("Print Format", name, function (doc) {
				mrinimitable.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = mrinimitable.ui.form.make_control({
		...
		this.name_input = mrinimitable.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				mrinimitable.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		mrinimitable.call({
			method: "mrinimitable.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		mrinimitable.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = mrinimitable.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				mrinimitable.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(mrinimitable.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			mrinimitable.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						mrinimitable.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!mrinimitable.model.layout_fields.includes(f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(mrinimitable.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						field = mrinimitable.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = mrinimitable.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				mrinimitable.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = mrinimitable.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				mrinimitable.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		mrinimitable.call({
			...
			callback: function (r) {
				...
				mrinimitable.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
