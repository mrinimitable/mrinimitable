...
from mrinimitable.utils import flt, is_image
...
class LetterHead(Document):
	...
	def validate_disabled_and_default(self):
		if self.disabled and self.is_default:
			mrinimitable.throw(_("Letter Head cannot be both disabled and default"))

		if not self.is_default and not self.disabled:
			if not mrinimitable.db.exists("Letter Head", dict(is_default=1)):
				self.is_default = 1
	...
	def set_image_as_html(
		...
		if not self.get(field) or not is_image(self.get(field)):
			mrinimitable.msgprint(failure_msg, alert=True, indicator="orange")
			return
...
{dimension}="{dimension_value}" style="{dimension}: {dimension_value}px;">
		...
		mrinimitable.msgprint(success_msg, alert=True)
	...
	def set_as_default(self):
		...
		if self.is_default:
			mrinimitable.db.set_value("Letter Head", {"name": ["!=", self.name]}, "is_default", 0)

			set_default("letter_head", self.name)

			# update control panel - so it loads new letter directly
			mrinimitable.db.set_default("default_letter_head_content", self.content)
		else:
			mrinimitable.defaults.clear_default("letter_head", self.name)
			mrinimitable.defaults.clear_default("default_letter_head_content", self.content)
