...
import mrinimitable
from mrinimitable.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/mrinimitable.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "mrinimitable.email.oauth.authorize_google_access",
	"contacts": "mrinimitable.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"indexing": "mrinimitable.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True, config=None):
		self.google_settings = mrinimitable.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			mrinimitable.throw(mrinimitable._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			mrinimitable.throw(mrinimitable._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		mrinimitable.log_error(mrinimitable._(error_title), mrinimitable._(response.get("error_description", error_message)))

		if raise_err:
			mrinimitable.throw(mrinimitable._(error_title), GoogleAuthenticationError, mrinimitable._(error_message))
...
def callback(state: str, code: str | None = None, error: str | None = None) -> None:
	...
	Invokes functions using `mrinimitable.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to mrinimitable site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			mrinimitable.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
	...
	mrinimitable.local.response["location"] = f"{redirect}?{failure_query_param if error else success_query_param}"
