...
class BaseUrlNotSetError(mrinimitable.ValidationError):
	pass


class AuthorizeUrlNotSetError(mrinimitable.ValidationError):
	pass


class AccessTokenUrlNotSetError(mrinimitable.ValidationError):
	pass


class RedirectUrlNotSetError(mrinimitable.ValidationError):
	pass


class ClientIDNotSetError(mrinimitable.ValidationError):
	pass


class ClientSecretNotSetError(mrinimitable.ValidationError):
	pass
...
class SocialLoginKey(Document):
	...
	def autoname(self):
		self.name = mrinimitable.scrub(self.provider_name)

	def validate(self):
		self.set_icon()
		if self.custom_base_url and not self.base_url:
			mrinimitable.throw(_("Please enter Base URL"), exc=BaseUrlNotSetError)
		if not self.authorize_url:
			mrinimitable.throw(_("Please enter Authorize URL"), exc=AuthorizeUrlNotSetError)
		if not self.access_token_url:
			mrinimitable.throw(_("Please enter Access Token URL"), exc=AccessTokenUrlNotSetError)
		if not self.redirect_url:
			mrinimitable.throw(_("Please enter Redirect URL"), exc=RedirectUrlNotSetError)
		if self.enable_social_login and not self.client_id:
			mrinimitable.throw(_("Please enter Client ID before social login is enabled"), exc=ClientIDNotSetError)
		...
		if self.auth_url_data:
			try:
				...
				mrinimitable.throw(_("Auth URL data should be valid JSON"))

		if self.api_endpoint_args:
			try:
				json.loads(self.api_endpoint_args)
			except json.JSONDecodeError:
				mrinimitable.throw(_("API Endpoint Args should be valid JSON"))

	def set_icon(self):
		icon_map = {
			"Google": "google.svg",
			"Mrinimitable": "mrinimitable.svg",
		...
		if self.provider_name in icon_map:
			...
			self.icon = f"/assets/mrinimitable/icons/social/{icon_file}"
	...
	def get_social_login_provider(self, provider, initialize=False):
		...
		providers["Office 365"] = {
			...
			"redirect_url": "/api/method/mrinimitable.integrations.oauth2_logins.login_via_office365",
			"api_endpoint": None,
		...
		providers["GitHub"] = {
			...
			"redirect_url": "/api/method/mrinimitable.integrations.oauth2_logins.login_via_github",
			"api_endpoint": "user",
			"api_endpoint_args": None,
		...
		providers["Google"] = {
			...
			"redirect_url": "/api/method/mrinimitable.integrations.oauth2_logins.login_via_google",
		...
		providers["Facebook"] = {
			...
			"redirect_url": "/api/method/mrinimitable.integrations.oauth2_logins.login_via_facebook",
		...
		providers["Mrinimitable"] = {
			"provider_name": "Mrinimitable",
			"enable_social_login": 1,
			"custom_base_url": 1,
			"icon": "/assets/mrinimitable/images/mrinimitable-favicon.svg",
			"redirect_url": "/api/method/mrinimitable.integrations.oauth2_logins.login_via_mrinimitable",
			"api_endpoint": "/api/method/mrinimitable.integrations.oauth2.openid_profile",
			"api_endpoint_args": None,
			"authorize_url": "/api/method/mrinimitable.integrations.oauth2.authorize",
			"access_token_url": "/api/method/mrinimitable.integrations.oauth2.get_token",
			"auth_url_data": json.dumps({"response_type": "code", "scope": "openid"}),
		}
		...
		providers["Salesforce"] = {
			...
			"redirect_url": "/api/method/mrinimitable.integrations.oauth2_logins.login_via_salesforce",
		...
		providers["fairlogin"] = {
			...
			"custom_base_url": 0,
			"icon": "fa fa-key",
			"redirect_url": "/api/method/mrinimitable.integrations.oauth2_logins.login_via_fairlogin",
		...
		providers["Keycloak"] = {
			...
			"enable_social_login": 1,
			"custom_base_url": 1,
			"redirect_url": "/api/method/mrinimitable.integrations.oauth2_logins.login_via_keycloak",
...
def provider_allows_signup(provider: str) -> bool:
	from mrinimitable.website.utils import is_signup_disabled

	sign_up_config = mrinimitable.db.get_value("Social Login Key", provider, "sign_ups")
