...
import mrinimitable
from mrinimitable import _, safe_encode
from mrinimitable.model.document import Document
from mrinimitable.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from mrinimitable.integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			mrinimitable.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			mrinimitable.throw(_("Invalid username or password"))
		except Exception as ex:
			mrinimitable.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = mrinimitable.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "mrinimitable.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def create_or_update_user(self, user_data: dict, groups: list | None = None):
		...
		if mrinimitable.db.exists("User", user_data["email"]):
			user = mrinimitable.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = mrinimitable.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			mrinimitable.throw(_("LDAP is not enabled."))
		...
		try:
			...
			mrinimitable.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					...
					mrinimitable.msgprint(_("Password changed successfully."))
				else:
					mrinimitable.throw(_("Failed to change password."))
			else:
				mrinimitable.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			mrinimitable.throw(_("No LDAP User found for email: {0}").format(user))
	...
	def convert_ldap_entry_to_dict(self, user_entry: Entry):
		...
		if isinstance(email, list):
			...
			for e in email:
				if mrinimitable.db.exists("User", e):
					email = e
					break
			else:
...
def login():
	...
	ldap: LDAPSettings = mrinimitable.get_doc("LDAP Settings")

	user = ldap.authenticate(mrinimitable.as_unicode(args.usr), mrinimitable.as_unicode(args.pwd))

	mrinimitable.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(mrinimitable.local.login_manager):
			return False

	mrinimitable.form_dict.pop("pwd", None)
	mrinimitable.local.login_manager.post_login()
