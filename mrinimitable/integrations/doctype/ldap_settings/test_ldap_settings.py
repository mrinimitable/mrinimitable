...
import mrinimitable
from mrinimitable.exceptions import MandatoryError, ValidationError
...
class LDAP_TestCase:
	...
	def mock_ldap_connection(f):
		...
		def wrapped(self, *args, **kwargs):
			with mock.patch(
				"mrinimitable.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.connect_to_ldap",
				...
				mrinimitable.get_doc(localdoc).save()
	...
	def clean_test_users():
		with contextlib.suppress(Exception):
			mrinimitable.get_doc("User", "posix.user1@unit.testing").delete()
		with contextlib.suppress(Exception):
			mrinimitable.get_doc("User", "posix.user2@unit.testing").delete()
		with contextlib.suppress(Exception):
			mrinimitable.get_doc("User", "website_ldap_user@test.com").delete()

	@classmethod
	def setUpClass(cls):
		cls.clean_test_users()
		# Save user data for restoration in tearDownClass()
		cls.user_ldap_settings = mrinimitable.get_doc("LDAP Settings")
		...
		user = mrinimitable.get_doc(cls.user1doc)
		...
		user = mrinimitable.get_doc(cls.user2doc)
		user.insert(ignore_permissions=True)
	...
	def tearDownClass(cls):
		with contextlib.suppress(Exception):
			mrinimitable.get_doc("LDAP Settings").delete()
	...
	def test_connect_to_ldap(self):
		...
		prevent_connection_parameters = {
			"mode": {
				"IP_V4_ONLY": "Locks the user to IPv4 without mrinimitable providing a way to configure",
				"IP_V6_ONLY": "Locks the user to IPv6 without mrinimitable providing a way to configure",
			},
			"auto_bind": {
	...
	def test_update_user_fields(self):
		...
		test_user = mrinimitable.get_doc("User", test_user_data["email"])
		self.test_class.update_user_fields(test_user, test_user_data)
		updated_user = mrinimitable.get_doc("User", test_user_data["email"])
		...
		self.assertIn(self.test_class.default_role, mrinimitable.get_roles(updated_user.name))
	...
	def test_create_website_user(self):
		...
		new_user = mrinimitable.get_doc("User", new_test_user_data["email"])
	...
	def test_sync_roles(self):
		if self.TEST_LDAP_SERVER.lower() == "openldap":
			test_user_data = {
				"posix.user1": [
					...
					"Administrators",
					"default_role",
					"mrinimitable_default_all",
					"mrinimitable_default_guest",
					"mrinimitable_default_desk_user",
				],
				"posix.user2": [
					"Users",
					"Group3",
		...
		role_to_group_map = {
			...
			"All": "mrinimitable_default_all",
			"Guest": "mrinimitable_default_guest",
			"Desk User": "mrinimitable_default_desk_user",
		}

		# re-create user1 to ensure clean
		mrinimitable.get_doc("User", "posix.user1@unit.testing").delete()
		user = mrinimitable.get_doc(self.user1doc)
		user.insert(ignore_permissions=True)

		for test_user in test_user_data:
			test_user_doc = mrinimitable.get_doc("User", f"{test_user}@unit.testing")
			test_user_roles = mrinimitable.get_roles(f"{test_user}@unit.testing")

			self.assertTrue(
				len(test_user_roles) == 2, "User should only be a part of the All and Guest roles"
			)  # check default mrinimitable roles

			self.test_class.sync_roles(test_user_doc, test_user_data[test_user])  # update user roles

			mrinimitable.get_doc("User", f"{test_user}@unit.testing")
			updated_user_roles = mrinimitable.get_roles(f"{test_user}@unit.testing")
	...
	def test_create_or_update_user(self):
		...
		mrinimitable.get_doc("User", f"{test_user}@unit.testing").delete()

		with self.assertRaises(
			mrinimitable.exceptions.DoesNotExistError
		):  # ensure user deleted so function can be tested
			mrinimitable.get_doc("User", f"{test_user}@unit.testing")

		with mock.patch(
			"mrinimitable.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.update_user_fields"
		) as update_user_fields_method:
			with mock.patch(
				"mrinimitable.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.sync_roles"
	...
	def test_authenticate(self):
		with mock.patch(
			"mrinimitable.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.fetch_ldap_groups"
		...
		for username, password in enumerate(invalid_users):
			with self.assertRaises(mrinimitable.exceptions.ValidationError) as display_massage:
...
class Test_OpenLDAP(LDAP_TestCase, TestCase):
	...
	DOCUMENT_GROUP_MAPPINGS: typing.ClassVar[list] = [
		...
		{"doctype": "LDAP Group Mapping", "ldap_group": "Users", "okayblue_role": "Website Manager"},
...
class Test_ActiveDirectory(LDAP_TestCase, TestCase):
	...
	DOCUMENT_GROUP_MAPPINGS: typing.ClassVar[list] = [
		...
		{"doctype": "LDAP Group Mapping", "ldap_group": "Domain Users", "okayblue_role": "Website Manager"},
		{
			"doctype": "LDAP Group Mapping",
