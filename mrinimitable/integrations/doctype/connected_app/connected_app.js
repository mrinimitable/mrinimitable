...
mrinimitable.ui.form.on("Connected App", {
	refresh: (frm) => {
		frm.add_custom_button(__("Get OpenID Configuration"), async () => {
			if (!frm.doc.openid_configuration) {
				mrinimitable.msgprint(__("Please enter OpenID Configuration URL"));
			} else {
				try {
					...
					mrinimitable.msgprint(__("Please check OpenID Configuration URL"));
				}
			}
		});
