...
import mrinimitable
from mrinimitable.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = mrinimitable.new_doc(doctype)
	connected_app.provider_name = "mrinimitable"
...
def get_oauth_client():
	oauth_client = mrinimitable.new_doc("OAuth Client")
...
class TestConnectedApp(IntegrationTestCase):
	def setUp(self):
		...
		Mrinimitable comes with it's own oAuth2 provider that we can test against. The
		...
		Mrinimitable's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/mrinimitable.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		mrinimitable.db.commit()  # Avoid snapshot violation issues

		delete_if_exists("token_cache")
		delete_if_exists("connected_app")

		if getattr(self, "oauth_client", None):
			tokens = mrinimitable.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = mrinimitable.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = mrinimitable.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = mrinimitable.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
