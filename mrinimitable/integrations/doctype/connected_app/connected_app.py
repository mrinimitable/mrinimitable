...
from mrinimitable.integrations.utils import make_get_request
from mrinimitable.model.document import Document

if any((os.getenv("CI"), mrinimitable.conf.developer_mode, mrinimitable.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	if TYPE_CHECKING:
		from mrinimitable.integrations.doctype.oauth_scope.oauth_scope import OAuthScope
		from mrinimitable.integrations.doctype.query_parameters.query_parameters import QueryParameters
	...
	def get_openid_configuration(self):
		if not self.openid_configuration:
			mrinimitable.throw(_("Please enter OpenID Configuration URL"))
		return make_get_request(self.openid_configuration)

	def validate(self):
		base_url = mrinimitable.utils.get_url()
		callback_path = (
			"/api/method/mrinimitable.integrations.doctype.connected_app.connected_app.callback/" + self.name
		)
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = mrinimitable.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		mrinimitable.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if mrinimitable.db.exists("Token Cache", token_cache_name):
			token_cache = mrinimitable.get_doc("Token Cache", token_cache_name)
...
@mrinimitable.whitelist(methods=["GET"], allow_guest=True)
def callback(code=None, state=None):
	...
	if mrinimitable.session.user == "Guest":
		mrinimitable.local.response["type"] = "redirect"
		mrinimitable.local.response["location"] = "/login?" + urlencode({"redirect-to": mrinimitable.request.url})
		return

	path = mrinimitable.request.path[1:].split("/")
	if len(path) != 4 or not path[3]:
		mrinimitable.throw(_("Invalid Parameters."))

	connected_app = mrinimitable.get_doc("Connected App", path[3])
	token_cache = mrinimitable.get_doc("Token Cache", connected_app.name + "-" + mrinimitable.session.user)

	if state != token_cache.state:
		mrinimitable.throw(_("Invalid token state! Check if the token has been created by the OAuth user."))
	...
	mrinimitable.local.response["type"] = "redirect"
	mrinimitable.local.response["location"] = token_cache.get("success_uri") or connected_app.get_url()


@mrinimitable.whitelist()
def has_token(connected_app, connected_user=None):
	app = mrinimitable.get_doc("Connected App", connected_app)
	token_cache = app.get_token_cache(connected_user or mrinimitable.session.user)
