...
class GoogleContacts(Document):
	...
	def validate(self):
		if not mrinimitable.db.get_single_value("Google Settings", "enable"):
			mrinimitable.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = mrinimitable.bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	"""
	contact = mrinimitable.get_doc("Google Contacts", g_contact)
...
def get_google_contacts_object(g_contact):
	...
	account = mrinimitable.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = mrinimitable.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	mrinimitable.db.set_value("Google Contacts", account.name, "last_sync_on", mrinimitable.utils.now_datetime())

	for idx, connection in enumerate(results):
		mrinimitable.publish_realtime(
			"import_google_contacts", dict(progress=idx + 1, total=len(results)), user=mrinimitable.session.user
		)
		# Work-around to fix
		# https://github.com/mrinimitable/mrinimitable/issues/22648
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not mrinimitable.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		mrinimitable.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		mrinimitable.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		mrinimitable.msgprint(
