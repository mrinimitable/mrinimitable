...
import mrinimitable
from mrinimitable.integrations.doctype.oauth_client.oauth_client import OAuthClient
from mrinimitable.utils import get_request_session
...
def make_request(method: str, url: str, auth=None, headers=None, data=None, json=None, params=None):
	...
	try:
		...
		response = mrinimitable.flags.integration_request = s.request(
		...
		if mrinimitable.flags.integration_request_doc:
			mrinimitable.flags.integration_request_doc.log_error()
		else:
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = mrinimitable.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else mrinimitable.as_json(obj, indent=1)
...
def validate_dynamic_client_metadata(client: OAuth2DynamicClientMetadata):
	...
	if not mrinimitable.conf.developer_mode and any(c.scheme != "https" for c in client.redirect_uris):
...
def create_new_oauth_client(client: OAuth2DynamicClientMetadata):
	doc = cast(OAuthClient, mrinimitable.get_doc({"doctype": "OAuth Client"}))
...
def get_oauth_settings():
	...
	settings: dict[str, Any] = mrinimitable._dict({"skip_authorization": None})
	if mrinimitable.get_cached_value("OAuth Settings", "OAuth Settings", "skip_authorization"):
		settings["skip_authorization"] = "Auto"  # based on legacy OAuth Provider Settings value

	elif value := mrinimitable.get_cached_value(
