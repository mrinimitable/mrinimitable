...
import mrinimitable
from mrinimitable import _, get_module_path, scrub
...
def export_module_json(doc: "Document", is_standard: bool, module: str) -> str | None:
	...
	`/home/gavin/mrinimitable-shashi/apps/mrinimitable/mrinimitable/core/print_format/_test_print_format_1/_test_print_format_1`
	"""
	if not mrinimitable.flags.in_import and is_standard and mrinimitable.conf.developer_mode:
		...
		return os.path.join(
			mrinimitable.get_module_path(module), scrub(doc.doctype), scrub(doc.name), scrub(doc.name)
		)
...
def get_doc_module(module: str, doctype: str, name: str) -> "ModuleType":
	...
	module_name = "{app}.{module}.{doctype}.{name}.{name}".format(
		app=mrinimitable.local.module_app[scrub(module)],
...
def export_customizations(
	...
	if not mrinimitable.conf.developer_mode:
		mrinimitable.throw(_("Only allowed to export customizations in developer mode"))

	custom = {
		"custom_fields": mrinimitable.get_all("Custom Field", fields="*", filters={"dt": doctype}, order_by="name"),
		"property_setters": mrinimitable.get_all(
			"Property Setter", fields="*", filters={"doc_type": doctype}, order_by="name"
		),
		"custom_perms": [],
		"links": mrinimitable.get_all("DocType Link", fields="*", filters={"parent": doctype}, order_by="name"),
		"doctype": doctype,
		"sync_on_migrate": sync_on_migrate,
	}

	if with_permissions:
		custom["custom_perms"] = mrinimitable.get_all(
	...
	for d in mrinimitable.get_meta(doctype).get_table_fields():
	...
	if custom["custom_fields"] or custom["property_setters"] or custom["custom_perms"]:
		...
		with open(path, "w") as f:
			f.write(mrinimitable.as_json(custom))

		mrinimitable.msgprint(_("Customizations for <b>{0}</b> exported to:<br>{1}").format(doctype, path))
		return path
...
def sync_customizations(app=None):
	...
	for app_name in apps:
		for module_name in mrinimitable.local.app_modules.get(app_name) or []:
			folder = mrinimitable.get_app_path(app_name, module_name, "custom")
			if os.path.exists(folder):
				for fname in os.listdir(folder):
					if fname.endswith(".json"):
						...
						elif mrinimitable.flags.in_install and app:
							sync_customizations_for_doctype(data, folder, fname)
...
def sync_customizations_for_doctype(data: dict, folder: str, filename: str = ""):
		...
		def sync_single_doctype(doc_type):
			def _insert(data):
				if data.get(doctype_fieldname) == doc_type:
					...
					doc = mrinimitable.get_doc(data)
					doc.db_insert()

			match custom_doctype:
				case "Custom Field":
					for d in data[key]:
						field = mrinimitable.db.get_value(
						...
						if not field:
							...
							custom_field = mrinimitable.get_doc("Custom Field", field)
	...
	if update_schema and not mrinimitable.db.get_value("DocType", doctype, "issingle"):
...
def reload_doc(
	...
	from mrinimitable.modules.import_file import import_files
...
def export_doc(doctype, name, module=None):
	...
	from mrinimitable.modules.export_file import write_document_file

	print(f"Exporting Document {doctype} {name}")
	module = module or mrinimitable.db.get_value("DocType", name, "module")
	write_document_file(mrinimitable.get_doc(doctype, name), module)


def get_doctype_module(doctype: str) -> str:
	"""Return **Module Def** name of given doctype."""
	doctype_module_map = mrinimitable.cache.get_value(
		"doctype_modules",
		generator=lambda: dict(mrinimitable.qb.from_("DocType").select("name", "module").run()),
	)

	if module_name := doctype_module_map.get(doctype):
		return module_name
	else:
		mrinimitable.throw(_("DocType {} not found").format(doctype), exc=mrinimitable.DoesNotExistError)
...
def load_doctype_module(doctype, module=None, prefix="", suffix=""):
	...
	if key not in doctype_python_modules:
		try:
			doctype_python_modules[key] = mrinimitable.get_module(module_name)
...
def get_module_app(module: str) -> str:
	app = mrinimitable.local.module_app.get(scrub(module))
	if app is None:
		mrinimitable.throw(_("Module {} not found").format(module), exc=mrinimitable.DoesNotExistError)
	return app
...
def get_app_publisher(module: str) -> str:
	app = get_module_app(module)
	if not app:
		mrinimitable.throw(_("App not found for module: {0}").format(module))
	return mrinimitable.get_hooks(hook="app_publisher", app_name=app)[0]


def make_boilerplate(
	template: str, doc: Union["Document", "mrinimitable._dict"], opts: Union[dict, "mrinimitable._dict"] = None
):
	...
	doc = doc or mrinimitable._dict()
	opts = opts or mrinimitable._dict()
	app_publisher = get_app_publisher(doc.module)
	base_class = "Document"
	base_class_import = "from mrinimitable.model.document import Document"
	controller_body = "pass"

	if doc.get("is_tree"):
		base_class = "NestedSet"
		base_class_import = "from mrinimitable.utils.nestedset import NestedSet"
	...
	with open(target_file_path, "w") as target, open(template_file_path) as source:
		...
		target.write(mrinimitable.as_unicode(controller_file_content))
