...
def run_all(skip_failing: bool = False, patch_type: PatchType | None = None) -> None:
	...
	executed = set(mrinimitable.get_all("Patch Log", filters={"skipped": 0}, fields="patch", pluck="patch"))
	...
	for patch in mrinimitable.flags.final_patches:
...
def get_all_patches(patch_type: PatchType | None = None) -> list[str]:
	if patch_type and not isinstance(patch_type, PatchType):
		mrinimitable.throw(f"Unsupported patch type specified: {patch_type}")
...
def get_patches_from_app(app: str, patch_type: PatchType | None = None) -> list[str]:
	...
	"""
	patches_file = mrinimitable.get_app_path(app, "patches.txt")
	...
	try:
		...
		if not patch_type or patch_type == PatchType.pre_model_sync:
			return mrinimitable.get_file_items(patches_file)
...
def parse_as_configfile(patches_file: str, patch_type: PatchType | None = None) -> list[str]:
	...
	if patch_type.value in parser.sections():
		...
		mrinimitable.throw(mrinimitable._("Patch type {} not found in patches.txt").format(patch_type))


def reload_doc(args):
	import mrinimitable.modules

	run_single(method=mrinimitable.modules.reload_doc, methodargs=args)


def run_single(patchmodule=None, method=None, methodargs=None, force=False):
	from mrinimitable import conf
...
def execute_patch(patchmodule: str, method=None, methodargs=None):
	...
	if patchmodule.startswith("execute:"):
		...
		_patch = mrinimitable.get_attr(patch)
	...
	print(
		f"Executing {patchmodule or methodargs} in {mrinimitable.local.site} ({mrinimitable.db.cur_db_name}){docstring}"
	)
	...
	try:
		if patchmodule:
			if patchmodule.startswith("finally:"):
				...
				mrinimitable.flags.final_patches.append(patchmodule)
			else:
				if has_patch_file:
					_patch()
				else:
...
def update_patch_log(patchmodule, skipped=False):
	...
	patch = mrinimitable.get_doc({"doctype": "Patch Log", "patch": patchmodule})
...
def executed(patchmodule):
	...
	return mrinimitable.db.get_value("Patch Log", {"patch": patchmodule, "skipped": 0})


def _patch_mode(enable):
	"""stop/start execution till patch is run"""
	mrinimitable.local.flags.in_patch = enable
	mrinimitable.db.commit()
