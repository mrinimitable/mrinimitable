...
import mrinimitable
from mrinimitable.core.doctype.doctype.doctype import InvalidFieldNameError
...
class TestCustomizeForm(IntegrationTestCase):
	def insert_custom_field(self):
		mrinimitable.delete_doc_if_exists("Custom Field", "Event-custom_test_field")
		self.field = mrinimitable.get_doc(
			{
				...
				"options": "\nCustom 1\nCustom 2\nCustom 3",
				"default": "Custom 3",
				"insert_after": mrinimitable.get_meta("Event").fields[-1].fieldname,
			}
		).insert()

	def setUp(self):
		self.insert_custom_field()
		mrinimitable.db.delete("Property Setter", dict(doc_type="Event"))
		mrinimitable.db.commit()
		mrinimitable.clear_cache(doctype="Event")

	def tearDown(self):
		mrinimitable.delete_doc("Custom Field", self.field.name)
		mrinimitable.db.commit()
		mrinimitable.clear_cache(doctype="Event")

	def get_customize_form(self, doctype=None):
		d = mrinimitable.get_doc("Customize Form")
		if doctype:
			d.doc_type = doctype
	...
	def test_fetch_to_customize(self):
		...
		self.assertEqual(len(d.get("fields")), len(mrinimitable.get_doc("DocType", d.doc_type).fields) + 1)
	...
	def test_save_customization_property(self):
		d = self.get_customize_form("Event")
		self.assertEqual(
			mrinimitable.db.get_value("Property Setter", {"doc_type": "Event", "property": "allow_copy"}, "value"),
			None,
		)
	...
	def test_save_customization_custom_field_property(self):
		d = self.get_customize_form("Event")
		self.assertEqual(mrinimitable.db.get_value("Custom Field", self.field.name, "reqd"), 0)

		custom_field = d.get("fields", {"fieldname": self.field.fieldname})[0]
		custom_field.reqd = 1
		custom_field.no_copy = 1
		d.run_method("save_customization")
		self.assertEqual(mrinimitable.db.get_value("Custom Field", self.field.name, "reqd"), 1)
		self.assertEqual(mrinimitable.db.get_value("Custom Field", self.field.name, "no_copy"), 1)
		...
		d.run_method("save_customization")
		self.assertEqual(mrinimitable.db.get_value("Custom Field", self.field.name, "reqd"), 0)
		self.assertEqual(mrinimitable.db.get_value("Custom Field", self.field.name, "no_copy"), 0)
	...
	def test_save_customization_new_field(self):
		...
		self.assertEqual(
			mrinimitable.db.get_value("Custom Field", custom_field_name, "fieldtype"),
			"Data",
		)

		self.assertEqual(
			mrinimitable.db.get_value("Custom Field", custom_field_name, "insert_after"),
			last_fieldname,
		)

		mrinimitable.delete_doc("Custom Field", custom_field_name)
		self.assertEqual(mrinimitable.db.get_value("Custom Field", custom_field_name), None)
	...
	def test_save_customization_remove_field(self):
		...
		self.assertEqual(mrinimitable.db.get_value("Custom Field", custom_field.name), None)
	...
	def test_core_doctype_customization(self):
		self.assertRaises(mrinimitable.ValidationError, self.get_customize_form, "User")
	...
	def test_custom_link(self):
		try:
			...
			event = mrinimitable.get_meta("Event")
	...
	def test_custom_internal_links(self):
		...
		mrinimitable.clear_cache()
		user_group = mrinimitable.get_meta("User Group")
		...
		mrinimitable.clear_cache()
		user_group = mrinimitable.get_meta("Event")
	...
	def test_system_generated_fields(self):
		...
		custom_field = mrinimitable.get_doc("Custom Field", {"dt": doctype, "fieldname": custom_field_name})
		...
		self.assertEqual(
			mrinimitable.db.get_value("Property Setter", property_setter_filters, "value"), "Test Description"
		)
	...
	def test_custom_field_order(self):
		...
		field_order_property = json.loads(
			mrinimitable.db.get_value("Property Setter", {"doc_type": "ToDo", "property": "field_order"}, "value")
		)

		self.assertEqual(field_order_property, [df.fieldname for df in mrinimitable.get_meta("ToDo").fields])
