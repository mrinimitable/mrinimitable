...
class PropertySetter(Document):
	...
	def validate_fieldtype_change(self):
		if self.property == "fieldtype" and self.field_name in not_allowed_fieldtype_change:
			mrinimitable.throw(_("Field type cannot be changed for {0}").format(self.field_name))
...
def make_property_setter(
	...
	property_setter = mrinimitable.get_doc(
		{
...
def delete_property_setter(doc_type, property=None, field_name=None, row_name=None):
	...
	property_setters = mrinimitable.db.get_values("Property Setter", filters)
	for ps in property_setters:
		mrinimitable.get_doc("Property Setter", ps).delete(ignore_permissions=True, force=True)
