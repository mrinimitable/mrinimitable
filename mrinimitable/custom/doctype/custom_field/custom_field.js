...
mrinimitable.ui.form.on("Custom Field", {
	setup: function (frm) {
	...
	dt: function (frm) {
		...
		return mrinimitable.call({
			method: "mrinimitable.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && mrinimitable.utils.has_special_chars(frm.doc.label)) {
			...
			frm.add_custom_button(__("Rename Fieldname"), () => {
				mrinimitable.prompt(
					...
					function (data) {
						mrinimitable
							.xcall(
								"mrinimitable.custom.doctype.custom_field.custom_field.rename_fieldname",
								{
...
mrinimitable.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";
