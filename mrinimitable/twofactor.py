...
from mrinimitable.permissions import ALL_USER_ROLE
from mrinimitable.utils import cint, get_datetime, get_url, time_diff_in_seconds
from mrinimitable.utils.background_jobs import enqueue
from mrinimitable.utils.password import decrypt, encrypt

PARENT_FOR_DEFAULTS = "__2fa"


def get_default(key):
	return mrinimitable.db.get_default(key, parent=PARENT_FOR_DEFAULTS)


def set_default(key, value):
	mrinimitable.db.set_default(key, value, parent=PARENT_FOR_DEFAULTS)


def clear_default(key):
	mrinimitable.defaults.clear_default(key, parent=PARENT_FOR_DEFAULTS)
...
def toggle_two_factor_auth(state, roles=None):
	...
	for role in roles or []:
		role = mrinimitable.get_doc("Role", {"role_name": role})
...
def two_factor_is_enabled(user=None):
	...
	enabled = cint(mrinimitable.get_system_settings("enable_two_factor_auth"))
	if enabled:
		bypass_two_factor_auth = cint(mrinimitable.get_system_settings("bypass_2fa_for_retricted_ip_users"))
		if bypass_two_factor_auth and user:
			...
			if restrict_ip_list and mrinimitable.local.request_ip:
...
def get_cached_user_pass():
	...
	user = pwd = None
	tmp_id = mrinimitable.form_dict.get("tmp_id")
	if tmp_id:
		user = mrinimitable.safe_decode(mrinimitable.cache.get(tmp_id + "_usr"))
		pwd = mrinimitable.safe_decode(mrinimitable.cache.get(tmp_id + "_pwd"))
	return (user, pwd)


def authenticate_for_2factor(user):
	"""Authenticate two factor for enabled user before login."""
	if mrinimitable.form_dict.get("otp"):
		return
	otp_secret = get_otpsecret_for_(user)
	token = int(pyotp.TOTP(otp_secret).now())
	tmp_id = mrinimitable.generate_hash(length=8)
	cache_2fa_data(user, token, otp_secret, tmp_id)
	verification_obj = get_verification_obj(user, token, otp_secret)
	# Save data in local
	mrinimitable.local.response["verification"] = verification_obj
	mrinimitable.local.response["tmp_id"] = tmp_id


def cache_2fa_data(user, token, otp_secret, tmp_id):
	"""Cache and set expiry for data."""
	pwd = mrinimitable.form_dict.get("pwd")
	verification_method = get_verification_method()

	pipeline = mrinimitable.cache.pipeline()

	# set increased expiry time for SMS and Email
	if verification_method in ["SMS", "Email"]:
		expiry_time = mrinimitable.flags.token_expiry or 300
		pipeline.set(tmp_id + "_token", token, expiry_time)
	else:
		expiry_time = mrinimitable.flags.otp_expiry or 180
...
def two_factor_is_enabled_for_(user):
	...
	role_doctype = mrinimitable.qb.DocType("Role")
	no_of_users = mrinimitable.db.count(
		role_doctype,
...
def get_verification_method():
	return mrinimitable.get_system_settings("two_factor_method")


def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	"""Confirm otp matches."""
	from mrinimitable.auth import get_login_attempt_tracker

	if not otp:
		otp = mrinimitable.form_dict.get("otp")
	if not otp:
	...
	if not tmp_id:
		tmp_id = mrinimitable.form_dict.get("tmp_id")
	hotp_token = mrinimitable.cache.get(tmp_id + "_token")
	otp_secret = mrinimitable.cache.get(tmp_id + "_otp_secret")
	if not otp_secret:
	...
	if hotp_token:
		if hotp.verify(otp, int(hotp_token)):
			mrinimitable.cache.delete(tmp_id + "_token")
...
def get_verification_obj(user, token, otp_secret):
	otp_issuer = mrinimitable.get_system_settings("otp_issuer_name")
	verification_method = get_verification_method()
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = mrinimitable.db.get_value("User", user, ["phone", "mobile_no"], as_dict=1)
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		mrinimitable.get_system_settings("otp_issuer_name")
	)
	return mrinimitable.render_template(subject_template, kwargs_dict)
...
def get_email_body_for_2fa(kwargs_dict):
	...
	"""
	return mrinimitable.render_template(body_template, kwargs_dict)
...
def get_link_for_qrcode(user, totp_uri):
	...
	key = mrinimitable.generate_hash(length=20)
	key_user = f"{key}_user"
	key_uri = f"{key}_uri"
	lifespan = int(mrinimitable.get_system_settings("lifespan_qrcode_image")) or 240
	mrinimitable.cache.set_value(key_uri, totp_uri, expires_in_sec=lifespan)
	mrinimitable.cache.set_value(key_user, user, expires_in_sec=lifespan)
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	send_token_hook_methods = mrinimitable.get_hooks("send_token_via_sms")
	if send_token_hook_methods:
		return mrinimitable.get_attr(send_token_hook_methods[-1])(otpsecret, token, phone_no)

	try:
		from mrinimitable.core.doctype.sms_settings.sms_settings import send_request
	except Exception:
		return False
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	user_email = mrinimitable.db.get_value("User", user, "email")
	if not user_email:
		return False
	hotp = pyotp.HOTP(otp_secret)
...
def create_barcode_folder():
	...
	folder = mrinimitable.db.exists("File", {"file_name": folder_name})
	if folder:
		return folder
	folder = mrinimitable.get_doc({"doctype": "File", "file_name": folder_name, "is_folder": 1, "folder": "Home"})
...
def delete_qrimage(user, check_expiry=False):
	...
	user_barcodes = mrinimitable.get_all(
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = mrinimitable.get_doc("File", barcode.name)
		mrinimitable.delete_doc("File", barcode.name, ignore_permissions=True)


def delete_all_barcodes_for_users():
	"""Task to delete all barcodes for user."""

	users = mrinimitable.get_all("User", {"enabled": 1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, str):
		barcode = mrinimitable.get_doc("File", barcode)
	lifespan = mrinimitable.get_system_settings("lifespan_qrcode_image") or 240
...
def disable():
	mrinimitable.db.set_single_value("System Settings", "enable_two_factor_auth", 0)


@mrinimitable.whitelist()
def reset_otp_secret(user: str):
	if mrinimitable.session.user != user:
		mrinimitable.only_for("System Manager", message=True)

	settings = mrinimitable.get_cached_doc("System Settings")
	...
	otp_issuer = settings.otp_issuer_name or "Mrinimitable Framework"
	user_email = mrinimitable.get_cached_value("User", user, "email")
	...
	mrinimitable.msgprint(_("OTP Secret has been reset. Re-registration will be required on next login."))
