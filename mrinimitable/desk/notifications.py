...
def get_notifications():
	...
	if mrinimitable.flags.in_install or not mrinimitable.is_setup_complete():
		return out
	...
	for name in groups:
		count = mrinimitable.cache.hget("notification_count:" + name, mrinimitable.session.user)
...
def get_notifications_for_doctypes(config, notification_count):
	...
	can_read = mrinimitable.get_user().get_can_read()
	open_count_doctype = {}
			...
			if d in notification_count:
				...
				try:
					if isinstance(condition, dict):
						...
						result = mrinimitable.get_attr(condition)()

				except mrinimitable.PermissionError:
					mrinimitable.clear_messages()
					pass
				# mrinimitable.msgprint("Permission Error in notifications for {0}".format(d))
					...
					mrinimitable.cache.hset("notification_count:" + d, mrinimitable.session.user, result)
	...
	for doctype in config.targets:
		if doctype in can_read:
			if doctype in notification_percent:
				...
				try:
					if isinstance(condition, dict):
						doc_list = mrinimitable.get_list(
							doctype,
...
def clear_notifications(user=None):
	...
	if user:
		mrinimitable.cache.hdel_names([f"notification_count:{name}" for name in groups], user)
	else:
		mrinimitable.cache.delete_value([f"notification_count:{name}" for name in groups])


def clear_notification_config(user):
	mrinimitable.cache.hdel("notification_config", user)


def delete_notification_count_for(doctype):
	mrinimitable.cache.delete_key("notification_count:" + doctype)
...
def get_notification_info():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(mrinimitable.db.sql("""select name, module from tabDocType"""))
...
def get_notification_config():
	...
	return mrinimitable.cache.hget("notification_config", user, _get)
...
def get_open_count(doctype: str, name: str, items=None):
	...
	if mrinimitable.flags.in_migrate or mrinimitable.flags.in_install:
		return {"count": []}

	# None of the count queries should take more than 1s individually
	mrinimitable.db.set_execution_timeout(1)

	try:
		return _get_linked_document_counts(doctype, name, items)
	except Exception as e:
		if mrinimitable.db.is_statement_timeout(e):
			return {"count": []}
		raise
...
def _get_linked_document_counts(doctype: str, name: str, items=None):
	...
	if not meta.custom:
		module = mrinimitable.get_meta_module(doctype)
...
def get_doc_count(doctype, filters) -> int | Literal["?"]:
	try:
		docs = mrinimitable.get_all(
			doctype, filters=filters, limit=100, distinct=True, ignore_ifnull=True, order_by=None
		)
		return len(docs)
	except Exception as e:
		if mrinimitable.db.is_statement_timeout(e):  # Skip fetching correct count if it's too slow
			return "?"
		raise
...
def notify_mentions(ref_doctype, ref_name, content):
	if ref_doctype and ref_name and content:
		...
		sender_fullname = get_fullname(mrinimitable.session.user)
		...
		notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
			mrinimitable.bold(sender_fullname), mrinimitable.bold(ref_doctype), get_title_html(title)
		)
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = mrinimitable.get_cached_doc("User Group", mention["data-id"])
