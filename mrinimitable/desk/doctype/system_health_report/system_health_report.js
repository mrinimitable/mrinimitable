...
mrinimitable.ui.form.on("System Health Report", {
	onload(frm) {
		let poll_attempts = 0;
		const interval = setInterval(() => {
			mrinimitable
				.xcall(
					"mrinimitable.desk.doctype.system_health_report.system_health_report.get_job_status",
	...
	refresh(frm) {
		...
		mrinimitable.realtime.on("pong", () => {
			frm.set_value("socketio_ping_check", "Pass");
			frm.set_value(
				"socketio_transport_mode",
				mrinimitable.realtime.socket.io?.engine?.transport?.name
			);
		});
		mrinimitable.realtime.emit("ping");
		frm.disable_save();
	...
	setup_highlight(frm) {
		...
		const update_fields = () => {
			if (!mrinimitable.get_route().includes(frm.doc.name)) {
				clearInterval(interval);
			}
