...
mrinimitable.ui.form.on("System Console", {
	onload: function (frm) {
	...
	refresh_processlist: function (frm) {
		...
		mrinimitable
			.call("mrinimitable.desk.doctype.system_console.system_console.show_processlist")
			.then((r) => {
				let rows = "";
	...
	load_completions(frm) {
		...
		setTimeout(() => {
			mrinimitable
				.call({
					method: "mrinimitable.core.doctype.server_script.server_script.get_autocompletion_items",
					type: "GET",
					cache: true,
				})
				.then((r) => r.message)
