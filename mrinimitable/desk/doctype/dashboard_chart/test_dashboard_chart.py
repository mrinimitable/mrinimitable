...
from mrinimitable.desk.doctype.dashboard_chart.dashboard_chart import get
from mrinimitable.tests import IntegrationTestCase
from mrinimitable.utils import formatdate, get_last_day, getdate
from mrinimitable.utils.dateutils import get_period, get_period_ending
...
class TestDashboardChart(IntegrationTestCase):
	...
	def test_dashboard_chart(self):
		if mrinimitable.db.exists("Dashboard Chart", "Test Dashboard Chart"):
			mrinimitable.delete_doc("Dashboard Chart", "Test Dashboard Chart")
	...
	def test_empty_dashboard_chart(self):
		if mrinimitable.db.exists("Dashboard Chart", "Test Empty Dashboard Chart"):
			mrinimitable.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart")

		mrinimitable.db.delete("Error Log")
	...
	def test_chart_wih_one_value(self):
		if mrinimitable.db.exists("Dashboard Chart", "Test Empty Dashboard Chart 2"):
			mrinimitable.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart 2")

		mrinimitable.db.delete("Error Log")

		# create one data point
		mrinimitable.get_doc(doctype="Error Log", creation="2018-06-01 00:00:00").insert()
	...
	def test_group_by_chart_type(self):
		if mrinimitable.db.exists("Dashboard Chart", "Test Group By Dashboard Chart"):
			mrinimitable.delete_doc("Dashboard Chart", "Test Group By Dashboard Chart")

		mrinimitable.get_doc({"doctype": "ToDo", "description": "test"}).insert()
		...
		todo_status_count = mrinimitable.db.count("ToDo", {"status": result.get("labels")[0]})
	...
	def test_daily_dashboard_chart(self):
		...
		if mrinimitable.db.exists("Dashboard Chart", "Test Daily Dashboard Chart"):
			mrinimitable.delete_doc("Dashboard Chart", "Test Daily Dashboard Chart")
	...
	def test_weekly_dashboard_chart(self):
		...
		if mrinimitable.db.exists("Dashboard Chart", "Test Weekly Dashboard Chart"):
			mrinimitable.delete_doc("Dashboard Chart", "Test Weekly Dashboard Chart")
	...
	def test_avg_dashboard_chart(self):
		...
		if mrinimitable.db.exists("Dashboard Chart", "Test Average Dashboard Chart"):
			mrinimitable.delete_doc("Dashboard Chart", "Test Average Dashboard Chart")
	...
	def test_user_date_label_dashboard_chart(self):
		mrinimitable.delete_doc_if_exists("Dashboard Chart", "Test Dashboard Chart Date Label")
		...
		with patch.object(mrinimitable.utils.data, "get_user_date_format", return_value="dd.mm.yyyy"):
		...
		with patch.object(mrinimitable.utils.data, "get_user_date_format", return_value="mm-dd-yyyy"):
			result = get(chart_name="Test Dashboard Chart Date Label")
...
def create_new_record(doctype_name, title, date, number):
	...
	if not mrinimitable.db.exists(doctype_name, {"title": doc.title}):
