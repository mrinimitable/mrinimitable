...
class TestEvent(IntegrationTestCase):
	...
	def test_allowed_public(self):
		mrinimitable.set_user(self.test_user)
		doc = mrinimitable.get_doc("Event", mrinimitable.db.get_value("Event", {"subject": "_Test Event 1"}))
		self.assertTrue(mrinimitable.has_permission("Event", doc=doc))

	def test_not_allowed_private(self):
		mrinimitable.set_user(self.test_user)
		doc = mrinimitable.get_doc("Event", mrinimitable.db.get_value("Event", {"subject": "_Test Event 2"}))
		self.assertFalse(mrinimitable.has_permission("Event", doc=doc))

	def test_allowed_private_if_in_event_user(self):
		name = mrinimitable.db.get_value("Event", {"subject": "_Test Event 3"})
		mrinimitable.share.add("Event", name, self.test_user, "read")
		mrinimitable.set_user(self.test_user)
		doc = mrinimitable.get_doc("Event", name)
		self.assertTrue(mrinimitable.has_permission("Event", doc=doc))
		mrinimitable.set_user("Administrator")
		mrinimitable.share.remove("Event", name, self.test_user)
	...
	def test_revert_logic(self):
		ev = mrinimitable.get_doc(self.globalTestRecords["Event"][0]).insert()
		name = ev.name

		mrinimitable.delete_doc("Event", ev.name)
