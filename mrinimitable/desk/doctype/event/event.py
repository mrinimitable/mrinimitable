...
from mrinimitable.contacts.doctype.contact.contact import get_default_contact
...
from mrinimitable.utils.user import get_enabled_system_users
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from mrinimitable.desk.doctype.event_participants.event_participants import EventParticipants
	...
	def validate(self):
		...
		if self.repeat_on == "Daily" and self.ends_on and getdate(self.starts_on) != getdate(self.ends_on):
			mrinimitable.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			mrinimitable.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		for communication in communications:
			mrinimitable.delete_doc("Communication", communication, force=True)
	...
	def sync_communication(self):
		...
		for participant in self.event_participants:
			if communications := mrinimitable.get_all(
				"Communication",
				filters=[
				...
				for comm in communications:
					communication = mrinimitable.get_doc("Communication", comm)
					self.update_communication(participant, communication)
			else:
				meta = mrinimitable.get_meta(participant.reference_doctype)
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				mrinimitable.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)
...
def delete_communication(event, reference_doctype, reference_docname):
	...
	deleted_participant = mrinimitable.get_doc(reference_doctype, reference_docname)

	comms = mrinimitable.get_list(
		"Communication",
		filters=[
	...
	for comm in comms:
		mrinimitable.delete_doc("Communication", comm)


def get_permission_query_conditions(user):
	if not user:
		user = mrinimitable.session.user
	query = f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={mrinimitable.db.escape(user)})"""
	if shared_events := mrinimitable.share.get_shared("Event", user=user):
		query += f" or `tabEvent`.`name` in ({', '.join([mrinimitable.db.escape(e) for e in shared_events])})"
	return query
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			mrinimitable.set_user_lang(user.name, user.language)
			...
			mrinimitable.sendmail(
				...
				subject=mrinimitable._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[mrinimitable._("Events in Today's Calendar"), "blue"],
			)
...
def get_events(
...
) -> list[mrinimitable._dict]:
	user = user or mrinimitable.session.user
	EventLikeDict: TypeAlias = Event | mrinimitable._dict
	...
	event_candidates: list[EventLikeDict] = mrinimitable.db.sql(
		"""
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = mrinimitable.get_all(
		"Event Participants",
	...
	if participations:
		for participation in participations:
			if delete_event:
				mrinimitable.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = mrinimitable.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					mrinimitable.db.delete("Event", {"name": participation.parent})
					mrinimitable.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = mrinimitable.get_list("Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			mrinimitable.db.set_value("Event", event.name, "status", "Closed")
