...
def update_follow(doctype: str, doc_name: str, following: bool):
	if following:
		return (follow_document(doctype, doc_name, mrinimitable.session.user) and True) or False
	else:
		return unfollow_document(doctype, doc_name, mrinimitable.session.user)
...
def follow_document(doctype, doc_name, user):
	...
	if not mrinimitable.get_meta(doctype).track_changes:
		mrinimitable.toast(_("Can't follow since changes are not tracked."))
		return False

	if user == "Administrator":
		mrinimitable.toast(_("Administrator can't follow"))
		return False

	if not mrinimitable.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		mrinimitable.toast(_("Document follow is not enabled for this user."))
		return False

	if not is_document_followed(doctype, doc_name, user):
		doc = mrinimitable.new_doc("Document Follow")
		doc.update({"ref_doctype": doctype, "ref_docname": doc_name, "user": user})
		doc.save()
		mrinimitable.toast(_("Following document {0}").format(doc_name))
		return doc
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		mrinimitable.delete_doc("Document Follow", doc[0].name, force=True)
		mrinimitable.toast(_("Un-following document {0}").format(doc_name))
		return False
	return False
...
def get_version(doctype, doc_name, frequency, user):
	...
	if version:
		for v in version:
			change = mrinimitable.parse_json(v.data)
			time = mrinimitable.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = mrinimitable.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append(
			{
...
def _get_filters(frequency, user):
	...
	if frequency == "Weekly":
		filters += [
			["modified", ">", mrinimitable.utils.add_days(mrinimitable.utils.nowdate(), -7)],
			["modified", "<", mrinimitable.utils.nowdate()],
		]

	elif frequency == "Daily":
		filters += [
			["modified", ">", mrinimitable.utils.add_days(mrinimitable.utils.nowdate(), -1)],
			["modified", "<", mrinimitable.utils.nowdate()],
		]

	elif frequency == "Hourly":
		filters += [
			["modified", ">", mrinimitable.utils.add_to_date(mrinimitable.utils.now_datetime(), hours=-1)],
			["modified", "<", mrinimitable.utils.now_datetime()],
		]
