...
from mrinimitable.model import child_table_fields, default_fields, get_permitted_fields, optional_fields
...
from mrinimitable.utils import add_user_info, cint, format_duration
...
def get():
	...
	if is_virtual_doctype(args.doctype):
		...
		data = compress(mrinimitable.call(controller.get_list, args=args, **args))
	else:
...
def get_list():
	...
	if is_virtual_doctype(args.doctype):
		controller = get_controller(args.doctype)
		data = mrinimitable.call(controller.get_list, args=args, **args)
	else:
		# uncompressed (refactored from mrinimitable.model.db_query.get_list)
		data = execute(**args)
...
def get_count() -> int | None:
	...
	if is_virtual_doctype(args.doctype):
		controller = get_controller(args.doctype)
		return mrinimitable.call(controller.get_count, args=args, **args)
	...
	timeout_clause = "SET STATEMENT max_statement_time=1 FOR" if mrinimitable.db.db_type == "mariadb" else ""

	try:
		count = mrinimitable.db.sql(f"{timeout_clause} select count(*) from ( {partial_query} ) p")[0][0]
	except Exception as e:
		if mrinimitable.db.is_statement_timeout(e):  # Skip fetching accurate count
			count = None
		else:
			raise

	if count == args.limit or count is None:
		mrinimitable.local.response_headers.set("Cache-Control", "private,max-age=600,stale-while-revalidate=10800")
...
def get_form_params():
	...
	data = mrinimitable._dict(mrinimitable.local.form_dict)
	clean_params(data)
	validate_args(data)
	return data
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = mrinimitable.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			mrinimitable.throw(_("Invalid aggregate function"))

		if mrinimitable.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
			data.fields.append(
...
def raise_invalid_field(fieldname):
	mrinimitable.throw(_("Field not permitted in query") + f": {fieldname}", mrinimitable.DataError)
...
def get_meta_and_docfield(fieldname, data):
	...
	meta = mrinimitable.get_meta(parenttype)
...
def get_parenttype_and_fieldname(field, data):
	if "." in field:
		...
		parenttype = parts[0]
		fieldname = parts[1]
		df = mrinimitable.get_meta(data.doctype).get_field(parenttype)
...
def compress(data, args=None):
	...
	from mrinimitable.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = mrinimitable.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if mrinimitable.db.exists("Report", name):
		report = mrinimitable.get_doc("Report", name)
		if report.is_standard == "Yes":
			mrinimitable.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			mrinimitable.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != mrinimitable.session.user and not report.has_permission("write"):
			mrinimitable.throw(_("Insufficient Permissions for editing Report"), mrinimitable.PermissionError)
	else:
	...
	mrinimitable.msgprint(
		_("Report {0} saved").format(mrinimitable.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
@mrinimitable.whitelist(methods=["POST", "DELETE"])
def delete_report(name):
	"""Delete reports of type Report Builder from Report View"""

	report = mrinimitable.get_doc("Report", name)
	if report.is_standard == "Yes":
		mrinimitable.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		mrinimitable.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != mrinimitable.session.user and not report.has_permission("delete"):
		mrinimitable.throw(_("Insufficient Permissions for deleting Report"), mrinimitable.PermissionError)

	report.delete(ignore_permissions=True)
	mrinimitable.msgprint(
		_("Report {0} deleted").format(mrinimitable.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	from mrinimitable.desk.utils import get_csv_bytes, pop_csv_params, provide_binary_file
	...
	if not mrinimitable.permissions.can_export(doctype):
		if mrinimitable.permissions.can_export(doctype, is_owner=True):
			for row in ret:
				if row[-1] != mrinimitable.session.user:
	...
	if mrinimitable.local.lang == "en" or not translate_values:
	...
	if file_format_type == "CSV":
		...
		content = get_csv_bytes(
			[[handle_html(mrinimitable.as_unicode(v)) if isinstance(v, str) else v for v in r] for r in data],
			csv_params,
		)
	elif file_format_type == "Excel":
		from mrinimitable.utils.xlsxutils import make_xlsx
...
def get_field_info(fields, doctype):
	...
	for key in fields:
		...
		if parenttype == doctype and fieldname == "name":
			...
			df = mrinimitable.get_meta(parenttype).get_field(fieldname)
			if df and df.fieldtype in ("Data", "Select", "Small Text", "Text"):
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = getattr(df, "translatable", False)
			elif df and df.fieldtype == "Link" and mrinimitable.get_meta(df.options).translated_doctype:
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = True
...
def delete_items():
	...
	items = sorted(json.loads(mrinimitable.form_dict.get("items")), reverse=True)
	doctype = mrinimitable.form_dict.get("doctype")

	if len(items) > 10:
		mrinimitable.enqueue("mrinimitable.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
...
def delete_bulk(doctype, items):
	...
	for i, d in enumerate(items):
		try:
			mrinimitable.flags.in_bulk_delete = True
			mrinimitable.delete_doc(doctype, d)
			if len(items) >= 5:
...
def get_sidebar_stats(stats, doctype, filters=None):
	...
	if is_virtual_doctype(doctype):
		...
		data = mrinimitable.call(controller.get_stats, args=args, **args)
	else:
...
def get_stats(stats, doctype, filters=None):
	...
	try:
		db_columns = mrinimitable.db.get_table_columns(doctype)
	except (mrinimitable.db.InternalError, mrinimitable.db.ProgrammingError):
	...
	for column in columns:
		if column not in db_columns:
			continue
		try:
			tag_count = mrinimitable.get_list(
				doctype,
				fields=[column, "count(*)"],
			...
			if column == "_user_tags":
				...
				no_tag_count = mrinimitable.get_list(
					doctype,
					fields=[column, "count(*)"],
		...
		except mrinimitable.db.SQLError:
			pass
...
def get_filter_dashboard_data(stats, doctype, filters=None):
	...
	for tag in tags:
		...
		if tag["type"] not in ["Date", "Datetime"]:
			tagcount = mrinimitable.get_list(
				doctype,
