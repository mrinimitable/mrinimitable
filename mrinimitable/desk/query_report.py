...
from mrinimitable.core.utils import ljust_list
from mrinimitable.desk.reportview import clean_params, parse_json
...
from mrinimitable.permissions import get_role_permissions, get_roles, has_permission
from mrinimitable.utils import cint, cstr, flt, format_duration, get_html_format, sbool
from mrinimitable.utils.caching import request_cache
...
def get_report_doc(report_name):
	...
	if not mrinimitable.has_permission(doc.ref_doctype, "report"):
		mrinimitable.throw(
	...
	if doc.disabled:
		mrinimitable.throw(_("Report {0} is disabled").format(_(report_name)))
...
def generate_report_result(
	...
	return {
		...
		"status": None,
		"execution_time": mrinimitable.cache.hget("report_execution_time", report.name) or 0,
	}
...
def get_script(report_name):
	...
	module = report.module or mrinimitable.db.get_value("DocType", report.ref_doctype, "module")
	...
	if not script:
		script = "mrinimitable.query_reports['{}']={{}}".format(report_name)

	return {
		"script": render_include(script),
		"html_format": html_format,
		"execution_time": mrinimitable.cache.hget("report_execution_time", report_name) or 0,
...
def get_reference_report(report):
	if report.report_type != "Custom Report":
		return report
	reference_report = mrinimitable.get_doc("Report", report.reference_report)
...
def run(
	...
	report = get_report_doc(report_name)
	if not mrinimitable.has_permission(report.ref_doctype, "report"):
	...
	try:
		...
		mrinimitable.log_error("Report Error")
		raise
...
def add_custom_column_data(custom_columns, result):
	...
	for column in custom_columns:
		if len(column["fieldname"].split("-")) > 1:
			...
			doctype_name = mrinimitable.unscrub(column["fieldname"].split("-")[1])
		...
		if key in custom_column_data:
			for row in result:
				...
				if isinstance(link_field, str):
					link_field = mrinimitable._dict({"fieldname": link_field, "names": []})
...
def get_prepared_report_result(report, filters, dn="", user=None):
	from mrinimitable.core.doctype.prepared_report.prepared_report import get_completed_prepared_report
	...
	doc = mrinimitable.get_doc("Prepared Report", dn) if dn else None
	if doc:
		try:
			...
			mrinimitable.msgprint(_("Prepared report render failed") + f": {e!s}")
			doc = None
...
def export_query():
	...
	form_params = mrinimitable._dict(mrinimitable.local.form_dict)
	csv_params = pop_csv_params(form_params)
	clean_params(form_params)
	parse_json(form_params)
	report_name = form_params.report_name
	mrinimitable.permissions.can_export(
		mrinimitable.get_cached_value("Report", report_name, "ref_doctype"),
		raise_exception=True,
	)

	file_format_type = form_params.file_format_type
	custom_columns = mrinimitable.parse_json(form_params.custom_columns or "[]")
	...
	if file_format_type == "CSV":
		...
		content = get_csv_bytes(
			[[handle_html(mrinimitable.as_unicode(v)) if isinstance(v, str) else v for v in r] for r in xlsx_data],
			csv_params,
		)
		file_extension = "csv"
...
def format_fields(data: mrinimitable._dict) -> None:
...
def get_data_for_custom_field(doctype, field, names=None):
	if not mrinimitable.has_permission(doctype, "read"):
		mrinimitable.throw(_("Not Permitted to read {0}").format(_(doctype)), mrinimitable.PermissionError)

	filters = {}
	if names:
		if isinstance(names, str | bytearray):
			names = mrinimitable.json.loads(names)
		filters.update({"name": ["in", names]})

	return mrinimitable._dict(mrinimitable.get_list(doctype, filters=filters, fields=["name", field], as_list=1))
...
def save_report(reference_report, report_name, columns, filters):
	...
	docname = mrinimitable.db.exists(
		"Report",
		{
	...
	if docname:
		report = mrinimitable.get_doc("Report", docname)
		...
		report.save()
		mrinimitable.msgprint(_("Report updated successfully"))

		return docname
	else:
		new_report = mrinimitable.get_doc(
			{
				"doctype": "Report",
				"report_name": report_name,
		...
		mrinimitable.msgprint(_("{0} saved successfully").format(_(new_report.name)))
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	shared = mrinimitable.share.get_shared(ref_doctype, user)
	columns_dict = get_columns_dict(columns)

	role_permissions = get_role_permissions(mrinimitable.get_meta(ref_doctype), user)
			...
			for match_filters in filter_list:
				...
				for dt, idx in linked_doctypes.items():
					...
					if (
						...
						and mrinimitable.db.exists(dt, cell_value)
					):
						match = False
						break

					if match:
						match = has_unrestricted_read_access(doctype=ref_doctype, user=mrinimitable.session.user)
...
def has_unrestricted_read_access(doctype, user):
	...
	standard_perm_exists = mrinimitable.db.exists(
		"DocPerm",
		permission_filters,
	)

	custom_perm_exists = mrinimitable.db.exists(
		"Custom DocPerm",
		permission_filters,
	)
...
def get_columns_dict(columns):
	...
	"""
	columns_dict = mrinimitable._dict()
...
def get_column_as_dict(col):
	...
	if isinstance(col, str):
		...
		col_dict["fieldname"] = mrinimitable.scrub(col[0])

	# dict
	else:
		col_dict.update(col)
		if "fieldname" not in col_dict:
			col_dict["fieldname"] = mrinimitable.scrub(col_dict["label"])
...
def get_user_match_filters(doctypes, user):
	...
	for dt in doctypes:
		filter_list = mrinimitable.desk.reportview.build_match_conditions(dt, user, False)
		if filter_list:
