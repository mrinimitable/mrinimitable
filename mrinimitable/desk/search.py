...
from mrinimitable import _, is_whitelisted, validate_and_sanitize_search_inputs
from mrinimitable.database.schema import SPECIAL_CHAR_PATTERN
from mrinimitable.model.db_query import get_order_by
from mrinimitable.permissions import has_permission
from mrinimitable.utils import cint, cstr, unique
from mrinimitable.utils.data import make_filter_tuple
...
def sanitize_searchfield(searchfield: str):
	...
	if SPECIAL_CHAR_PATTERN.search(searchfield):
		mrinimitable.throw(_("Invalid Search Field {0}").format(searchfield), mrinimitable.DataError)
...
def search_widget(
	...
	standard_queries = mrinimitable.get_hooks().standard_queries or {}
	...
	if query:  # Query = custom search query i.e. python function
		try:
			is_whitelisted(mrinimitable.get_attr(query))
		...
		except (mrinimitable.PermissionError, mrinimitable.AppNotInstalledError, ImportError):
			if mrinimitable.local.conf.developer_mode:
				raise
			else:
	...
	if not meta.translated_doctype:
		_txt = mrinimitable.db.escape((txt or "").replace("%", "").replace("@", ""))
...
def get_names_for_mentions(search_term):
	users_for_mentions = mrinimitable.cache.get_value("users_for_mentions", get_users_for_mentions)
	user_groups = mrinimitable.cache.get_value("user_groups", get_user_groups)
	...
	for mention_data in users_for_mentions + user_groups:
		...
		mention_data["link"] = mrinimitable.utils.get_url_to_form(
...
def get_user_groups():
	return mrinimitable.get_all("User Group", fields=["name as id", "name as value"], update={"is_group": True})
...
def get_link_title(doctype, docname):
	...
	if meta.show_title_field_in_link:
		return mrinimitable.db.get_value(doctype, docname, meta.title_field)
