...
# Author - Shivam Mishra <shivam@mrinimitable.io>

from functools import wraps
from json import JSONDecodeError, dumps, loads

import mrinimitable
from mrinimitable import DoesNotExistError, ValidationError, _, _dict
from mrinimitable.boot import get_allowed_pages, get_allowed_reports
from mrinimitable.cache_manager import (
...
class Workspace:
	def __init__(self, page, minimal=False):
		...
		self.workspace_manager = "Workspace Manager" in mrinimitable.get_roles()

		self.user = mrinimitable.get_user()
		self.allowed_modules = self.get_cached("user_allowed_modules", self.get_allowed_modules)

		self.doc = mrinimitable.get_cached_doc("Workspace", self.page_name)
		if (
			self.doc
			and self.doc.module
		...
		self.restricted_doctypes = (
			mrinimitable.cache.get_value("domain_restricted_doctypes") or build_domain_restricted_doctype_cache()
		)
		self.restricted_pages = (
			mrinimitable.cache.get_value("domain_restricted_pages") or build_domain_restricted_page_cache()
		)
	...
	def get_cached(self, cache_key, fallback_fn):
		value = mrinimitable.cache.get_value(cache_key, user=mrinimitable.session.user)
		...
		mrinimitable.cache.set_value(cache_key, value, mrinimitable.session.user, 21600)
		return value
	...
	def get_onboarding_doc(self, onboarding):
		...
		if mrinimitable.db.get_value("Module Onboarding", onboarding, "is_complete"):
			return None

		doc = mrinimitable.get_doc("Module Onboarding", onboarding)

		# Check if user is allowed
		allowed_roles = set(doc.get_allowed_roles())
		user_roles = set(mrinimitable.get_roles())
	...
	def _doctype_contains_a_record(self, name):
		...
		if not exists and mrinimitable.db.exists(name):
			if not mrinimitable.db.get_value("DocType", name, "issingle"):
				exists = bool(mrinimitable.get_all(name, limit=1))
			else:
				exists = True
	...
	def _prepare_item(self, item):
		...
		if item.get("link_type") == "DocType":
			item["description"] = mrinimitable.get_meta(item.link_to).description
	...
	def is_custom_block_permitted(self, custom_block_name):
		...
		allowed = [
			d.role for d in mrinimitable.get_all("Has Role", fields=["role"], filters={"parent": custom_block_name})
		]
	...
	def get_links(self):
		...
		default_country = mrinimitable.db.get_default("country")
	...
	def get_charts(self):
		all_charts = []
		if mrinimitable.has_permission("Dashboard Chart", throw=False):
			charts = self.doc.charts

			for chart in charts:
				if mrinimitable.has_permission("Dashboard Chart", doc=chart.chart_name):
					# Translate label
	...
	def get_shortcuts(self):
		def _in_active_domains(item):
			if not item.restrict_to_domain:
				...
				return item.restrict_to_domain in mrinimitable.get_active_domains()
	...
	def get_onboarding_steps(self, onboarding_doc):
		...
		for doc in onboarding_doc.get_steps():
			...
			if step.action == "Create Entry":
				step.is_submittable = mrinimitable.db.get_value(
	...
	def get_number_cards(self):
		all_number_cards = []
		if mrinimitable.has_permission("Number Card", throw=False):
			number_cards = self.doc.number_cards
			for number_card in number_cards:
				if mrinimitable.has_permission("Number Card", doc=number_card.number_card_name):
					# Translate label
					number_card.label = (
	...
	def get_custom_blocks(self):
		all_custom_blocks = []
		if mrinimitable.has_permission("Custom HTML Block", throw=False):
			custom_blocks = self.doc.custom_blocks

			for custom_block in custom_blocks:
				if mrinimitable.has_permission("Custom HTML Block", doc=custom_block.custom_block_name):
...
def get_desktop_page(page):
	...
	try:
		...
		mrinimitable.log_error("Workspace Missing")
		return {}
...
def get_workspace_sidebar_items():
	...
	has_access = "Workspace Manager" in mrinimitable.get_roles()

	# don't get domain restricted pages
	blocked_modules = mrinimitable.get_cached_doc("User", mrinimitable.session.user).get_blocked_modules()
	blocked_modules.append("Dummy Module")

	# adding None to allowed_domains to include pages without domain restriction
	allowed_domains = [None, *mrinimitable.get_active_domains()]
	...
	all_pages = mrinimitable.get_all(
	...
	try:
		workspace_visibilty = loads(
			mrinimitable.db.get_single_value("Workspace Settings", "workspace_visibility_json") or "{}"
		)
		...
		try:
			...
			if has_access or workspace.is_permitted():
				if page.public and (has_access or not page.is_hidden) and page.title != "Welcome Workspace":
					pages.append(page)
				elif page.for_user == mrinimitable.session.user:
			...
			if not page["app"] and page["module"]:
				page["app"] = mrinimitable.db.get_value("Module Def", page["module"], "app_name") or get_module_app(
					page["module"]
				)
			if page["link_type"] == "Report":
				report_type, ref_doctype = mrinimitable.db.get_value(
	...
	return {
		"workspace_setup_completed": mrinimitable.db.get_single_value(
			"Workspace Settings", "workspace_setup_completed"
		),
		"pages": pages,
		"has_access": has_access,
		"has_create_access": mrinimitable.has_permission(doctype="Workspace", ptype="create"),
	}


def get_table_with_counts():
	counts = mrinimitable.cache.get_value("information_schema:counts")
	if not counts:
...
def update_onboarding_step(name, field, value):
	...
	mrinimitable.db.set_value("Onboarding Step", name, field, value)

	capture(mrinimitable.scrub(name), app="mrinimitable_onboarding", properties={field: value})
