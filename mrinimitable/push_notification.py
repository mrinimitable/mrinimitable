...
import mrinimitable
from mrinimitable import sbool
from mrinimitable.utils.data import cstr
from mrinimitable.utils.response import Response

from .mrinimitableclient import MrinimitableClient
...
class PushNotification:
	...
	def send_notification_to_user(
		...
		if strip_html:
			body = mrinimitable.utils.strip_html(body)
	...
	def is_enabled(self) -> bool:
		...
		"""
		return sbool(
			mrinimitable.db.get_single_value("Push Notification Settings", "enable_push_notification_relay")
		)
	...
	def _get_credential(self) -> tuple[str, str]:
		...
		"""
		notification_settings = mrinimitable.get_doc("Push Notification Settings")
		...
		token = mrinimitable.generate_hash(length=48)
		...
		body = {
			...
			"token": token,
			"webhook_route": "/api/method/mrinimitable.push_notification.auth_webhook",
		}
	...
	def _send_post_request(self, method: str, params: dict, use_authentication: bool = True):
		...
		relay_server_endpoint = mrinimitable.conf.get("push_relay_server_url")
		if use_authentication:
			api_key, api_secret = self._get_credential()
			client = MrinimitableClient(relay_server_endpoint, api_key=api_key, api_secret=api_secret)
		else:
			client = MrinimitableClient(relay_server_endpoint)
	...
	def _site_name(self) -> str:
		return urlparse(mrinimitable.utils.get_url()).hostname

	@property
	def _site_protocol(self) -> str:
		return urlparse(mrinimitable.utils.get_url()).scheme

	@property
	def _site_port(self) -> str:
		site_uri = urlparse(mrinimitable.utils.get_url())
...
def auth_webhook():
	url = urlparse(mrinimitable.utils.get_url()).hostname
	token = mrinimitable.cache().get_value(f"{url}:push_relay_registration_token")
	response = Response()
...
def subscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).add_token(mrinimitable.session.user, fcm_token)
	return {"success": success, "message": message}


@mrinimitable.whitelist(methods=["GET"])
def unsubscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).remove_token(mrinimitable.session.user, fcm_token)
	return {"success": success, "message": message}
