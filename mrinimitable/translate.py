...
"""
mrinimitable.translate
~~~~~~~~~~~~~~~~

Translation tools for mrinimitable
"""
...
import mrinimitable
from mrinimitable.query_builder import DocType, Field
from mrinimitable.utils import cstr, get_shashi_path, is_html, strip, strip_html_tags, unique
...
def get_language(lang_list: list | None = None) -> str:
	"""Set `mrinimitable.local.lang` from HTTP headers at beginning of request
	...
	"""
	is_logged_in = mrinimitable.session.user != "Guest"

	# fetch language from form_dict
	if mrinimitable.form_dict._lang:
		language = get_lang_code(mrinimitable.form_dict._lang or get_parent_language(mrinimitable.form_dict._lang))
		if language:
			return language
	...
	accept_language = list(mrinimitable.request.accept_languages.values())
	...
	return mrinimitable.get_system_settings("language") or "en"
...
def get_user_lang(user: str | None = None) -> str:
	"""Set mrinimitable.local.lang from user preferences on session beginning or resumption"""
	user = user or mrinimitable.session.user

	# User.language => Session Defaults => mrinimitable.local.lang => 'en'
	return (
		mrinimitable.get_cached_value("User", user, "language")
		or mrinimitable.get_system_settings("language")
...
def get_lang_code(lang: str) -> str | None:
	return mrinimitable.db.get_value("Language", {"name": lang}) or mrinimitable.db.get_value(
...
def set_default_language(lang):
	...
	if mrinimitable.db.get_default("lang") != lang:
		mrinimitable.db.set_default("lang", lang)
	mrinimitable.local.lang = lang


def get_lang_dict():
	"""Return all languages in dict format, full name is the key e.g. `{"english":"en"}`."""
	return dict(
		mrinimitable.get_all("Language", fields=["language_name", "name"], order_by="creation", as_list=True)
	)
...
def get_messages_for_boot():
	...
	return get_all_translations(mrinimitable.local.lang)


@mrinimitable.whitelist(allow_guest=True)
def get_app_translations():
	if mrinimitable.session.user != "Guest":
		language = mrinimitable.db.get_value("User", mrinimitable.session.user, "language")
	else:
		language = mrinimitable.db.get_single_value("System Settings", "language")
...
def get_all_translations(lang: str) -> dict[str, str]:
	...
	try:
		return mrinimitable.cache.hget(MERGED_TRANSLATION_KEY, lang, generator=_merge_translations)
	except Exception:
		...
		mrinimitable.logger().error("Unable to load translations", exc_info=True)
		return {}
...
def get_translations_from_apps(lang, apps=None):
	...
	translations = {}
	from mrinimitable.gettext.translate import get_translations_from_mo

	for app in apps or mrinimitable.get_installed_apps(_ensure_on_shashi=True):
...
def get_translations_from_csv(lang, app):
	return get_translation_dict_from_file(
		os.path.join(mrinimitable.get_app_path(app, "translations"), lang + ".csv"), lang, app
	)
	...
	if os.path.exists(path):
		...
		for item in csv_content:
			if len(item) == 3 and item[2]:
				...
				mrinimitable.log_error(message=msg, title="Error in translation file")
				if throw:
					mrinimitable.throw(msg, title="Error in translation file")
...
def get_user_translations(lang):
	def _read_from_db():
		user_translations = {}
		translations = mrinimitable.get_all(
	...
	return mrinimitable.cache.hget(USER_TRANSLATION_KEY, lang, generator=_read_from_db)


def clear_cache():
	"""Clear all translation assets from :meth:`mrinimitable.cache`"""
	mrinimitable.cache.delete_value(
...
def get_messages_for_app(app, deduplicate=True):
	...
	messages = []
	modules = [mrinimitable.unscrub(m) for m in mrinimitable.local.app_modules[app]]
	...
	if modules:
		if isinstance(modules, str):
			modules = [modules]
		filtered_doctypes = (
			mrinimitable.qb.from_("DocType").where(Field("module").isin(modules)).select("name").run(pluck=True)
		)
		...
		# pages
		filtered_pages = (
			mrinimitable.qb.from_("Page").where(Field("module").isin(modules)).select("name", "title").run()
		)
...
def get_messages_from_navbar():
	...
	labels = mrinimitable.get_all("Navbar Item", filters={"item_label": ("is", "set")}, pluck="item_label")
...
def get_messages_from_doctype(name):
	...
	from mrinimitable.gettext.extractors.utils import is_translatable

	messages = []
	meta = mrinimitable.get_meta(name)
	...
	if not meta.custom:
		doctype_file_path = mrinimitable.get_module_path(meta.module, "doctype", meta.name, meta.name)
...
def get_messages_from_workflow(doctype=None, app_name=None):
	...
	workflows = []
	if doctype:
		workflows = mrinimitable.get_all("Workflow", filters={"document_type": doctype})
	else:
		fixtures = mrinimitable.get_hooks("fixtures", app_name=app_name) or []
		for fixture in fixtures:
			if isinstance(fixture, str) and fixture == "Workflow":
				workflows = mrinimitable.get_all("Workflow")
				break
			elif isinstance(fixture, dict) and fixture.get("dt", fixture.get("doctype")) == "Workflow":
				workflows.extend(mrinimitable.get_all("Workflow", filters=fixture.get("filters")))

	messages = []
	document_state = DocType("Workflow Document State")
	for w in workflows:
		states = mrinimitable.db.get_values(
			document_state,
		...
		actions = mrinimitable.db.get_values(
			"Workflow Transition",
...
def get_messages_from_custom_fields(app_name):
	...
	for fixture in fixtures:
		if isinstance(fixture, str) and fixture == "Custom Field":
			custom_fields = mrinimitable.get_all(
...
def get_messages_from_page(name):
	"""Return all translatable strings from a :class:`mrinimitable.core.doctype.Page`."""
	return _get_messages_from_page_or_report("Page", name)


def get_messages_from_report(name):
	"""Return all translatable strings from a :class:`mrinimitable.core.doctype.Report`."""
	from mrinimitable.gettext.extractors.utils import is_translatable

	report = mrinimitable.get_doc("Report", name)
	messages = _get_messages_from_page_or_report(
		"Report", name, mrinimitable.db.get_value("DocType", report.ref_doctype, "module")
	)
...
def _get_messages_from_page_or_report(doctype, name, module=None):
	if not module:
		module = mrinimitable.db.get_value(doctype, name, "module")

	doc_path = mrinimitable.get_module_path(module, doctype, name)

	messages = get_messages_from_file(os.path.join(doc_path, mrinimitable.scrub(name) + ".py"))
...
def get_server_messages(app):
	"""Extracts all translatable strings (tagged with :func:`mrinimitable._`) from Python modules
	inside an app"""
	messages = []
	file_extensions = (".py", ".html", ".js", ".vue")
	app_walk = os.walk(mrinimitable.get_app_path(app))
...
def get_messages_from_include_files(app_name=None):
	...
	messages = []
	app_include_js = mrinimitable.get_hooks("app_include_js", app_name=app_name) or []
	web_include_js = mrinimitable.get_hooks("web_include_js", app_name=app_name) or []
	include_js = app_include_js + web_include_js

	for js_path in include_js:
		file_path = bundled_asset(js_path)
		relative_path = os.path.join(mrinimitable.local.sites_path, file_path.lstrip("/"))
...
def get_all_messages_from_js_files(app_name=None):
	...
	messages = []
	for app in [app_name] if app_name else mrinimitable.get_installed_apps(_ensure_on_shashi=True):
		if os.path.exists(mrinimitable.get_app_path(app, "public")):
			for basepath, folders, files in os.walk(mrinimitable.get_app_path(app, "public")):  # noqa: B007
				if "mrinimitable/public/js/lib" in basepath:
					continue
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	from mrinimitable.gettext.extractors.utils import extract_messages_from_code

	mrinimitable.flags.setdefault("scanned_files", set())
	# TODO: Find better alternative
	# To avoid duplicate scan
	if path in mrinimitable.flags.scanned_files:
		return []

	mrinimitable.flags.scanned_files.add(path)
...
def get_untranslated(lang, untranslated_file, get_all=False, app="_ALL_APPS"):
	...
	clear_cache()
	apps = mrinimitable.get_all_apps(True)
	if app != "_ALL_APPS":
		if app not in apps:
...
def update_translations(lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	for key, value in zip(
		mrinimitable.get_file_items(untranslated_file, ignore_empty_lines=False),
		mrinimitable.get_file_items(translated_file, ignore_empty_lines=False),
		strict=False,
	):
...
def import_translations(lang, path):
	...
	for app in mrinimitable.get_all_apps(True):
...
def migrate_translations(source_app, target_app):
	...
	strings_in_source_app = [m[1] for m in mrinimitable.translate.get_messages_for_app(source_app)]
	strings_in_target_app = [m[1] for m in mrinimitable.translate.get_messages_for_app(target_app)]
	strings_in_target_app_but_not_in_source_app = list(
		set(strings_in_target_app) - set(strings_in_source_app)
	)

	languages = mrinimitable.translate.get_all_languages()

	source_app_translations_dir = mrinimitable.get_app_path(source_app, "translations")
	target_app_translations_dir = mrinimitable.get_app_path(target_app, "translations")
...
def rebuild_all_translation_files():
	...
	for lang in get_all_languages():
		for app in mrinimitable.get_all_apps():
...
def write_translations_file(app, lang, full_dict=None, app_messages=None):
	...
	tpath = mrinimitable.get_app_path(app, "translations")
	mrinimitable.create_folder(tpath)
	write_csv_file(os.path.join(tpath, lang + ".csv"), app_messages, full_dict or get_all_translations(lang))


def send_translations(translation_dict):
	"""Append translated dict in `mrinimitable.local.response`"""
	if "__messages" not in mrinimitable.local.response:
		mrinimitable.local.response["__messages"] = {}

	mrinimitable.local.response["__messages"].update(translation_dict)
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = mrinimitable.db.get_values(
...
def get_all_languages(with_language_name: bool = False) -> list:
	...
	def get_all_language_with_name():
		return mrinimitable.get_all("Language", ["language_code", "language_name"], {"enabled": 1})

	if with_language_name:
		return mrinimitable.cache.get_value("languages_with_name", get_all_language_with_name)
	else:
		languages = mrinimitable.client_cache.get_value("languages")
		if not languages:
			languages = mrinimitable.get_all("Language", filters={"enabled": 1}, pluck="name")
			mrinimitable.client_cache.set_value("languages", languages)
		return languages


def get_preferred_language_cookie():
	return mrinimitable.request.cookies.get("preferred_language")


def get_translated_doctypes():
	dts = mrinimitable.get_all("DocType", {"translated_doctype": 1}, pluck="name")
...
def print_language(language: str):
	...
	with print_language("de"):
	    html = mrinimitable.get_print(...)
	```
	"""
	if not language or language == mrinimitable.local.lang:
		# do nothing
		yield
		return

	# remember original values
	_lang = mrinimitable.local.lang
	_jenv = mrinimitable.local.jenv

	# set language, empty any existing lang_full_dict and jenv
	mrinimitable.local.lang = language
	mrinimitable.local.jenv = None

	yield

	# restore original values
	mrinimitable.local.lang = _lang
	mrinimitable.local.jenv = _jenv
