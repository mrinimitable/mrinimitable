...
import mrinimitable
from mrinimitable import _dict, get_file_json
from mrinimitable.exceptions import IncorrectSitePath
...
def get_site_config(
	...
	*,
	...
	sites_path = sites_path or getattr(mrinimitable.local, "sites_path", ".")
	site_path = site_path or getattr(mrinimitable.local, "site_path", None)
...
def _get_site_config(sites_path: str, site_path: str) -> _dict[str, Any]:
	...
	if site_path:
		...
		if os.path.exists(site_config):
			try:
				...
				click.secho(f"{mrinimitable.local.site}/site_config.json is invalid", fg="red")
				print(error)
				raise
		elif mrinimitable.local.site and not mrinimitable.local.flags.new_site:
			error_msg = f"{mrinimitable.local.site} does not exist."
	...
	def db_default_ports(db_type):
		if db_type == "mariadb":
			from mrinimitable.database.mariadb.database import MariaDBDatabase
	...
	config["redis_queue"] = (
		os.environ.get("MRINIMITABLE_REDIS_QUEUE") or config.get("redis_queue") or "redis://127.0.0.1:11311"
	)
	config["redis_cache"] = (
		os.environ.get("MRINIMITABLE_REDIS_CACHE") or config.get("redis_cache") or "redis://127.0.0.1:13311"
	)
	config["db_type"] = os.environ.get("MRINIMITABLE_DB_TYPE") or config.get("db_type") or "mariadb"

	if config["db_type"] in ("mariadb", "postgres"):
		config["db_socket"] = os.environ.get("MRINIMITABLE_DB_SOCKET") or config.get("db_socket")
		config["db_host"] = os.environ.get("MRINIMITABLE_DB_HOST") or config.get("db_host") or "127.0.0.1"
		config["db_port"] = int(
			os.environ.get("MRINIMITABLE_DB_PORT") or config.get("db_port") or db_default_ports(config["db_type"])
		)

		# Set the user as database name if not set in config
		config["db_user"] = os.environ.get("MRINIMITABLE_DB_USER") or config.get("db_user") or config.get("db_name")

		# read password
		config["db_password"] = os.environ.get("MRINIMITABLE_DB_PASSWORD") or config.get("db_password")

	# vice versa for dbname if not defined
	config["db_name"] = os.environ.get("MRINIMITABLE_DB_NAME") or config.get("db_name") or config["db_user"]
...
def get_conf(site: str | None = None) -> _dict[str, Any]:
	if hasattr(mrinimitable.local, "conf"):
		return mrinimitable.local.conf
