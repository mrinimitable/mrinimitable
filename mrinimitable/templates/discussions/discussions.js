mrinimitable.ready(() => {
	...
	mrinimitable.require("controls.bundle.js", () => {
...
const setup_socket_io = () => {
	mrinimitable.realtime.init(window.socketio_port || "9000");

	mrinimitable.realtime.on("publish_message", (data) => {
		publish_message(data);
	});

	mrinimitable.realtime.on("update_message", (data) => {
		update_message(data);
	});

	mrinimitable.realtime.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const insert_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == mrinimitable.session.user) {
	...
	} else if (topic.owner == mrinimitable.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (strip_html(reply).trim() != "" || reply.includes("img")) {
		...
		mrinimitable.call({
			method: "mrinimitable.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = mrinimitable.get_palette(element.attr("title"));
...
const delete_reply = (e) => {
	mrinimitable.call({
		method: "mrinimitable.website.doctype.discussion_reply.discussion_reply.delete_message",
		args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	mrinimitable.session.user != owner && $template.find(".dropdown").remove();
	return $template.prop("outerHTML");
};
...
const make_comment_editor = (element) => {
	this.comment_editor = new mrinimitable.ui.FieldGroup({
		fields: [
			{
	...
	element.find(".mrinimitable-control").removeClass("hide-control");
