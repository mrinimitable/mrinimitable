...
default_log_level = logging.WARNING if mrinimitable._dev_server else logging.ERROR
stream_logging = os.environ.get("MRINIMITABLE_STREAM_LOGGING")


def create_handler(module, site=None, max_size=100_000, file_count=20, stream_only=False):
	"""Create and return a Mrinimitable-specific logging handler."""
...
def get_logger(
	...
	try:
		return mrinimitable.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(mrinimitable.log_level or default_log_level)
	...
	mrinimitable.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record) -> bool:
		if "Form Dict" not in str(record.msg):
			site = getattr(mrinimitable.local, "site", None)
			form_dict = sanitized_dict(getattr(mrinimitable.local, "form_dict", None))
...
def set_log_level(level: Literal["ERROR", "WARNING", "WARN", "INFO", "DEBUG"]) -> None:
	...
	mrinimitable.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	mrinimitable.loggers = {}
