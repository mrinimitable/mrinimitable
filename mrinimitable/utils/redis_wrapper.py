...
class RedisWrapper(redis.Redis):
	...
	def __call__(self):
		"""WARNING: Added for backward compatibility to support mrinimitable.cache().method(...)"""
		return self
	...
	def make_key(self, key, user=None, shared=False):
		...
		if user:
			if user is True:
				user = mrinimitable.local.session.get("user")

			key = f"user:{user}:{key}"

		return f"{mrinimitable.local.conf.get('db_name')}|{key}".encode()
	...
	def set_value(self, key, val, user=None, expires_in_sec=None, shared=False):
		...
		mrinimitable.local.cache[key] = val
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False, *, use_local_cache=True):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in mrinimitable.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		local_cache = mrinimitable.local.cache
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(mrinimitable.local.cache) if regex.match(cstr(k))]
	...
	def hset(
		...
		**kwargs,
		...
		# set in local
		mrinimitable.local.cache.setdefault(_name, {})[key] = value
	...
	def hdel(
		...
		:param shared: shared mrinimitable key or not
		:param pipeline: A redis.client.Pipeline object, if this transaction is to be run in a pipeline
		"""
		_name = self.make_key(name, shared=shared)

		name_in_local_cache = _name in mrinimitable.local.cache

		if not isinstance(keys, list | tuple):
			if name_in_local_cache and keys in mrinimitable.local.cache[_name]:
				del mrinimitable.local.cache[_name][keys]
			if pipeline:
		...
		for key in keys:
			if name_in_local_cache:
				if key in mrinimitable.local.cache[_name]:
					del mrinimitable.local.cache[_name][key]
...
def setup_cache() -> RedisWrapper:
	if mrinimitable.conf.redis_cache_sentinel_enabled:
		sentinels = [tuple(node.split(":")) for node in mrinimitable.conf.get("redis_cache_sentinels", [])]
		sentinel = get_sentinel_connection(
			sentinels=sentinels,
			sentinel_username=mrinimitable.conf.get("redis_cache_sentinel_username"),
			sentinel_password=mrinimitable.conf.get("redis_cache_sentinel_password"),
			master_username=mrinimitable.conf.get("redis_cache_master_username"),
			master_password=mrinimitable.conf.get("redis_cache_master_password"),
		)
		return sentinel.master_for(
			mrinimitable.conf.get("redis_cache_master_service"),
			redis_class=RedisWrapper,
		)

	return RedisWrapper.from_url(mrinimitable.conf.get("redis_cache"))
	...
	def _enable_client_tracking(self, conn):
		try:
			...
			if "client ID" in str(e) and "does not exist" in str(e):
				...
				mrinimitable.client_cache.healthy = False
...
class ClientCache:
	...
	This feature is internal to Mrinimitable Framework and is subjected to change without any notice.
	...
	Usage/Notes:
		...
		- Same keys can be accessed with `mrinimitable.cache` too, but that won't implement invalidation.
		...
		- Never use `mrinimitable.cache`'s request local cache along with client-side cache. Two
	...
	def __init__(self, maxsize: int = 1024, ttl=10 * 60, monitor: RedisWrapper | None = None) -> None:
		...
		self.invalidator = mrinimitable.cache
		self.healthy = True
		...
		self.redis: RedisWrapper = RedisWrapper.from_url(
			mrinimitable.conf.get("redis_cache"),
	...
	def get_doc(self, doctype: str, name: str | None = None):
		...
		key = mrinimitable.get_document_cache_key(doctype, name)
		return self.get_value(key, generator=lambda: mrinimitable.get_doc(doctype, name))
	...
	def erase_persistent_caches(self, *, doctype=None):
		...
		try:
			self.redis.publish(
				...
				json.dumps({"doctype": doctype, "site": mrinimitable.local.site}),
			)
		except redis.exceptions.ConnectionError:
	...
	def _handle_persistent_cache_invalidation(self, message):
		...
		payload = mrinimitable._dict(json.loads(message["data"]))
