...
from mrinimitable.types.filter import Filters, FilterSignature, FilterTuple
from mrinimitable.utils.deprecations import deprecated
from mrinimitable.utils.number_format import NUMBER_FORMAT_MAP, NumberFormat
...
# these options are necessary to use orjson with mrinimitable
#
...
# mrinimitable converts datetime objects differently (__str__) from orjson (RFC 3339)
#
...
def getdate(
	...
	try:
		...
		try:
			...
			mrinimitable.throw(
				mrinimitable._("{} is not a valid date string.").format(mrinimitable.bold(string_date)),
				title=mrinimitable._("Invalid Date"),
			)
...
def add_to_date(
	...
	if isinstance(date, str):
		...
		try:
			...
			mrinimitable.throw(mrinimitable._("Please select a valid date filter"), title=mrinimitable._("Invalid Date"))
...
def get_system_timezone() -> str:
	...
	return mrinimitable.get_system_settings("time_zone") or "Asia/Kolkata"  # Default to India ?!
...
def now() -> str:
	...
	if mrinimitable.flags.current_date:
		return (
			getdate(mrinimitable.flags.current_date).strftime(DATE_FORMAT)
			+ " "
...
def get_user_date_format() -> str:
	...
	if getattr(mrinimitable.local, "user_date_format", None) is None:
		mrinimitable.local.user_date_format = get_date_format()

	return mrinimitable.local.user_date_format
...
def get_user_time_format() -> str:
	...
	if getattr(mrinimitable.local, "user_time_format", None) is None:
		mrinimitable.local.user_time_format = get_time_format()

	return mrinimitable.local.user_time_format
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string, locale=(mrinimitable.local.lang or "").replace("-", "_")
		)
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(mrinimitable.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(mrinimitable.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
...
def validate_duration_format(duration):
	if not DURATION_PATTERN.match(duration):
		mrinimitable.throw(
			mrinimitable._("Value {0} must be in the valid duration format: d h m s").format(mrinimitable.bold(duration))
		)
...
def global_date_format(date: DateTimeLikeObject, format="long") -> str:
	...
	date = getdate(date)
	return babel.dates.format_date(date, locale=(mrinimitable.local.lang or "en").replace("-", "_"), format=format)
...
def cast_fieldtype(fieldtype, value, show_warning=True):
	if show_warning:
		message = (
			"Function `mrinimitable.utils.data.cast_fieldtype` has been deprecated in favour"
			" of `mrinimitable.utils.data.cast`. Use the newer util for safer type casting."
		)
...
def cast(fieldtype, value=None):
	"""Cast the value to the Python native object of the Mrinimitable fieldtype provided.
	...
	Mapping of Python types => Mrinimitable types:
...
def flt(
	...
	try:
		...
		if isinstance(e, mrinimitable.InvalidRoundingMethod):
			raise
		num = 0.0
...
def cstr(s, encoding="utf-8") -> str:
	...
	return mrinimitable.as_unicode(s, encoding)
...
def rounded(num, precision=0, rounding_method=None):
	...
	rounding_method = (
		rounding_method or mrinimitable.get_system_settings("rounding_method") or "Banker's Rounding (legacy)"
	)
	...
	if rounding_method == "Banker's Rounding":
		...
		mrinimitable.throw(
			mrinimitable._("Unknown Rounding Method: {}").format(rounding_method),
			exc=mrinimitable.InvalidRoundingMethod,
		)
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(
		mrinimitable.db.get_value("Currency", currency, "smallest_currency_fraction_value", cache=True)
	)
...
def fmt_money(
	...
	if precision is None:
		precision = cint(mrinimitable.db.get_default("currency_precision")) or None
	...
	if number_format.decimal_separator:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction = mrinimitable.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and mrinimitable.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = mrinimitable.db.get_value("Currency", currency, "symbol", cache=True) or currency
		symbol_on_right = mrinimitable.db.get_value("Currency", currency, "symbol_on_right", cache=True)

		if symbol_on_right:
			amount = f"{amount} {mrinimitable._(symbol)}"
		else:
			amount = f"{mrinimitable._(symbol)} {amount}"

	return amount


from mrinimitable.deprecation_dumpster import get_number_format_info
...
def __getattr__(name):
	if name == "number_format_info":
		...
		deprecation_warning(
			"unkown",
			"v16",
			"use mrinimitable.utils.number_format.NUMBER_FORMAT_MAP instead of mrinimitable.utils.data.number_format_info",
		)
	else:
		try:
			globals()[name]
...
def money_in_words(
	...
	from mrinimitable.utils import get_defaults
	...
	if not fraction_currency:
		fraction_currency = mrinimitable.db.get_value("Currency", main_currency, "fraction", cache=True)

	number_format = get_number_format()

	fraction_units = mrinimitable.db.get_value("Currency", main_currency, "fraction_units", cache=True)
	if fraction_units:
...
def in_words(integer: int, in_million=True) -> str:
	...
	locale = "en_IN" if not in_million else mrinimitable.local.lang
	integer = int(integer)
	try:
...
def get_thumbnail_base64_for_image(src: str) -> dict[str, str] | None:
	...
	from mrinimitable import cache, safe_decode
	from mrinimitable.core.doctype.file.utils import get_local_image

	if not src:
		mrinimitable.throw(f"Invalid source for image: {src}")
	...
	def _get_base64():
		file_path = mrinimitable.get_site_path("public", src.lstrip("/"))
...
def pdf_to_base64(filename: str) -> bytes | None:
	...
	from mrinimitable.utils.file_manager import get_file_path
...
def pretty_date(iso_datetime: datetime.datetime | str, mini=False) -> str:
	...
	now_dt = now_datetime()
	locale = mrinimitable.local.lang.replace("-", "_") if mrinimitable.local.lang else None
...
def comma_or(some_list: list | tuple, add_quotes=True) -> str:
	...
	e.g. ['a', 'b', 'c'] -> "'a', 'b' or 'c'"
	"""
	return comma_sep(some_list, mrinimitable._("{0} or {1}"), add_quotes)
...
def comma_and(some_list: list | tuple, add_quotes=True) -> str:
	...
	e.g. ['a', 'b', 'c'] -> "'a', 'b' and 'c'"
	"""
	return comma_sep(some_list, mrinimitable._("{0} and {1}"), add_quotes)
...
def comma_sep(some_list: list | tuple, pattern: str, add_quotes=True) -> str:
	...
	if isinstance(some_list, list | tuple):
		...
		if not some_list:
			...
			return pattern.format(", ".join(mrinimitable._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(
	...
	host_name = mrinimitable.local.conf.host_name or mrinimitable.local.conf.hostname
	...
	if not host_name:
		...
		elif mrinimitable.local.site:
			protocol = "http://"

			if mrinimitable.local.conf.ssl_certificate:
				protocol = "https://"

			elif mrinimitable.local.conf.wildcard:
				domain = mrinimitable.local.conf.wildcard.get("domain")
				if (
					domain
					and mrinimitable.local.site.endswith(domain)
					and mrinimitable.local.conf.wildcard.get("ssl_certificate")
				):
					protocol = "https://"

			host_name = protocol + mrinimitable.local.site

		else:
			host_name = mrinimitable.db.get_single_value("Website Settings", "subdomain")
	...
	if not uri and full_address:
		uri = mrinimitable.get_request_header("REQUEST_URI", "")
	...
	if (
		...
		not mrinimitable.conf.restart_supervisor_on_update
		and not mrinimitable.conf.restart_systemd_on_update
		and host_name
...
def get_host_name_from_request() -> str:
	...
	if hasattr(mrinimitable.local, "request") and mrinimitable.local.request and mrinimitable.local.request.host:
		protocol = "https://" if "https" == mrinimitable.get_request_header("X-Forwarded-Proto", "") else "http://"
		return protocol + mrinimitable.local.request.host
...
def get_host_name() -> str:
	...
	e.g. If site is 'https://cloud.mrinimitable.io', returns 'cloud.mrinimitable.io'.
	"""
...
def get_link_to_form(doctype: str, name: str | None = None, label: str | None = None) -> str:
	...
	e.g. get_link_to_form("Sales Invoice", "INV-0001", "Link Label") returns:
	    '<a href="https://mrinimitable.io/app/sales-invoice/INV-0001">Link Label</a>'.
	"""
...
def get_link_to_report(
	...
	e.g. get_link_to_report("Revenue Report", "Link Label") returns:
	        '<a href="https://mrinimitable.io/app/query-report/Revenue%20Report">Link Label</a>'.
	"""
	from mrinimitable import _
...
def get_url_to_form(doctype: str, name: str | None = None) -> str:
	...
	e.g. when doctype="Sales Invoice" and your site URL is "https://mrinimitable.io",
	         returns 'https://mrinimitable.io/app/sales-invoice/INV-00001'
	"""
	if not name:
...
def get_url_to_list(doctype: str) -> str:
	...
	e.g. when doctype="Sales Invoice" and your site URL is "https://mrinimitable.io",
	         returns 'https://mrinimitable.io/app/sales-invoice'
	"""
...
def get_url_to_report(name, report_type: str | None = None, doctype: str | None = None) -> str:
	...
	e.g. when name="Sales Register" and your site URL is "https://mrinimitable.io",
	         returns 'https://mrinimitable.io/app/query-report/Sales%20Register'

	You can optionally pass `report_type` and `doctype` to get the URL for a Report Builder report.

	get_url_to_report("Revenue", "Report Builder", "Sales Invoice") -> 'https://mrinimitable.io/app/sales-invoice/view/report/Revenue'
	"""
...
def filter_operator_is(value: str, pattern: str) -> bool:
	...
	if pattern == "set":
		...
		mrinimitable.throw(mrinimitable._(f"Invalid argument for operator 'IS': {pattern}"))
...
def get_filter(doctype: str, filters: FilterSignature, filters_config=None) -> "mrinimitable._dict":
	"""Return a `_dict` like:
	...
	from mrinimitable.model import child_table_fields, default_fields, optional_fields
	...
	f = mrinimitable._dict(doctype=ft[0], fieldname=ft[1], operator=ft[2], value=ft[3])
	...
	if f.operator.lower() not in valid_operators:
		mrinimitable.throw(mrinimitable._("Operator must be one of {0}").format(", ".join(valid_operators)))
	...
	if f.doctype and (f.fieldname not in default_fields + optional_fields + child_table_fields):
		...
		meta = mrinimitable.get_meta(f.doctype)
		if not meta.has_field(f.fieldname):
			# try and match the doctype name from child tables
			for df in meta.get_table_fields():
				if mrinimitable.get_meta(df.options).has_field(f.fieldname):
					f.doctype = df.options
					break

	try:
		df = mrinimitable.get_meta(f.doctype).get_field(f.fieldname) if f.doctype else None
	except mrinimitable.exceptions.DoesNotExistError:
		df = None
...
def sanitize_column(column_name: str) -> str:
	return _sanitize_column(column_name, (mrinimitable.db and mrinimitable.db.db_type) or None)
...
def _sanitize_column(column_name: str, db_type: str) -> str:
	...
	def _raise_exception():
		mrinimitable.throw(_("Invalid field name {0}").format(column_name), mrinimitable.DataError)
...
def scrub_urls(html: str) -> str:
	...
	e.g. If HTML is '<a href="/files/abc.jpeg">View Image</a>' and site URL is 'https://mrinimitable.io',
	        returns '<a href="https://mrinimitable.io/files/abc.jpeg">View Image</a>'.
	"""
...
def quoted(url: str) -> str:
	...
	e.g. 'https://mrinimitable.io/files/my Image file.jpeg' -> 'https://mrinimitable.io/files/my%20Image%20file.jpeg'
	"""
...
def generate_hash(*args, **kwargs) -> str:
	...
	"""
	return mrinimitable.generate_hash(*args, **kwargs)
...
def get_user_info_for_avatar(user_id: str) -> _UserInfo:
	...
	e.g. {
	        "email": "faris@mrinimitable.io",
	        "image": "/assets/mrinimitable/images/ui/avatar.png",
	        "name": "Faris Ansari"
	}
	"""
	try:
		user = mrinimitable.get_cached_doc("User", user_id)
...
def validate_python_code(string: str, fieldname: str | None = None, is_expression: bool = True) -> None:
	...
	try:
		...
		msg = mrinimitable._("{} Invalid python code on line {}").format(
			fieldname + ":" if fieldname else "", line_no + 1
		)
		msg += f"<br><pre>{error_line}</pre>"
		msg += f"<pre>{' ' * offset}^</pre>"

		mrinimitable.throw(msg, title=mrinimitable._("Syntax Error"))
	except Exception as e:
		mrinimitable.msgprint(
			mrinimitable._("{} Possibly invalid python code. <br>{}").format(fieldname + ": " or "", str(e)),
			indicator="orange",
		)
...
def format_timedelta(o: datetime.timedelta | str) -> str:
	...
	# but Mrinimitable doesn't - I think via babel : only allows 0..23 range for hour
...
def is_site_link(link: str) -> bool:
	...
	if link.startswith("/"):
		return True
	return urlparse(link).netloc == urlparse(mrinimitable.utils.get_url()).netloc
...
def mock(type, size=1, locale="en"):
	...
	from mrinimitable.utils import squashify
...
def add_trackers_to_url(
	...
	def get_utm_values():
		_source = mrinimitable.db.get_value("UTM Source", source, "slug") or source
		_campaign = mrinimitable.db.get_value("UTM Campaign", campaign, "slug") or campaign
		_medium = mrinimitable.db.get_value("UTM Medium", medium, "slug") or medium
	...
	source, medium, campaign = mrinimitable.cache.get_value(
...
def map_trackers(url_trackers: dict, create: bool = False):
	mrinimitable_trackers = {}

	if url_source := url_trackers.get("utm_source", url_trackers.get("source")):
		source = mrinimitable.db.get_value("UTM Source", {"slug": slug(url_source)}, "name") or url_source
		if create and source == url_source and not mrinimitable.db.exists("UTM Source", source):
			source = mrinimitable.new_doc("UTM Source")
			source.name = url_source
			source.description = f"Autogenerated from {url_trackers}"
			source.save(ignore_permissions=True)
		mrinimitable_trackers["utm_source"] = source

	if url_medium := url_trackers.get("utm_medium", url_trackers.get("medium")):
		medium = mrinimitable.db.get_value("UTM Medium", {"slug": slug(url_medium)}, "name") or url_medium
		if create and medium == url_medium and not mrinimitable.db.exists("UTM Medium", medium):
			medium = mrinimitable.new_doc("UTM Medium")
			medium.name = url_medium
			medium.description = f"Autogenerated from {url_trackers}"
			medium.save(ignore_permissions=True)
		mrinimitable_trackers["utm_medium"] = medium

	if url_campaign := url_trackers.get("utm_campaign", url_trackers.get("campaign")):
		campaign = mrinimitable.db.get_value("UTM Campaign", {"slug": slug(url_campaign)}, "name") or url_campaign
		if create and campaign == url_campaign and not mrinimitable.db.exists("UTM Campaign", campaign):
			campaign = mrinimitable.new_doc("UTM Campaign")
			campaign.name = url_campaign
			campaign.campaign_description = f"Autogenerated from {url_trackers}"
			campaign.save(ignore_permissions=True)
		mrinimitable_trackers["utm_campaign"] = campaign

	if url_content := url_trackers.get("utm_content", url_trackers.get("content")):
		mrinimitable_trackers["utm_content"] = url_content

	return mrinimitable_trackers
