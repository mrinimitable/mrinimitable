...
from mrinimitable.database.database import Database, EmptyQueryValues


class MrinimitableIntegration(Integration):
	identifier = "mrinimitable"
...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = mrinimitable._dict({"scheduled": False, "wait": 0})
		...
		if mrinimitable.form_dict.cmd:
			path = f"/api/method/{mrinimitable.form_dict.cmd}"
		else:
	...
	scope.set_user({"id": mrinimitable.local.site})
	user = getattr(mrinimitable.session, "user", "Unidentified")
	scope.set_tag("mrinimitable_user", user)
	# Extract `X-Mrinimitable-Request-ID` to store as a separate field if its present
	if trace_id := mrinimitable.monitor.get_trace_id():
		scope.set_tag("mrinimitable_trace_id", trace_id)
	...
	try:
		...
		with hub.configure_scope() as scope:
			...
			if mrinimitable.request:
				evt_processor = _make_wsgi_event_processor(mrinimitable.request.environ, False)
				scope.add_event_processor(evt_processor)
				if mrinimitable.request.is_json:
					scope.set_context("JSON Body", mrinimitable.request.json)
				elif mrinimitable.request.form:
					scope.set_context("Form Data", mrinimitable.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], mrinimitable.ValidationError | mrinimitable.PermissionError):
					return
		...
		mrinimitable.logger().error("Failed to capture exception", exc_info=True)
