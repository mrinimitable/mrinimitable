...
from mrinimitable.utils import cint, get_file_size, get_url, now, now_datetime
...
class BackupGenerator:
	...
	def __init__(
		...
		site = mrinimitable.local.site or mrinimitable.generate_hash(length=8)
	...
	def _set_existing_tables(self):
		...
		if not hasattr(self, "_existing_tables"):
			self._existing_tables = mrinimitable.db.get_tables()
	...
	def set_backup_tables_from_config(self):
		...
		backup_conf = mrinimitable.conf.get("backup", {})
		self._set_existing_tables()
	...
	def get_backup(self, older_than=24, ignore_files=False, force=False):
		...
		if not (last_db and last_file and last_private_file and site_config_backup_path):
			...
			if mrinimitable.get_system_settings("encrypt_backup"):
	...
	def set_backup_file_name(self):
		...
		enc = "-enc" if mrinimitable.get_system_settings("encrypt_backup") else ""
		...
		for path in paths:
			if os.path.exists(path):
				...
				try:
					...
					mrinimitable.utils.execute_in_shell(command)
	...
	def print_summary(self):
		...
		print(f"Backup Summary for {mrinimitable.local.site} at {now()}")
	...
	def backup_files(self):
		for folder in ("public", "private"):
			files_path = mrinimitable.get_site_path(folder, "files")
			...
			try:
				mrinimitable.utils.execute_in_shell(
					cmd_string.format(backup_path, files_path),
					verbose=self.verbose,
					low_priority=True,
					check_exit_code=True,
				)
			except mrinimitable.CommandFailedError as e:
	...
	def copy_site_config(self):
		...
		site_config_path = os.path.join(mrinimitable.get_site_path(), "site_config.json")
	...
	def take_dump(self):
		if self.db_type == "sqlite":
			...
			db_path = Path(mrinimitable.get_site_path()) / "db" / f"{self.db_name}.db"
			command = f"gzip -k {db_path} -c > {self.backup_path_db}"

			mrinimitable.utils.execute_in_shell(command, low_priority=True, check_exit_code=True)
		...
		if not gzip_exc:
			mrinimitable.throw(
				_("gzip not found in PATH! This is required to take a backup."), exc=mrinimitable.ExecutableNotFound
			)

		if self.old_backup_metadata:
			database_header_content = [
				f"Backup generated by Mrinimitable {mrinimitable.__version__} on branch {get_app_branch('mrinimitable') or 'N/A'}",
				"",
			]
		else:
			database_header_content = [
				"begin mrinimitable metadata",
				"[mrinimitable]",
				f"version = {mrinimitable.__version__}",
				f"branch = {get_app_branch('mrinimitable') or 'N/A'}",
				"end mrinimitable metadata",
				"",
			]
		...
		if self.partial:
			...
			database_header_content.extend(
				[
					f"Partial Backup of Mrinimitable Site {mrinimitable.local.site}",
	...
	def send_email(self):
		...
		"""
		from mrinimitable.email import get_system_managers
...
download only after 24 hours."""
		...
		mrinimitable.sendmail(recipients=recipient_list, message=msg, subject=subject)
...
def fetch_latest_backups(partial=False) -> dict:
	...
	odb = BackupGenerator(
		...
		mrinimitable.conf.db_user,
		mrinimitable.conf.db_password,
...
def delete_temp_backups(older_than=24):
	...
	"""
	older_than = cint(mrinimitable.conf.keep_backups_for_hours) or older_than
	backup_path = get_backup_path()
...
def get_backup_path():
	return mrinimitable.utils.get_site_path(conf.get("backup_path", "private/backups"))
...
def get_or_generate_backup_encryption_key():
	...
	key = mrinimitable.conf.get(BACKUP_ENCRYPTION_CONFIG_KEY)
	if key:
		return key
