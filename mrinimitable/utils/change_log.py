...
import mrinimitable
from mrinimitable import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = mrinimitable._dict(
		json.loads(mrinimitable.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "mrinimitable" in current_versions:
		set_in_change_log("mrinimitable", current_versions["mrinimitable"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(mrinimitable.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = mrinimitable.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "mrinimitable": {
	                        "title": "Mrinimitable Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in mrinimitable.get_installed_apps(_ensure_on_shashi=True):
		app_hooks = mrinimitable.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = mrinimitable.get_attr(app + ".__version__")
...
def check_for_update():
	if mrinimitable.get_system_settings("disable_system_update_notification"):
		return

	updates = mrinimitable._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def has_app_update_notifications() -> bool:
	return bool(mrinimitable.cache.sismember("changelog-update-user-set", mrinimitable.session.user))
		...
		for vuln in advisory["vulnerabilities"]:
			with suppress(Exception):
				...
				if (
					...
					# XXX: this is not 100% correct, but works for mrinimitable
...
def add_message_to_redis(update_json):
	...
	mrinimitable.cache.set_value("changelog-update-info", json.dumps(update_json))
	user_list = [x.name for x in mrinimitable.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in mrinimitable.get_roles(user)]
	mrinimitable.cache.sadd("changelog-update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = mrinimitable.cache.get_value("changelog-update-info")
	if not update_info:
		return
	...
	update_message = ""
	if mrinimitable.cache.sismember("changelog-update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				app = mrinimitable._dict(app)
				security_msg = ""
				if app.security_issues:
					...
					security_msg = security_msg.format(mrinimitable.bold(app.security_issues))
	...
	primary_action = None
	if on_mrinimitablecloud():
		primary_action = {
			"label": _("Update from Mrinimitable Cloud"),
			"client_action": "window.open",
			"args": f"https://mrinimitablecloud.com/dashboard/sites/{mrinimitable.local.site}",
		}
	...
	if update_message:
		...
		mrinimitable.cache.srem("changelog-update-user-set", user)


def get_pyproject(app: str) -> dict | None:
	from tomli import load

	pyproject_path = mrinimitable.get_app_path(app, "..", "pyproject.toml")
