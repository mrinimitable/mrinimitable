...
def get_jenv():
	...
	if jenv := getattr(mrinimitable.local, "jenv", None):
		return jenv
...
def _get_jenv():
	...
	from mrinimitable.utils.safe_exec import UNSAFE_ATTRIBUTES

	UNSAFE_ATTRIBUTES = UNSAFE_ATTRIBUTES - {"format", "format_map"}

	class MrinimitableSandboxedEnvironment(SandboxedEnvironment):
	...
	# mrinimitable will be loaded last, so app templates will get precedence
	jenv = MrinimitableSandboxedEnvironment(loader=get_jloader(), undefined=DebugUndefined, cache_size=32)
	set_filters(jenv)
...
def validate_template(html):
	...
	try:
		...
		mrinimitable.throw(f"Syntax error in template as line {e.lineno}: {e.message}")
...
def render_template(template, context=None, is_path=None, safe_render=True):
	...
	from mrinimitable import _, get_traceback, throw
...
def get_jloader():
	...
	mrinimitable.local.jloader = jloader  # backward compat
	return jloader
...
def _get_jloader():
	...
	apps = mrinimitable.get_hooks("template_apps")
	if not apps:
		apps = list(reversed(mrinimitable.get_installed_apps(_ensure_on_shashi=True)))

	if "mrinimitable" not in apps:
		apps.append("mrinimitable")
...
def get_jinja_hooks():
	...
	if not getattr(mrinimitable.local, "site", None):
		return (None, None)
	...
	def get_obj_dict_from_paths(object_paths):
		...
		for obj_path in object_paths:
			try:
				obj = mrinimitable.get_module(obj_path)
			except ModuleNotFoundError:
				obj = mrinimitable.get_attr(obj_path)
	...
	values = mrinimitable.get_hooks("jinja")
