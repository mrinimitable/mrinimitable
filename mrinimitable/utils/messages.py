...
from mrinimitable.utils.data import safe_decode
...
def msgprint(
	...
	*,
	...
	msg = safe_decode(msg)
	out = mrinimitable._dict(message=msg)
	...
	def _raise_exception():
		if raise_exception:
			if inspect.isclass(raise_exception) and issubclass(raise_exception, Exception):
				...
				exc = mrinimitable.ValidationError(msg)
			if out.__mrinimitable_exc_id:
				exc.__mrinimitable_exc_id = out.__mrinimitable_exc_id
			raise exc

	if mrinimitable.flags.mute_messages:
		_raise_exception()
		return
	...
	if mrinimitable.flags.print_messages and out.message:
	...
	if raise_exception:
		...
		out.__mrinimitable_exc_id = mrinimitable.generate_hash()
	...
	if realtime:
		mrinimitable.publish_realtime(event="msgprint", message=out)
	else:
		mrinimitable.message_log.append(out)
	_raise_exception()
...
def get_message_log() -> list[dict]:
	return [msg_out for msg_out in mrinimitable.local.message_log]


def clear_last_message():
	if len(mrinimitable.local.message_log) > 0:
		mrinimitable.local.message_log = mrinimitable.local.message_log[:-1]


def throw(
	msg: str,
	exc: type[Exception] | Exception = mrinimitable.ValidationError,
	title: str | None = None,
	is_minimizable: bool = False,
	wide: bool = False,
	as_list: bool = False,
	...
	:param msg: Message.
	:param exc: Exception class. Default `mrinimitable.ValidationError`
	:param title: [optional] Message title. Default: "Message".
	:param is_minimizable: [optional] Allow users to minimize the modal
	:param wide: [optional] Show wide modal
	:param as_list: [optional] If `msg` is a list, render as un-ordered list.
...
def throw_permission_error():
	throw(_("Not permitted"), mrinimitable.PermissionError)
