...
def request_cache(func: Callable) -> Callable:
	...
	Cache is stored in `mrinimitable.local.request_cache`.
	...
	def wrapper(*args, **kwargs):
		_cache = getattr(mrinimitable.local, "request_cache", None)
		if _cache is None:
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	The cache is stored in `mrinimitable.utils.caching._SITE_CACHE`.
	...
	workers, use redis (mrinimitable.cache API) instead.
	...
	def time_cache_wrapper(func: Callable | None = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			site = getattr(mrinimitable.local, "site", None)
			if not site:
				return func(*args, **kwargs)
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None, shared: bool = False) -> Callable:
	...
	def wrapper(func: Callable | None = None) -> Callable:
		...
		def clear_cache():
			mrinimitable.cache.delete_keys(func_key)
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			cached_val = mrinimitable.cache.get_value(func_call_key, user=user, shared=shared)
			...
			if mrinimitable.cache.exists(func_call_key, user=user, shared=shared):
				return None

			val = func(*args, **kwargs)
			ttl = getattr(func, "ttl", 3600)
			mrinimitable.cache.set_value(func_call_key, val, expires_in_sec=ttl, user=user, shared=shared)
			return val
	...
	*,
	...
	def outer(func: Callable) -> Callable:
		...
		def inner(*args, **kwargs):
			...
			if mrinimitable.request and mrinimitable.request.method == "GET" and qualified_name in mrinimitable.request.path:
				mrinimitable.local.response_headers.set("Cache-Control", cache_headers)
			return ret
...
def deprecated_local_cache(namespace, key, generator, regenerate_if_none=False):
	if namespace not in mrinimitable.local.cache:
		mrinimitable.local.cache[namespace] = {}

	if key not in mrinimitable.local.cache[namespace]:
		mrinimitable.local.cache[namespace][key] = generator()

	elif mrinimitable.local.cache[namespace][key] is None and regenerate_if_none:
		# if key exists but the previous result was None
		mrinimitable.local.cache[namespace][key] = generator()

	return mrinimitable.local.cache[namespace][key]
