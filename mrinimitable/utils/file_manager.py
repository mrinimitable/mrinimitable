...
import mrinimitable
from mrinimitable import _, conf
from mrinimitable.query_builder.utils import DocType
from mrinimitable.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method
...
def get_file_url(file_data_name):
	data = mrinimitable.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = mrinimitable.form_dict.doctype
	dn = mrinimitable.form_dict.docname
	file_url = mrinimitable.form_dict.file_url
	filename = mrinimitable.form_dict.filename
	mrinimitable.form_dict.is_private = cint(mrinimitable.form_dict.is_private)

	if not filename and not file_url:
		mrinimitable.msgprint(_("Please select a file or url"), raise_exception=True)
	...
	if dt and dn:
		...
		comment = mrinimitable.get_doc(dt, dn).add_comment(
			"Attachment",
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	mrinimitable.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = mrinimitable.form_dict.file_size
	...
	try:
		...
		return mrinimitable.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using mrinimitable.form
	if "filedata" in mrinimitable.form_dict:
		if "," in mrinimitable.form_dict.filedata:
			mrinimitable.form_dict.filedata = mrinimitable.form_dict.filedata.rsplit(",", 1)[1]
		mrinimitable.uploaded_content = safe_b64decode(mrinimitable.form_dict.filedata)
		mrinimitable.uploaded_filename = mrinimitable.form_dict.filename
		return mrinimitable.uploaded_filename, mrinimitable.uploaded_content
	else:
		mrinimitable.msgprint(_("No file attached"))
		return None, None
...
def save_file(fname, content, dt, dn, folder=None, decode=False, is_private=0, df=None):
	...
	f = mrinimitable.get_doc(file_data)
	f.flags.ignore_permissions = True
	try:
		f.insert()
	except mrinimitable.DuplicateEntryError:
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in mrinimitable.get_all(
		"File", {"content_hash": content_hash, "is_private": is_private}, pluck="name"
	):
		b = mrinimitable.get_doc("File", name)
		return {k: b.get(k) for k in mrinimitable.get_hooks()["write_file_keys"]}
	return False
...
def write_file(content, fname, is_private=0):
	...
	mrinimitable.create_folder(file_path)
	# write the file
	if isinstance(content, str):
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in mrinimitable.get_all("File", {"attached_to_doctype": dt, "attached_to_name": dn}, pluck="name"):
			if from_delete:
				# If deleting a doc, directly delete files
				mrinimitable.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if not (attached_to_doctype and attached_to_name):
		attached = mrinimitable.db.get_value("File", fid, ["attached_to_doctype", "attached_to_name", "file_name"])
		if attached:
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = mrinimitable.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = mrinimitable.flags.in_web_form or doc.has_permission("write")
		if not file_name:
			file_name = mrinimitable.db.get_value("File", fid, "file_name")
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = mrinimitable.utils.get_site_path("public", "files", parts[-1])

		else:
			path = mrinimitable.utils.get_site_path("private", "files", parts[-1])
...
def get_file_name(fname, optional_suffix):
	...
	n_records = mrinimitable.get_all("File", {"file_name": fname}, pluck="name")
...
def add_attachments(doctype, name, attachments):
	...
	for a in attachments:
		if isinstance(a, str):
			attach = mrinimitable.db.get_value(
...
def is_safe_path(path: str) -> bool:
	...
	basedir = mrinimitable.get_site_path()
