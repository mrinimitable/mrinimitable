...
from mrinimitable.deprecation_dumpster import gzip_compress, gzip_decompress, make_esc
...
def get_fullname(user=None):
	...
	if not hasattr(mrinimitable.local, "fullnames"):
		mrinimitable.local.fullnames = {}

	if not mrinimitable.local.fullnames.get(user):
		p = mrinimitable.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			mrinimitable.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			mrinimitable.local.fullnames[user] = user

	return mrinimitable.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return mrinimitable.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number: str, fieldname: str) -> None:
	...
	try:
		...
		if not valid_number:
			mrinimitable.throw(
				error_message.format(mrinimitable.bold(phone_number), mrinimitable.bold(fieldname)),
				title=error_title,
				exc=mrinimitable.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		mrinimitable.throw(
			mrinimitable._("{0} is not a valid Phone Number").format(phone_number), mrinimitable.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	* anything else is considered invalid
	...
	Note: "Name" here is name of a person, not the primary key in Mrinimitable doctypes.
	"""
	if not name:
		return False
	...
	if not match and throw:
		mrinimitable.throw(mrinimitable._("{0} is not a valid Name").format(name), mrinimitable.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = mrinimitable.utils.escape_html(e)
				mrinimitable.throw(
					mrinimitable._("{0} is not a valid Email Address").format(invalid_email),
...
def validate_url(
	...
	if not is_valid and throw:
		mrinimitable.throw(mrinimitable._("'{0}' is not a valid URL").format(mrinimitable.bold(txt)))
...
def has_gravatar(email: str) -> str:
	...
	if mrinimitable.flags.in_import or mrinimitable.flags.in_install or mrinimitable.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	...
	"""
	hexdigest = hashlib.md5(mrinimitable.as_unicode(email).encode("utf-8"), usedforsecurity=False).hexdigest()
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", mrinimitable._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	mrinimitable.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return mrinimitable.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return mrinimitable.db.set_default(key, val)
...
def execute_in_shell(cmd, verbose=False, low_priority=False, check_exit_code=False):
	...
	if failed:
		raise mrinimitable.CommandFailedError(
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = mrinimitable.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return mrinimitable.local.site_path
...
def get_shashi_path():
	return os.environ.get("MRINIMITABLE_SHASHI_ROOT") or os.path.realpath(
		os.path.join(os.path.dirname(mrinimitable.__file__), "..", "..", "..")
	)


def get_shashi_id():
	return mrinimitable.get_conf().get("shashi_id", get_shashi_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or mrinimitable.local.site}@{get_shashi_id()}"
...
def get_site_url(site):
	conf = mrinimitable.get_conf(site)
	if conf.host_name:
...
def get_test_client(use_cookies=True) -> Client:
	"""Return an test instance of the Mrinimitable WSGI."""
	from mrinimitable.app import application

	return Client(application, use_cookies=use_cookies)


def get_hook_method(hook_name, fallback=None):
	method = mrinimitable.get_hooks().get(hook_name)
	if method:
		method = mrinimitable.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in mrinimitable.get_hooks(hook):
		out = out or mrinimitable.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(mrinimitable.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in mrinimitable.get_installed_apps():
				include_path = mrinimitable.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(mrinimitable.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else mrinimitable.utils.md_to_html(text)
...
def get_site_info():
	from mrinimitable.email.queue import get_emails_sent_this_month
	from mrinimitable.utils.user import get_system_managers

	# only get system users
	users = mrinimitable.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", mrinimitable.STANDARD_USERS)},
	...
	system_settings = mrinimitable.db.get_singles_dict("System Settings")
	space_usage = mrinimitable._dict((mrinimitable.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": mrinimitable.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in mrinimitable.get_hooks("get_site_info"):
		site_info.update(mrinimitable.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return orjson.loads(mrinimitable.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                shashi --site okayblue.local execute mrinimitable.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = mrinimitable.db.count(doctype)

	return orjson.loads(mrinimitable.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: mrinimitable function to be called
	...
	Example:
	        via terminal:
	                shashi --site okayblue.local execute mrinimitable.utils.call --args '''["mrinimitable.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return orjson.loads(mrinimitable.as_json(mrinimitable.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, int | float):
			filters = mrinimitable.as_unicode(filters)
...
def set_request(**kwargs):
	...
	mrinimitable.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	return mrinimitable.safe_decode(response.get_data())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(mrinimitable.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return mrinimitable.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = mrinimitable.parse_json(mrinimitable.read_file("assets/assets.json"))

		if assets_rtl := mrinimitable.read_file("assets/assets-rtl.json"):
			assets.update(mrinimitable.parse_json(assets_rtl))
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
	...
	missing_info = mrinimitable.get_all(
		"User",
	...
	for info in missing_info:
		user_info.setdefault(info.name, mrinimitable._dict()).update(
...
class CallbackManager:
	...
	Example usage: mrinimitable.db.after_commit
	"""
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from mrinimitable.utils.safe_exec import safe_eval
...
def create_folder(path, with_init=False):
	...
	from mrinimitable.utils import touch_file
