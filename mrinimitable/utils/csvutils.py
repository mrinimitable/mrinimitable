...
from mrinimitable.core.doctype.file.file import FILE_ENCODING_OPTIONS
from mrinimitable.utils import cint, comma_or, cstr, flt


def read_csv_content_from_attached_file(doc):
	fileid = mrinimitable.get_all(
		"File",
		fields=["name"],
	...
	try:
		_file = mrinimitable.get_doc("File", fileid)
...
def read_csv_content(fcontent, use_sniffer: bool = False):
	...
	content = [mrinimitable.safe_decode(line) for line in fcontent.splitlines(True)]
	...
	if use_sniffer:
		...
		try:
			...
			dialect = sniffer.sniff(
				sample="\n".join(sample_content), delimiters=mrinimitable.flags.delimiter_options
			)
		except csv.Error:
	...
	try:
		...
		mrinimitable.msgprint(_("Not a valid Comma Separated Value (CSV File)"))
		raise
...
def send_csv_to_client(args):
	...
	mrinimitable.response["result"] = cstr(to_csv(args.data))
	mrinimitable.response["doctype"] = args.filename
...
def build_csv_response(data, filename):
	mrinimitable.response["result"] = cstr(to_csv(data))
	mrinimitable.response["doctype"] = filename
	mrinimitable.response["type"] = "csv"
...
def check_record(d):
	...
	from mrinimitable.utils.dateutils import parse_date
	...
	for key in d:
		...
		if docfield:
			if docfield.reqd and (val == "" or val is None):
				mrinimitable.msgprint(_("{0} is required").format(docfield.label), raise_exception=1)
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
	...
	if d.get("name") and mrinimitable.db.exists(doctype, d["name"]):
		if overwrite:
			doc = mrinimitable.get_doc(doctype, d["name"])
