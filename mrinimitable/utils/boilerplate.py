...
def make_boilerplate(dest, app_name, no_git=False):
	...
	app_name = mrinimitable.scrub(app_name)
...
def _get_user_inputs(app_name):
	...
	new_app_config = {
		...
		"branch_name": {"prompt": "Branch Name", "default": get_app_branch("mrinimitable")},
	}
...
def copy_from_mrinimitable(rel_path: str, new_app_path: str):
	"""Copy files from mrinimitable app to new app."""
	src = Path(mrinimitable.get_app_path("mrinimitable", "..")) / rel_path
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	mrinimitable.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, mrinimitable.scrub(hooks.app_title)),
		with_init=True,
	)
	mrinimitable.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True)
	mrinimitable.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	mrinimitable.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	mrinimitable.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	mrinimitable.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	mrinimitable.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	mrinimitable.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "__init__.py"), "w") as f:
		f.write(mrinimitable.as_unicode(init_template))

	with open(os.path.join(dest, hooks.app_name, "pyproject.toml"), "w") as f:
		f.write(mrinimitable.as_unicode(pyproject_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, ".pre-commit-config.yaml"), "w") as f:
		f.write(mrinimitable.as_unicode(precommit_template.format(**hooks)))

	license_body = get_license_text(license_name=hooks.app_license)
	with open(os.path.join(dest, hooks.app_name, "license.txt"), "w") as f:
		f.write(mrinimitable.as_unicode(license_body))

	with open(
		os.path.join(dest, hooks.app_name, hooks.app_name, mrinimitable.scrub(hooks.app_title), ".mrinimitable"), "w"
	) as f:
		f.write("")

	from mrinimitable.deprecation_dumpster import boilerplate_modules_txt
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(mrinimitable.as_unicode(hooks_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w") as f:
		f.write(mrinimitable.as_unicode(patches_template.format(**hooks)))

	app_directory = os.path.join(dest, hooks.app_name)

	copy_from_mrinimitable(".editorconfig", app_directory)
	copy_from_mrinimitable(".eslintrc", app_directory)
	...
	with open(os.path.join(dest, hooks.app_name, "README.md"), "w") as f:
		f.write(mrinimitable.as_unicode(readme_template.format(**hooks)))

	if not no_git:
		with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
			f.write(mrinimitable.as_unicode(gitignore_template.format(app_name=hooks.app_name)))
...
class PatchCreator:
	def __init__(self):
		self.all_apps = mrinimitable.get_all_apps(sites_path=".", with_internal_apps=False)
	...
	def _ask_app_name(self):
		...
		self.app_dir = pathlib.Path(mrinimitable.get_app_path(self.app))
	...
	def _ask_patch_meta_info(self):
		...
		default_filename = mrinimitable.scrub(self.docstring) + ".py"
...
dynamic = ["version"]
dependencies = [
    # "mrinimitable~=15.0.0" # Installed and managed by shashi.
]
...
# Desk Notifications
# ------------------
# See mrinimitable.core.notifications.get_notification_config
...
# 	"mrinimitable.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
# each overriding function accepts a `data` argument;
# generated from the base implementation of the doctype dashboard,
# along with any modifications made in other Mrinimitable apps
...
jobs:
  tests:
    ...
    steps:
      ...
      - name: Setup
        run: |
          pip install mrinimitable-shashi
          shashi init --skip-redis-config-generation --skip-assets --python "$(which python)" ~/mrinimitable-shashi
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL character_set_server = 'utf8mb4'"
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL collation_server = 'utf8mb4_unicode_ci'"

      - name: Install
        working-directory: /home/runner/mrinimitable-shashi
        run: |
...
# Read docs to understand patches: https://mrinimitableframework.com/docs/v14/user/en/database-migrations
  ...
  linter:
    ...
    steps:
      ...
      - name: Download Semgrep rules
        run: git clone --depth 1 https://github.com/mrinimitable/semgrep-rules.git mrinimitable-semgrep-rules

      - name: Run Semgrep rules
        run: |
          pip install semgrep
          semgrep ci --config ./mrinimitable-semgrep-rules/rules --config r/python.lang.correctness
...
- Linters: Runs [Mrinimitable Semgrep Rules](https://github.com/mrinimitable/semgrep-rules) and [pip-audit](https://pypi.org/project/pip-audit/) on every pull request.
