...
import mrinimitable
from mrinimitable.model.meta import get_field_currency, get_field_precision
...
def format_value(value, df=None, doc=None, currency=None, translated=False, format=None):
	...
	if isinstance(df, str):
		df = mrinimitable._dict(fieldtype=df)

	if not df:
		df = mrinimitable._dict()
	...
	if not df:
		...
		default_currency = mrinimitable.db.get_default("currency")
		...
		if not BLOCK_TAGS_PATTERN.search(value):
			return mrinimitable.safe_decode(value).replace("\n", "<br>")

	elif df.get("fieldtype") == "Markdown Editor":
		return mrinimitable.utils.markdown(value)

	elif df.get("fieldtype") == "Table MultiSelect":
		values = []
		meta = mrinimitable.get_meta(df.options)
		...
		if isinstance(value, str):
			return mrinimitable._(value, context=df.parent or "")
