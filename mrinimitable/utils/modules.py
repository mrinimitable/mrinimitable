...
def get_modules_from_all_apps_for_user(user: str | None = None) -> list[dict]:
	...
	global_blocked_modules = mrinimitable.get_doc("User", "Administrator").get_blocked_modules()
	user_blocked_modules = mrinimitable.get_doc("User", user).get_blocked_modules()
...
def get_modules_from_app(app):
	return mrinimitable.get_all("Module Def", filters={"app_name": app}, fields=["module_name", "app_name as app"])


def get_all_empty_tables_by_module():
	table_rows = mrinimitable.qb.Field("table_rows")
	table_name = mrinimitable.qb.Field("table_name")
	information_schema = mrinimitable.qb.Schema("information_schema")

	empty_tables = (
		mrinimitable.qb.from_(information_schema.tables).select(table_name).where(table_rows == 0)
	).run()

	empty_tables = {r[0] for r in empty_tables}

	results = mrinimitable.get_all("DocType", fields=["name", "module"])
