...
def web_blocks(blocks):
	import mrinimitable
	from mrinimitable import _, _dict, throw
	from mrinimitable.website.doctype.web_page.web_page import get_web_blocks_html
	...
	if not mrinimitable.flags.web_block_scripts:
	...
	for template, scripts in out.scripts.items():
		...
		if template not in mrinimitable.flags.web_block_scripts:
			for script in scripts:
				html += f"<script data-web-template='{template}'>{script}</script>"
			mrinimitable.flags.web_block_scripts[template] = True

	for template, styles in out.styles.items():
		# deduplication of styles when web_blocks methods are used in web pages
		# see render_dynamic method web_page.py
		if template not in mrinimitable.flags.web_block_styles:
			for style in styles:
				html += f"<style data-web-template='{template}'>{style}</style>"
			mrinimitable.flags.web_block_styles[template] = True

	return html


def get_dom_id(seed=None):
	from mrinimitable import generate_hash
...
def include_script(path, preload=True):
	...
	if preload:
		...
		mrinimitable.local.preload_assets["script"].append(path)
...
def include_icons(path, preload=True):
	...
	if preload:
		...
		mrinimitable.local.preload_assets["icons"].append(path)
...
def include_style(path, rtl=None, preload=True):
	...
	if preload:
		...
		mrinimitable.local.preload_assets["style"].append(path)

	return f'<link type="text/css" rel="stylesheet" href="{path}">'


def bundled_asset(path, rtl=None):
	from mrinimitable.utils import get_assets_json
	from mrinimitable.website.utils import abs_url
...
def is_rtl(rtl=None):
	from mrinimitable import local
