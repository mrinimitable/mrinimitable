...
from mrinimitable.utils import cstr, scrub_urls
from mrinimitable.utils.caching import redis_cache
from mrinimitable.utils.jinja_globals import bundled_asset, is_rtl

cssutils.log.setLog(mrinimitable.logger("cssutils"))
...
def pdf_body_html(template, args, **kwargs):
	try:
		...
		mrinimitable.throw(
			...
			exc=mrinimitable.PrintFormatError,
...
def get_pdf(html, options=None, output: PdfWriter | None = None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				print(html, options)
				mrinimitable.throw(_("PDF generation failed because of broken image links"))
...
def prepare_options(html, options):
	...
	# page size
	pdf_page_size = (
		options.get("page-size") or mrinimitable.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
	)

	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or mrinimitable.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or mrinimitable.db.get_single_value(
...
def get_cookie_options():
	options = {}
	if mrinimitable.session and mrinimitable.session.sid and hasattr(mrinimitable.local, "request"):
		# Use wkhtmltopdf's cookie-jar feature to set cookies and restrict them to host domain
		cookiejar = f"/tmp/{mrinimitable.generate_hash()}.jar"

		# Remove port from request.host
		# https://werkzeug.palletsprojects.com/en/0.16.x/wrappers/#werkzeug.wrappers.BaseRequest.host
		domain = mrinimitable.utils.get_host_name().split(":", 1)[0]
		with open(cookiejar, "w") as f:
			f.write(f"sid={mrinimitable.session.sid}; Domain={domain};\n")
...
def _get_base64_image(src):
	...
	try:
		...
		mrinimitable.logger("pdf").error("Failed to convert inline images to base64", exc_info=True)
...
def prepare_header_footer(soup: BeautifulSoup):
	...
	css = mrinimitable.read_file(os.path.join(mrinimitable.local.sites_path, print_css))
	...
	for html_id in ("header-html", "footer-html"):
		if content := soup.find(id=html_id):
			...
			hook_func = mrinimitable.get_hooks(id_map.get(html_id))
			html = mrinimitable.call(
				hook_func[-1],
				soup=soup,
				head=head,
				content=content,
			...
			# create temp file
			fname = os.path.join("/tmp", f"mrinimitable-pdf-{mrinimitable.generate_hash()}.html")
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/mrinimitable-pdf-random.html"}
...
def get_wkhtmltopdf_version():
	wkhtmltopdf_version = mrinimitable.cache.hget("wkhtmltopdf_version", None)

	if not wkhtmltopdf_version:
		try:
			res = subprocess.check_output(["wkhtmltopdf", "--version"])
			wkhtmltopdf_version = res.decode("utf-8").split(" ")[1]
			mrinimitable.cache.hset("wkhtmltopdf_version", None, wkhtmltopdf_version)
		except Exception:
			pass
