...
from mrinimitable.utils import cint, strip_html_tags
...
def reset():
	...
	:return:
	"""
	mrinimitable.db.delete("__global_search")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in mrinimitable.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = mrinimitable.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = mrinimitable.get_installed_apps()
		module_app = mrinimitable.local.module_app

		doctypes = [
			d.name
			for d in global_search_doctypes
			if module_app.get(mrinimitable.scrub(d.module)) and module_app[mrinimitable.scrub(d.module)] in installed_apps
		]

		return doctypes

	return mrinimitable.cache.get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if mrinimitable.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = mrinimitable._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = mrinimitable.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", mrinimitable.model.table_fields], "options": doctype},
		)
	...
	all_records = mrinimitable.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = mrinimitable.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": mrinimitable.db.escape(doctype),
					"name": mrinimitable.db.escape(doc.name),
					"content": mrinimitable.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": mrinimitable.db.escape((title or "")[: int(mrinimitable.db.VARCHAR_LEN)]),
					"route": mrinimitable.db.escape((route or "")[: int(mrinimitable.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def delete_global_search_records_for_doctype(doctype):
	mrinimitable.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = mrinimitable._dict()
	child_search_fields = mrinimitable._dict()

	for child in meta.get_table_fields():
		child_meta = mrinimitable.get_meta(child.options)
		...
		if search_fields:
			...
			child_records = mrinimitable.get_all(
				child.options,
			...
			for record in child_records:
				all_children.setdefault(record.parent, mrinimitable._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	content = [
		...
		if doc.get(field.fieldname) and field.fieldtype not in mrinimitable.model.table_fields
	]
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(mrinimitable.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if mrinimitable.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	...
	routes_to_index = []
	for app in apps:
		base = mrinimitable.get_app_path(app, "www")
		path_to_index = mrinimitable.get_app_path(app, "www")
...
def add_route_to_global_search(route):
	...
	mrinimitable.local.no_cache = True
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(mrinimitable.safe_decode(value))
...
def sync_global_search():
	...
	def get_search_queue_item_generator():
		while value := mrinimitable.cache.rpop("global_search_queue"):
			yield value
...
def sync_values(values: list):
	...
	GlobalSearch = mrinimitable.qb.Table("__global_search")
	conflict_fields = ["content", "published", "title", "route"]

	query = mrinimitable.qb.into(GlobalSearch).columns(["doctype", "name", *conflict_fields]).insert(*values)
...
def sync_value_in_queue(value):
	try:
		...
		mrinimitable.cache.lpush("global_search_queue", json.dumps(value))
...
def delete_for_document(doc):
	...
	"""
	mrinimitable.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	from mrinimitable.query_builder.functions import Match

	results = []
	sorted_results = []

	allowed_doctypes = set(get_doctypes_for_global_search()) & set(mrinimitable.get_user().get_can_read())
	...
	for doctype in allowed_doctypes:
		for r in results:
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = mrinimitable.get_meta(r.doctype)
					if meta.image_field:
						r.image = mrinimitable.db.get_value(r.doctype, r.name, meta.image_field)
					if meta.title_field:
						r.title = mrinimitable.db.get_value(r.doctype, r.name, meta.title_field)
				except Exception:
...
def web_search(text: str, scope: str | None = None, start: int = 0, limit: int = 20):
	...
	for text in texts:
		...
		mariadb_conditions += "MATCH(`content`) AGAINST ({} IN BOOLEAN MODE)".format(
			mrinimitable.db.escape("+" + text + "*")
		)
		postgres_conditions += f'TO_TSVECTOR("content") @@ PLAINTO_TSQUERY({mrinimitable.db.escape(text)})'

		values = {"scope": "".join([scope, "%"]) if scope else "", "limit": limit, "start": start}

		result = mrinimitable.db.multisql(
			{
