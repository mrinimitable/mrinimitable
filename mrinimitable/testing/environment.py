"""
This module handles the setup and teardown of the test environment for Mrinimitable applications.
...
def _initialize_test_environment(site, config):
	...
	mrinimitable.flags.print_messages = logger.getEffectiveLevel() < logging.INFO
	mrinimitable.flags.tests_verbose = logger.getEffectiveLevel() < logging.INFO
...
def _decorate_all_methods_and_functions_with_type_checker():
	...
	def _decorate_callable(obj, parent_module):
		# whitelisted methods are already checked, see mrinimitable.whitelist
		if getattr(obj, "__func__", obj) in mrinimitable.whitelisted:
			return obj
		# Check if the function is already decorated
			...
			config = _get_config_from_pyproject(mrinimitable.get_app_source_path(app))
	...
	for app in (apps := mrinimitable.get_installed_apps()):
		...
		for _, submodule_name, _ in pkgutil.walk_packages(path=pkg.__path__, prefix=pkg.__name__ + "."):
			...
			submodule = mrinimitable.get_module(submodule_name)
...
class IntegrationTestPreparation:
	...
	def _run_before_test_hooks(app: str, category: str):
		...
		for hook_function in mrinimitable.get_hooks("before_tests", app_name=app):
			logger.info(f'Running "before_tests" hook function {hook_function}')
			mrinimitable.get_attr(hook_function)()
	...
	def _create_global_test_record_dependencies(app: str, category: str):
		...
		try:
			test_module = mrinimitable.get_module(f"{app}.tests")
