...
import mrinimitable
from mrinimitable.modules.utils import get_module_name
from mrinimitable.testing.config import TestParameters


class MrinimitableTestLoader(unittest.TestLoader):
	...
	def discover_tests(self, params: TestParameters) -> unittest.TestSuite:
		...
		if self.params.tests:
			...
			files = self.get_files(mrinimitable.get_installed_apps())
			...
			module = mrinimitable.get_cached_value("DocType", self.params.doctype, "module")
			app = mrinimitable.get_cached_value("Module Def", module, "app_name")
