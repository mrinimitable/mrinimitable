"""
This module provides functionality for discovering and organizing tests in the Mrinimitable framework.
...
logger = logging.getLogger("mrinimitable.testing.discovery")
...
def discover_doctype_tests(doctypes: list[str], runner, app: str, force: bool = False) -> "TestRunner":
	...
	for doctype in doctypes:
		try:
			...
			doctype_app = mrinimitable.db.get_value("Module Def", module, "app_name")
			if app is None:
				_app = doctype_app
			...
			test_module = mrinimitable.modules.utils.get_module_name(doctype, module, "test_")
			force and mrinimitable.db.delete(doctype)
	...
	for test in runner._iterate_suite(test_suite):
		...
		match test:
			...
			case _:
				...
				if any(b.__name__ == "MrinimitableTestCase" for b in test.__class__.__bases__):
					from mrinimitable.deprecation_dumpster import deprecation_warning

					deprecation_warning(
						"2024-20-08",
						"v17",
						"accurate categorization of MrinimitableTestCase will be removed from this runner",
					)
					category = "old-mrinimitable-test-class-category"
