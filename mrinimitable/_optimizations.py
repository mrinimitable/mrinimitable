"""This file houses all Mrinimitable specific optimizations and hooks that run on startup or during fork.
...
def optimize_all():
	...
	# Note:
	# - This function is ALWAYS executed as soon as `import mrinimitable` ends.
...
def optimize_gc_parameters():
	from mrinimitable.utils import sbool

	if not bool(sbool(os.environ.get("MRINIMITABLE_TUNE_GC", True))):
		return

	# generational GC gets triggered after certain allocs (g0) which is 700 by default.
	# This number is quite small for mrinimitable where a single query can potentially create 700+
	# objects easily.
...
def freeze_gc():
	...
	#
	# Refer to issue for more info: https://github.com/mrinimitable/mrinimitable/issues/18927
	gc.collect()
	gc.freeze()
...
def optimize_for_gil_contention():
	if not os.environ.get("MRINIMITABLE_PERF_PIN_WORKERS"):
		return

	if "gunicorn" not in str(sys.argv[0]):
		return

	if os.environ.get("MRINIMITABLE_PERF_PIN_WORKERS_DETERMINISTIC"):
