mrinimitable.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = mrinimitable.ui.make_app_page({
		parent: wrapper,
	...
	$(mrinimitable.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new mrinimitable.PermissionEngine(wrapper);
};

mrinimitable.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

mrinimitable.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			change: function () {
				mrinimitable.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return mrinimitable.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (mrinimitable.get_route()[1]) {
			this.doctype_select.set_value(mrinimitable.get_route()[1]);
		} else if (mrinimitable.route_options) {
			if (mrinimitable.route_options.doctype) {
				this.doctype_select.set_value(mrinimitable.route_options.doctype);
			}
			if (mrinimitable.route_options.role) {
				this.role_select.set_value(mrinimitable.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = mrinimitable.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return mrinimitable
				.call({
					module: "mrinimitable.core",
					page: "permission_manager",
		...
		let $d = $(d.wrapper)
			.find(".mrinimitable-confirm-message")
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(mrinimitable.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${mrinimitable.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return mrinimitable.call({
					...
					callback: (r) => {
						if (r.exc) {
							mrinimitable.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			mrinimitable.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					mrinimitable.call({
						...
						callback: (r) => {
							if (r.exc) {
								mrinimitable.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						},
	...
	get_link_fields(doctype) {
		return mrinimitable.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
