...
import mrinimitable
from mrinimitable.core.doctype.log_settings.log_settings import _supports_log_clearing, run_log_clean_up
...
class TestLogSettings(IntegrationTestCase):
	...
	def setUp(self) -> None:
		if self._testMethodName == "test_delete_logs":
			self.datetime = mrinimitable._dict()
	...
	def test_delete_logs(self):
		...
		activity_log_count = mrinimitable.db.count("Activity Log", {"creation": ("<=", self.datetime.past)})
		error_log_count = mrinimitable.db.count("Error Log", {"creation": ("<=", self.datetime.past)})
		email_queue_count = mrinimitable.db.count("Email Queue", {"creation": ("<=", self.datetime.past)})
...
def setup_test_logs(past: datetime) -> None:
	activity_log = mrinimitable.get_doc(
		{
