...
from mrinimitable.utils import cint, cstr, format_datetime, format_duration, formatdate, parse_json
from mrinimitable.utils.csvutils import UnicodeWriter
...
class DataExporter:
	...
	def prepare_args(self):
		...
		if self.all_doctypes:
			self.child_doctypes = [
				...
				for df in mrinimitable.get_meta(self.doctype).get_table_fields()
			]
	...
	def build_response(self):
		...
		if self.file_type == "Excel":
			...
			mrinimitable.response["result"] = cstr(self.writer.getvalue())
			mrinimitable.response["type"] = "csv"
			mrinimitable.response["doctype"] = self.doctype
	...
	def build_field_columns(self, dt, parentfield=None):
		...
		for f in mrinimitable.db.get_table_columns_description(table_name):
			field = meta.get_field(f.name)
			if f.name in ["owner", "creation"]:
				std_field = next((x for x in mrinimitable.model.std_fields if x["fieldname"] == f.name), None)
				if std_field:
					field = mrinimitable._dict(
						{
		...
		_column_start_end = mrinimitable._dict(start=0)
		...
		if dt == self.doctype:
			...
			_column_start_end = mrinimitable._dict(start=len(self.columns))
	...
	def getinforow(docfield):
		...
		if docfield.fieldtype == "Select":
			...
			return cstr(mrinimitable.defaults.get_defaults().date_format)
	...
	def add_data(self):
		...
		mrinimitable.permissions.can_export(self.parent_doctype, raise_exception=True)

		# sort nested set doctypes by `lft asc`
		order_by = None
		table_columns = mrinimitable.db.get_table_columns(self.parent_doctype)
		if "lft" in table_columns and "rgt" in table_columns:
			order_by = f"`tab{self.parent_doctype}`.`lft` asc"
		# get permitted data only
		self.data = mrinimitable.get_list(
		...
		for doc in self.data:
			...
			if self.all_doctypes:
				...
				for c in self.child_doctypes:
					...
					data_row = (
						mrinimitable.qb.from_(child_doctype_table)
						.select("*")
			...
			for i, c in enumerate(self.columns[_column_start_end.start : _column_start_end.end]):
				...
				if value:
					if fieldtype == "Date":
						...
						value = mrinimitable.core.utils.html2text(value)
	...
	def build_response_as_excel(self):
		...
		filename = mrinimitable.generate_hash(length=10)
