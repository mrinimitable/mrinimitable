...
mrinimitable.ui.form.on("Data Export", {
	refresh: (frm) => {
		frm.disable_save();
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					issingle: 0,
					istable: 0,
					name: ["in", mrinimitable.boot.user.can_export],
				},
			};
		});
	},
	reference_doctype: (frm) => {
		...
		if (doctype) {
			mrinimitable.model.with_doctype(doctype, () => set_field_options(frm));
		} else {
...
const can_export = (frm) => {
	...
	if (!doctype) {
		mrinimitable.msgprint(__("Please select the Document Type."));
	} else if (!parent_multicheck_options.length) {
		mrinimitable.msgprint(__("At least one field of Parent Document Type is mandatory"));
	} else {
		is_valid_form = true;
	}
...
const export_data = (frm) => {
	let get_template_url = "/api/method/mrinimitable.core.doctype.data_export.exporter.export_data";
	var export_params = () => {
		let columns = {};
...
const set_field_options = (frm) => {
	...
	frm.filter_list = new mrinimitable.ui.FilterGroup({
...
const make_multiselect_buttons = (parent_wrapper) => {
	...
	["Select All", "Unselect All"].map((d) => {
		mrinimitable.ui.form.make_control({
			parent: $(button_container),
			df: {
				label: __(d),
				fieldname: mrinimitable.scrub(d),
				fieldtype: "Button",
				click: () => {
...
const get_doctypes = (parentdt) => {
	return [parentdt].concat(mrinimitable.meta.get_table_fields(parentdt).map((df) => df.options));
};

const add_doctype_field_multicheck_control = (doctype, parent_wrapper) => {
	const fields = get_fields(doctype);

	mrinimitable.model.std_fields
	...
	const multicheck_control = mrinimitable.ui.form.make_control({
...
const filter_fields = (df) => mrinimitable.model.is_value_type(df) && !df.hidden;
const get_fields = (dt) => mrinimitable.meta.get_docfields(dt).filter(filter_fields);
