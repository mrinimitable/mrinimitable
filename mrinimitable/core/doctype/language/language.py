...
from mrinimitable.defaults import clear_default, set_default
...
class Language(Document):
	...
	def on_update(self):
		mrinimitable.cache.delete_value("languages_with_name")
		mrinimitable.client_cache.delete_value("languages")
	...
	def update_user_defaults(self):
		...
		should be updated.
		"""
		users = mrinimitable.get_all("User", filters={"language": self.name}, pluck="name")
...
def sync_languages():
	"""Create Language records from mrinimitable/geo/languages.csv"""
	from csv import DictReader

	with open(mrinimitable.get_app_path("mrinimitable", "geo", "languages.csv")) as f:
		reader = DictReader(f)
		for row in reader:
			if not mrinimitable.db.exists("Language", row["language_code"]):
				doc = mrinimitable.new_doc("Language")
				doc.update(row)
				doc.insert()
