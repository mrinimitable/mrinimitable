...
mrinimitable.ui.form.on("Audit Trail", {
	refresh(frm) {
	...
	start_date(frm) {
		...
		mrinimitable.db
			...
			.then((creation) => {
				if (mrinimitable.datetime.obj_to_str(creation) < frm.doc.start_date) {
					frm.doc.document = "";
					frm.refresh_fields();
				}
			});
	...
	end_date(frm) {
		mrinimitable.db
			.get_value(frm.doc.doctype_name, frm.doc.document, "creation")
			.then((creation) => {
				if (mrinimitable.datetime.obj_to_str(creation) > frm.doc.end_date) {
					frm.doc.document = "";
					frm.refresh_fields();
				}
			});
	...
	render_changed_fields(frm, document_names, changed_fields) {
		...
		$(mrinimitable.render_template("audit_trail", render_dict)).appendTo(
	...
	render_rows_added_or_removed(frm, changed_fields) {
		...
		for (let key in added_or_removed) {
			...
			$(mrinimitable.render_template("audit_trail_rows_added_removed", section_dict)).appendTo(
