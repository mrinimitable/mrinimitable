mrinimitable.ui.form.on("Communication", {
	onload: function (frm) {
		if (frm.doc.content) {
			frm.doc.content = mrinimitable.dom.remove_script_and_style(frm.doc.content);
		}
			...
			fields: [
				{
					...
					get_query: function () {
						return { query: "mrinimitable.email.get_communication_doctype" };
					},
				},
				{
			...
			if (values) {
				mrinimitable.confirm(
					...
					function () {
						d.hide();
						mrinimitable.call({
							method: "mrinimitable.email.relink",
							args: {
								name: frm.doc.name,
	...
	show_move_dialog: function (frm) {
		var d = new mrinimitable.ui.Dialog({
			...
			primary_action(values) {
				d.hide();
				mrinimitable.call({
					method: "mrinimitable.email.inbox.move_email",
					args: {
	...
	mark_as_read_unread: function (frm) {
		...
		return mrinimitable.call({
			method: "mrinimitable.email.inbox.create_email_flag_queue",
			args: {
				names: [frm.doc.name],
				action: action,
				flag: flag,
	...
	mark_as_closed_open: function (frm) {
		...
		return mrinimitable.call({
			method: "mrinimitable.email.inbox.mark_as_closed_open",
			args: {
				communication: frm.doc.name,
				status: status,
			},
	...
	get_mail_args: function (frm) {
		var sender_email_id = "";
		$.each(mrinimitable.boot.email_accounts, function (idx, account) {
	...
	add_to_contact: function (frm) {
		...
		mrinimitable.new_doc("Contact");
	},

	mark_as_spam: function (frm) {
		mrinimitable.call({
			method: "mrinimitable.email.inbox.mark_as_spam",
			args: {
				communication: frm.doc.name,
			...
			freeze: true,
			callback: function (r) {
				mrinimitable.msgprint(__("Email has been marked as spam"));
			},
		});
	},
	...
	move_to_trash: function (frm) {
		mrinimitable.call({
			method: "mrinimitable.email.inbox.mark_as_trash",
			args: {
				communication: frm.doc.name,
			},
			freeze: true,
			callback: function (r) {
				mrinimitable.msgprint(__("Email has been moved to trash"));
			},
		});
	},
});
