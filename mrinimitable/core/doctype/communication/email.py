...
import mrinimitable
import mrinimitable.email.smtp
...
def make(
	...
	**kwargs,
	...
	"""
	if kwargs:
		from mrinimitable.utils.commands import warn

		warn(
			f"Options {kwargs} used in mrinimitable.core.doctype.communication.email.make "
			"are deprecated or unsupported",
			category=DeprecationWarning,
		)

	if doctype and name:
		mrinimitable.has_permission(doctype, doc=name, ptype="email", throw=True)
...
def _make(
	...
	sender = sender or get_formatted_email(mrinimitable.session.user)
	...
	if cint(send_email):
		if not comm.get_outgoing_email_account():
			mrinimitable.throw(
				...
				exc=mrinimitable.OutgoingEmailError,
			)
...
def add_attachments(name: str, attachments: Iterable[str | dict]) -> None:
	...
	for a in attachments:
		if isinstance(a, str):
			attach = mrinimitable.db.get_value("File", {"name": a}, ["file_url", "is_private"], as_dict=1)
			file_args = {
				"file_url": attach.file_url,
				"is_private": attach.is_private,
			}
		elif isinstance(a, dict) and "fcontent" in a and "fname" in a:
			# dict returned by mrinimitable.attach_print()
			file_args = {
				"file_name": a["fname"],
				"content": a["fcontent"],
				"is_private": 1,
		...
		_file = mrinimitable.new_doc("File")
...
def mark_email_as_seen(name: str | None = None):
	...
	mrinimitable.response.update(mrinimitable.utils.get_imaginary_pixel_response())
...
def _mark_email_as_seen(name):
	try:
		...
		mrinimitable.log_error("Unable to mark as seen", None, "Communication", name)
...
def update_communication_as_read(name):
	...
	communication = mrinimitable.db.get_value("Communication", name, "read_by_recipient", as_dict=True)
