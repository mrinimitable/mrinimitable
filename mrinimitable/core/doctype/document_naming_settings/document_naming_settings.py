...
from mrinimitable.core.doctype.doctype.doctype import validate_series
from mrinimitable.model.document import Document
from mrinimitable.model.naming import NamingSeries
from mrinimitable.permissions import get_doctypes_with_read


class NamingSeriesNotSetError(mrinimitable.ValidationError):
	pass
...
class DocumentNamingSettings(Document):
	...
	if TYPE_CHECKING:
		from mrinimitable.core.doctype.amended_document_naming_settings.amended_document_naming_settings import (
	...
	def _get_transactions(self) -> list[str]:
		...
		standard = mrinimitable.get_all("DocField", {"fieldname": "naming_series"}, "parent", pluck="parent")
		custom = mrinimitable.get_all("Custom Field", {"fieldname": "naming_series"}, "dt", pluck="dt")
	...
	def _get_prefixes(self, doctypes) -> list[str]:
		...
		for d in doctypes:
			try:
				options = mrinimitable.get_meta(d).get_naming_series_options()
				series_templates.update(options)
			except mrinimitable.DoesNotExistError:
				mrinimitable.msgprint(_("Unable to find DocType {0}").format(d))
				continue

		custom_templates = mrinimitable.get_all(
			"DocType",
			fields=["autoname"],
			filters={
	...
	def _evaluate_and_clean_templates(self, series_templates: set[str]) -> list[str]:
		...
		prefixes_from_db = mrinimitable.qb.from_(series).select(series.name).run(pluck=True)
		...
		for series_template in series_templates:
			try:
				...
				mrinimitable.log_error(f"Invalid naming series {series_template}")
	...
	def validate_set_series(self):
		if self.transaction_type and not self.naming_series_options:
			mrinimitable.throw(_("Please set the series to be used."))
	...
	def check_duplicate(self):
		...
		for doctype in all_doctypes_with_naming_series:
			for series in mrinimitable.get_meta(doctype).get_naming_series_options():
				existing_series[stripped_series(series)] = doctype

		dt = mrinimitable.get_doc("DocType", self.transaction_type)

		options = self.get_options_list(self.naming_series_options)
		for series in options:
			if stripped_series(series) in existing_series:
				mrinimitable.throw(_("Series {0} already used in {1}").format(series, existing_series[series]))
	...
	def get_options(self, doctype=None):
		...
		if mrinimitable.get_meta(doctype or self.transaction_type).get_field("naming_series"):
			return mrinimitable.get_meta(doctype or self.transaction_type).get_field("naming_series").options
	...
	def update_amendment_rule(self):
		...
		existing_overrides = mrinimitable.db.get_all(
		...
		for override in existing_overrides:
			mrinimitable.delete_doc("Amended Document Naming Settings", override)

		for row in self.amend_naming_override:
			row.save()

		mrinimitable.msgprint(_("Amendment naming rules updated."), indicator="green", alert=True)
	...
	def update_series_start(self):
		...
		if self.prefix is None:
			mrinimitable.throw(_("Please select prefix first"))
	...
	def create_version_log_for_change(self, series, old, new):
		...
		version.data = mrinimitable.as_json({"changed": [["current", old, new]]})
	...
	def _fetch_last_doc_if_available(self):
		...
		try:
			return mrinimitable.get_last_doc(self.transaction_type)
		except Exception:
			return None
