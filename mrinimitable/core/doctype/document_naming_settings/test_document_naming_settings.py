...
from mrinimitable.core.doctype.document_naming_settings.document_naming_settings import (
	DocumentNamingSettings,
)
from mrinimitable.model.naming import NamingSeries, get_default_naming_series
...
class TestNamingSeries(IntegrationTestCase):
	...
	def setUp(self):
		self.dns: DocumentNamingSettings = mrinimitable.get_doc("Document Naming Settings")
	...
	def test_get_transactions(self):
		...
		existing_naming_series = mrinimitable.get_meta(self.ns_doctype).get_field("naming_series").options
	...
	def test_updates_naming_options(self):
		...
		self.assertIn(test_series, mrinimitable.get_meta(self.ns_doctype).get_naming_series_options())
	...
	def test_amended_naming(self):
		...
		new_amended_doc = mrinimitable.get_doc(
			doctype=self.ns_doctype, some_fieldname="test doc with submit", amended_from=submittable_doc.name
		).insert()
