...
mrinimitable.ui.form.on("Prepared Report", {
	render_filter_values: function (frm, filters) {
		...
		Object.keys(filters).forEach((key) => {
			const filter_row = $(`<tr>
				<td>${mrinimitable.model.unscrub(key)}</td>
				<td>${filters[key]}</td>
			</tr>`);
	...
	refresh: function (frm) {
		...
		if (frm.doc.status == "Completed") {
			frm.page.set_primary_action(__("Show Report"), () => {
				mrinimitable.route_options = { prepared_report_name: frm.doc.name };
				mrinimitable.set_route("query-report", frm.doc.report_name);
			});
			let csv_attached = (frm.get_files() || []).some((f) => f.file_url.endsWith(".csv"));
			if (!csv_attached) {
				frm.add_custom_button(__("Download as CSV"), function () {
					mrinimitable.call({
						method: "mrinimitable.core.doctype.prepared_report.prepared_report.enqueue_json_to_csv_conversion",
						args: {
