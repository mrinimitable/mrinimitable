...
import mrinimitable
from mrinimitable.model import datetime_fields, no_value_fields, table_fields
...
class Version(Document):
	...
	def set_impersonator(data):
		if not mrinimitable.session:
			return
		if impersonator := mrinimitable.session.data.get("impersonated_by"):
			data["impersonated_by"] = impersonator

		if audit_user := mrinimitable.session.data.get("audit_user"):
	...
	def set_diff(self, old: Document, new: Document) -> bool:
		...
		if diff:
			...
			self.data = mrinimitable.as_json(diff, indent=None, separators=(",", ":"))
			return True
		else:
			return False
	...
	def for_insert(self, doc: Document) -> bool:
		...
		self.data = mrinimitable.as_json(data, indent=None, separators=(",", ":"))
		return True
				...
				if doctype:
					...
					if (field_meta := meta.get_field(df.fieldname)) and field_meta.fieldtype == "Link":
						...
						if link_meta.show_title_field_in_link and (
							...
							result = mrinimitable.db.get_values(
								field_meta.options,
...
def on_doctype_update():
	mrinimitable.db.add_index("Version", ["ref_doctype", "docname"])
