mrinimitable.listview_settings["Recorder"] = {
	...
	onload(listview) {
		...
		listview.page.add_button(__("Clear"), () => {
			mrinimitable.xcall("mrinimitable.recorder.delete").then(listview.refresh);
		});
		...
		listview.page.add_menu_item(__("Import"), () => {
			new mrinimitable.ui.FileUploader({
				...
				on_success: (file) => {
					...
					mrinimitable.call({
						method: "mrinimitable.recorder.import_data",
						args: {
							file: file.file_url,
						},
						callback: function () {
		...
		listview.page.add_menu_item(__("Export"), () => {
			mrinimitable.call({
				method: "mrinimitable.recorder.export_data",
				callback: function (r) {
	...
	fetch_recorder_status(listview) {
		return mrinimitable.xcall("mrinimitable.recorder.status").then((status) => {
	...
	stop_recorder(listview) {
		let me = this;
		mrinimitable.xcall("mrinimitable.recorder.stop", {}).then(() => {
			listview.refresh();
	...
	start_recorder(listview) {
		...
		mrinimitable.prompt(
			...
			(values) => {
				mrinimitable.xcall("mrinimitable.recorder.start", values).then(() => {
					listview.refresh();
