...
mrinimitable.ui.form.on("Recorder", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		...
		frm.add_custom_button(__("Suggest Optimizations"), () => {
			mrinimitable.xcall("mrinimitable.core.doctype.recorder.recorder.optimize", {
				recorder_id: frm.doc.name,
			});
		});

		mrinimitable.realtime.on("recorder-analysis-complete", () => {
			frm.reload_doc();
		...
		index_grid.add_custom_button(__("Add Indexes"), function () {
			...
			if (!indexes_to_add.length) {
				mrinimitable.toast(__("You need to select indexes you want to add first."));
				return;
			}
			mrinimitable.xcall("mrinimitable.core.doctype.recorder.recorder.add_indexes", {
...
mrinimitable.ui.form.on("Recorder Query", "form_render", function (frm, cdt, cdn) {
	let row = mrinimitable.get_doc(cdt, cdn);
