...
import mrinimitable
from mrinimitable.core.doctype.report.report import is_prepared_report_enabled
...
class RolePermissionforPageandReport(Document):
	...
	def set_custom_roles(self):
		...
		name = mrinimitable.db.get_value("Custom Role", args, "name")
		if name:
			doc = mrinimitable.get_doc("Custom Role", name)
			roles = doc.roles
		else:
	...
	def update_custom_roles(self):
		...
		if self.report:
			args.update({"ref_doctype": mrinimitable.db.get_value("Report", self.report, "ref_doctype")})

		if name:
			custom_role = mrinimitable.get_doc("Custom Role", name)
			custom_role.set("roles", roles)
			custom_role.save()
		else:
			mrinimitable.get_doc(args).insert()

	def update_disable_prepared_report(self):
		if self.report:
			# intentionally written update query in mrinimitable.db.sql instead of mrinimitable.db.set_value
			mrinimitable.db.sql(
