...
from mrinimitable.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			mrinimitable.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> mrinimitable._dict:
	...
	if job_name == "mrinimitable.utils.background_jobs.run_doc_method":
	...
	return mrinimitable._dict(
		...
		exc_info=exc_info,
		arguments=mrinimitable.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == mrinimitable.local.site
...
@mrinimitable.whitelist()
def stop_job(job_id):
	mrinimitable.get_doc("RQ Job", job_id).stop_job()
