mrinimitable.ui.form.on("System Settings", {
	refresh: function (frm) {
		mrinimitable.call({
			method: "mrinimitable.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				mrinimitable.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", mrinimitable.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					mrinimitable.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					mrinimitable.app.setup_moment();
	...
	after_save: function (frm) {
		...
		const attr_tuples = [
			[frm.doc.language, mrinimitable.boot.sysdefaults.language, mrinimitable.boot.user.language],
			[frm.doc.rounding_method, mrinimitable.boot.sysdefaults.rounding_method], // no user override.
		];
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == mrinimitable.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		mrinimitable.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", mrinimitable.boot.sysdefaults.rounding_method);
			}
		);
	},
