...
mrinimitable.ui.form.on("Installed Applications", {
	...
	show_update_order_dialog() {
		const dialog = new mrinimitable.ui.Dialog({
			...
			primary_action: function () {
				...
				mrinimitable.call({
					method: "mrinimitable.core.doctype.installed_applications.installed_applications.update_installed_apps_order",
					freeze: true,
					args: {
						new_order: new_order,
					},
		...
		mrinimitable
			.xcall(
				"mrinimitable.core.doctype.installed_applications.installed_applications.get_installed_app_order"
			)
			.then((data) => {
				data.forEach((app) => {
				...
				grid.find(".grid-row-check:first").remove() &&
					grid.find(".grid-row-check").replaceWith(mrinimitable.utils.icon("menu"));
				dialog.show();
			});
	},
});
