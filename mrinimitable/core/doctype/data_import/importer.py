...
from mrinimitable.utils import cint, cstr, duration_to_seconds, flt, update_progress_bar
from mrinimitable.utils.csvutils import get_csv_content_from_google_sheets, read_csv_content
from mrinimitable.utils.xlsxutils import (
...
class Importer:
	def __init__(
		...
		if not self.data_import:
			self.data_import = mrinimitable.get_doc(doctype="Data Import")
			if import_type:
				self.data_import.import_type = import_type

		self.template_options = mrinimitable.parse_json(self.data_import.template_options or "{}")
	...
	def get_data_for_import_preview(self):
		...
		out.import_log = mrinimitable.get_all(
			"Data Import Log",
	...
	def before_import(self):
		...
		mrinimitable.cache.hdel("lang", mrinimitable.session.user)
		mrinimitable.set_user_lang(mrinimitable.session.user)

		# set flags
		mrinimitable.flags.in_import = True
		mrinimitable.flags.mute_emails = self.data_import.mute_emails
	...
	def import_data(self):
		...
		if (
			...
			mrinimitable.db.delete("Data Import Log", {"success": 0, "data_import": self.data_import.name})

		# get successfully imported rows
		imported_rows = []
		for log in import_log:
			log = mrinimitable._dict(log)
		...
		batch_size = mrinimitable.conf.data_import_batch_size or 1000

		for batch_index, batched_payloads in enumerate(mrinimitable.utils.create_batch(payloads, batch_size)):
			for i, payload in enumerate(batched_payloads):
				...
				try:
					...
					messages = mrinimitable.local.message_log
					...
					create_import_log(
						...
						{
							"success": False,
							"exception": mrinimitable.get_traceback(),
							"messages": messages,
							"row_indexes": row_indexes,
						},
					)
	...
	def insert_record(self, doc):
		...
		new_doc = mrinimitable.new_doc(self.doctype)
		new_doc.update(doc)
	...
	def update_record(self, doc):
		...
		existing_doc = mrinimitable.get_doc(self.doctype, doc.get(id_field.fieldname))

		updated_doc = mrinimitable.get_doc(self.doctype, doc.get(id_field.fieldname))
		...
		if get_diff(existing_doc, updated_doc):
			...
			# throw if no changes
			mrinimitable.throw(_("No changes to update"))
	...
	def export_import_log(self):
		...
		import_log = mrinimitable.get_all(
			"Data Import Log",
		...
		for log in import_log:
			...
			exception = mrinimitable.utils.cstr(log.get("exception", ""))
	...
	def print_import_log(self, import_log):
		...
		if failed_records:
			...
			file_name = f"{self.doctype}_import_on_{mrinimitable.utils.now()}.txt"
...
class ImportFile:
	def __init__(
		...
		self.doctype = doctype
		self.template_options = template_options or mrinimitable._dict(column_to_field_map=mrinimitable._dict())
		...
		if isinstance(file, str):
			if mrinimitable.db.exists("File", {"file_url": file}):
				self.file_doc = mrinimitable.get_doc("File", {"file_url": file})
		...
		if not self.file_doc and not self.file_path and not self.google_sheets_url:
			mrinimitable.throw(_("Invalid template file for import"))
	...
	def get_data_from_template_file(self):
		...
		if not content:
			mrinimitable.throw(_("Invalid or corrupted content for import"))
	...
	def get_data_for_import_preview(self):
		...
		columns = [mrinimitable._dict({"header_title": "Sr. No", "skip_import": True})]
	...
	def get_payloads_for_import(self):
		...
		while data:
			...
			payloads.append(mrinimitable._dict(doc=doc, rows=rows))
		return payloads
	...
	def read_file(self, file_path: str):
		...
		if self.console:
			file_content = mrinimitable.read_file(file_path, True)
			return file_content, extn

		file_name = mrinimitable.db.get_value("File", {"file_url": file_path})
		if file_name:
			file = mrinimitable.get_doc("File", file_name)
	...
	def read_content(self, content, extension):
		...
		if extension not in ("csv", "xlsx", "xls"):
			mrinimitable.throw(_("Import template should be of type .csv, .xlsx or .xls"), title=error_title)
...
class Row:
	...
	def _parse_doc(self, doctype, columns, values, parent_doc=None, table_df=None):
		doc = mrinimitable._dict()
		if self.import_type == INSERT:
			# new_doc returns a dict with default values set
			doc = mrinimitable.new_doc(
				doctype,
		...
		for key in mrinimitable.model.default_fields + mrinimitable.model.child_table_fields + ("__islocal",):
			doc.pop(key, None)
		...
		is_table = mrinimitable.get_meta(doctype).istable
		...
		if is_table and is_update:
			...
			if id_value and mrinimitable.db.exists(doctype, id_value):
				existing_doc = mrinimitable.get_doc(doctype, id_value)
				existing_doc.update(doc)
				doc = existing_doc
			else:
				# for table rows being inserted in update
				# create a new doc with defaults set
				new_doc = mrinimitable.new_doc(doctype, as_dict=True)
				new_doc.update(doc)
				doc = new_doc
	...
	def validate_value(self, value, col):
		...
		if df.fieldtype == "Select":
			...
			if select_options and cstr(value) not in select_options:
				options_string = ", ".join(mrinimitable.bold(d) for d in select_options)
			...
			if not exists:
				msg = _("Value {0} missing for {1}").format(mrinimitable.bold(value), mrinimitable.bold(df.options))
				self.warnings.append(
					{
						"row": self.row_number,
						"field": df_as_json(df),
				...
				self.warnings.append(
					{
						...
						"message": _("Value {0} must in {1} format").format(
							mrinimitable.bold(value), mrinimitable.bold(get_user_format(col.date_format))
						),
					}
				)
				return
	...
	def link_exists(self, value, df):
		return bool(mrinimitable.db.exists(df.options, value, cache=True))
...
class Header(Row):
	def __init__(self, index, row, doctype, raw_data, column_to_field_map=None):
		...
		self.data = row
		self.doctype = doctype
		column_to_field_map = column_to_field_map or mrinimitable._dict()
...
class Column:
	def __init__(self, index, header, doctype, column_values, map_to_field=None, seen=None):
		...
		self.meta = mrinimitable.get_meta(doctype)
		self.parse()
	...
	def parse(self):
				...
				self.warnings.append(
					{
						"message": _("Mapping column {0} to field {1}").format(
							mrinimitable.bold(header_title or "<i>Untitled Column</i>"), mrinimitable.bold(df.label)
						),
						"type": "info",
					}
				)
		...
		if header_title in self.seen:
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Skipping Duplicate Column {0}").format(mrinimitable.bold(header_title)),
					"type": "info",
				}
			)
			df = None
			...
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Skipping column {0}").format(mrinimitable.bold(header_title)),
					"type": "info",
				}
			)
			...
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Cannot match column {0} with any field").format(mrinimitable.bold(header_title)),
					"type": "info",
				}
			)
		...
		def guess_date_format(d):
			if isinstance(d, datetime | date | time):
				...
				if self.df.fieldtype == "Time":
					return "%H:%M:%S"
			if isinstance(d, str):
				return mrinimitable.utils.guess_date_format(d)
			...
			self.warnings.append(
				{
					...
					"message": message.format(
						mrinimitable.bold(self.header_title),
						len(unique_date_formats),
						mrinimitable.bold(user_date_format),
					),
					"type": "info",
				}
			)
	...
	def validate_values(self):
		...
		if self.df.fieldtype == "Link":
			...
			transform = (lambda v: cstr(v).lower()) if mrinimitable.db.db_type == "mariadb" else cstr
			values = list({transform(v) for v in self.column_values if v})
			exists = [
				transform(d.name) for d in mrinimitable.get_all(self.df.options, filters={"name": ("in", values)})
			]
			...
			if options:
				...
				if invalid:
					valid_values = ", ".join(mrinimitable.bold(o) for o in options)
					invalid_values = ", ".join(mrinimitable.bold(i) for i in invalid)
...
def build_fields_dict_for_column_matching(parent_doctype):
	...
	for doctype, table_df in doctypes:
		...
		fields = get_standard_fields(doctype) + mrinimitable.get_meta(doctype).fields
		for df in fields:
			...
			if parent_doctype == doctype:
				...
				if isinstance(df, dict):
					new_df = mrinimitable._dict(df.copy())
				else:
					new_df = df.as_dict()
...
def get_df_for_column_header(doctype, header):
	...
	df_by_labels_and_fieldname = mrinimitable.cache.hget(
...
def get_id_field(doctype):
	...
	return mrinimitable._dict({"label": "ID", "fieldname": "name", "fieldtype": "Data"})
