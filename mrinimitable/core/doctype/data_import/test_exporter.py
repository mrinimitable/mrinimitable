...
from mrinimitable.core.doctype.data_import.test_importer import create_doctype_if_not_exists
...
class TestExporter(IntegrationTestCase):
	...
	def test_exports_specified_fields(self):
		if not mrinimitable.db.exists(doctype_name, "Test"):
			...
			doc = mrinimitable.get_doc(doctype_name, "Test")
	...
	def test_export_csv_response(self):
		...
		self.assertEqual(mrinimitable.response["doctype"], doctype_name)
