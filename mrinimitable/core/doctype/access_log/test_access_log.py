...
class TestAccessLog(IntegrationTestCase):
	def setUp(self):
		...
		self.file_name = mrinimitable.utils.random_string(10) + ".txt"
		self.test_content = mrinimitable.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = mrinimitable.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = mrinimitable.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(mrinimitable.local.site) + new_private_file.file_url
