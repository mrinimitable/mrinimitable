...
import mrinimitable
from mrinimitable.cache_manager import clear_doctype_cache
...
from mrinimitable.core.doctype.rq_job.test_rq_job import wait_for_completion
...
from mrinimitable.model.delete_doc import delete_controllers
from mrinimitable.model.sync import remove_orphan_doctypes
...
class TestDocType(IntegrationTestCase):
	...
	def test_validate_name(self):
		self.assertRaises(mrinimitable.NameError, new_doctype("_Some DocType").insert)
		self.assertRaises(mrinimitable.NameError, new_doctype("8Some DocType").insert)
		self.assertRaises(mrinimitable.NameError, new_doctype("Some (DocType)").insert)
		self.assertRaises(
		...
		for name in ("Some DocType", "Some_DocType", "Some-DocType"):
			if mrinimitable.db.exists("DocType", name):
				mrinimitable.delete_doc("DocType", name)
	...
	def test_making_sequence_on_change(self):
		mrinimitable.delete_doc_if_exists("DocType", self._testMethodName)
		...
		self.assertEqual(
			...
			"varchar" if mrinimitable.db.db_type == "mariadb" else "character varying",
		)

	def test_doctype_unique_constraint_dropped(self):
		if mrinimitable.db.exists("DocType", "With_Unique"):
			mrinimitable.delete_doc("DocType", "With_Unique")

		dt = new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = mrinimitable.new_doc("With_Unique")
		doc2 = mrinimitable.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(mrinimitable.UniqueValidationError, doc2.insert)
		mrinimitable.db.rollback()
	...
	def test_validate_search_fields(self):
		...
		self.assertRaises(mrinimitable.ValidationError, doc.save)
	...
	def test_all_depends_on_fields_conditions(self):
		...
		docfields = mrinimitable.get_all(
			"DocField",
			or_filters={
				"ifnull(depends_on, '')": ("!=", ""),
	...
	def test_sync_field_order(self):
		...
		# create test doctype
		test_doctype = mrinimitable.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"fields": [
		...
		mrinimitable.delete_doc_if_exists("DocType", "Test Field Order DocType")
		if os.path.isfile(path):
			os.remove(path)

		try:
			mrinimitable.flags.allow_doctype_export = 1
			test_doctype.save()

			# assert that field_order list is being created with the default order
			test_doctype_json = mrinimitable.get_file_json(path)
			...
			with open(path, "w+") as txtfile:
				txtfile.write(mrinimitable.as_json(test_doctype_json))
			...
			mrinimitable.reload_doctype(test_doctype.name, force=True)
			test_doctype.reload()
			...
			mrinimitable.flags.allow_doctype_export = 0
	...
	def test_cancel_link_doctype(self):
		...
		from mrinimitable.desk.form.linked_with import cancel_all_linked_docs, get_submitted_linked_docs
		...
		# create doctype data
		data_link_doc = mrinimitable.new_doc("Test Linked Doctype")
		data_link_doc.some_fieldname = "Data1"
		data_link_doc.insert()
		data_link_doc.save()
		data_link_doc.submit()

		data_doc = mrinimitable.new_doc("Test Doctype")
	...
	def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
		...
		# create doctype data
		data_link_doc_1 = mrinimitable.new_doc("Test Linked Doctype 1")
		data_link_doc_1.some_fieldname = "Data1"
		data_link_doc_1.insert()
		data_link_doc_1.save()
		data_link_doc_1.submit()

		data_doc_2 = mrinimitable.new_doc("Test Doctype 1")
		...
		data_doc = mrinimitable.new_doc("Test Doctype 2")
		data_doc.some_fieldname = "Data1"
		...
		self.assertRaises(mrinimitable.LinkExistsError, data_link_doc_1.cancel)
	...
	def test_create_virtual_doctype(self):
		...
		virtual_doc.reload()
		doc = mrinimitable.get_doc("DocType", "Test Virtual Doctype")

		self.assertDictEqual(doc.as_dict(), virtual_doc.as_dict())
		self.assertEqual(doc.is_virtual, 1)
		self.assertFalse(mrinimitable.db.table_exists("Test Virtual Doctype"))

	def test_create_virtual_doctype_as_child_table(self):
		"""Test virtual DocType as Child Table below a normal DocType."""
		mrinimitable.delete_doc_if_exists("DocType", "Test Parent Virtual DocType", force=1)
		mrinimitable.delete_doc_if_exists("DocType", "Test Virtual DocType as Child Table", force=1)
		...
		doc = mrinimitable.get_doc("DocType", "Test Virtual DocType as Child Table")

		self.assertEqual(doc.is_virtual, 1)
		self.assertEqual(doc.istable, 1)
		self.assertFalse(mrinimitable.db.table_exists("Test Virtual DocType as Child Table"))
		...
		self.assertRaises(mrinimitable.exceptions.ValidationError, parent_doc.insert)
		parent_doc.is_virtual = 1
		parent_doc.insert(ignore_permissions=True)
		self.assertFalse(mrinimitable.db.table_exists("Test Parent Virtual DocType"))
	...
	def test_autoincremented_doctype_transition(self):
		mrinimitable.delete_doc_if_exists("DocType", "testy_autoinc_dt")
		dt = new_doctype("testy_autoinc_dt", autoname="autoincrement").insert(ignore_permissions=True)
		dt.autoname = "hash"

		dt.save(ignore_permissions=True)

		dt_data = mrinimitable.get_doc({"doctype": dt.name, "some_fieldname": "test data"}).insert(
	...
	def test_json_field(self):
		...
		json_doc.insert()
		json_doc.save()
		doc = mrinimitable.get_doc("DocType", "Test Json Doctype")
		...
		test_json = mrinimitable.get_doc("Test Json Doctype", doc.name)
	...
	def test_no_delete_doc(self):
		self.assertRaises(mrinimitable.ValidationError, mrinimitable.delete_doc, "DocType", "Address")
	...
	def test_custom_field_deletion(self):
		...
		mrinimitable.delete_doc("DocType", child)
		self.assertFalse(mrinimitable.get_meta(doctype).get_field(field))
	...
	def test_delete_doctype_with_customization(self):
		...
		# Create 1 record
		original_doc = mrinimitable.get_doc(doctype=doctype, custom_field_name="wat").insert()
		self.assertEqual(original_doc.some_fieldname, "DELETETHIS")

		# delete doctype
		mrinimitable.delete_doc("DocType", doctype)
		...
		restored_doc = mrinimitable.get_last_doc(doctype)
		...
		self.assertEqual(mrinimitable.get_meta(doctype).get_field(field).default, "DELETETHIS")
		mrinimitable.delete_doc("DocType", doctype)
	...
	def test_delete_orphaned_doctypes(self):
		...
		job = mrinimitable.enqueue(remove_orphan_doctypes)
		wait_for_completion(job)

		mrinimitable.db.rollback()
		self.assertFalse(mrinimitable.db.exists("DocType", doctype.name))
	...
	def test_not_in_list_view_for_not_allowed_mandatory_field(self):
		...
		mrinimitable.delete_doc("DocType", doctype.name)
	...
	def test_no_recursive_fetch(self):
		...
		self.assertRaises(mrinimitable.ValidationError, recursive_dt.insert)
	...
	def test_meta_serialization(self):
		...
		doc = mrinimitable.new_doc(doctype.name, some_fieldname="something").insert()
		doc.save()
		doc.submit()
		mrinimitable.get_meta(doctype.name).as_dict()
