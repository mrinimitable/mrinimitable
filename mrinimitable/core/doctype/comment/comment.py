...
from mrinimitable.desk.notifications import notify_mentions
from mrinimitable.exceptions import ImplicitCommitError
...
class Comment(Document):
	...
	def validate(self):
		if not self.comment_email:
			self.comment_email = mrinimitable.session.user
		self.content = mrinimitable.utils.sanitize_html(self.content, always_sanitize=True)
...
def on_doctype_update():
	mrinimitable.db.add_index("Comment", ["reference_doctype", "reference_name"])
...
def get_comments_from_parent(doc):
	...
	try:
		if is_virtual_doctype(doc.reference_doctype):
			...
			_comments = mrinimitable.db.get_value(doc.reference_doctype, doc.reference_name, "_comments") or "[]"

	except Exception as e:
		if mrinimitable.db.is_missing_table_or_column(e):
			_comments = "[]"
...
def update_comments_in_parent(reference_doctype, reference_name, _comments):
	...
	if (
		...
		or mrinimitable.db.get_value("DocType", reference_doctype, "issingle")
	...
	try:
		...
		if mrinimitable.db.is_missing_column(e) and getattr(mrinimitable.local, "request", None):
			pass
		elif mrinimitable.db.is_data_too_long(e):
			raise mrinimitable.DataTooLongException
		else:
			raise
	else:
		...
		# Clear route cache
		if route := mrinimitable.get_cached_value(reference_doctype, reference_name, "route"):
			clear_cache(route)
