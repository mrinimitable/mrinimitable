...
from mrinimitable.website.doctype.blog_post.test_blog_post import make_test_blog
...
class TestUserPermission(IntegrationTestCase):
	...
	def test_default_user_permission_corectness(self):
		...
		mrinimitable.delete_doc("Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not mrinimitable.db.exists("Blog Category", category):
				mrinimitable.get_doc({"doctype": "Blog Category", "title": category}).insert()
