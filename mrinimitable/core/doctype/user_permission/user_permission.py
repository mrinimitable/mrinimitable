...
from mrinimitable.desk.form.linked_with import get_linked_doctypes
...
class UserPermission(Document):
	...
	def on_update(self):
		mrinimitable.cache.hdel("user_permissions", self.user)
		mrinimitable.publish_realtime("update_user_permissions", user=self.user, after_commit=True)
	...
	def validate_user_permission(self):
		...
		duplicate_exists = mrinimitable.get_all(
			self.doctype,
			filters={
				"allow": self.allow,
		...
		if duplicate_exists:
			mrinimitable.throw(_("User permission already exists"), mrinimitable.DuplicateEntryError)

	def validate_default_permission(self):
		"""validate user permission overlap for default value of a particular doctype"""
		overlap_exists = []
		if self.is_default:
			overlap_exists = mrinimitable.get_all(
				self.doctype,
		...
		if overlap_exists:
			ref_link = mrinimitable.get_desk_link(self.doctype, overlap_exists[0].name)
			mrinimitable.throw(_("{0} has already assigned default value for {1}.").format(ref_link, self.allow))
...
def get_user_permissions(user=None):
	...
	if mrinimitable.request and mrinimitable.local.form_dict.cmd == "get_user_permissions":
	...
	cached_user_permissions = mrinimitable.cache.hget("user_permissions", user)
	...
	try:
		for perm in mrinimitable.get_all(
			...
			meta = mrinimitable.get_meta(perm.allow)
			...
			if meta.is_nested_set() and not perm.hide_descendants:
				decendants = mrinimitable.db.get_descendants(perm.allow, perm.for_value)
				for doc in decendants:
					add_doc_to_perm(perm, doc, False, False)

		out = mrinimitable._dict(out)
		mrinimitable.cache.hset("user_permissions", user, out)
	except mrinimitable.db.SQLError as e:
...
def check_applicable_doc_perm(user, doctype, docname):
	...
	applicable = []
	doc_exists = mrinimitable.get_all(
		"User Permission",
		fields=["name"],
		filters={
			"user": user,
...
def clear_user_permissions(user, for_doctype):
	mrinimitable.only_for("System Manager")
	total = mrinimitable.db.count("User Permission", {"user": user, "allow": for_doctype})
...
def insert_user_perm(
	...
	user_perm = mrinimitable.new_doc("User Permission")
	user_perm.user = user
