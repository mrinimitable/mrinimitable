...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(mrinimitable.conf.db_type)
...
def get_query(*args, **kwargs) -> QueryBuilder:
	from mrinimitable.database.query import Engine
...
def DocType(*args, **kwargs):
	return mrinimitable.qb.DocType(*args, **kwargs)


def Table(*args, **kwargs):
	return mrinimitable.qb.Table(*args, **kwargs)


def execute_query(query, *args, **kwargs):
	child_queries = query._child_queries
	query, params = prepare_query(query)
	result = mrinimitable.local.db.sql(query, params, *args, **kwargs)  # nosemgrep
...
def prepare_query(query):
	...
	if mrinimitable.local.flags.get("in_safe_exec", False):
		from mrinimitable.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX, check_safe_sql_query
		...
		if not check_safe_sql_query(query, throw=False):
			...
			if len(callstack) >= 3 and SERVER_SCRIPT_FILE_PREFIX in callstack[2].filename:
				raise mrinimitable.PermissionError("Only SELECT SQL allowed in scripting")
...
def patch_query_execute():
	...
	This excludes the use of `mrinimitable.db.sql` method while
...
def patch_query_aggregation():
	"""Patch aggregation functions to mrinimitable.qb"""
	from mrinimitable.query_builder.functions import _avg, _max, _min, _sum
