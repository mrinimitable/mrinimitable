...
class TestDBQuery(IntegrationTestCase):
	...
	def test_child_table_field_syntax(self):
		note = mrinimitable.get_doc(
			doctype="Note",
			title=f"Test {mrinimitable.utils.random_string(8)}",
			content="test",
	...
	def test_child_table_join(self):
		mrinimitable.delete_doc_if_exists("DocType", "Parent DocType 1")
		mrinimitable.delete_doc_if_exists("DocType", "Parent DocType 2")
		mrinimitable.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		...
		# clear records
		mrinimitable.db.delete("Parent DocType 1")
		mrinimitable.db.delete("Parent DocType 2")
		mrinimitable.db.delete("Child DocType")
		...
		# test query
		results1 = mrinimitable.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = mrinimitable.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_link_field_syntax(self):
		todo = mrinimitable.get_doc(doctype="ToDo", description="Test ToDo", allocated_to="Administrator").insert()
		result = mrinimitable.get_all(
			"ToDo",
	...
	def test_string_as_field(self):
		self.assertEqual(
			mrinimitable.get_all("DocType", as_list=True), mrinimitable.get_all("DocType", fields="name", as_list=True)
		)

	def test_none_filter(self):
		query = mrinimitable.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_between_filters_date_bounds(self):
		date_df = mrinimitable._dict(fieldtype="Date")
		datetime_df = mrinimitable._dict(fieldtype="Datetime")
		today = mrinimitable.utils.nowdate()
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(mrinimitable.DataError):
			DatabaseQuery("DocType").execute(
				fields=[
					"name",
					"issingle",
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = mrinimitable.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = mrinimitable.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = mrinimitable.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = mrinimitable.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = mrinimitable.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = mrinimitable.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = mrinimitable.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = mrinimitable.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = mrinimitable.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(mrinimitable.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(mrinimitable.get_all("Nested DocType"))
		)
	...
	def test_virtual_field_get_list(self):
		try:
			mrinimitable.get_list("Prepared Report", ["*"])
			mrinimitable.get_list("Scheduled Job Type", ["*"])
		except Exception:
	...
	def test_prepare_select_args(self):
		# mrinimitable.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = mrinimitable.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = mrinimitable.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_cast_name(self):
		...
		mrinimitable.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		mrinimitable.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		mrinimitable.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = mrinimitable.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from mrinimitable.desk.doctype.dashboard_settings.dashboard_settings import (
		...
		dashboard_settings = mrinimitable.db.sql(
			f"""
				SELECT name
	...
	def test_virtual_doctype(self):
		...
		class VirtualDocType:
			...
			def get_list(args=None, limit_page_length=0, doctype=None):
				from mrinimitable.types.filter import FilterTuple
		...
		with patch(
			"mrinimitable.controllers",
			new={mrinimitable.local.site: {"Virtual DocType": VirtualDocType}},
		):
			mrinimitable.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)

	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", mrinimitable.get_all("User", {"first_name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", mrinimitable.get_all("User", {"first_name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", mrinimitable.get_all("User", {"first_name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", mrinimitable.get_all("User", {"first_name": ("in", [])}, run=0))
		self.assertIn("ifnull", mrinimitable.get_all("User", {"first_name": ("not in", ["a"])}, run=0))
		self.assertIn("ifnull", mrinimitable.get_all("User", {"first_name": ("not in", [])}, run=0))
		self.assertIn("ifnull", mrinimitable.get_all("User", {"first_name": ("not in", [""])}, run=0))

		# primary key is never nullable
		self.assertNotIn("ifnull", mrinimitable.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertNotIn("ifnull", mrinimitable.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertNotIn("ifnull", mrinimitable.get_all("User", {"name": ("in", (""))}, run=0))
		self.assertNotIn("ifnull", mrinimitable.get_all("User", {"name": ("in", ())}, run=0))

	def test_coalesce_with_datetime_ops(self):
		self.assertNotIn("ifnull", mrinimitable.get_all("User", {"last_active": (">", "2022-01-01")}, run=0))
		self.assertNotIn("ifnull", mrinimitable.get_all("User", {"creation": ("<", "2022-01-01")}, run=0))
		self.assertNotIn(
			"ifnull",
			mrinimitable.get_all("User", {"last_active": ("between", ("2022-01-01", "2023-01-01"))}, run=0),
		)
		self.assertIn("ifnull", mrinimitable.get_all("User", {"last_active": ("<", "2022-01-01")}, run=0))

	def test_ambiguous_linked_tables(self):
		from mrinimitable.desk.reportview import get

		if not mrinimitable.db.exists("DocType", "Related Todos"):
			mrinimitable.get_doc(
				{
					"doctype": "DocType",
					"custom": 1,
			...
			mrinimitable.db.delete("Related Todos")

		todo_one = mrinimitable.get_doc(
			{
				"doctype": "ToDo",
		...
		mrinimitable.form_dict.doctype = "Related Todos"
	...
	def test_select_star_expansion(self):
		count = mrinimitable.get_list("Language", ["SUM(1)", "COUNT(*)"], as_list=1, order_by=None)[0]
		self.assertEqual(count[0], mrinimitable.db.count("Language"))
		self.assertEqual(count[1], mrinimitable.db.count("Language"))

	def test_ifnull_none(self):
		query = mrinimitable.get_all("DocField", {"fieldname": None}, run=0)
		self.assertIn("''", query)
		self.assertNotIn("\\'", query)
		self.assertNotIn("ifnull", query)
		self.assertFalse(mrinimitable.get_all("DocField", {"name": None}))
		self.assertFalse(mrinimitable.get_all("DocField", {"parent": None}))
		self.assertNotIn("0", mrinimitable.get_all("DocField", {"parent": None}, run=0))
...
class TestReportView(IntegrationTestCase):
	...
	def test_get_count(self):
		...
		count = execute_cmd("mrinimitable.desk.reportview.get_count")
		mrinimitable.local.form_dict = mrinimitable._dict(
			{
				"doctype": "DocType",
		...
		dict_filter_response = execute_cmd("mrinimitable.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("mrinimitable.desk.reportview.get_count")
		current_value = mrinimitable.db.sql(
	...
	def test_reportview_get(self):
		...
		response = execute_cmd("mrinimitable.desk.reportview.get")
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			self.assertEqual(mrinimitable.get_list("Test Blog Post", "published"), [])
	...
	def test_db_filter_not_set(self):
		...
		"""
		mrinimitable.get_doc({"doctype": "ToDo", "description": "filter test"}).insert()
		mrinimitable.get_doc({"doctype": "ToDo", "description": "filter test", "reference_name": ""}).insert()
		...
		self.assertEqual(
			len(mrinimitable.get_all("ToDo", filters={"reference_name": ["is", "not set"]})),
			mrinimitable.db.count("ToDo", {"reference_name": ["is", "not set"]}),
		)


def add_child_table_to_blog_post():
	child_table = mrinimitable.get_doc(
		{
			"doctype": "DocType",
			"istable": 1,
			"custom": 1,
...
def create_nested_doctype():
	if mrinimitable.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = mrinimitable.new_doc("Nested DocType")
		d.update(r)
