...
import mrinimitable
from mrinimitable.desk.reportview import export_query, extract_fieldnames
...
class TestReportview(IntegrationTestCase):
	def test_csv(self):
		...
		for delimiter in (",", ";", "\t", "|"):
			mrinimitable.local.form_dict.csv_delimiter = delimiter
			for quoting in (QUOTE_ALL, QUOTE_MINIMAL, QUOTE_NONE, QUOTE_NONNUMERIC):
				mrinimitable.local.form_dict.csv_quoting = quoting
