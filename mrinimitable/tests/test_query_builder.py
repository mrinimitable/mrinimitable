...
from mrinimitable.query_builder.builder import Function
from mrinimitable.query_builder.custom import ConstantColumn
from mrinimitable.query_builder.functions import (
	Cast_,
	Coalesce,
	CombineDatetime,
	Date,
...
def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(mrinimitable.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")
...
class TestCustomFunctionsMariaDB(IntegrationTestCase):
	...
	def test_constant_column(self):
		query = mrinimitable.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
	...
	def test_time(self):
		...
		select_query = mrinimitable.qb.from_(note).select(CombineDatetime(note.posting_date, note.posting_time))
	...
	def test_cast(self):
		...
		self.assertEqual(
			mrinimitable.qb.from_("red").from_(note).select("other", Cast_(note.name, "varchar")).get_sql(),
	...
	def test_round(self):
		...
		query = mrinimitable.qb.from_(note).select(Round(note.price))
		self.assertEqual("select round(`price`,0) from `tabnote`", str(query).lower())

		query = mrinimitable.qb.from_(note).select(Round(note.price, 3))
		self.assertEqual("select round(`price`,3) from `tabnote`", str(query).lower())

	def test_truncate(self):
		note = mrinimitable.qb.DocType("Note")
		query = mrinimitable.qb.from_(note).select(Truncate(note.price, 3))
...
class TestBuilderBase:
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", mrinimitable.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", mrinimitable.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", mrinimitable.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = mrinimitable.qb.from_("ToDo").select("*").limit(1)
	...
	def test_agg_funcs(self):
		...
		mrinimitable.db.truncate(self.doctype_name)
		sample_data = {
			"doctype": self.doctype_name,
			"number": 1,
		}
		mrinimitable.get_doc(sample_data).insert(ignore_mandatory=True)
		sample_data["number"] = 3
		mrinimitable.get_doc(sample_data).insert(ignore_mandatory=True)
		sample_data["number"] = 4
		mrinimitable.get_doc(sample_data).insert(ignore_mandatory=True)
		self.assertEqual(mrinimitable.qb.max(self.doctype_name, "number"), 4)
		self.assertEqual(mrinimitable.qb.min(self.doctype_name, "number"), 1)
		self.assertAlmostEqual(mrinimitable.qb.avg(self.doctype_name, "number"), 2.666, places=2)
		self.assertEqual(mrinimitable.qb.sum(self.doctype_name, "number"), 8.0)
...
class TestParameterization(IntegrationTestCase):
	def test_where_conditions(self):
		...
		query = mrinimitable.qb.from_(DocType).select(DocType.name).where(DocType.owner == "Administrator' --")
	...
	def test_set_conditions(self):
		DocType = mrinimitable.qb.DocType("DocType")
		query = mrinimitable.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_where_conditions_functions(self):
		DocType = mrinimitable.qb.DocType("DocType")
		query = (
			mrinimitable.qb.from_(DocType).select(DocType.name).where(Coalesce(DocType.search_fields == "subject"))
		)
	...
	def test_case(self):
		DocType = mrinimitable.qb.DocType("DocType")
		query = mrinimitable.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = mrinimitable.qb.DocType("DocType")
		query = mrinimitable.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
	...
	def test_named_parameter_wrapper(self):
		from mrinimitable.query_builder.terms import NamedParameterWrapper
...
class TestBuilderMaria(IntegrationTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual("SELECT * FROM `tabNotes`", mrinimitable.qb.from_("Notes").select("*").get_sql())
		self.assertEqual("SELECT * FROM `__Auth`", mrinimitable.qb.from_("__Auth").select("*").get_sql())

	def test_get_qb_type(self):
		from mrinimitable.query_builder import get_query_builder

		qb = get_query_builder(mrinimitable.db.db_type)
...
class TestBuilderPostgres(IntegrationTestCase, TestBuilderBase):
	...
	def test_replace_tables(self):
		info_schema = mrinimitable.qb.Schema("information_schema")
		self.assertEqual(
			'SELECT * FROM "pg_stat_all_tables"',
			mrinimitable.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", mrinimitable.qb.Field("table_name").get_sql())
...
class TestMisc(IntegrationTestCase):
	...
	def test_union(self):
		...
		role = mrinimitable.qb.DocType("Role")
		users = mrinimitable.qb.from_(user).select(user.name)
		roles = mrinimitable.qb.from_(role).select(role.name)
