...
from mrinimitable.utils import cint, get_test_client, get_url

try:
	_site = mrinimitable.local.site
except Exception:
	_site = None
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("mrinimitable.app.get_site_name", return_value=self.site):
				header_patch = patch("mrinimitable.get_request_header", new=patch_request_header)
...
class MrinimitableAPITestCase(IntegrationTestCase):
	...
	def sid(self) -> str:
		...
		return mrinimitable.session.sid
...
class TestResourceAPI(MrinimitableAPITestCase):
	DOCTYPE = "ToDo"
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "mrinimitable.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(MrinimitableAPITestCase):
	def test_ping(self):
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/mrinimitable.realtime.get_user_info
	...
	def test_logs(self):
		...
		def get_message(resp, msg_type):
			return mrinimitable.parse_json(mrinimitable.parse_json(mrinimitable.parse_json(resp.json)[msg_type])[0])
	...
	def test_array_response(self):
		method = "mrinimitable.tests.test_api.test_array"
...
class TestWSGIApp(MrinimitableAPITestCase):
	def test_request_hooks(self):
		self.addCleanup(lambda: _test_REQ_HOOK.clear())

		with self.patch_hooks(
			{
				"before_request": ["mrinimitable.tests.test_api.before_request"],
				"after_request": ["mrinimitable.tests.test_api.after_request"],
			}
		):
...
class TestResponse(MrinimitableAPITestCase):
	def test_generate_pdf(self):
		response = self.get(
			"/api/method/mrinimitable.utils.print_format.download_pdf",
	...
	def test_binary_and_csv_response(self):
		def download_template(file_type):
			...
			return self.post(
				"/api/method/mrinimitable.core.doctype.data_import.data_import.download_template",
				{
					"sid": self.sid,
					"doctype": "User",
		...
		from mrinimitable.desk.utils import provide_binary_file
	...
	def test_download_private_file_with_unique_url(self):
		test_content = mrinimitable.generate_hash()
