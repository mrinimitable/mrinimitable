...
class TestPermissions(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		mrinimitable.clear_cache(doctype="Blog Post")
	...
	def tearDown(self):
		...
		mrinimitable.db.set_value("Blogger", "_Test Blogger 1", "user", None)
	...
	def test_basic_permission(self):
		post = mrinimitable.get_doc("Blog Post", "-test-blog-post")
	...
	def test_select_permission(self):
		...
		permitted_record = mrinimitable.get_list("Blog Post", fields="*", limit=1)[0]
		full_record = mrinimitable.get_all("Blog Post", fields="*", limit=1)[0]
	...
	def test_user_permissions_in_doc(self):
		...
		post1 = mrinimitable.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_user_permissions_in_report(self):
		...
		mrinimitable.set_user("test2@example.com")
		names = [d.name for d in mrinimitable.get_list("Blog Post", fields=["name", "blog_category"])]

		self.assertTrue("-test-blog-post-1" in names)
		self.assertFalse("-test-blog-post" in names)

	def test_default_values(self):
		doc = mrinimitable.new_doc("Blog Post")
	...
	def test_user_link_match_doc(self):
		blogger = mrinimitable.get_doc("Blogger", "_Test Blogger 1")
		blogger.user = "test2@example.com"
		blogger.save()

		mrinimitable.set_user("test2@example.com")

		post = mrinimitable.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(post.has_permission("read"))
	...
	def test_user_link_match_report(self):
		...
		names = [d.name for d in mrinimitable.get_list("Blog Post", fields=["name", "owner"])]
	...
	def test_not_allowed_to_set_user_permissions(self):
		...
		self.assertRaises(
			mrinimitable.PermissionError, add_user_permission, "Blog Post", "-test-blog-post", "test2@example.com"
		)
	...
	def test_read_if_explicit_user_permissions_are_set(self):
		...
		doc = mrinimitable.get_doc("Blog Post", "-test-blog-post")
		self.assertTrue(doc.has_permission("read"))

		# and not this one
		doc = mrinimitable.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_only_once(self):
		blog_post = mrinimitable.get_meta("Blog Post")
		doc = mrinimitable.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_only_once_child_table_rows(self):
		...
		doc = mrinimitable.get_doc("DocType", "Blog Post")
	...
	def test_user_permission_doctypes(self):
		...
		doc = mrinimitable.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(doc.has_permission("read"))
	...
	def test_insert_if_owner_with_user_permissions(self):
		...
		mrinimitable.delete_doc("Blog Post", "-test-blog-post-title")
	...
	def test_contextual_user_permission(self):
		...
		posts = mrinimitable.get_all("Blog Post", fields=["name", "blogger"])
		...
		posts = mrinimitable.get_list("Blog Post", fields=["name", "blogger"])
	...
	def test_if_owner_permission_overrides_properly(self):
		...
		mrinimitable.set_user("test2@example.com")
		self.assertEqual(mrinimitable.get_list("Blog Post"), [])
	...
	def test_if_owner_permission_on_getdoc(self):
		...
		mrinimitable.set_user("test2@example.com")
		self.assertRaises(mrinimitable.PermissionError, getdoc, "Blog Post", doc.name)
	...
	def test_if_owner_permission_on_get_list(self):
		...
		mrinimitable.set_user("test2@example.com")
		self.assertIn(doc.name, mrinimitable.get_list("Blog Post", pluck="name"))
		...
		mrinimitable.set_user("test2@example.com")
		self.assertNotIn(doc.name, mrinimitable.get_list("Blog Post", pluck="name"))
	...
	def test_if_owner_permission_on_delete(self):
		...
		with self.set_user("testperm@example.com"):
			...
			self.assertRaises(mrinimitable.PermissionError, mrinimitable.delete_doc, "Blog Post", doc.name)

		with self.set_user("test2@example.com"):
			mrinimitable.delete_doc("Blog Post", "-test-blog-post-title-new-1")
