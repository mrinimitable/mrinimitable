...
from mrinimitable.model.rename_doc import bulk_rename, update_document_title
from mrinimitable.modules.utils import get_doc_path
...
def patch_db(endpoints: list[str] | None = None):
	...
	for point in endpoints:
		x = patch(f"mrinimitable.db.{point}", new=lambda: True)
		patched_endpoints.append(x)

	savepoint = "SAVEPOINT_for_test_bulk_rename"
	mrinimitable.db.savepoint(save_point=savepoint)
	try:
...
class TestRenameDoc(IntegrationTestCase):
	...
	def setUpClass(self):
		...
		super().setUpClass()
		self._original_developer_flag = mrinimitable.conf.developer_mode
		...
		self.doctype = mrinimitable._dict(
			{
	...
	def tearDownClass(self):
		...
		mrinimitable.delete_doc_if_exists("DocType", "Renamed Doc")
		mrinimitable.db.sql_ddl("drop table if exists `tabRenamed Doc`")

		# delete the documents created
		for docname in self.available_documents:
			mrinimitable.delete_doc(self.test_doctype, docname)

		for dt in self.doctype.values():
			if mrinimitable.db.exists("DocType", dt):
				mrinimitable.delete_doc("DocType", dt)
				mrinimitable.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{dt}`")

		# reset original value of developer_mode conf
		mrinimitable.conf.developer_mode = self._original_developer_flag

	def setUp(self):
		mrinimitable.flags.link_fields = {}
		if self._testMethodName == "test_doc_rename_method":
			self.property_setter = mrinimitable.get_doc(
				{
	...
	def test_rename_doc(self):
		"""Rename an existing document via mrinimitable.rename_doc"""
		old_name = choice(self.available_documents)
		new_name = old_name + ".new"
		self.assertEqual(new_name, mrinimitable.rename_doc(self.test_doctype, old_name, new_name, force=True))
		self.available_documents.remove(old_name)
		self.available_documents.append(new_name)

	def test_merging_docs(self):
		"""Merge two documents via mrinimitable.rename_doc"""
		first_todo, second_todo = sample(self.available_documents, 2)

		second_todo_doc = mrinimitable.get_doc(self.test_doctype, second_todo)
		second_todo_doc.priority = "High"
		second_todo_doc.save()

		merged_todo = mrinimitable.rename_doc(self.test_doctype, first_todo, second_todo, merge=True, force=True)
		merged_todo_doc = mrinimitable.get_doc(self.test_doctype, merged_todo)
		self.available_documents.remove(first_todo)

		with self.assertRaises(DoesNotExistError):
			mrinimitable.get_doc(self.test_doctype, first_todo)
	...
	def test_rename_controllers(self):
		...
		self.assertNotEqual(get_controller(self.doctype.old), mrinimitable.model.document.Document)

		old_doctype_path = get_doc_path("Custom", "DocType", self.doctype.old)

		# rename doc via wrapper API accessible via /desk
		mrinimitable.rename_doc("DocType", self.doctype.old, self.doctype.new)

		# check if database and controllers are updated
		self.assertTrue(mrinimitable.db.exists("DocType", self.doctype.new))
		self.assertFalse(mrinimitable.db.exists("DocType", self.doctype.old))
		self.assertFalse(os.path.exists(old_doctype_path))

	def test_rename_doctype(self):
		"""Rename DocType via mrinimitable.rename_doc"""
		from mrinimitable.core.doctype.doctype.test_doctype import new_doctype

		if not mrinimitable.db.exists("DocType", "Rename This"):
			new_doctype(
				"Rename This",
				fields=[
					{
		...
		to_rename_record = mrinimitable.get_doc(
			{"doctype": "Rename This", "linked_to_doctype": "Rename This"}
		).insert()

		# Rename doctype
		self.assertEqual(
			"Renamed Doc", mrinimitable.rename_doc("DocType", "Rename This", "Renamed Doc", force=True)
		)

		# Test if Doctype value has changed in Link field
		linked_to_doctype = mrinimitable.db.get_value("Renamed Doc", to_rename_record.name, "linked_to_doctype")
		...
		new_name = "ToDo"
		self.assertEqual(new_name, mrinimitable.rename_doc("Renamed Doc", old_name, new_name, force=True))

	def test_update_document_title_api(self):
		test_doctype = "Module Def"
		test_doc = mrinimitable.get_doc(
			{
				"doctype": test_doctype,
				"module_name": f"Test-test_update_document_title_api-{mrinimitable.generate_hash()}",
				"custom": True,
			}
		)
		...
		doc_before = mrinimitable.get_doc(test_doctype, dn)
		return_value = update_document_title(doctype=dt, docname=dn, new_name=new_name)
		doc_after = mrinimitable.get_doc(test_doctype, return_value)
	...
	def test_bulk_rename(self):
		...
		with patch_db(["commit", "rollback"]), patch("mrinimitable.enqueue") as enqueue:
			...
			enqueue.assert_called_with(
				"mrinimitable.utils.global_search.rebuild_for_doctype",
				doctype=self.test_doctype,
			)

	def test_doc_rename_method(self):
		name = choice(self.available_documents)
		new_name = f"{name}-{mrinimitable.generate_hash(length=4)}"
		doc = mrinimitable.get_doc(self.test_doctype, name)
		doc.rename(new_name, merge=mrinimitable.db.exists(self.test_doctype, new_name))
	...
	def test_parenttype(self):
		...
		parent_a_instance = mrinimitable.get_doc(
			doctype=parent_a.name, test_table=[{"some_fieldname": "x"}], name="XYZ"
		).insert()

		parent_b_instance = mrinimitable.get_doc(
	...
	def test_rename_autoincrement_doc(self):
		if not mrinimitable.db.exists("DocType", "Autoincrement DocType"):
			new_doctype(
		...
		if not mrinimitable.db.exists("DocType", "Autoincrement Linked DocType"):
			new_doctype(
		...
		# create records
		mary = mrinimitable.new_doc("Autoincrement DocType", first_name="Mary").insert()
		marilyn = mrinimitable.new_doc("Autoincrement DocType", first_name="Marilyn").insert()
		linked_with_marilyn = mrinimitable.new_doc(
		...
		mrinimitable.rename_doc("Autoincrement DocType", marilyn.name, mary.name, merge=True)
		linked_with_marilyn.reload()

		self.assertTrue(mrinimitable.db.exists("Autoincrement DocType", marilyn.name) is None)
		self.assertEqual(linked_with_marilyn.autoincrement_doctype, mrinimitable.utils.cstr(mary.name))
