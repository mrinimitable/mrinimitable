...
import mrinimitable
from mrinimitable.modules import patch_handler
...
execute:mrinimitable.db.updatedb("Item")
execute:mrinimitable.function(arg="1")
...
class TestPatches(IntegrationTestCase):
	def test_patch_module_names(self):
		mrinimitable.flags.final_patches = []
		mrinimitable.flags.in_install = True
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(mrinimitable.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))

		mrinimitable.flags.in_install = False

	def test_get_patch_list(self):
		pre = patch_handler.get_patches_from_app("mrinimitable", patch_handler.PatchType.pre_model_sync)
		post = patch_handler.get_patches_from_app("mrinimitable", patch_handler.PatchType.post_model_sync)
		all_patches = patch_handler.get_patches_from_app("mrinimitable")
	...
	def test_all_patches_are_marked_completed(self):
		all_patches = patch_handler.get_patches_from_app("mrinimitable")
		finished_patches = mrinimitable.db.count("Patch Log")
...
class TestPatchReader(IntegrationTestCase):
	def get_patches(self):
		return (
			patch_handler.get_patches_from_app("mrinimitable"),
			patch_handler.get_patches_from_app("mrinimitable", patch_handler.PatchType.pre_model_sync),
			patch_handler.get_patches_from_app("mrinimitable", patch_handler.PatchType.post_model_sync),
		)
	...
	def test_verify_patch_txt(self):
		...
		check_patch_files("mrinimitable")
...
def check_patch_files(app):
	...
	patch_dir = Path(mrinimitable.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = mrinimitable.get_module(module)
...
def _get_dotted_path(file: Path, app) -> str:
	app_path = Path(mrinimitable.get_app_path(app))
