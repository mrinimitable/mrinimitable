...
from mrinimitable.custom.doctype.customize_form.test_customize_form import TestCustomizeForm
from mrinimitable.desk.notifications import get_open_count
...
class TestDashboardConnections(IntegrationTestCase):
	...
	def test_external_doctype_link_with_dashboard_override(self):
		...
		# create a test doc
		todo_doc = mrinimitable.get_doc(doctype="ToDo", description="test").insert()
		mrinimitable.get_doc(doctype="Test Doctype D", title="d-001", doclink=todo_doc.name).insert()
		mrinimitable.get_doc(doctype="Test Doctype E", title="e-001", todo=todo_doc.name).insert()
		...
		with self.patch_hooks(
			{
				"override_doctype_dashboards": {
					"ToDo": ["mrinimitable.tests.test_dashboard_connections.get_dashboard_for_todo"]
				}
			}
		):
			connections = get_open_count("ToDo", todo_doc.name)["count"]
...
def delete_test_data():
	...
	for doctype in doctypes:
		if mrinimitable.db.table_exists(doctype):
			mrinimitable.db.delete(doctype)
...
def add_links_in_child_tables():
	test_child_table_with_link_to_doctype_a = mrinimitable.get_doc(
	...
	test_child_table_with_link_to_doctype_b = mrinimitable.get_doc(
