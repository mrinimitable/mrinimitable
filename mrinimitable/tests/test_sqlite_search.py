...
import mrinimitable
from mrinimitable.search.sqlite_search import SQLiteSearch, SQLiteSearchIndexMissingError
...
class TestSQLiteSearch(SQLiteSearch):
	...
	def get_search_filters(self):
		...
		return {"owner": mrinimitable.session.user}
...
class TestSQLiteSearchAPI(IntegrationTestCase):
	...
	def setUp(self):
		...
		for data in note_data:
			note = mrinimitable.get_doc({"doctype": "Note", "title": data["title"], "content": data["content"]})
			note.insert()
		...
		for data in todo_data:
			todo = mrinimitable.get_doc({"doctype": "ToDo", "description": data["description"], "status": "Open"})
			todo.insert()
	...
	def test_search_filtering_and_permissions(self):
		...
		original_user = mrinimitable.session.user
		try:
			# Create a test user and switch to them
			test_user_email = "test_search_user@example.com"
			if not mrinimitable.db.exists("User", test_user_email):
				test_user = mrinimitable.get_doc(
					{
						"doctype": "User",
			...
			mrinimitable.set_user(original_user)
	...
	def test_content_processing_and_html_handling(self):
		...
		html_note = mrinimitable.get_doc(
			{
				"doctype": "Note",
	...
	@patch("mrinimitable.enqueue")
	def test_background_operations(self, mock_enqueue):
		"""Test background job integration and module-level functions."""
		from mrinimitable.search.sqlite_search import (
			build_index_in_background,
			get_search_classes,
		)

		# Test getting search classes
		with patch("mrinimitable.get_hooks") as mock_get_hooks:
			mock_get_hooks.return_value = ["mrinimitable.tests.test_sqlite_search.TestSQLiteSearch"]
