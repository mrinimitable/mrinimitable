...
from mrinimitable.permissions import add_permission, update_permission_property
from mrinimitable.query_builder import Field
from mrinimitable.query_builder.functions import Abs, Count, Ifnull, Max, Now, Timestamp
from mrinimitable.tests import IntegrationTestCase
from mrinimitable.tests.classes.context_managers import enable_safe_exec
from mrinimitable.tests.test_db_query import (
...
from mrinimitable.utils.nestedset import get_ancestors_of, get_descendants_of
...
def create_tree_docs():
	...
	for record in records:
		d = mrinimitable.new_doc("Test Tree DocType")
		d.update(record)
		d.insert()
...
class TestQuery(IntegrationTestCase):
	...
	def test_string_fields(self):
		self.assertEqual(
			mrinimitable.qb.get_query("User", fields="name, email", filters={"name": "Administrator"}).get_sql(),
		...
		self.assertEqual(
			mrinimitable.qb.get_query("User", fields=[Count("*")]).get_sql(),
			mrinimitable.qb.from_("User").select(Count("*")).get_sql(),
		)
	...
	def test_qb_fields(self):
		...
		self.assertEqual(
			...
			mrinimitable.qb.from_(user_doctype).select(user_doctype.name, user_doctype.email).get_sql(),
		)

		self.assertEqual(
			mrinimitable.qb.get_query(user_doctype, fields=user_doctype.email, filters={}).get_sql(),
			mrinimitable.qb.from_(user_doctype).select(user_doctype.email).get_sql(),
		)
	...
	def test_field_validation_select(self):
		...
		for field in valid_fields:
			try:
				mrinimitable.qb.get_query("User", fields=field).get_sql()
		...
		for field in invalid_fields:
			with self.assertRaises(
				(mrinimitable.PermissionError, mrinimitable.ValidationError),
	...
	def test_field_validation_filters(self):
		...
		for field in valid_fields:
			try:
				# Test in dict filter
				mrinimitable.qb.get_query("User", filters={field: "value"}).get_sql()
				# Test in list filter
				mrinimitable.qb.get_query("User", filters=[[field, "=", "value"]]).get_sql()
				# Test in list filter with doctype
				mrinimitable.qb.get_query("User", filters=[["User", field, "=", "value"]]).get_sql()
			except Exception as e:
				self.fail(f"Valid filter field '{field}' failed validation: {e}")

		for field in invalid_fields:
			with self.assertRaises(
				mrinimitable.ValidationError, msg=f"Invalid filter field '{{{field}: val}}' passed validation"
			):
				mrinimitable.qb.get_query("User", filters={field: "value"}).get_sql()
	...
	def test_field_validation_group_by(self):
		...
		for group_by_str in valid_fields:
			try:
				mrinimitable.qb.get_query("User", group_by=group_by_str).get_sql()
			except Exception as e:
	...
	def test_field_validation_order_by(self):
		...
		for order_by_str in valid_fields:
			try:
				mrinimitable.qb.get_query("User", order_by=order_by_str).get_sql()
			except Exception as e:
				self.fail(f"Valid ORDER BY string '{order_by_str}' failed validation: {e}")

		for order_by_str in invalid_fields:
			with self.assertRaises(
				(mrinimitable.PermissionError, ValueError, mrinimitable.ValidationError),
	...
	def test_aliasing(self):
		user_doctype = mrinimitable.qb.DocType("User")
		self.assertEqual(
			mrinimitable.qb.get_query("User", fields=["name as owner", "email as id"], filters={}).get_sql(),
		...
		self.assertEqual(
			mrinimitable.qb.get_query(user_doctype, fields="name as owner, email as id", filters={}).get_sql(),
			mrinimitable.qb.from_(user_doctype)
			.select(user_doctype.name.as_("owner"), user_doctype.email.as_("id"))
			.get_sql(),
		)
	...
	def test_filters(self):
		self.assertEqual(
			mrinimitable.qb.get_query(
				"DocType",
				fields=["name"],
				filters={"module.app_name": "mrinimitable"},
			).get_sql(),
			"SELECT `tabDocType`.`name` FROM `tabDocType` LEFT JOIN `tabModule Def` ON `tabModule Def`.`name`=`tabDocType`.`module` WHERE `tabModule Def`.`app_name`='mrinimitable'".replace(
				"`", '"' if mrinimitable.db.db_type == "postgres" else "`"
			),
		)
		...
		self.assertEqual(
			...
			"SELECT `name` FROM `tabDocType`".replace("`", '"' if mrinimitable.db.db_type == "postgres" else "`"),
		)
	...
	def test_nested_filters(self):
		...
		self.assertEqual(mrinimitable.qb.get_query("User", filters=filters_and).get_sql(), expected_sql_and)
		...
		self.assertEqual(mrinimitable.qb.get_query("User", filters=filters_or).get_sql(), expected_sql_or)
		...
		self.assertEqual(mrinimitable.qb.get_query("User", filters=filters_mixed).get_sql(), expected_sql_mixed)
		...
		self.assertEqual(mrinimitable.qb.get_query("User", filters=filters_nested).get_sql(), expected_sql_nested)
		...
		self.assertEqual(
			mrinimitable.qb.get_query("User", filters=filters_single_group).get_sql(), expected_sql_single_group
		)
		...
		self.assertEqual(mrinimitable.qb.get_query("User", filters=filters_complex).get_sql(), expected_sql_complex)

	def test_invalid_nested_filters(self):
		"""Test invalid formats for nested filters."""
		# Invalid operator
		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", filters=[["email", "=", "a"], "xor", ["email", "=", "b"]]).get_sql()
		self.assertIn("Expected 'and' or 'or' operator", str(cm.exception))

		# Missing condition after operator
		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", filters=[["email", "=", "a"], "and"]).get_sql()
		self.assertIn("Filter condition missing after operator", str(cm.exception))

		# Starting with operator
		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", filters=["and", ["email", "=", "a"]]).get_sql()
		self.assertIn("Invalid start for filter condition", str(cm.exception))

		# Invalid condition type (string instead of list/tuple)
		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", filters=[["email", "=", "a"], "and", "enabled = 1"]).get_sql()
		...
		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", filters=["email", "and", ["enabled", "=", 1]]).get_sql()
		self.assertIn("Invalid start for filter condition", str(cm.exception))
	...
	# 	self.assertNotIn(
	# 		"email", mrinimitable.qb.get_query("User", fields=["name", "#email"], filters={}).get_sql()
	# 	)

	def test_nestedset(self):
		mrinimitable.db.sql("delete from `tabDocType` where `name` = 'Test Tree DocType'")
		mrinimitable.db.sql_ddl("drop table if exists `tabTest Tree DocType`")
		create_tree_docs()
		descendants_result = mrinimitable.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		ancestors_result = mrinimitable.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		not_descendants_result = mrinimitable.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		not_ancestors_result = mrinimitable.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
	...
	def test_child_field_syntax(self):
		note1 = mrinimitable.get_doc(doctype="Note", title="Note 1", seen_by=[{"user": "Administrator"}]).insert()
		note2 = mrinimitable.get_doc(
			doctype="Note", title="Note 2", seen_by=[{"user": "Administrator"}, {"user": "Guest"}]
		).insert()

		result = mrinimitable.qb.get_query(
			"Note",
	...
	def test_build_match_conditions(self):
		from mrinimitable.permissions import add_user_permission, clear_user_permissions_for_doctype

		clear_user_permissions_for_doctype("Test Blog Post", "test2@example.com")

		test2user = mrinimitable.get_doc("User", "test2@example.com")
		...
		query = mrinimitable.qb.get_query("Test Blog Post", ignore_permissions=False)
		...
		query = str(mrinimitable.qb.get_query("Test Blog Post", ignore_permissions=False))
	...
	def test_ignore_permissions_for_query(self):
		...
		with self.assertRaises(mrinimitable.PermissionError):
			mrinimitable.qb.get_query("DocType", filters={"istable": 1}, ignore_permissions=False)

		result = mrinimitable.qb.get_query("DocType", filters={"istable": 1}, ignore_permissions=True).run()
	...
	def test_permlevel_fields(self):
		...
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			test_post = mrinimitable.get_doc(
				{
			...
			data = mrinimitable.qb.get_query(
				"Test Blog Post",
	...
	def test_child_table_access_with_select_permission(self):
		...
		# Cleanup
		mrinimitable.set_user("Administrator")
		test_user = mrinimitable.get_doc("User", test_user_email)
		test_user.remove_roles(test_role)
		mrinimitable.delete_doc("Role", test_role, ignore_missing=True, force=True)
		mrinimitable.delete_doc("Note", {"title": test_note_title}, ignore_missing=True, force=True)

		# Setup Role with 'select' on Note and 'read' on Note Seen By
		mrinimitable.get_doc({"doctype": "Role", "role_name": test_role}).insert(ignore_if_duplicate=True)
		...
		mrinimitable.set_user(test_user_email)
		...
		test_user.remove_roles(test_role)
		mrinimitable.delete_doc("Role", test_role, force=True)
	...
	def test_nested_permission(self):
		...
		from mrinimitable.core.page.permission_manager.permission_manager import update
		...
		with self.set_user("test2@example.com"):
			data = mrinimitable.qb.get_query("Nested DocType", ignore_permissions=False).run(as_dict=1)
	...
	def test_is_set_is_not_set(self):
		...
		result = mrinimitable.qb.get_query("DocType", filters={"autoname": ["is", "not set"]}).run(as_dict=1)
		...
		result = mrinimitable.qb.get_query("DocType", filters={"autoname": ["is", "set"]}).run(as_dict=1)
		...
		mrinimitable.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_permission_query_condition(self):
		...
		from mrinimitable.desk.doctype.dashboard_settings.dashboard_settings import create_dashboard_settings

		# Create a Dashboard Settings for test user
		self.doctype = "Dashboard Settings"
		self.user = "test@example.com"

		original_hooks = mrinimitable.get_hooks("permission_query_conditions") or {}
		...
		query = mrinimitable.qb.get_query("Dashboard Settings", user=self.user, ignore_permissions=False)
		self.assertIn("`tabDashboard Settings`.name = ", str(query))

		# Create a server script for permission query
		script = mrinimitable.new_doc(
		...
		mrinimitable.hooks.permission_query_conditions = {}  # Clear hooks to test server script alone
		...
		script.delete()
		mrinimitable.clear_cache()
		mrinimitable.hooks.permission_query_conditions = original_hooks
	...
	def test_link_field_target_permission(self):
		...
		mrinimitable.set_user("Administrator")
		mrinimitable.delete_doc("DocType", target_dt_name, ignore_missing=True, force=True)
		mrinimitable.delete_doc("DocType", source_dt_name, ignore_missing=True, force=True)
		mrinimitable.delete_doc("Role", test_role, ignore_missing=True, force=True)
		test_user_doc = mrinimitable.get_doc("User", test_user)
		...
		# Create Records
		target_doc = mrinimitable.get_doc(
			doctype=target_dt_name, target_field="Secret Data", other_target_field="Public Data"
		).insert(ignore_permissions=True)
		source_doc = mrinimitable.get_doc(doctype=source_dt_name, link_field=target_doc.name).insert(
		...
		result_restricted = mrinimitable.qb.get_query(
			source_dt_name,
		...
		mrinimitable.set_user("Administrator")
		result_admin = mrinimitable.qb.get_query(
			source_dt_name,
			filters={"name": source_doc.name},
			fields=[
				"name",
	...
	def test_filter_linked_field_permission(self):
		...
		with setup_test_user(set_user=True) as user:
			...
			mrinimitable.delete_doc("DocType", source_dt_name, ignore_missing=True, force=True)
			mrinimitable.delete_doc("Role", test_role, ignore_missing=True, force=True)
			test_user_doc = mrinimitable.get_doc("User", user.name)
			test_user_doc.remove_roles(test_role)
			...
			with self.assertRaises(mrinimitable.PermissionError) as cm_link:
				mrinimitable.qb.get_query(
					source_dt_name,
	...
	def test_child_table_group_by_order_by_permissions(self):
		...
		mrinimitable.set_user("Administrator")
		mrinimitable.delete_doc("DocType", child_dt_name, ignore_missing=True, force=True)
		mrinimitable.delete_doc("DocType", parent_dt_name, ignore_missing=True, force=True)
		mrinimitable.delete_doc("Role", test_role, ignore_missing=True, force=True)

		test_user_doc = mrinimitable.get_doc("User", test_user_email)
		test_user_doc.remove_roles(test_role)
	...
	def test_group_by_order_by_validation_errors(self):
		...
		for field in invalid_group_by_fields:
			with self.assertRaises(
				mrinimitable.ValidationError, msg=f"Invalid GROUP BY field '{field}' passed validation"
			):
				mrinimitable.qb.get_query("User", group_by=field).get_sql()
		...
		for field in invalid_order_by_fields:
			with self.assertRaises(
				(mrinimitable.ValidationError, ValueError),
				msg=f"Invalid ORDER BY field '{field}' passed validation",
			):
				mrinimitable.qb.get_query("User", order_by=field).get_sql()

	def test_backtick_rejection_group_order(self):
		"""Test that backticks are properly rejected in GROUP BY and ORDER BY."""
		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", group_by="`name`").get_sql()
		self.assertIn("cannot contain backticks", str(cm.exception))

		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", order_by="`name` ASC").get_sql()
		self.assertIn("cannot contain backticks", str(cm.exception))

		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", group_by="`name`, `email`").get_sql()
		self.assertIn("cannot contain backticks", str(cm.exception))
	...
	def test_sql_functions_in_fields(self):
		...
		query = mrinimitable.qb.get_query("User", fields=["user_type", {"COUNT": "name"}], group_by="user_type")
		sql = query.get_sql()
		...
		# Test ABS function
		query = mrinimitable.qb.get_query("User", fields=[{"ABS": "enabled", "as": "abs_enabled"}])
		sql = query.get_sql()
		...
		query = mrinimitable.qb.get_query("User", fields=[{"TIMESTAMP": "creation", "as": "ts"}])
		sql = query.get_sql()
		...
		query = mrinimitable.qb.get_query("User", fields=[{"NOW": None, "as": "current_time"}])
		sql = query.get_sql()
		...
		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", fields=[{"UNSUPPORTED_FUNC": "name"}]).get_sql()
		...
		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", fields=[{"UPPER": ["first_name"]}]).get_sql()
		self.assertIn("Unsupported function or invalid field name: UPPER", str(cm.exception))

		# Test SQL injection attempt
		with self.assertRaises(mrinimitable.ValidationError) as cm:
			mrinimitable.qb.get_query("User", fields=[{"DROP": "TABLE users"}]).get_sql()
