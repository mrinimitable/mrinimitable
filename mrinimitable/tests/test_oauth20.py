...
import mrinimitable
from mrinimitable.integrations.oauth2 import encode_params
from mrinimitable.tests import IntegrationTestCase
from mrinimitable.tests.test_api import get_test_client, make_request, suppress_stdout
from mrinimitable.tests.utils import make_test_records

if TYPE_CHECKING:
	from mrinimitable.integrations.doctype.social_login_key.social_login_key import SocialLoginKey


class MrinimitableRequestTestCase(IntegrationTestCase):
	@property
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			mrinimitable.local.login_manager.login_as("test@example.com")
			self._sid = mrinimitable.session.sid
...
class TestOAuth20(MrinimitableRequestTestCase):
	...
	def setUpClass(cls):
		...
		# Set Mrinimitable server URL reqired for id_token generation
		mrinimitable_login_key: SocialLoginKey = mrinimitable.new_doc("Social Login Key")
		mrinimitable_login_key.get_social_login_provider("Mrinimitable", initialize=True)
		mrinimitable_login_key.base_url = mrinimitable.utils.get_url()
		mrinimitable_login_key.enable_social_login = 0
		mrinimitable_login_key.insert(ignore_if_duplicate=True)
		mrinimitable.db.commit()

	def setUp(self):
		self.TEST_CLIENT = get_test_client()
		self.oauth_client = mrinimitable.new_doc("OAuth Client")
	...
	def test_login_using_authorization_code(self):
		...
		resp = self.get(
			"/api/method/mrinimitable.integrations.oauth2.authorize",
			{
		...
		token_response = self.post(
			"/api/method/mrinimitable.integrations.oauth2.get_token",
	...
	def test_revoke_token(self):
		client = mrinimitable.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = self.post(
			"/api/method/mrinimitable.integrations.oauth2.revoke_token",
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = mrinimitable.get_doc("OAuth Client", self.client_id)
		...
		try:
			session.get(
				get_full_url("/api/method/mrinimitable.integrations.oauth2.authorize"),
	...
	def test_openid_code_id_token(self):
		update_client_for_auth_code_grant(self.client_id)
		nonce = mrinimitable.generate_hash()
...
def check_valid_openid_response(access_token=None, client: "MrinimitableRequestTestCase" = None):
	"""Return True for valid response."""
	# Use token in header
	headers = {}
	URL = "/api/method/mrinimitable.integrations.oauth2.openid_profile"
...
def get_full_url(endpoint):
	...
	return urljoin(mrinimitable.utils.get_url(), endpoint)
