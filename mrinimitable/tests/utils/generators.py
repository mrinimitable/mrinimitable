...
from mrinimitable.modules import get_doctype_module, get_module_path, load_doctype_module

if TYPE_CHECKING:
	from mrinimitable.model.document import Document

logger = logging.getLogger(__name__)
testing_logger = logging.getLogger("mrinimitable.testing.generators")
...
def get_modules(doctype) -> tuple[str, ModuleType]:
	...
	module = mrinimitable.db.get_value("DocType", doctype, "module")
	try:
...
def get_missing_records_module_overrides(module) -> [list, list]:
	...
	if hasattr(module, "test_dependencies"):
		...
		deprecation_warning(
			"2024-10-09",
			"v17",
			"""test_dependencies was clarified to EXTRA_TEST_RECORD_DEPENDENCIES; migration script: https://github.com/mrinimitable/mrinimitable/pull/28060""",
		)
	...
	if hasattr(module, "test_ignore"):
		...
		deprecation_warning(
			"2024-10-09",
			"v17",
			"""test_ignore was clarified to IGNORE_TEST_RECORD_DEPENDENCIES; migration script: https://github.com/mrinimitable/mrinimitable/pull/28060""",
		)
...
def load_test_records_for(index_doctype) -> dict[str, Any]:
	module_path = get_module_path(get_doctype_module(index_doctype), "doctype", mrinimitable.scrub(index_doctype))
...
def _generate_all_records_towards(
	...
	visited = set(mrinimitable.local.test_objects.keys())
...
def _generate_records_for(
	...
	if hasattr(test_module, "_make_test_records"):
		...
		if not test_records:
			...
			mrinimitable.local.test_objects[index_doctype] = []  # avoid noisy retries on multiple invocations
...
def _sync_records(
	...
	def _load(do_create=True):
		...
		for _sub_doctype, records in test_records.items():
			for record in records:
				...
				mrinimitable.local.test_objects[index_doctype].append(MappingProxyType(record))
	...
	if index_doctype in test_record_manager_instance.get():
		...
		if reset:
			...
			mrinimitable.local.test_objects[index_doctype] = []
			...
			if index_doctype not in mrinimitable.local.test_objects:
...
def _try_create(record, reset=False, commit=False) -> tuple["Document", bool]:
	...
	if not reset:
		mrinimitable.db.savepoint("creating_test_record")

	d = mrinimitable.copy_doc(record)
	...
	if mrinimitable.db.exists(d.doctype, d.name) and not reset:
		mrinimitable.db.rollback(save_point="creating_test_record")
		# do not create test records, if already exists
		return mrinimitable.get_doc(d.doctype, d.name), False
...
class TestRecordManager:
	def __init__(self):
		...
		self.log_file = Path(mrinimitable.get_site_path(PERSISTENT_TEST_LOG_FILE))
		self._log = None
	...
	def _append_to_log(self, index_doctype, records: list["Document"]):
		...
		with self.log_file.open("a") as f:
			f.write(mrinimitable.as_json(entry, indent=None) + "\n")
		...
		if self.log_file.exists():
			with self.log_file.open() as f:
				for line in f:
					...
					try:
						for r in records:
							log.setdefault(index_doctype, []).append(mrinimitable.get_doc(r["doctype"], r["name"]))
					except mrinimitable.DoesNotExistError as e:
						raise ValueError(
...
def _clear_test_log():
	log_file_path = mrinimitable.get_site_path(PERSISTENT_TEST_LOG_FILE)
