...
from mrinimitable.utils import global_search, now_datetime
...
class TestGlobalSearch(IntegrationTestCase):
	def setUp(self):
		...
		self.assertTrue("__global_search" in mrinimitable.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		mrinimitable.db.delete("Property Setter", {"doc_type": "Event"})
	...
	def test_update_doc(self):
		...
		event = mrinimitable.get_doc("Event", mrinimitable.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		self.insert_test_events()
		event_name = mrinimitable.get_all("Event")[0].name
		event = mrinimitable.get_doc("Event", event_name)
		...
		mrinimitable.delete_doc("Event", event_name)
		global_search.sync_global_search()
		mrinimitable.db.commit()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = mrinimitable.get_doc({"doctype": "Event", "subject": text, "starts_on": now_datetime()})
			doc.insert()
