...
if TYPE_CHECKING:
	from mrinimitable.model import Document
...
def freeze_time(time_to_freeze: Any, is_utc: bool = False, *args: Any, **kwargs: Any) -> None:
	...
	from mrinimitable.utils.data import get_datetime, get_system_timezone
...
def patch_hooks(overridden_hooks: dict) -> None:
	...
	get_hooks = mrinimitable.get_hooks
	...
	with patch.object(mrinimitable, "get_hooks", patched_hooks):
		yield
...
def change_settings(doctype, settings_dict=None, /, commit=False, **settings) -> None:
	...
	settings = mrinimitable.get_doc(doctype)
...
def enable_safe_exec() -> None:
	...
	conf = os.path.join(mrinimitable.local.sites_path, "common_site_config.json")
...
def trace_fields(
	...
	**field_configs: dict[str, list | Callable | None],
	...
	Yields:
	    Document class
	"""
	from mrinimitable.model.trace import traced_field
...
# NOTE: declare those who should also be made available directly mrinimitable.tests.* namespace
