...
import mrinimitable
from mrinimitable.installer import parse_app_name
from mrinimitable.model.document import Document
from mrinimitable.tests import IntegrationTestCase, MockedRequestTestCase
...
from mrinimitable.utils.change_log import (
	get_source_url,
	parse_github_url,
)
from mrinimitable.utils.data import (
	add_to_date,
	add_trackers_to_url,
	add_years,
	cast,
...
from mrinimitable.utils.dateutils import get_dates_from_timegrain
from mrinimitable.utils.diff import _get_value_from_version, get_version_diff, version_query
from mrinimitable.utils.identicon import Identicon
from mrinimitable.utils.image import optimize_image, strip_exif_data
from mrinimitable.utils.make_random import can_make, get_random, how_many
from mrinimitable.utils.response import json_handler
from mrinimitable.utils.synchronization import LockTimeoutError, filelock
	...
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/mrinimitable/test.jpg">
			</div>
			<div style="background-image: url('/assets/mrinimitable/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/mrinimitable/test.jpg">' in html)
		self.assertTrue(f"style=\"background-image: url('{url}/assets/mrinimitable/bg.jpg') !important\"" in html)
...
class TestHTMLUtils(IntegrationTestCase):
	def test_clean_email_html(self):
		from mrinimitable.utils.html_utils import clean_email_html
...
class TestValidationUtils(IntegrationTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://mrinimitable.io", throw=True))
		...
		self.assertRaises(mrinimitable.ValidationError, validate_url, "mrinimitable", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://mrinimitable.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(validate_url("bolo://mrinimitable.io", valid_schemes=("http", "https", "ftp", "ftps")))
		self.assertRaises(
			mrinimitable.ValidationError,
			validate_url,
			"gopher://mrinimitable.io",
			valid_schemes="https",
			throw=True,
		)
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@mrinimitable.com"))
		self.assertTrue(validate_email_address("someone@mrinimitable.com, anyone@mrinimitable.io"))
		self.assertTrue(validate_email_address("test%201@mrinimitable.com"))

		# Invalid address
		self.assertFalse(validate_email_address("someone"))
		self.assertFalse(validate_email_address("someone@----.com"))
		self.assertFalse(validate_email_address("test 1@mrinimitable.com"))
		self.assertFalse(validate_email_address("test@example.com test2@example.com,undisclosed-recipient"))

		# Invalid with throw
		self.assertRaises(
			mrinimitable.InvalidEmailAddressError,
			validate_email_address,
			"someone.com",
			throw=True,
		)

		self.assertEqual(validate_email_address("Some%20One@mrinimitable.com"), "Some%20One@mrinimitable.com")
		self.assertEqual(
			validate_email_address("erp+Job%20Applicant=JA00004@mrinimitable.com"),
			"erp+Job%20Applicant=JA00004@mrinimitable.com",
		)
	...
	def test_valid_phone(self):
		...
		self.assertRaises(
			mrinimitable.InvalidPhoneNumberError,
			validate_phone_number_with_country_code,
			"+420 1234567890",
			"field",
		)
	...
	def test_validate_name(self):
		...
		for name in invalid_names:
			self.assertRaises(mrinimitable.InvalidNameError, validate_name, name, True)


class TestImage(IntegrationTestCase):
	def test_strip_exif_data(self):
		original_image = Image.open(mrinimitable.get_app_path("mrinimitable", "tests", "data", "exif_sample_image.jpg"))
		original_image_content = open(
			mrinimitable.get_app_path("mrinimitable", "tests", "data", "exif_sample_image.jpg"),
			mode="rb",
		).read()
	...
	def test_optimize_image(self):
		image_file_path = mrinimitable.get_app_path("mrinimitable", "tests", "data", "sample_image_for_optimization.jpg")
...
class TestPythonExpressions(IntegrationTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(mrinimitable.ValidationError, validate_python_code, expr)
...
class TestDiffUtils(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = mrinimitable.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(IntegrationTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(mrinimitable.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				mrinimitable.utils.get_first_day_of_week("2020-12-25"),
				mrinimitable.utils.getdate("2020-12-21"),
			)
			self.assertEqual(
				mrinimitable.utils.get_first_day_of_week("2020-12-20"),
				mrinimitable.utils.getdate("2020-12-14"),
			)

		# Sunday as start of the week
		self.assertEqual(
			mrinimitable.utils.get_first_day_of_week("2020-12-25"),
			mrinimitable.utils.getdate("2020-12-20"),
		)
		self.assertEqual(
			mrinimitable.utils.get_first_day_of_week("2020-12-21"),
			mrinimitable.utils.getdate("2020-12-20"),
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			mrinimitable.utils.get_last_day_of_week("2020-12-24"),
			mrinimitable.utils.getdate("2020-12-26"),
		)
		self.assertEqual(
			mrinimitable.utils.get_last_day_of_week("2020-12-28"),
			mrinimitable.utils.getdate("2021-01-02"),
		)

	def test_is_last_day_of_the_month(self):
		self.assertEqual(mrinimitable.utils.is_last_day_of_the_month("2020-12-24"), False)
		self.assertEqual(mrinimitable.utils.is_last_day_of_the_month("2020-12-31"), True)
...
class TestResponse(IntegrationTestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				mrinimitable.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
...
class TestXlsxUtils(IntegrationTestCase):
	def test_unescape(self):
		from mrinimitable.utils.xlsxutils import handle_html
...
class TestLinkTitle(IntegrationTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		"""
		custom_doctype = mrinimitable.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		prop_setter = mrinimitable.get_doc(
			{
		...
		from mrinimitable.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = mrinimitable.local.response["_link_titles"]
...
class TestAppParser(MockedRequestTestCase):
	def test_app_name_parser(self):
		self.responses.add(
			"HEAD",
			"https://api.github.com/repos/mrinimitable/healthcare",
			status=200,
			json={},
		)
		shashi_path = get_shashi_path()
		mrinimitable_app = os.path.join(shashi_path, "apps", "mrinimitable")
		self.assertEqual("mrinimitable", parse_app_name(mrinimitable_app))
		...
		self.assertEqual("healthcare", parse_app_name("git@github.com:mrinimitable/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("mrinimitable/healthcare@develop"))
...
class TestIntrospectionMagic(IntegrationTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(mrinimitable.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(mrinimitable.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(mrinimitable.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(mrinimitable.get_newargs(lambda: None, args), {})
...
class TestLazyLoader(IntegrationTestCase):
	def test_lazy_import_module(self):
		from mrinimitable.utils.lazy_loader import lazy_import

		with Capturing() as output:
			ls = lazy_import("mrinimitable.tests.data.load_sleep")
		self.assertEqual(output, [])

		with Capturing() as output:
			ls.time
		self.assertEqual(["Module `mrinimitable.tests.data.load_sleep` loaded"], output)


class TestIdenticon(IntegrationTestCase):
	def test_get_gravatar(self):
		# developers@mrinimitable.io has a gravatar linked so str URL will be returned
		toggle_test_mode(False)
		gravatar_url = get_gravatar("developers@mrinimitable.io")
		...
		gravatar_url = get_gravatar(f"developers{random_string(6)}@mrinimitable.io")
		self.assertIsInstance(gravatar_url, str)
...
class TestMiscUtils(IntegrationTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(mrinimitable.local.site, get_sites())

	def test_get_site_info(self):
		info = get_site_info()

		installed_apps = [app["app_name"] for app in info["installed_apps"]]
		self.assertIn("mrinimitable", installed_apps)
...
class TestTypingValidations(IntegrationTestCase):
	...
	def test_validate_whitelisted_api(self):
		...
		for fn, args, kwargs in test_cases:
			with self.assertRaisesRegex(mrinimitable.MrinimitableTypeError, self.ERR_REGEX):
				fn(*args, **kwargs)

	def test_validate_whitelisted_doc_method(self):
		report = mrinimitable.get_last_doc("Report")
...
class TestTBSanitization(IntegrationTestCase):
	def test_traceback_sanitzation(self):
		try:
			...
			args = mrinimitable._dict({"password": "42", "pwd": "42", "safe": "safe_value"})  # noqa: F841
			raise Exception
		except Exception:
...
class TestRounding(IntegrationTestCase):
	...
	def test_default_rounding(self):
		self.assertEqual(mrinimitable.get_system_settings("rounding_method"), "Banker's Rounding")
...
class TestArgumentTypingValidations(IntegrationTestCase):
	def test_validate_argument_types(self):
		...
		from mrinimitable.utils.typing_validations import (
			MrinimitableTypeError,
		...
		self.assertEqual(test_simple_types(1, 2, "1"), (1, 2.0, True))
		with self.assertRaises(MrinimitableTypeError):
		...
		doctype = mrinimitable.get_last_doc("DocType")
...
class TestChangeLog(IntegrationTestCase):
	def test_get_remote_url(self):
		self.assertIsInstance(get_source_url("mrinimitable"), str)
	...
	def test_parse_github_url(self):
		...
		self.assertEqual(owner, "mrinimitable")
		...
		owner, repo = parse_github_url("git@github.com:mrinimitable/okayblue.git")
		self.assertEqual(owner, "mrinimitable")
		self.assertEqual(repo, "okayblue")
...
class TestURLTrackers(IntegrationTestCase):
	def test_add_trackers_to_url(self):
		...
		with patch("mrinimitable.db.get_value") as mock_get_value:
	...
	def test_map_trackers(self):
		...
		expected = {
			"utm_source": mrinimitable.get_doc("UTM Source", "test_source"),
			"utm_medium": mrinimitable.get_doc("UTM Medium", "test_medium"),
			"utm_campaign": mrinimitable.get_doc("UTM Campaign", "test_campaign"),
			"utm_content": "test_content",
		}
