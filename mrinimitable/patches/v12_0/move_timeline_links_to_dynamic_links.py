...
def execute():
	communications = mrinimitable.db.sql(
		"""
		SELECT
		...
		if communication.timeline_doctype and communication.timeline_name:
			...
			values.append(
				"""({}, "{}", "timeline_links", "Communication", "{}", "{}", "{}", "{}", "{}", "{}")""".format(
					counter,
					str(name),
					mrinimitable.db.escape(communication.name),
					mrinimitable.db.escape(communication.timeline_doctype),
					mrinimitable.db.escape(communication.timeline_name),
		...
		if communication.link_doctype and communication.link_name:
			...
			values.append(
				"""({}, "{}", "timeline_links", "Communication", "{}", "{}", "{}", "{}", "{}", "{}")""".format(
					...
					str(name),
					mrinimitable.db.escape(communication.name),
					mrinimitable.db.escape(communication.link_doctype),
					mrinimitable.db.escape(communication.link_name),
					communication.creation,
					communication.modified,
					communication.modified_by,
				)
