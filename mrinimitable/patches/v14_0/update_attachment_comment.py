...
def execute():
	...
	for name, content in mrinimitable.get_all(
		...
		mrinimitable.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in mrinimitable.get_all(
		...
		if content and content.startswith("Removed "):
			mrinimitable.db.set_value("Comment", name, "content", content[8:], update_modified=False)
