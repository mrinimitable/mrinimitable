import mrinimitable
from mrinimitable.desk.doctype.notification_log.notification_log import make_notification_logs


def execute():
	if mrinimitable.get_all("Email Account", {"auth_method": "OAuth", "connected_user": ["is", "set"]}, limit=1):
		return
	...
	mrinimitable.db.set_value("Email Account", {"auth_method": "OAuth"}, "awaiting_password", 1)
	...
	if sysmanagers := get_system_managers():
		make_notification_logs(
			{
				"type": "Alert",
				"subject": mrinimitable._(message),
			},
			sysmanagers,
		)
...
def get_system_managers():
	user_doctype = mrinimitable.qb.DocType("User").as_("user")
	user_role_doctype = mrinimitable.qb.DocType("Has Role").as_("user_role")
	return (
