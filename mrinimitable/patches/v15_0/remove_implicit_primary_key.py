import mrinimitable
from mrinimitable.model.naming import is_autoincremented
...
def execute():
	...
	for doctype in possible_log_types:
		if (
			mrinimitable.db.exists("DocType", doctype)
			and _is_implicit_int_pk(doctype)
			and not is_autoincremented(doctype)
		):
			if mrinimitable.db.db_type == "mariadb":
				mrinimitable.db.sql(f"ALTER TABLE `tab{doctype}` MODIFY name varchar({mrinimitable.db.VARCHAR_LEN})")
			else:
				mrinimitable.db.sql(
					f"ALTER TABLE `tab{doctype}` ALTER COLUMN name TYPE varchar({mrinimitable.db.VARCHAR_LEN}) USING name::varchar"
				)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if mrinimitable.db.db_type == "mariadb":
		...
		values = (mrinimitable.db.cur_db_name,)

	col_type = mrinimitable.db.sql(query, values)
