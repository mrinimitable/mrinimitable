...
class TestAutoAssign(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		mrinimitable.db.delete("Assignment Rule")
	...
	def test_load_balancing(self):
		...
		for user in ("test@example.com", "test1@example.com", "test2@example.com"):
			self.assertEqual(
				len(mrinimitable.get_all("ToDo", dict(allocated_to=user, reference_type=TEST_DOCTYPE))), 10
			)
		...
		for d in mrinimitable.get_all(
			...
			mrinimitable.db.delete("ToDo", {"name": d.name})
	...
	def check_assignment_rule_scheduling(self):
		...
		mrinimitable.flags.assignment_day = "Monday"
		note = _make_test_record(public=1)
		...
		mrinimitable.flags.assignment_day = "Friday"
		note = _make_test_record(public=1)
	...
	def test_assignment_rule_condition(self):
		...
		assignment_rule = mrinimitable.get_doc(
		...
		expiry_date = mrinimitable.utils.add_days(mrinimitable.utils.nowdate(), 2)
		note1 = _make_test_record(expiry_date=expiry_date)
		note2 = _make_test_record(expiry_date=expiry_date)

		note1_todo = mrinimitable.get_all(
			"ToDo", filters=dict(reference_type=TEST_DOCTYPE, reference_name=note1.name, status="Open")
		)[0]

		note1_todo_doc = mrinimitable.get_doc("ToDo", note1_todo.name)
		self.assertEqual(mrinimitable.utils.get_date_str(note1_todo_doc.date), expiry_date)

		# due date should be updated if the reference doc's date is updated.
		note1.expiry_date = mrinimitable.utils.add_days(expiry_date, 2)
		note1.save()
		note1_todo_doc.reload()
		self.assertEqual(mrinimitable.utils.get_date_str(note1_todo_doc.date), note1.expiry_date)

		# saving one note's expiry should not update other note todo's due date
		note2_todo = mrinimitable.get_all(
			"ToDo",
			filters=dict(reference_type=TEST_DOCTYPE, reference_name=note2.name, status="Open"),
			fields=["name", "date"],
		)[0]
		self.assertNotEqual(mrinimitable.utils.get_date_str(note2_todo.date), note1.expiry_date)
		self.assertEqual(mrinimitable.utils.get_date_str(note2_todo.date), expiry_date)
		assignment_rule.delete()
		mrinimitable.db.commit()  # undo changes commited by DDL
	...
	def test_submittable_assignment(self):
		...
		dt = mrinimitable.get_doc("DocType", submittable_doctype)
		dt.is_submittable = 1
		dt.save()

		# create a rule for the submittable doctype
		assignment_rule = mrinimitable.new_doc("Assignment Rule")
		...
		doc = mrinimitable.new_doc(submittable_doctype)
		doc.save()
		doc.submit()
...
def clear_assignments():
	mrinimitable.db.delete("ToDo", {"reference_type": TEST_DOCTYPE})


def get_assignment_rule(days, assign=None):
	mrinimitable.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 1")
	...
	mrinimitable.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 2")
...
def _make_test_record(
	...
	**kwargs,
	...
	doc = mrinimitable.new_doc(TEST_DOCTYPE)
