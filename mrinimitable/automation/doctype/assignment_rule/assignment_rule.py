...
from mrinimitable.cache_manager import clear_doctype_map, get_doctype_map
from mrinimitable.desk.form import assign_to
from mrinimitable.model import log_types
from mrinimitable.model.document import Document
from mrinimitable.utils.data import comma_and
...
class AssignmentRule(Document):
	...
	if TYPE_CHECKING:
		from mrinimitable.automation.doctype.assignment_rule_day.assignment_rule_day import AssignmentRuleDay
		from mrinimitable.automation.doctype.assignment_rule_user.assignment_rule_user import (
			AssignmentRuleUser,
		)
	...
	def validate_document_types(self):
		if self.document_type == "ToDo":
			mrinimitable.throw(
				_("Assignment Rule is not allowed on document type {0}").format(mrinimitable.bold(_("ToDo")))
			)
		...
		if user:
			assign_to.add(
				dict(
					...
					name=doc.get("name"),
					description=mrinimitable.render_template(self.description, doc),
	...
	def get_user_based_on_field(self, doc):
		...
		if mrinimitable.db.exists("User", val):
			return val

	def safe_eval(self, fieldname, doc):
		try:
			if self.get(fieldname):
				return mrinimitable.safe_eval(self.get(fieldname), None, doc)
		except Exception as e:
			# when assignment fails, don't block the document as it may be
			# a part of the email pulling
			mrinimitable.msgprint(mrinimitable._("Auto assignment failed: {0}").format(str(e)), indicator="orange")
	...
	def is_rule_not_applicable_today(self):
		today = mrinimitable.flags.assignment_day or mrinimitable.utils.get_weekday()
		assignment_days = self.get_assignment_days()
...
def reopen_closed_assignment(doc):
	todo_list = mrinimitable.get_all(
		"ToDo",
		filters={
	...
	for todo in todo_list:
		todo_doc = mrinimitable.get_doc("ToDo", todo)
...
def apply(doc=None, method=None, doctype=None, name=None):
	...
	assignment_rule_docs: list[AssignmentRule] = [
		mrinimitable.get_cached_doc("Assignment Rule", d.get("name")) for d in assignment_rules
	]
	...
	if assignments:
		for assignment_rule in assignment_rule_docs:
			...
			if not new_apply:
				...
				if to_close_todos:
					# close todo status
					todos_to_close = mrinimitable.get_all(
						"ToDo",
						filters={
							"reference_type": doc.doctype,
							"reference_name": str(doc.name),
					...
					for todo in todos_to_close:
						_todo = mrinimitable.get_doc("ToDo", todo)
						_todo.status = "Closed"
...
def update_due_date(doc, state=None):
	...
	for rule in assignment_rules:
		rule_doc = mrinimitable.get_cached_doc("Assignment Rule", rule.get("name"))
		...
		if field_updated:
			assignment_todos = mrinimitable.get_all(
				"ToDo",
				filters={
...
def get_assignment_rules() -> list[str]:
	return mrinimitable.get_all("Assignment Rule", filters={"disabled": 0}, pluck="document_type")
