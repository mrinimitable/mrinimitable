...
"""
Mrinimitable - Low Code Open Source Framework in Python and JS

Mrinimitable, pronounced fra-pay, is a full stack, batteries-included, web
...
Read the documentation: https://mrinimitableframework.com/docs
"""
...
from mrinimitable.utils.caching import deprecated_local_cache as local_cache
from mrinimitable.utils.caching import request_cache, site_cache
from mrinimitable.utils.data import as_unicode, bold, cint, cstr, safe_decode, safe_encode, sbool
from mrinimitable.utils.local import Local, LocalProxy, release_local
from mrinimitable.utils.translations import _, _lt, set_user_lang
...
__title__ = "Mrinimitable Framework"
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from mrinimitable.database.mariadb.database import MariaDBDatabase as PyMariaDBDatabase
	from mrinimitable.database.mariadb.mysqlclient import MariaDBDatabase
	from mrinimitable.database.postgres.database import PostgresDatabase
	from mrinimitable.database.sqlite.database import SQLiteDatabase
	from mrinimitable.model.document import Document
	from mrinimitable.query_builder.builder import MariaDB, Postgres, SQLite
	from mrinimitable.utils.redis_wrapper import ClientCache, RedisWrapper
...
# this global may be subsequently changed by mrinimitable.tests.utils.toggle_test_mode()
in_test = False
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force: bool = False) -> None:
	"""Initialize mrinimitable for the current site. Reset thread locals `mrinimitable.local`"""
	...
	local.conf = get_site_config(sites_path=sites_path, site_path=site_path, cached=bool(mrinimitable.request))
	...
	setup_module_map(include_all_apps=not (mrinimitable.request or mrinimitable.job or mrinimitable.flags.in_migrate))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: (Deprecated) If site is given, calls `mrinimitable.init`.
	:param db_name: (Deprecated) Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from mrinimitable.database import get_db
	...
	if site:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling mrinimitable.connect with the site argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke mrinimitable.init(site) prior to calling mrinimitable.connect(), if initializing the site is necessary.",
		)
		init(site)
	...
	if db_name:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling mrinimitable.connect with the db_name argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke mrinimitable.init(site) with the right config prior to calling mrinimitable.connect(), if necessary.",
		)
	...
	if mrinimitable.conf.db_type in ("mariadb", "postgres"):
...
def connect_replica() -> bool:
	...
	if hasattr(mrinimitable.local, "_recorder"):
		mrinimitable.local._recorder._patch_sql(local.db)
...
def setup_redis_cache_connection():
	"""Defines `mrinimitable.cache` as `RedisWrapper` instance"""
	from mrinimitable.utils.redis_wrapper import ClientCache, setup_cache
...
def print_sql(enable: bool = True) -> None:
	if not local.conf.allow_tests:
		mrinimitable.throw("`mrinimitable.print_sql` only works with `allow_tests` site config enabled.")
...
def get_roles(username=None) -> list[str]:
	...
	return mrinimitable.permissions.get_roles(username or local.session.user)
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from mrinimitable.utils.typing_validations import validate_argument_types
...
def is_whitelisted(method):
	from mrinimitable.utils import sanitize_html
...
def read_only():
	def innfn(fn):
		...
		def wrapper_fn(*args, **kwargs):
			# mrinimitable.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `mrinimitable.PermissionError` if the user does not have any of the permitted roles.
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	from mrinimitable.permissions import get_role_permissions
...
def has_permission(
	...
	*,
	...
	Raise `mrinimitable.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = mrinimitable.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		if doc:
			mrinimitable.permissions.check_doctype_permission(doctype, ptype)

		document_label = f"{_(doctype)} {doc if isinstance(doc, str) else doc.name}" if doc else _(doctype)
		mrinimitable.flags.error_message = _("No permission for {0}").format(document_label)
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `mrinimitable.PermissionError` if not permitted.
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `mrinimitable.client.set_value`"""
	import mrinimitable.client

	return mrinimitable.client.set_value(doctype, docname, fieldname, value)


def get_meta_module(doctype):
	import mrinimitable.modules

	return mrinimitable.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `mrinimitable.model.delete_doc.delete_doc`.
	...
	import mrinimitable.model.delete_doc

	return mrinimitable.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return mrinimitable.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `mrinimitable.model.rename_doc.rename_doc`
	"""
...
def get_all_apps(with_internal_apps=True, sites_path=None):
	...
	if "mrinimitable" in apps:
		apps.remove("mrinimitable")
	apps.insert(0, "mrinimitable")
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = mrinimitable.utils.strip(content)
...
def redirect(url):
	...
	from mrinimitable.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        mrinimitable.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import mrinimitable.desk.reportview

	return mrinimitable.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `mrinimitable.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        mrinimitable.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        mrinimitable.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import mrinimitable.model.db_query

	return mrinimitable.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `mrinimitable.model.db_query`. Will **not** check for permissions.
	Parameters are same as `mrinimitable.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        mrinimitable.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        mrinimitable.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])
	"""
...
def get_value(*args, **kwargs):
	...
	Alias for `mrinimitable.db.get_value`
...
from mrinimitable.deprecation_dumpster import mrinimitable_get_test_records as get_test_records
...
def logger(
	...
	from mrinimitable.utils.logger import get_logger
...
def get_active_domains():
	from mrinimitable.core.doctype.domain_settings.domain_settings import get_active_domains
...
def is_setup_complete():
	...
	if not mrinimitable.db.table_exists("Installed Application"):
		return setup_complete

	if all(mrinimitable.get_all("Installed Application", {"has_setup_wizard": 1}, pluck="is_setup_complete")):
		setup_complete = True
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from mrinimitable.desk.search import sanitize_searchfield
...
def override_whitelisted_method(original_method: str) -> str:
	...
	overrides = mrinimitable.get_hooks("override_whitelisted_methods", {}).get(original_method, [])
	return overrides[-1] if overrides else original_method


# Backward compatibility
from mrinimitable.utils.messages import *  # noqa: I001

import mrinimitable._optimizations
from mrinimitable.cache_manager import clear_cache, reset_metadata_version
from mrinimitable.config import get_common_site_config, get_conf, get_site_config
from mrinimitable.core.doctype.system_settings.system_settings import get_system_settings
from mrinimitable.model.document import (
	get_doc,
	get_lazy_doc,
	copy_doc,
	new_doc,
...
from mrinimitable.model.meta import get_meta
from mrinimitable.realtime import publish_progress, publish_realtime
from mrinimitable.utils import get_traceback, mock, parse_json, safe_eval, create_folder
from mrinimitable.utils.background_jobs import enqueue, enqueue_doc
from mrinimitable.utils.error import log_error
from mrinimitable.utils.formatters import format_value
from mrinimitable.utils.print_utils import get_print, attach_print
from mrinimitable.email import sendmail

# for backwards compatibility
format = format_value
delete_doc_if_exists = delete_doc

mrinimitable._optimizations.optimize_all()
mrinimitable._optimizations.register_fault_handler()
