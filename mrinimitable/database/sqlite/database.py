...
import mrinimitable
from mrinimitable.database.database import (
	TRANSACTION_DISABLED_MSG,
	Database,
	ImplicitCommitError,
)
from mrinimitable.database.sqlite.schema import SQLiteTable
...
class SQLiteDatabase(SQLiteExceptionUtil, Database):
	...
	def get_db_path(self):
		return Path(mrinimitable.get_site_path()) / "db" / f"{self.cur_db_name}.db"
	...
	def change_column_type(
		...
		if not column_exists:
			raise mrinimitable.InvalidColumnName(f"Column {column} does not exist in table {table_name}")
	...
	def rename_column(self, doctype: str, old_column_name: str, new_column_name: str):
		...
		if not column_exists:
			raise mrinimitable.InvalidColumnName(f"Column {old_column_name} does not exist in table {table_name}")
	...
	def add_index(self, doctype: str, fields: list, index_name: str | None = None):
		...
		from mrinimitable.custom.doctype.property_setter.property_setter import (
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = mrinimitable.cache.get_value("db_tables")
			to_query = not tables

		if to_query:
			tables = self.sql("SELECT name FROM sqlite_master WHERE type='table';", pluck=True)
			mrinimitable.cache.set_value("db_tables", tables)
	...
	def begin(self, *, read_only=False):
		if read_only or mrinimitable.flags.read_only:
			if self._conn:
				self._conn.close()
