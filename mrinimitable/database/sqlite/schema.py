...
class SQLiteTable(DBTable):
	def create(self):
		...
		mrinimitable.db.sql_ddl(create_table_query)

		# Create indexes separately
		for index_query in index_defs:
			mrinimitable.db.sql_ddl(index_query)
	...
	def alter(self):
		...
		existing_columns = []
		for column in mrinimitable.db.sql(f"PRAGMA table_info(`{self.table_name}`)", as_dict=1):
		...
		mrinimitable.db.sql_ddl(create_table)

		# Copy data
		existing_columns = [col.split()[0] for col in existing_columns]
		column_list = ", ".join(existing_columns)
		mrinimitable.db.sql_ddl(f"INSERT INTO `{temp_table}` SELECT {column_list} FROM `{self.table_name}`")

		# Drop old table
		mrinimitable.db.sql_ddl(f"DROP TABLE `{self.table_name}`")

		# Rename new table
		mrinimitable.db.sql_ddl(f"ALTER TABLE `{temp_table}` RENAME TO `{self.table_name}`")
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname == "UUID" and mrinimitable.db.get_column_type(self.doctype, "name") != "TEXT":
