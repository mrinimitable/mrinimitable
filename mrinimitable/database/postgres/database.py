...
from mrinimitable.database.postgres.schema import PostgresTable
from mrinimitable.database.utils import EmptyQueryValues, LazyDecode
from mrinimitable.utils import cstr, get_table_name
...
class PostgresExceptionUtil:
	...
	def is_statement_timeout(e):
		return PostgresDatabase.is_timedout(e) or isinstance(e, mrinimitable.QueryTimeoutError)
...
class PostgresDatabase(PostgresExceptionUtil, Database):
	...
	@property
	def db_schema(self):
		return mrinimitable.conf.get("db_schema", "public").replace("'", "").replace('"', "")
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		if (columns := mrinimitable.client_cache.get_value(key)) is not None:
			return columns
		...
		mrinimitable.client_cache.set_value(key, columns)
	...
	def describe(self, doctype: str) -> list | tuple:
		...
		return self.sql(
			f"SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_NAME = '{table_name}' and table_schema='{mrinimitable.conf.get('db_schema', 'public')}'"
		)
