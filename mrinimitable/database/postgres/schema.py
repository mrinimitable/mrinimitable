...
from mrinimitable.database.schema import DBTable, get_definition
...
class PostgresTable(DBTable):
	def create(self):
		varchar_len = mrinimitable.db.VARCHAR_LEN
		...
		if not self.meta.issingle and self.meta.autoname == "autoincrement":
			mrinimitable.db.create_sequence(self.doctype, check_not_exists=True)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			mrinimitable.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{mrinimitable.db.escape(col.default)}"
		...
		for col in self.change_nullability:
			...
			if col.not_nullable:
				try:
					...
					mrinimitable.qb.update(table).set(
		...
		try:
			if query:
				...
				# nosemgrep
				mrinimitable.db.sql(final_alter_query)
			if change_nullability:
				# nosemgrep
				mrinimitable.db.sql(f"ALTER TABLE `{self.table_name}` {','.join(change_nullability)}")
			if create_contraint_query:
				# nosemgrep
				mrinimitable.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				mrinimitable.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if mrinimitable.db.is_duplicate_fieldname(e):
				mrinimitable.throw(str(e))
			elif mrinimitable.db.is_duplicate_entry(e):
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname == "UUID" and mrinimitable.db.get_column_type(self.doctype, "name") != "uuid":
			if not mrinimitable.db.get_value(self.doctype, {}, order_by=None):
		...
		if autoname != "UUID" and mrinimitable.db.get_column_type(self.doctype, "name") == "uuid":
			return f"alter column `name` TYPE varchar({mrinimitable.db.VARCHAR_LEN})"
