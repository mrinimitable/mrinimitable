...
from mrinimitable.query_builder.functions import Function
...
class LazyMogrify(LazyString):
	...
	def _setup(self) -> str:
		return mrinimitable.db.mogrify(self.query, self.values)
...
def dangerously_reconnect_on_connection_abort(func):
	...
	def wrapper(*args, **kwargs):
		try:
			...
			if mrinimitable.db.is_interface_error(e) or isinstance(e, mrinimitable.db.OperationalError):
	...
	def run(self):
		...
		while self._functions:
			...
			try:
				...
				mrinimitable.log_error(title="Error executing commit_after_response callback")
...
def commit_after_response(func):
	...
	request = getattr(mrinimitable.local, "request", False)
	if not request or mrinimitable.in_test:
		func()
		return
