...
from mrinimitable.database.operator_map import NESTED_SET_OPERATORS, OPERATOR_MAP
from mrinimitable.database.utils import DefaultOrderBy, FilterValue, convert_to_value, get_doctype_name
from mrinimitable.model import get_permitted_fields
from mrinimitable.query_builder import Criterion, Field, Order, functions
from mrinimitable.query_builder.utils import PseudoColumnMapper
from mrinimitable.utils.data import MARIADB_SPECIFIC_COMMENT
...
# less restrictive version of mrinimitable.core.doctype.doctype.doctype.START_WITH_LETTERS_PATTERN
...
class Engine:
	def get_query(
		...
		*,
		...
		qb = mrinimitable.local.qb
		db_type = mrinimitable.local.db.db_type
		...
		self.user = user or mrinimitable.session.user
		...
		if limit:
			if not isinstance(limit, int) or limit < 0:
				mrinimitable.throw(_("Limit must be a non-negative integer"), TypeError)
			self.query = self.query.limit(limit)

		if offset:
			if not isinstance(offset, int) or offset < 0:
				mrinimitable.throw(_("Offset must be a non-negative integer"), TypeError)
	...
	def validate_doctype(self):
		if not TABLE_NAME_PATTERN.match(self.doctype):
			mrinimitable.throw(_("Invalid DocType: {0}").format(self.doctype))
		...
		if isinstance(filters, list | tuple):
			...
			if is_nested_structure:
				...
				try:
					...
					mrinimitable.log_error(f"Filter parsing error: {filters}", "Query Engine Error")
					mrinimitable.throw(_("Error parsing nested filters: {0}").format(e), exc=e)
	...
	def _build_criterion_for_simple_filter(
		...
		if _operator in NESTED_SET_OPERATORS:
			...
			main_meta = mrinimitable.get_meta(self.doctype)
	...
	def _parse_nested_filters(self, nested_list: list | tuple) -> "Criterion | None":
		...
		if not isinstance(nested_list, list | tuple):
			mrinimitable.throw(_("Nested filters must be provided as a list or tuple."))
		...
		while idx < len(nested_list):
			...
			if idx >= len(nested_list):
				mrinimitable.throw(_("Filter condition missing after operator: {0}").format(operator_str))
	...
	def _condition_to_criterion(self, condition: list | tuple) -> "Criterion":
		...
		if not isinstance(condition, list | tuple):
			mrinimitable.throw(_("Invalid condition type in nested filters: {0}").format(type(condition)))
		...
		if is_nested:
			...
			if len(condition) == 3 and isinstance(condition[1], str) and condition[1] in OPERATOR_MAP:
				...
				mrinimitable.throw(_("Invalid simple filter format: {0}").format(condition))
	...
	def _validate_and_prepare_filter_field(self, field: str | Field, doctype: str | None = None) -> Field:
		...
		if "." in field:
			...
			if doctype and doctype != self.doctype:
				...
				parent_meta = mrinimitable.get_meta(self.doctype)
				...
				return mrinimitable.qb.DocType(target_doctype)[target_fieldname]
		...
		if fieldname not in permitted_fields:
			mrinimitable.throw(
				_("You do not have permission to access field: {0}").format(
					mrinimitable.bold(f"{doctype}.{fieldname}")
				),
	...
	def parse_string_field(self, field: str):
		...
		if not match:
			mrinimitable.throw(_("Could not parse field: {0}").format(field))
		...
		if table_name:
			...
			if not TABLE_NAME_PATTERN.match(table_name.lstrip("tab")):
				mrinimitable.throw(_("Invalid characters in table name: {0}").format(table_name))
			table_obj = mrinimitable.qb.DocType(table_name)
	...
	def parse_fields(
		...
		if isinstance(fields, str):
			...
			mrinimitable.throw(_("Fields must be a string, list, tuple, pypika Field, or pypika Function"))
	...
	def _parse_single_field_item(
		...
		if not isinstance(field, str):
			mrinimitable.throw(_("Invalid field type: {0}").format(type(field)))
	...
	def _validate_group_by(self, group_by: str) -> list[Field]:
		...
		if not isinstance(group_by, str):
			mrinimitable.throw(_("Group By must be a string"), TypeError)
	...
	def _validate_order_by(self, order_by: str) -> list[tuple[Field | str, Order]]:
		...
		if not isinstance(order_by, str):
			mrinimitable.throw(_("Order By must be a string"), TypeError)
	...
	def check_read_permission(self):
		...
		def has_permission(ptype):
			return mrinimitable.has_permission(
				self.doctype,
				ptype,
				user=self.user,
		...
		if not has_permission("select") and not has_permission("read"):
			mrinimitable.throw(
				_("Insufficient Permission for {0}").format(mrinimitable.bold(self.doctype)), mrinimitable.PermissionError
			)
		...
		for field in self.fields:
			if isinstance(field, ChildTableField):
				...
				if field.link_fieldname in permitted_fields_set:
					...
					has_target_perm = mrinimitable.has_permission(
						target_doctype, "select", user=self.user
					) or mrinimitable.has_permission(target_doctype, "read", user=self.user)
	...
	def get_user_permission_conditions(self, role_permissions):
		...
		user_permissions = mrinimitable.permissions.get_user_permissions(self.user)
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				...
				if docs:
					...
					strict_user_permissions = mrinimitable.get_system_settings("apply_strict_user_permissions")
	...
	def get_doctype_link_fields(self):
		meta = mrinimitable.get_meta(self.doctype)
	...
	def add_permission_conditions(self):
		conditions = []
		role_permissions = mrinimitable.permissions.get_role_permissions(self.doctype, user=self.user)
		fetch_shared_docs = False
		...
		if fetch_shared_docs:
			shared_docs = mrinimitable.share.get_shared(self.doctype, self.user)
	...
	def get_permission_query_conditions(self):
		...
		conditions = []
		hooks = mrinimitable.get_hooks("permission_query_conditions", {})
		condition_methods = hooks.get(self.doctype, []) + hooks.get("*", [])

		for method in condition_methods:
			if c := mrinimitable.call(mrinimitable.get_attr(method), self.user, doctype=self.doctype):
		...
		if permission_script_name := get_server_script_map().get("permission_query", {}).get(self.doctype):
			script = mrinimitable.get_doc("Server Script", permission_script_name)
	...
	def get_permission_type(self, doctype) -> str:
		...
		if mrinimitable.only_has_select_perm(doctype, user=self.user):
			return "select"
		return "read"
...
class Permission:
	...
	def check_permissions(cls, query, **kwargs):
		...
		for dt in doctype:
			...
			if not mrinimitable.has_permission(
			...
			) and not mrinimitable.has_permission(
				dt,
				"read",
				user=kwargs.get("user"),
				parent_doctype=kwargs.get("parent_doctype"),
				...
				mrinimitable.throw(
					_("Insufficient Permission for {0}").format(mrinimitable.bold(dt)), mrinimitable.PermissionError
				)
	...
	def parse(field: str, doctype: str, allow_tab_notation: bool = True):
		if "." in field:
			...
			if child_match:
				...
				try:
					meta = mrinimitable.get_meta(doctype)  # Get meta of the *parent* doctype
				...
				if linked_field:
					...
					elif linked_field.fieldtype in mrinimitable.model.table_fields:
...
class ChildTableField(DynamicTableField):
	def __init__(
		...
		self.table = mrinimitable.qb.DocType(self.doctype)
		self.field = self.table[self.fieldname]

	def apply_select(self, query: QueryBuilder) -> QueryBuilder:
		table = mrinimitable.qb.DocType(self.doctype)
	...
	def apply_join(self, query: QueryBuilder) -> QueryBuilder:
		main_table = mrinimitable.qb.DocType(self.parent_doctype)
...
class ChildQuery:
	def __init__(
		...
		field = mrinimitable.get_meta(parent_doctype).get_field(fieldname)
		if field.fieldtype not in mrinimitable.model.table_fields:
			return
		self.fieldname = fieldname
		self.fields = fields
		self.parent_doctype = parent_doctype
	...
	def get_query(self, parent_names=None) -> QueryBuilder:
		...
		return mrinimitable.qb.get_query(
			self.doctype,
...
def get_nested_set_hierarchy_result(doctype: str, name: str, hierarchy: str) -> list[str]:
	...
	try:
		lft, rgt = mrinimitable.qb.from_(table).select("lft", "rgt").where(table.name == name).run()[0]
	except IndexError:
		lft, rgt = None, None
...
class RawCriterion(Term):
	...
	Allows using raw SQL strings in pypika queries:
		mrinimitable.qb.from_("DocType").where(RawCriterion("name like 'a%'"))
	"""
...
class SQLFunctionParser:
	...
	def parse_function(self, function_dict: dict) -> Field:
		...
		if not function_name:
			mrinimitable.throw(_("Invalid function dictionary format"), mrinimitable.ValidationError)
	...
	def _validate_string_argument(self, arg: str):
		...
		if not arg:
			mrinimitable.throw(_("Empty string arguments are not allowed"), mrinimitable.ValidationError)
	...
	def _validate_string_literal(self, literal: str):
		...
		if literal.startswith("'") and literal.endswith("'"):
			...
			mrinimitable.throw(_("Invalid string literal format: {0}").format(literal), mrinimitable.ValidationError)
	...
	def _validate_alias(self, alias: str):
		...
		if not isinstance(alias, str):
			mrinimitable.throw(_("Alias must be a string"), mrinimitable.ValidationError)

		alias = alias.strip()
		if not alias:
			mrinimitable.throw(_("Empty alias is not allowed"), mrinimitable.ValidationError)
