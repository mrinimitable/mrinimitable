...
from mrinimitable.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via mrinimitable.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = mrinimitable.as_unicode(escape_string(mrinimitable.as_unicode(s)), "utf-8").replace("`", "\\`")
