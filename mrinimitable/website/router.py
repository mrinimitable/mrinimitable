...
import mrinimitable
from mrinimitable.website.utils import extract_title, get_frontmatter
...
def get_page_info_from_web_page_with_dynamic_routes(path):
	...
	"""
	from mrinimitable.website.doctype.web_page.web_page import get_dynamic_web_pages
...
def get_page_info_from_web_form(path):
	...
	from mrinimitable.website.doctype.web_form.web_form import get_published_web_forms
	...
	for d in get_published_web_forms():
		...
		if end_point:
			if path.endswith("/list"):
				mrinimitable.form_dict.is_list = True
			elif path.endswith("/new"):
				mrinimitable.form_dict.is_new = True
			elif path.endswith("/edit"):
				mrinimitable.form_dict.is_edit = True
			else:
				mrinimitable.form_dict.is_read = True
			return d
...
def evaluate_dynamic_routes(rules, path):
	...
	if hasattr(mrinimitable.local, "request") and mrinimitable.local.request.environ:
		urls = route_map.bind_to_environ(mrinimitable.local.request.environ)
		try:
			endpoint, args = urls.match("/" + path)
			if args:
				# don't cache when there's a query string!
				mrinimitable.local.no_cache = 1
				mrinimitable.local.form_dict.update(args)
		except NotFound:
			pass
...
def get_pages(app=None):
	...
	return mrinimitable.cache.get_value("website_pages", lambda: _build(app))
...
def get_pages_from_path(start, app, app_path):
	...
	if os.path.exists(start_path):
		for basepath, folders, files in os.walk(start_path):  # noqa: B007
			for fname in files:
				fname = mrinimitable.utils.cstr(fname)
				if "." not in fname:
					continue
...
def setup_source(page_info):
	...
	if page_info.template.endswith((".md", ".html")):
		...
		if page_info.template.endswith(".md"):
			source = mrinimitable.utils.md_to_html(source)
...
def get_base_template(path=None):
	...
	"""
	if not path:
		path = mrinimitable.local.request.path

	base_template_map = mrinimitable.get_hooks("base_template_map") or {}
...
def load_properties_from_controller(page_info):
	...
	module = mrinimitable.get_module(page_info.controller)
	if not module:
		return
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes = mrinimitable.get_hooks("website_generators")
		doctypes_with_web_view = mrinimitable.get_all(
			"DocType", fields=["name", "module"], filters=dict(has_web_view=1)
		)
		module_app_map = mrinimitable.local.module_app
		doctypes += [
			d.name
			for d in doctypes_with_web_view
			if module_app_map.get(mrinimitable.scrub(d.module)) in installed_apps
		]
		return doctypes

	return mrinimitable.cache.get_value("doctypes_with_web_view", _get)


def get_start_folders():
	return mrinimitable.local.flags.web_pages_folders or ("www", "templates/pages")


def clear_routing_cache():
	from mrinimitable.website.doctype.web_form.web_form import get_published_web_forms
	from mrinimitable.website.doctype.web_page.web_page import get_dynamic_web_pages
	from mrinimitable.website.page_renderers.document_page import _find_matching_document_webview
	from mrinimitable.www.sitemap import get_public_pages_from_doctypes
	...
	mrinimitable.cache.delete_value("home_page")
