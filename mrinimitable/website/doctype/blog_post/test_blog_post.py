...
from mrinimitable.utils import random_string, set_request
from mrinimitable.website.doctype.blog_post.blog_post import get_blog_list
...
class TestBlogPost(IntegrationTestCase):
	...
	def test_generator_not_found(self):
		pages = mrinimitable.get_all("Blog Post", fields=["name", "route"], filters={"published": 0}, limit=1)

		route = f"test-route-{mrinimitable.generate_hash(length=5)}"

		mrinimitable.db.set_value("Blog Post", pages[0].name, "route", route)
	...
	def test_category_link(self):
		...
		blog_page_html = mrinimitable.safe_decode(blog_page_response.get_data())
		...
		category_page_html = mrinimitable.safe_decode(category_page_response.get_data())
		# Category page should contain the blog post title
		self.assertIn(blog.title, category_page_html)

		# Cleanup
		mrinimitable.delete_doc("Blog Post", blog.name)
		mrinimitable.delete_doc("Blog Category", blog.blog_category)
	...
	def test_blog_pagination(self):
		...
		filters = mrinimitable._dict({"blog_category": scrub(category_title)})
		...
		for blog in blogs:
			mrinimitable.delete_doc(blog.doctype, blog.name)
		mrinimitable.delete_doc("Blog Category", blogs[0].blog_category)

	def test_caching(self):
		# to enable caching
		mrinimitable.flags.force_website_cache = True
		print(mrinimitable.session.user)
	...
	def test_like_dislike(self):
		...
		mrinimitable.db.delete("Comment", {"comment_type": "Like", "reference_doctype": "Blog Post"})

		from mrinimitable.templates.includes.likes.likes import like
...
def make_test_blog(category_title="Test Blog Category"):
	...
	if not mrinimitable.db.exists("Blog Category", category_name):
		mrinimitable.get_doc(doctype="Blog Category", title=category_title).insert()
	if not mrinimitable.db.exists("Blogger", "test-blogger"):
		mrinimitable.get_doc(doctype="Blogger", short_name="test-blogger", full_name="Test Blogger").insert()
