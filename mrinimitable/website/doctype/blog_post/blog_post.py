...
from mrinimitable.website.utils import (
	clear_cache,
	find_first_image,
	get_comment_list,
...
class BlogPost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return (
				mrinimitable.db.get_value("Blog Category", self.blog_category, "route")
				+ "/"
	...
	def validate(self):
		...
		if self.featured:
			if not self.meta_image:
				mrinimitable.throw(_("A featured post must have a cover image"))
	...
	def reset_featured_for_other_blogs(self):
		all_posts = mrinimitable.get_all("Blog Post", {"featured": 1})
		for post in all_posts:
			mrinimitable.db.set_value("Blog Post", post.name, "featured", 0)
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and mrinimitable.db.get_single_value(
			"Blog Settings", "show_cta_in_blog", cache=True
		)

		if self.blogger:
			context.blogger_info = mrinimitable.get_doc("Blogger", self.blogger).as_dict()
		...
		context.category = mrinimitable.db.get_value(
		...
		context.guest_allowed = mrinimitable.db.get_single_value("Blog Settings", "allow_guest_to_comment")

	def fetch_cta(self):
		if mrinimitable.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
			blog_settings = mrinimitable.get_cached_doc("Blog Settings")
	...
	def fetch_social_links_info(self):
		if not mrinimitable.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
			return []

		url = mrinimitable.local.site + "/" + self.route
	...
	def load_likes(self, context):
		...
		context.like_count = mrinimitable.db.count("Comment", filters)

		filters["comment_email"] = user

		if user == "Guest":
			filters["ip_address"] = mrinimitable.local.request_ip

		context.like = mrinimitable.db.count("Comment", filters)
...
def get_list_context(context=None):
	...
	blog_settings = mrinimitable.get_doc("Blog Settings").as_dict(no_default_fields=True)
	list_context.update(blog_settings)

	category_name = mrinimitable.utils.escape_html(
		mrinimitable.local.form_dict.blog_category or mrinimitable.local.form_dict.category
	)
	if category_name:
		category = mrinimitable.get_doc("Blog Category", category_name)
	...
	elif mrinimitable.local.form_dict.blogger:
		blogger = mrinimitable.db.get_value("Blogger", {"name": mrinimitable.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif mrinimitable.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(mrinimitable.local.form_dict.txt))
...
def get_blog_categories():
	...
	post, category = mrinimitable.qb.DocType("Blog Post"), mrinimitable.qb.DocType("Blog Category")
	return (
		mrinimitable.qb.from_(category)
		.select(category.name, category.route, category.title)
		.where(
			(category.published == 1)
			& ExistsCriterion(
				mrinimitable.qb.from_(post)
				.select("name")
...
def clear_blog_cache():
	for blog in mrinimitable.db.get_list("Blog Post", fields=["route"], pluck="route", filters={"published": True}):
		clear_cache(blog)
...
def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
	...
	if filters and filters.get("blog_category"):
		...
		category = mrinimitable.utils.escape_html(
			mrinimitable.local.form_dict.blog_category or mrinimitable.local.form_dict.category
		)

	if filters and filters.get("blogger"):
		conditions.append("t1.blogger={}".format(mrinimitable.db.escape(filters.get("blogger"))))

	if category:
		conditions.append("t1.blog_category={}".format(mrinimitable.db.escape(category)))

	if txt:
		conditions.append(
			"(t1.content like {0} or t1.title like {0})".format(mrinimitable.db.escape("%" + txt + "%"))
		)

	if conditions:
		mrinimitable.local.no_cache = 1
	...
	posts = mrinimitable.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = mrinimitable.db.get_value(
