...
from mrinimitable.utils import cint, encode, get_request_site_address
from mrinimitable.website.utils import get_boot_data
...
class WebsiteSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from mrinimitable.website.doctype.top_bar_item.top_bar_item import TopBarItem
		from mrinimitable.website.doctype.website_route_redirect.website_route_redirect import (
	...
	def validate_google_settings(self):
		if self.enable_google_indexing and not mrinimitable.get_cached_value("Google Settings", None, "enable"):
	...
	def validate_redirects(self):
		for idx, row in enumerate(self.route_redirects):
			try:
				...
				if not mrinimitable.flags.in_migrate:
					mrinimitable.throw(_("Invalid redirect regex in row #{}: {}").format(idx, str(e)))
	...
	def get_access_token(self):
		...
		if not self.indexing_refresh_token:
			button_label = mrinimitable.bold(_("Allow API Indexing Access"))
			raise mrinimitable.ValidationError(_("Click on {0} to generate Refresh Token.").format(button_label))
...
def get_website_settings(context=None):
	...
	context = mrinimitable._dict(context or {})
	settings: WebsiteSettings = mrinimitable.client_cache.get_doc("Website Settings")
	...
	if context.disable_website_theme:
		context.theme = mrinimitable._dict()

	else:
		from mrinimitable.website.doctype.website_theme.website_theme import get_active_theme

		context.theme = get_active_theme() or mrinimitable._dict()

	if not context.get("favicon"):
		context["favicon"] = "/assets/mrinimitable/images/mrinimitable-favicon.svg"
	...
	context.read_only_mode = mrinimitable.flags.read_only
...
def get_auto_account_deletion():
	return mrinimitable.db.get_single_value("Website Settings", "auto_account_deletion")
