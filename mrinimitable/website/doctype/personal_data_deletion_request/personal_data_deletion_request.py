...
from mrinimitable.desk.doctype.notification_settings.notification_settings import is_email_notifications_enabled
from mrinimitable.model.document import Document
from mrinimitable.utils import get_datetime, get_fullname, time_diff_in_hours
...
class PersonalDataDeletionRequest(Document):
	...
	if TYPE_CHECKING:
		...
		from mrinimitable.website.doctype.personal_data_deletion_step.personal_data_deletion_step import (
	...
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = mrinimitable.get_hooks("user_data_fields")
	...
	def autoname(self):
		from mrinimitable.model.naming import set_name_from_naming_options
		...
		domain = mrinimitable.local.site.replace("_", "-")
	...
	def generate_url_for_confirmation(self):
		params = {"email": self.email, "name": self.name, "host_name": mrinimitable.local.site}
		api = mrinimitable.utils.get_url(
			"/api/method/mrinimitable.website.doctype.personal_data_deletion_request"
	...
	def disable_user(self):
		user = mrinimitable.get_doc("User", self.email)
		user.enabled = False
		user.save()
	...
	def send_verification_mail(self):
		...
		mrinimitable.sendmail(
			...
			args={
				"email": self.email,
				"name": self.name,
				"host_name": mrinimitable.utils.get_url(),
				"link": url,
			},
	...
	def notify_system_managers(self):
		...
		mrinimitable.sendmail(
			...
			args={"user": self.email, "url": mrinimitable.utils.get_url(self.get_url())},
	...
	def validate_data_anonymization(self):
		...
		if self.status != "Pending Approval":
			mrinimitable.throw(_("This request has not yet been approved by the user."))
	...
	def anonymize_data(self):
		return mrinimitable.enqueue_doc(
			self.doctype,
			self.name,
			"_anonymize_data",
			queue="long",
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = mrinimitable.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			mrinimitable.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		mrinimitable.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in mrinimitable.get_meta(doctype["doctype"]).fields:
...
def process_data_deletion_request():
	auto_account_deletion = mrinimitable.db.get_single_value("Website Settings", "auto_account_deletion")
	if auto_account_deletion < 1:
		return

	requests = mrinimitable.get_all(
		"Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="name"
	)

	for request in requests:
		doc = mrinimitable.get_doc("Personal Data Deletion Request", request)
...
def confirm_deletion(email, name, host_name):
	...
	doc = mrinimitable.get_doc("Personal Data Deletion Request", name)
	host_name = mrinimitable.utils.get_url()
