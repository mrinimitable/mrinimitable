...
import mrinimitable
from mrinimitable.website.page_renderers.base_template_page import BaseTemplatePage
from mrinimitable.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def set_template_path(self):
		...
		for app in reversed(mrinimitable.get_installed_apps()):
	...
	def get_index_path_options(search_path):
		return (
			mrinimitable.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = mrinimitable.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = mrinimitable.get_module(self.pymodule_name)
		...
		self.context.developer_mode = mrinimitable.conf.developer_mode
	...
	def render_template(self):
		if self.template_path.endswith("min.js"):
			...
			html = mrinimitable.render_template(
	...
	def get_raw_template(self):
		return mrinimitable.get_jloader().get_source(mrinimitable.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = mrinimitable.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "mrinimitable"
		self.app_path = mrinimitable.get_app_path("mrinimitable")
		self.path = path
	...
	def set_user_info(self):
		from mrinimitable.utils.user import get_fullname_and_avatar

		info = get_fullname_and_avatar(mrinimitable.session.user)
