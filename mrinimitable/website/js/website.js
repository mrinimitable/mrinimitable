...
mrinimitable.provide("website");
mrinimitable.provide("mrinimitable.awesome_bar_path");
window.cur_frm = null;
...
$.extend(mrinimitable, {
	...
	require: async function (links, callback) {
		...
		links = links.map((link) => mrinimitable.bundled_asset(link));
	...
	bundled_asset(path, is_rtl = null) {
		if (!path.startsWith("/assets") && path.includes(".bundle.")) {
			...
			path = mrinimitable.boot.assets_json[path] || path;
			return path;
		}
		return path;
	},
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (mrinimitable._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				mrinimitable._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		mrinimitable.prepare_call(opts);
		if (opts.freeze) {
			mrinimitable.freeze();
		}
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					mrinimitable.msgprint(__("Not found"));
				},
				403: function () {
					mrinimitable.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			mrinimitable.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				mrinimitable.msgprint(server_messages);
			}
		}
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return mrinimitable.call({
			type: "GET",
			method: "mrinimitable.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (mrinimitable.is_user_logged_in()) {
			...
			$(".user-image").attr("src", mrinimitable.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				mrinimitable.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				mrinimitable.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				mrinimitable.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	freeze: function (msg) {
		...
		mrinimitable.freeze_count++;
	},
	unfreeze: function () {
		if (!mrinimitable.freeze_count) return; // anything open?
		mrinimitable.freeze_count--;
		if (!mrinimitable.freeze_count) {
	...
	trigger_ready: function () {
		mrinimitable.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = mrinimitable.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		mrinimitable.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				mrinimitable.do_search(val);
				return false;
			}
		});
	},
	do_search: function (val) {
		var path =
			(mrinimitable.awesome_bar_path && mrinimitable.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		mrinimitable.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return mrinimitable.get_cookie("user_id") !== "Guest" && mrinimitable.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (mrinimitable.session.user === "Guest" && window.show_language_picker) {
			mrinimitable
				.call("mrinimitable.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = mrinimitable.get_cookie("preferred_language");
					let language_codes = [];
...
mrinimitable.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		mrinimitable.utils.debounce(() => {
			...
			mrinimitable
				.call({
					method: "mrinimitable.search.web_search",
					args: {
...
window.valid_email = function (id) {
	// copied regex from mrinimitable/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = mrinimitable.get_cookie("full_name");
	var logged_in = mrinimitable.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	mrinimitable.bind_navbar_search();

	// add apps link
	let apps = mrinimitable.boot?.apps_data?.apps;
	let obj = {
		label: __("Apps"),
		route: "/apps",
	};
	if (apps?.length) {
		...
		let is_desk_apps = mrinimitable.boot?.apps_data?.is_desk_apps;
		!is_desk_apps && mrinimitable.add_apps(obj);
	}

	// switch to app link
	if (mrinimitable.get_cookie("system_user") === "yes" && logged_in) {
		mrinimitable.add_switch_to_desk();
	}

	mrinimitable.render_user();
...
$(document).on("page-change", function () {
	...
	$.extend(mrinimitable, mrinimitable.get_cookies());
	mrinimitable.session = { user: mrinimitable.user_id };

	mrinimitable.datetime.refresh_when();
	mrinimitable.trigger_ready();
	mrinimitable.bind_filters();
	mrinimitable.highlight_code_blocks();
	mrinimitable.add_link_to_headings();
	mrinimitable.make_navbar_active();
	// scroll to hash
...
mrinimitable.ready(function () {
	mrinimitable.show_language_picker();
	mrinimitable.setup_videos();
	mrinimitable.realtime.init(window.socketio_port, true); // lazy connection
});
