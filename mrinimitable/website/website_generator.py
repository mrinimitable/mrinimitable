...
from mrinimitable.search.website_search import remove_document_from_index, update_index_for_path
from mrinimitable.website.utils import cleanup_page_name, clear_cache


class WebsiteGenerator(Document):
	website = mrinimitable._dict()
	...
	def clear_cache(self):
		...
		mrinimitable.db.after_commit.add(lambda: clear_cache(self.route))
		mrinimitable.db.after_rollback.add(lambda: clear_cache(self.route))
	...
	def on_trash(self):
		...
		if self.allow_website_search_indexing():
			mrinimitable.enqueue(remove_document_from_index, path=self.route, enqueue_after_commit=True)
	...
	def send_indexing_request(self, operation_type="URL_UPDATED"):
		...
		if (
			mrinimitable.db.get_single_value("Website Settings", "enable_google_indexing")
			and self.is_website_published()
			and self.meta.allow_guest_to_view
		):
			url = mrinimitable.utils.get_url(self.route)
			mrinimitable.enqueue(
				"mrinimitable.website.doctype.website_settings.google_indexing.publish_site",
				url=url,
	...
	def remove_old_route_from_index(self):
		...
		if self.allow_website_search_indexing() or mrinimitable.in_test:
			return
	...
	def update_website_search_index(self):
		...
		"""
		if not self.allow_website_search_indexing() or mrinimitable.in_test:
			return

		if self.is_website_published():
			mrinimitable.enqueue(update_index_for_path, path=self.route, enqueue_after_commit=True)
		elif self.route:
