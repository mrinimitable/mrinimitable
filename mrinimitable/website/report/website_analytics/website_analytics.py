# Copyright (c) 2013, Mrinimitable Technologies and contributors
# License: MIT. See LICENSE

from datetime import datetime

import mrinimitable
from mrinimitable.query_builder.functions import Coalesce, Count
...
class WebsiteAnalytics:
	def __init__(self, filters=None):
		self.filters = mrinimitable._dict(filters or {})

		if not self.filters.to_date:
			self.filters.to_date = datetime.now()

		if not self.filters.from_date:
			self.filters.from_date = mrinimitable.utils.add_days(self.filters.to_date, -7)

		if not self.filters.range:
			self.filters.range = "Daily"

		self.filters.to_date = mrinimitable.utils.add_days(self.filters.to_date, 1)
	...
	def get_columns(self):
		meta = mrinimitable.get_meta("Web Page View")
	...
	def get_data(self):
		WebPageView = mrinimitable.qb.DocType("Web Page View")
		count_all = Count("*").as_("count")
		case = mrinimitable.qb.terms.Case().when(WebPageView.is_unique == "1", "1")
		count_is_unique = Count(case).as_("unique_count")

		return (
			mrinimitable.qb.from_(WebPageView)
			...
			.orderby("count", order=mrinimitable.qb.desc)
		).run()
	...
	def get_chart_data(self):
		current_dialect = mrinimitable.db.db_type or "mariadb"
		...
		self.chart_data = mrinimitable.db.sql(query, values=values, as_dict=1)
	...
	def prepare_chart_data(self, data):
		...
		if self.filters.range == "Monthly":
			date_range = [mrinimitable.utils.add_days(dd, 1) for dd in date_range]
