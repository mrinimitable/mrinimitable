...
{% if enable_view_tracking %}
	if (navigator.doNotTrack != 1 && !window.is_404) {
		mrinimitable.ready(() => {
			let browser = mrinimitable.utils.get_browser();
			let query_params = mrinimitable.utils.get_query_params();

			// Get visitor ID based on browser uniqueness
			import('/assets/mrinimitable/js/lib/fingerprintjs.js')
				.then(fingerprint_js => fingerprint_js.load())
				.then(fp => fp.get())
				.then(result => {
					mrinimitable.call("mrinimitable.website.doctype.web_page_view.web_page_view.make_view_log", {
