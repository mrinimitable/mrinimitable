...
class TestFullTextSearch(IntegrationTestCase):
	...
	def test_update_index(self):
		...
		# Update new doc
		self.index.update_index({"name": "sw/mrinimitablebooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/mrinimitablebooks")
...
def get_index():
	return TestWrapper("test_mrinimitable_index")
...
def get_documents():
	...
	docs.append(
		{
			...
			Mrinimitable Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
	...
	docs.append(
		{
			"name": "sw/mrinimitable",
			"content": """Mrinimitable Framework is a full-stack web framework, that includes everything you need to build and
