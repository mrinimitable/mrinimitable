# Copyright (c) 2025, Mrinimitable Technologies Pvt. Ltd. and Contributors
...
class SQLiteSearch(ABC):
	"""
	Abstract base class for SQLite FTS5-based full-text search for Mrinimitable.
	...
	def search(self, query, title_only=False, filters=None):
		...
		try:
			...
			mrinimitable.log_error(f"Search query failed: {e}")
			raw_results = []
			total_matches = 0
	...
	def drop_index(self):
		...
		if os.path.exists(self.db_path):
			try:
				...
				mrinimitable.log_error(f"Failed to remove search index file {self.db_path}: {e}")
				raise
	...
	def get_documents(self):
		...
		for doctype, config in self.doc_configs.items():
			docs = mrinimitable.qb.get_query(
	...
	def _get_connection(self, read_only=False):
		...
		try:
			...
			mrinimitable.log_error(f"Failed to connect to search database: {e}")
	...
	def _update_progress(self, message, progress, total=100, absolute=True):
		...
		if not hasattr(mrinimitable.local, "request") and not mrinimitable.flags.in_test:
	...
	def _get_db_path(self, is_temp=False):
		...
		site_path = mrinimitable.get_site_path()
			...
			if field == "modified":
				...
				if modified_value:
					if not isinstance(modified_value, datetime.datetime):
						modified_value = mrinimitable.utils.get_datetime(modified_value)
...
def build_index(
	...
	if search_class_path:
		SearchClass = mrinimitable.get_attr(search_class_path)
...
def build_index_in_background():
	...
	for SearchClass in search_classes:
		...
		mrinimitable.enqueue(
			"mrinimitable.search.sqlite_search.build_index",
			queue="long",
...
def get_search_classes() -> list[type[SQLiteSearch]]:
	module_paths = mrinimitable.get_hooks("sqlite_search")
	search_classes = [mrinimitable.get_attr(path) for path in module_paths]
