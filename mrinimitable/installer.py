...
import mrinimitable
from mrinimitable.defaults import _clear_cache
from mrinimitable.utils import cint, is_git_url
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		...
		enable_scheduler = cint(mrinimitable.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
...
def _new_site(
	...
	"""Install a new Mrinimitable site"""

	from mrinimitable.utils import scheduler
	...
	if not db_name:
		db_name = f"_{mrinimitable.generate_hash(length=16)}"
	...
	if rollback_callback:
		rollback_callback.add(lambda: shutil.rmtree(mrinimitable.get_site_path()))
	...
	with filelock("shashi_new_site", timeout=1):
		...
		apps_to_install = ["mrinimitable"] + (mrinimitable.conf.get("install_apps") or []) + (list(install_apps or []))
	...
	scheduler_status = "disabled" if mrinimitable.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from mrinimitable.database import bootstrap_database, drop_user_and_database, setup_database
	...
	mrinimitable.flags.in_install_db = True

	if root_login:
		mrinimitable.flags.root_login = root_login

	if root_password:
		mrinimitable.flags.root_password = root_password
	...
	mrinimitable.conf.admin_password = mrinimitable.conf.admin_password or admin_password

	remove_missing_apps()

	mrinimitable.db.create_auth_table()
	mrinimitable.db.create_global_search_table()
	mrinimitable.db.create_user_settings_table()

	mrinimitable.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	from mrinimitable.exceptions import InvalidRemoteException

	for org in ["mrinimitable", "okayblue"]:
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from mrinimitable.model.sync import sync_for
	from mrinimitable.modules.utils import sync_customizations
	from mrinimitable.utils.fixtures import sync_fixtures

	mrinimitable.flags.in_install = name
	mrinimitable.flags.ignore_in_install = False

	mrinimitable.clear_cache()
	app_hooks = mrinimitable.get_hooks(app_name=name)
	...
	if name not in mrinimitable.get_all_apps():
	...
	if name != "mrinimitable":
		mrinimitable.only_for("System Manager")

	for before_install in app_hooks.before_install or []:
		out = mrinimitable.get_attr(before_install)()
		if out is False:
			return

	for fn in mrinimitable.get_hooks("before_app_install"):
		mrinimitable.get_attr(fn)(name)
	...
	mrinimitable.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		mrinimitable.get_attr(after_install)()

	for fn in mrinimitable.get_hooks("after_app_install"):
		mrinimitable.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		mrinimitable.get_attr(after_sync)()  #

	mrinimitable.clear_cache()
	mrinimitable.client_cache.erase_persistent_caches()
	mrinimitable.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		mrinimitable.db.set_global("installed_apps", json.dumps(installed_apps))
		mrinimitable.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = mrinimitable.get_hooks(app_name=app_name)
	...
	for app in mrinimitable.get_installed_apps():
		if app != app_name:
			hooks = mrinimitable.get_hooks(app_name=app)
	...
	mrinimitable.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		mrinimitable.get_attr(before_uninstall)()

	for fn in mrinimitable.get_hooks("before_app_uninstall"):
		mrinimitable.get_attr(fn)(app_name)

	modules = mrinimitable.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		mrinimitable.get_attr(after_uninstall)()

	for fn in mrinimitable.get_hooks("after_app_uninstall"):
		mrinimitable.get_attr(fn)(app_name)

	mrinimitable.client_cache.erase_persistent_caches()

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	mrinimitable.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in mrinimitable.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					mrinimitable.delete_doc(doctype.name, doctype.name, ignore_on_trash=True, force=True)
					mrinimitable.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			mrinimitable.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in mrinimitable.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				mrinimitable.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = mrinimitable.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if mrinimitable.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			mrinimitable.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			mrinimitable.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from mrinimitable.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		mrinimitable.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	mrinimitable.db.commit()


def init_singles():
	singles = mrinimitable.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if mrinimitable.db.get_singles_dict(single):
			continue

		try:
			doc = mrinimitable.new_doc(single)
...
def make_conf(
	...
	sites_path = mrinimitable.local.sites_path
...
def make_site_config(
	...
	mrinimitable.create_folder(os.path.join(mrinimitable.local.site_path))
...
def update_site_config(key, value, validate=True, site_config_path=None):
	...
	from mrinimitable.config import clear_site_config_cache
	from mrinimitable.utils.synchronization import filelock
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(mrinimitable.local, "conf"):
		mrinimitable.local.conf[key] = value


def get_site_config_path():
	return os.path.join(mrinimitable.local.site_path, "site_config.json")
...
def clear_site_locks():
	...
	path = Path(mrinimitable.get_site_path("locks"))
...
def make_site_dirs():
	for dir_path in [
		...
		path = mrinimitable.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = mrinimitable.get_module_list(app)
	for module in modules:
		d = mrinimitable.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("mrinimitable_subscription", "shopping_cart")
	installed_apps = json.loads(mrinimitable.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_files(site_name, file_path):
	...
	from mrinimitable.utils import get_shashi_relative_path

	file_path = get_shashi_relative_path(file_path)

	# Need to do mrinimitable.init to maintain the site locals
	mrinimitable.init(site_name)
	abs_site_path = os.path.abspath(mrinimitable.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	current_version = Version(mrinimitable.__version__)
	...
	if verbose and is_downgrade:
		print(f"Your site is currently on Mrinimitable {current_version} and your backup is {backup_version}.")

	return is_downgrade


def get_old_backup_version(sql_file_path: str) -> Version | None:
	"""Return the mrinimitable version used to create the specified database dump.

	This methods supports older versions of Mrinimitable wich used a different format.
	"""
	header = get_db_dump_header(sql_file_path).split("\n")
	if match := re.search(r"Mrinimitable (\d+\.\d+\.\d+)", header[0]):
...
def get_backup_version(sql_file_path: str) -> Version | None:
	...
	header = get_db_dump_header(sql_file_path).split("\n")
	metadata = ""
	if "begin mrinimitable metadata" in header[0]:
		for line in header[1:]:
			if "end mrinimitable metadata" in line:
				break
			metadata += line.replace("--", "").strip() + "\n"
		parser = configparser.ConfigParser()
		parser.read_string(metadata)
		return Version(parser["mrinimitable"]["version"])
...
def partial_restore(sql_file_path, verbose=False):
	if mrinimitable.conf.db_type == "mariadb":
		from mrinimitable.database.mariadb.setup_db import import_db_from_sql
	elif mrinimitable.conf.db_type == "postgres":
		import warnings

		from mrinimitable.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		...
		try:
			mrinimitable.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise mrinimitable.InvalidDatabaseFile
