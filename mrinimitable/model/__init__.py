...
def delete_fields(args_dict, delete=0):
	...
	* If single, deletes record from `tabSingles`
	...
	for dt in args_dict:
		...
		if mrinimitable.db.get_value("DocType", dt, "issingle"):
			mrinimitable.db.delete(
				"Singles",
				{
					"doctype": dt,
			...
			existing_fields = mrinimitable.db.describe(dt)
...
def get_permitted_fields(
	...
	*,
	...
	if permission_type is None:
		permission_type = "select" if mrinimitable.only_has_select_perm(doctype, user=user) else "read"
