...
def set_default(doc, key):
	...
	if not doc.is_default:
		mrinimitable.db.set(doc, "is_default", 1)
...
def set_field_property(filters, key, value):
	...
	docs = [
		mrinimitable.get_doc("DocType", d.parent)
		for d in mrinimitable.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(mrinimitable.ValidationError):
	pass
	...
	for _ in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with open(mrinimitable.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def get_fetch_values(doctype, fieldname, value):
	...
	db_values = mrinimitable.db.get_value(
...
def is_virtual_doctype(doctype: str):
	if mrinimitable.flags.in_install or mrinimitable.flags.in_migrate:
		return (
			mrinimitable.db.get_value("DocType", doctype, "is_virtual")
			if mrinimitable.db.has_column("DocType", "is_virtual")
			else False
		)

	return getattr(mrinimitable.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if mrinimitable.flags.in_install or mrinimitable.flags.in_migrate:
		return mrinimitable.db.get_value("DocType", doctype, "issingle")
	else:
		return getattr(mrinimitable.get_meta(doctype), "issingle", False)
