...
from mrinimitable.database.utils import DefaultOrderBy, FallBackDateTimeStr, NestedSetHierarchy
from mrinimitable.model import OPTIONAL_FIELDS, get_permitted_fields
from mrinimitable.model.meta import get_table_columns
from mrinimitable.model.utils import is_virtual_doctype
from mrinimitable.model.utils.user_settings import get_user_settings, update_user_settings
from mrinimitable.query_builder.utils import Column
from mrinimitable.types import Filters, FilterSignature, FilterTuple
...
from mrinimitable.utils.data import DateTimeLikeObject, get_datetime, getdate, sbool
...
class DatabaseQuery:
	...
	def get_meta(self, doctype: str):
		if doctype not in self._metas:
			self._metas[doctype] = mrinimitable.get_meta(doctype)
	...
	def execute(
		...
		*,
		...
		if as_list and not isinstance(self.fields, (Sequence | str)) and len(self.fields) > 1:
			mrinimitable.throw(_("Fields must be a list or tuple when as_list is enabled"))
		...
		if is_virtual_doctype(self.doctype):
			...
			return mrinimitable.call(controller.get_list, args=kwargs, **kwargs)
	...
	def build_and_run(self):
		...
		if mrinimitable.db.db_type == "postgres" and args.order_by and args.group_by:
...
{limit}""".format(**args)
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {mrinimitable.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			mrinimitable.throw(_("Use of sub-query or function is restricted"), mrinimitable.DataError)
		...
		for field in self.fields:
			...
			if SUB_QUERY_PATTERN.match(field):
				...
				if function in blacklisted_functions:
					mrinimitable.throw(
						_("Use of function {0} in field is restricted").format(function), exc=mrinimitable.DataError
					)
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					mrinimitable.throw(_("Illegal SQL Query"))
	...
	def append_link_table(self, doctype, fieldname):
		...
		linked_table = mrinimitable._dict(
			doctype=doctype,
			fieldname=fieldname,
	...
	def _set_permission_map(self, doctype: str, parent_doctype: str | None = None):
		ptype = "select" if mrinimitable.only_has_select_perm(doctype) else "read"
		mrinimitable.has_permission(
			doctype,
			ptype=ptype,
	...
	def apply_fieldlevel_read_permissions(self):
		...
		Note: Does not apply to `mrinimitable.model.core_doctype_list`
		...
		Example:
		        ...
		        - Query: fields=["*"]
		        - Result: fields=["title", ...] // will also include Mrinimitable's meta field like `name`, `owner`, etc.
		"""
		from mrinimitable.desk.reportview import extract_fieldnames
		...
		for i, field in fields_to_check:
			...
			# labels / pseudo columns or mrinimitable internals
			if column[0] in {"'", '"'}:
				continue
			...
			if doctype and doctype != self.doctype:
				if wrap_grave_quotes(table) not in self.query_tables:
					raise mrinimitable.PermissionError(doctype)
	...
	def prepare_filter_condition(self, ft: FilterTuple) -> str:
		...
		from mrinimitable.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in NestedSetHierarchy:
			...
			lft, rgt = "", ""
			if f.value:
				lft, rgt = mrinimitable.db.get_value(ref_doctype, f.value, ["lft", "rgt"]) or (0, 0)
			...
			fallback = "''"
			value = [mrinimitable.db.escape((cstr(v)).strip(), percent=False) for v in nodes]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
		...
		if f.operator.lower() in ("in", "not in"):
			...
			if value is None:
				...
				fallback = "''"
				value = [mrinimitable.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
				if len(value):
					value = f"({', '.join(value)})"
				else:
					value = "('')"
			...
			if f.operator in (">", "<", ">=", "<=") and (f.fieldname in ("creation", "modified")):
				...
				value = mrinimitable.db.format_date(f.value)
				fallback = "'0001-01-01'"

			elif (df and df.fieldtype == "Datetime") or isinstance(f.value, datetime.datetime):
				value = mrinimitable.db.format_datetime(f.value)
				fallback = f"'{FallBackDateTimeStr}'"
				...
				df
				and (db_type := cstr(mrinimitable.db.type_map.get(df.fieldtype, " ")[0]))
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if mrinimitable.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif escape and isinstance(value, str):
				value = f"{mrinimitable.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and mrinimitable.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True) -> str | list:
		...
		role_permissions = mrinimitable.permissions.get_role_permissions(self.doctype_meta, user=self.user)
		if (
			...
			only_if_shared = True
			self.shared = mrinimitable.share.get_shared(self.doctype, self.user)
			if not self.shared:
				mrinimitable.throw(_("No permission to read {0}").format(_(self.doctype)), mrinimitable.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				...
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {mrinimitable.db.escape(self.user, percent=False)}"
				)
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(mrinimitable.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if mrinimitable.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(mrinimitable.db.escape(doc, percent=False) for doc in docs)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if any(re.search("\b" + pattern + "\b", _lower) for pattern in subquery_indicators):
			mrinimitable.throw(_("Cannot use sub-query here."))
		...
		for field in parameters.split(","):
			...
			if full_field_name:
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					mrinimitable.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))
			...
			for func in blacklisted_sql_functions:
				if re.search(r"\b" + re.escape(func) + r"\W*\(", field.lower()):
					mrinimitable.throw(_("Cannot use {0} in order/group by").format(field))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
			mrinimitable.db.exists("DocField", {"parent": parent, "options": child_doctype})
			or mrinimitable.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
			raise mrinimitable.PermissionError

		if mrinimitable.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = mrinimitable.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	from_date = mrinimitable.utils.nowdate()
	to_date = mrinimitable.utils.nowdate()
	...
	if fieldtype == "Datetime":
		cond = f"'{mrinimitable.db.format_datetime(from_date)}' AND '{mrinimitable.db.format_datetime(to_date)}'"
	else:
		cond = f"'{mrinimitable.db.format_date(from_date)}' AND '{mrinimitable.db.format_date(to_date)}'"
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = mrinimitable._dict(mrinimitable.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = mrinimitable._dict(option)
