...
from mrinimitable.model.document import Document, bulk_insert
...
def get_countries_and_currencies():
	from mrinimitable.geo.country_info import get_all as get_geo_data
	...
	for name, country in data.items():
		country = mrinimitable._dict(country)
		countries.append(
		...
		if country.currency and country.currency not in added_currencies:
			...
			currencies.append(
				mrinimitable.get_doc(
					...
					number_format=mrinimitable.db.escape(country.number_format)[1:-1],
				)
			)
