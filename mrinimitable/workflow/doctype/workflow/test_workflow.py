...
class TestWorkflow(IntegrationTestCase):
	...
	def setUp(self):
		self.patcher = patch("mrinimitable.attach_print", return_value={})
		self.patcher.start()
		mrinimitable.db.delete("Workflow Action")
	...
	def tearDown(self):
		...
		mrinimitable.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		...
		workflow_actions = mrinimitable.get_all("Workflow Action", fields=["*"])
	...
	def test_sync_tasks(self, doc=None):
		...
		domain = mrinimitable.new_doc("Domain")
		...
		with self.patch_hooks(
			{
				"workflow_methods": [
					{
						"name": "Create Note",
						"method": "mrinimitable.workflow.doctype.workflow.test_workflow.create_new_note",
					}
				]
			}
		):
		...
		self.assertTrue(
			mrinimitable.db.exists("Note", {"title": "workflow - " + domain.name, "content": "workflow test"})
		)
		self.assertTrue(mrinimitable.db.exists("Domain", {"name": "workflow - " + domain.name}))
		self.assertTrue(mrinimitable.db.exists("Webhook Request Log", {"url": "https://workflowtasks.org/post"}))
...
def create_todo_workflow():
	from mrinimitable.tests.ui_test_helpers import UI_TEST_USER

	if mrinimitable.db.exists("Workflow", "Test ToDo"):
		mrinimitable.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not mrinimitable.db.exists("Role", TEST_ROLE):
		mrinimitable.get_doc(doctype="Role", role_name=TEST_ROLE).insert(ignore_if_duplicate=True)
		if mrinimitable.db.exists("User", UI_TEST_USER):
			mrinimitable.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = mrinimitable.new_doc("Workflow")
...
def create_domain_workflow():
	...
	if mrinimitable.db.exists("Workflow", "Test Domain"):
		mrinimitable.delete_doc("Workflow", "Test Domain")
	...
	pending_to_approved_transition = mrinimitable.new_doc("Workflow Transition Tasks")
...
def create_new_todo():
	return mrinimitable.get_doc(doctype="ToDo", description="workflow " + random_string(10)).insert()
...
def create_new_server_script():
	server_script = mrinimitable.new_doc("Server Script")
