"""
MrinimitableClient is a library that helps you connect with other mrinimitable systems
"""
...
class MrinimitableException(Exception):
	pass


class MrinimitableClient:
	def __init__(
		self,
		url,
		username=None,
		...
		api_key=None,
		api_secret=None,
		mrinimitable_authorization_source=None,
	):
		import requests
		...
		self.mrinimitable_authorization_source = mrinimitable_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.mrinimitable_authorization_source:
				auth_source = {"Mrinimitable-Authorization-Source": self.mrinimitable_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": mrinimitable.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return mrinimitable._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "mrinimitable.client.insert_many", "docs": mrinimitable.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": mrinimitable.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return mrinimitable._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "mrinimitable.client.bulk_update", "docs": mrinimitable.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "mrinimitable.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "mrinimitable.client.submit", "doc": mrinimitable.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "mrinimitable.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": mrinimitable.as_json(filters),
			}
		)
	...
	def set_value(self, doctype, docname, fieldname, value):
		...
		return self.post_request(
			{
				"cmd": "mrinimitable.client.set_value",
				"doctype": doctype,
				"name": docname,
				"fieldname": fieldname,
				"value": value,
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "mrinimitable.client.cancel", "doctype": doctype, "name": name})
	...
	def rename_doc(self, doctype, old_name, new_name):
		...
		params = {
			"cmd": "mrinimitable.client.rename_doc",
			"doctype": doctype,
			"old_name": old_name,
			"new_name": new_name,
		}
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not mrinimitable.db.exists("User", doc.get("owner")):
			...
			doc["doctype"] = doctype
			new_doc = mrinimitable.get_doc(doc)
			new_doc.insert()
	...
	def migrate_single(self, doctype):
		...
		doc.modified = mrinimitable.db.get_single_value(doctype, "modified")
		mrinimitable.get_doc(doc).insert()
	...
	def post_process(self, response):
		...
		if rjson and (rjson.get("exc") or rjson.get("exc_type") or rjson.get("errors")):
			try:
				...
				exc = "MrinimitableClient Request Failed\n\n" + exception
			except Exception:
				exc = rjson.get("exc")

			raise MrinimitableException(exc)
		if "message" in rjson:
...
class MrinimitableOAuth2Client(MrinimitableClient):
