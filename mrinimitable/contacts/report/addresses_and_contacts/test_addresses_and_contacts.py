...
from mrinimitable.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from mrinimitable.tests import IntegrationTestCase


def get_custom_linked_doctype():
	if bool(mrinimitable.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if mrinimitable.flags.test_address_created:
		return

	address = mrinimitable.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	mrinimitable.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if mrinimitable.flags.test_contact_created:
		return
	...
	mrinimitable.flags.test_contact_created = True
