...
const html_plugin = require("./mrinimitable-html");
const vue_style_plugin = require("./mrinimitable-vue-style");
const rtlcss = require("rtlcss");
const postCssPlugin = require("@mrinimitable/esbuild-plugin-postcss2").default;
...
const argv = yargs
	...
	.option("skip_mrinimitable", {
		type: "boolean",
		description: "Skip building mrinimitable assets",
	})
	.option("files", {
		type: "string",
	...
	.example("node esbuild --apps mrinimitable,okayblue", "Run build only for mrinimitable and okayblue")
	.example(
		"node esbuild --files mrinimitable/website.bundle.js,mrinimitable/desk.bundle.js",
		"Run build only for specified bundles"
	)
	.version(false).argv;

const APPS = (!argv.apps ? app_list : argv.apps.split(",")).filter(
	(app) => !(argv.skip_mrinimitable && app == "mrinimitable")
);
...
function get_files_to_build(files) {
	// files: ['mrinimitable/website.bundle.js', 'okayblue/main.bundle.js']
	let include_patterns = [];
	let ignore_patterns = [];
...
async function update_assets_json_in_cache() {
	...
	if (process.env.MRINIMITABLE_DOCKER_BUILD) {
		return;
	}
