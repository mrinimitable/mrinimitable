	...
	it("check value changes", () => {
		...
		TEST_CASES.forEach((test_case) => {
			cy.window()
				.its("mrinimitable")
				.then((mrinimitable) => {
					mrinimitable.boot.sysdefaults.currency = test_case.currency;
					mrinimitable.boot.sysdefaults.currency_precision = test_case.default_precision ?? 2;
					mrinimitable.boot.sysdefaults.number_format = test_case.number_format ?? "#,###.##";
				});
