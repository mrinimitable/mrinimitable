...
context("Grid Search", () => {
		...
		return cy
			...
			.then((mrinimitable) => {
				return mrinimitable.xcall(
					"mrinimitable.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("mrinimitable")
			.then((mrinimitable) => {
				mrinimitable.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.mrinimitable-control[data-fieldname="child_table_1"]').as("table");
