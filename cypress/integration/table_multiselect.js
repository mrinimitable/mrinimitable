context("Table MultiSelect", () => {
	...
	it("select value from multiselect dropdown", () => {
		...
		cy.get(
			'.mrinimitable-control[data-fieldname="users"] .form-control .tb-selected-value .btn-link-to-form'
		).as("selected-value");
		cy.get("@selected-value").should("contain", "test@okayblue.com");

		cy.intercept("POST", "/api/method/mrinimitable.desk.form.save.savedocs").as("save_form");
		// trigger save
	...
	it("delete value using backspace", () => {
		...
		cy.get('.mrinimitable-control[data-fieldname="users"] .form-control .tb-selected-value').should(
			"not.exist"
		);
	});
	...
	it("delete value using x", () => {
		cy.go_to_list("Assignment Rule");
		cy.get(`.list-subject:contains("table multiselect")`).last().find("a").click();
		cy.get('.mrinimitable-control[data-fieldname="users"] .form-control .tb-selected-value').as(
			"existing_value"
		);
