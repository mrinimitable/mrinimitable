context("Workspace Blocks", () => {
	before(() => {
		...
		return cy
			...
			.its("mrinimitable")
			.then((mrinimitable) => {
				return mrinimitable.xcall("mrinimitable.tests.ui_test_helpers.setup_workflow");
			});
	});
	...
	it("Create Test Page", () => {
		cy.intercept({
			method: "POST",
			url: "api/method/mrinimitable.desk.doctype.workspace.workspace.new_page",
		}).as("new_page");
	...
	it.skip("Quick List Block", () => {
		...
		cy.intercept({
			method: "GET",
			url: "api/method/mrinimitable.desk.form.load.getdoctype?**",
		}).as("get_doctype");
		...
		cy.intercept({
			method: "POST",
			url: "api/method/mrinimitable.desk.reportview.get",
		}).as("refresh-list");
