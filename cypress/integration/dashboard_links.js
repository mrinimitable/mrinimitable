...
context("Dashboard links", () => {
	before(() => {
		...
		return cy
			...
			.its("mrinimitable")
			.then((mrinimitable) => {
				mrinimitable.call("mrinimitable.tests.ui_test_helpers.update_child_table", {
	...
	it("check if child table is populated with linked field on creation from dashboard link", () => {
		...
		cy.get(
			'.mrinimitable-control[data-fieldname="child_table"] .rows .data-row .col[data-fieldname="doctype_to_link"]'
