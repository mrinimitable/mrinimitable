context("Control Link", () => {
	...
	it("should set the valid value", () => {
		...
		cy.intercept("POST", "/api/method/mrinimitable.desk.search.search_link").as("search_link");

		cy.get(".mrinimitable-control[data-fieldname=link] input").focus().as("input");
		cy.wait("@search_link");
		cy.get("@input").type("todo for link", { delay: 200 });
		cy.wait("@search_link");
		cy.wait(500);
		cy.get(".mrinimitable-control[data-fieldname=link]").findByRole("listbox").should("be.visible");
		cy.get(".mrinimitable-control[data-fieldname=link] input").type("{enter}", { delay: 100 });
		cy.get(".mrinimitable-control[data-fieldname=link] input").blur();
	...
	it("should unset invalid value", () => {
		...
		cy.intercept("/api/method/mrinimitable.client.validate_link*").as("validate_link");

		cy.get(".mrinimitable-control[data-fieldname=link] input")
			.type("invalid value", { delay: 100 })
			.blur();
		cy.wait("@validate_link");
		cy.wait(500);
		cy.get(".mrinimitable-control[data-fieldname=link] input").should("have.value", "");
	});
	...
	it("should be possible set empty value explicitly", () => {
		...
		cy.get(".mrinimitable-control[data-fieldname=link] input").type("  ", { delay: 100 }).blur();
		cy.wait("@validate_link");
		cy.wait(500);
		cy.get(".mrinimitable-control[data-fieldname=link] input").should("have.value", "");
		cy.window()
	...
	it("should show open link button", () => {
		...
		cy.get("@todos").then((todos) => {
			cy.get(".mrinimitable-control[data-fieldname=link] input").as("input");
			cy.get("@input").focus();
			cy.wait("@search_link");
			cy.get("@input").type(todos[0]).blur();
			cy.wait("@validate_link");
			cy.get("@input").trigger("mouseover");
			cy.get(".mrinimitable-control[data-fieldname=link] .btn-open")
				.should("be.visible")
	...
	it("show title field in link", () => {
		...
		cy.window()
			...
			.then((mrinimitable) => {
				if (!mrinimitable.boot) {
					...
					mrinimitable.boot.link_title_doctypes = ["ToDo"];
				}
			});
		...
		cy.get("@input").type("todo for link", { delay: 200 });
		cy.wait("@search_link");
		cy.get(".mrinimitable-control[data-fieldname=link] ul").should("be.visible");
		cy.get(".mrinimitable-control[data-fieldname=link] input").type("{enter}", { delay: 100 });
		cy.get(".mrinimitable-control[data-fieldname=link] input").blur();
		cy.get("@dialog").then((dialog) => {
			cy.get("@todos").then((todos) => {
	...
	it("should update dependant fields (via fetch_from)", () => {
		cy.get("@todos").then((todos) => {
			...
			cy.get(".mrinimitable-control[data-fieldname=assigned_by] input").focus().as("input");
			cy.get("@input").clear().type(cy.config("testUser"), { delay: 300 }).blur();
			cy.wait("@validate_link");
			cy.get(".mrinimitable-control[data-fieldname=assigned_by_full_name] .control-value").should(
				"contain",
				"Mrinimitable"
			);
	...
	it("show translated text for Gender link field with language de with input in de", () => {
		cy.call("mrinimitable.tests.ui_test_helpers.insert_translations").then(() => {
			cy.window()
				.its("mrinimitable")
				.then((mrinimitable) => {
					cy.set_value("User", mrinimitable.user.name, { language: "de" });
				});
	...
	it("show text for Gender link field with language en", () => {
		cy.window()
			.its("mrinimitable")
			.then((mrinimitable) => {
				cy.set_value("User", mrinimitable.user.name, { language: "en" });
			});
	...
	it("show custom link option", () => {
		cy.window()
			.its("mrinimitable")
			.then((mrinimitable) => {
				mrinimitable.ui.form.ControlLink.link_options = (link) => {
					return [
						{
							html:
