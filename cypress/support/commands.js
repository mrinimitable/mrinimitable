			...
			return cy
				.request({
					...
					headers: {
						...
						"X-Mrinimitable-CSRF-Token": csrf_token,
					},
				})
				.then((res) => {
...
Cypress.Commands.add("create_records", (doc) => {
	return cy
		.call("mrinimitable.tests.ui_test_helpers.create_if_not_exists", { doc: JSON.stringify(doc) })
		.then((r) => r.message);
});

Cypress.Commands.add("set_value", (doctype, name, obj) => {
	return cy.call("mrinimitable.client.set_value", {
		doctype,
		name,
		fieldname: obj,
	});
...
Cypress.Commands.add(
	...
	(tablefieldname, row_idx, fieldname, fieldtype = "Data") => {
		let selector = `.mrinimitable-control[data-fieldname="${tablefieldname}"]`;
...
Cypress.Commands.add("dialog", (opts) => {
	return cy
		.window({ log: false })
		.its("mrinimitable", { log: false })
		.then((mrinimitable) => {
			Cypress.log({
				name: "dialog",
				displayName: "dialog",
				message: "mrinimitable.ui.Dialog",
				consoleProps: () => {
					return {
						options: opts,
						dialog: d,
			...
			var d = new mrinimitable.ui.Dialog(opts);
			d.show();
			return d;
		});
});
...
Cypress.Commands.add("save", () => {
	cy.intercept("/api/method/mrinimitable.desk.form.save.savedocs").as("save_call");
...
Cypress.Commands.add("add_role", (user, role) => {
	cy.window()
		.its("mrinimitable")
		.then((mrinimitable) => {
			const session_user = mrinimitable.session.user;
...
const add_remove_role = (action, user, role, session_user) => {
	...
	cy.call("mrinimitable.tests.ui_test_helpers.add_remove_role", {
		action: action,
		user: user,
		role: role,
	});
...
Cypress.Commands.add("select_listview_row_checkbox", (row_no) => {
	cy.get(".mrinimitable-list .select-like > .list-row-checkbox").eq(row_no).click();
});
